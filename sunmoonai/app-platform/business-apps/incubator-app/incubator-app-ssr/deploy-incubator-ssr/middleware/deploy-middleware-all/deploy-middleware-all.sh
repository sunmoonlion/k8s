...
# Incubator App SSR 中间件总控部署脚本

set -e

# 脚本目录（保存为变量，防止被统一部署模板覆盖）
INCUBATOR_SSR_MIDDLEWARE_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# 计算项目根目录（k8s目录）
# 从 deploy-middleware-all/ -> middleware/ -> deploy-incubator-ssr/ -> incubator-app-ssr/ -> incubator-app/ -> business-apps/ -> app-platform/ -> sunmoonai/ -> k8s/
...
load_config() {
    # 使用保存的脚本目录确保路径正确
    local config_file="${INCUBATOR_SSR_MIDDLEWARE_CONFIG_FILE:-$INCUBATOR_SSR_MIDDLEWARE_SCRIPT_DIR/deploy-middleware-all.conf}"
    
    if [[ ! -f "$config_file" ]]; then
        log_error "Incubator App SSR 中间件总控配置文件不存在: $config_file"
        log_error "INCUBATOR_SSR_MIDDLEWARE_SCRIPT_DIR: $INCUBATOR_SSR_MIDDLEWARE_SCRIPT_DIR"
        log_error "INCUBATOR_SSR_MIDDLEWARE_CONFIG_FILE: ${INCUBATOR_SSR_MIDDLEWARE_CONFIG_FILE:-未设置}"
        exit 1
    fi

    source "$config_file"
    log_success "✅ Incubator App SSR 中间件总控配置加载成功"
}
...
deploy_all_middleware() {
    ...
    log_info "开始部署 Incubator App SSR 中间件组件..."
    log_info "命名空间: $namespace"
    log_warn "⚠️ 注意：当前 middleware.yaml 是占位符，可根据需要扩展"
    
    # 当前中间件组件（占位符，可根据需要扩展）
    local components=(
        # 示例：如果需要自定义请求头中间件，可以添加：
        # "headers:${incubator_ssr_headers_enabled:-false}:${incubator_ssr_headers_priority:-100}:Incubator App SSR Headers:$INCUBATOR_SSR_MIDDLEWARE_SCRIPT_DIR/../incubator-ssr-headers/deploy-incubator-ssr-headers/deploy-incubator-ssr-headers.sh"
    )
    ...
    if [[ ${#enabled[@]} -eq 0 ]]; then
        log_info "当前没有启用的中间件组件（middleware.yaml 是占位符，可根据需要扩展）"
        return 0
    fi
    ...
    log_success "✅ Incubator App SSR 所有中间件组件部署完成！"
    return 0
}
...
delete_all_middleware() {
    ...
    log_info "开始删除 Incubator App SSR 所有中间件组件..."
    log_info "命名空间: $namespace"
    ...
    log_success "✅ Incubator App SSR 所有中间件组件删除完成！"
    return 0
}
...
check_all_middleware_status() {
    ...
    log_info "检查 Incubator App SSR 所有中间件组件状态..."
    log_info "命名空间: $namespace"
    ...
    log_success "✅ Incubator App SSR 所有中间件组件状态检查完成！"
    return 0
}
...
main() {
    ...
    case "$action" in
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 Incubator App SSR 所有中间件组件（默认）"
            echo "  uninstall        删除 Incubator App SSR 所有中间件组件"
            ...
            echo ""
            echo "注意：当前 middleware.yaml 是占位符，可根据需要扩展"
            echo ""
            echo "示例:"
            echo "  $0 deploy"
