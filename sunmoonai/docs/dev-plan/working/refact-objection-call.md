...
| **截止** | 见 §5，是**可观测判据**不是时钟时间。逾期视为**弃权**，流程继续，处置记录写「未响应」——**不写成「认可」** |
...
④ 对你是轻活（四条处置，逐条确认或反驳即可），⑤ 才是重活。
**若你触发 §5 的弃权判据，验收方会按顺位改派 qwen**——
理由只会是「不可用」，不会是别的，且会附上观测值。

## 5. 逾期与换人（预先声明，避免事后裁量）

本轮按 `round-protocol.md`「参与方不可用」执行，规则**现在就写明**，不在事后决定：

**截止不是时钟时间，是判据。**「几点合适」会变成每轮都要有人拍板的新问题，
而判据由本轮自己的数据算出，谁跑都得到同一个结论。

**本环节的截止判据（同侪基准 + 停滞，二者先满足者为准）：**

```text
① 半数参与方（本轮 4 家，即 2 家）交付异议或「无异议」后开始计时；
② 窗长 W = 已交付各家「从本通知提交到交付」用时的中位数；
③ W 内某家的 refact-objection-<名>.md 字节数无变化、且其分支无新提交
   → 判为弃权。
```

**停滞判据优先：产物还在长就不算逾期。**裁决方在宣布任何一家弃权前，
会跑一次判据并把观测值（字节数、时间戳、分支 HEAD）写进处置记录——
「等太久了」不构成理由，观测值才是。

| 环节 | 逾期后果 |
| --- | --- |
| ④ 异议 | 视为**弃权**，流程继续。④ 之后还有 ⑤ 验收与 ⑥ 人确认两道独立检查，故此处允许默认放行 |
| ⑤ 验收 | 验收方视为**不可用**，按下方顺位换人。**⑤ 不设默认通过**——最后的检查不允许超时放行 |
| ⑥ 确认 | **不设逾期默认。**人不确认就不发布 |

**⑤ 验收方顺位**（排除裁决方 opus、基座作者 luna，按处置表采纳条数由少到多）：

```text
kimi（4 条） → qwen（5 条） → cursor（12 条）
```

> **⚠ 已发生变更，见 `refact-rulings.md`：**
> **R1** 免除 kimi 参加 ④（发起人裁定，方向「更省事」，已人确认）；
> **R2** ⑤ 验收方按上述顺位改派 **qwen**。
> 两条裁定的代价已如实登记在裁定记录里。kimi 依「裁量权 · 推翻」
> 仍可随时就该裁定或 `F-EXEC-02` 的改判提异议——**本裁定不剥夺申辩权，只是不再等**。

换人只有一个合法理由：**该家不可用**（不响应、逾期、退出）。
**不得因为对结果不满意换人。**换人会记入处置记录：原因、原指定、新指定、顺位计算。

**弃权不等于同意。**逾期的家在处置记录里记为「未响应」——
