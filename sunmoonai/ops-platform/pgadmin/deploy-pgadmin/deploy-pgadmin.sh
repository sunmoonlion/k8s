...
process_pgadmin_values() {
    ...
    if [[ -f "$env_values_file" ]]; then
        ...
        sed -i "s/{{PGADMIN_UNIFIED_HOST}}/${PGADMIN_UNIFIED_HOST:-www.sunmoonai.com}/g" "$pgadmin_values_file"
...
check_pgadmin_status() {
    ...
    # 优先尝试恢复 kubectl/helm 连接（若主流程清理了隧道，避免 status 误判）
    ...
    if [[ "$pods_running" =~ ^[0-9]+$ ]] && [[ "$pods_running" -gt 0 ]]; then
        log_success "✅ pgAdmin Pod 运行正常 ($pods_running 个)"
    elif [[ "$pods_pending" =~ ^[0-9]+$ ]] && [[ "$pods_pending" -gt 0 ]]; then
        log_warn "⏳ pgAdmin Pod 正在启动中（$pods_pending/$pods_count 个 Pending/ContainerCreating，0 个 Running）"
...
show_pgadmin_connection_info() {
    ...
    echo "=== pgAdmin 连接信息 ==="
    echo "命名空间: $namespace"
    echo "外部访问: https://${PGADMIN_UNIFIED_HOST:-www.sunmoonai.com}/pgadmin"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件（如果需要）:"
    echo "   echo '${PGADMIN_NODE_IP:-115.190.153.150} ${PGADMIN_UNIFIED_HOST:-www.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 访问 pgAdmin:"
    echo "   https://${PGADMIN_UNIFIED_HOST:-www.sunmoonai.com}/pgadmin"
    echo ""
    echo "3. 查看服务状态:"
