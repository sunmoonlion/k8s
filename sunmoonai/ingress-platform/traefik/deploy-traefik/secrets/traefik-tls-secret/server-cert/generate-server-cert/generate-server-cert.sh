...
main() {
    ...
    if [[ ! -f "$ca_cert" || ! -f "$ca_key" ]]; then
        ...
        log_error "     cd ~/k8s/sunmoonai/infrastructure/deploy-infrastructure-all"
        log_error "     bash deploy-infrastructure-all.sh deploy"
        log_error ""
        log_error "  2. 或者单独运行 Step12："
        log_error "     bash ~/k8s/sunmoonai/infrastructure/steps/step12_ca_generation.sh all"
        log_error ""
        log_error "  3. 或者手动运行CA生成脚本（不推荐，仅用于测试）："
        log_error "     ~/k8s/utils/ca-management/generate-ca.sh"
        log_error ""
        log_error "注意："
