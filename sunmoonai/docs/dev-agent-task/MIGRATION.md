...
| guide 没查什么（覆盖声明） | I02-097 | P-uat + 史 | 拆：查了/没查/不能排除与三类盲区进 P-uat；`runtime-refact` 轮当时的覆盖清单留作历史 |
...
| `pipeline.md` | — | B + P（总流程） | 整份移入 B（agent 项目的具体流程）；流程原则抽象后写进 `dev-process` 总则 |
