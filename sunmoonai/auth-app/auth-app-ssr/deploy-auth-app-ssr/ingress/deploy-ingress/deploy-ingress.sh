...
# 从 deploy-ingress/ -> ingress/ -> deploy-auth-app-ssr/ -> auth-app-ssr/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/auth-app-ssr-ingress/generate-ingress/auth-app-ssr-ingress-generated.yaml"
...
load_config() {
    ...
    # 主配置文件路径：从 deploy-ingress/ -> ingress/ -> deploy-auth-app-ssr/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-auth-app-ssr.conf"
    ...
    # 从主配置文件构建配置
    SERVICE_NAME="auth-app-ssr"
    NAMESPACE="${AUTH_APP_SSR_NAMESPACE:-app-platform-dev}"
    UNIFIED_HOST="${AUTH_APP_SSR_UNIFIED_HOST:-auth-app-ssr.sunmoonai.com}"
...
deploy_http_route() {
    ...
    kubectl get ingressroute -n "$namespace" auth-app-ssr-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" auth-app-ssr-headers 2>/dev/null || log_warn "Middleware 尚未创建"
...
check_deployment_status() {
    ...
    kubectl get ingressroute -n "$NAMESPACE" -l component=auth-app-ssr 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=auth-app-ssr 2>/dev/null || echo "Middleware 不存在"
