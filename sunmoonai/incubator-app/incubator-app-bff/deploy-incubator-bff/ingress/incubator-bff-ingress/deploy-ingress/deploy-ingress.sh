...
# 从 deploy-ingress/ -> incubator-bff-ingress/ -> ingress/ -> deploy-incubator-bff/ -> incubator-app-bff/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/incubator-bff-ingress/generate-ingress/incubator-app-bff-ingress-generated.yaml"
...
verify_service() {
    ...
    if kubectl get service -n "$namespace" "$service_name" >/dev/null 2>&1; then
        log_success "✅ Incubator App BFF 服务存在: $service_name"
        return 0
    else
        if [[ "$warn_only" == "true" ]]; then
            log_warning "⚠️ Incubator App BFF 服务不存在: $service_name（将在服务部署后自动生效）"
            return 0  # 返回成功，允许继续部署
        else
            log_error "❌ Incubator App BFF 服务不存在: $service_name"
            return 1
        fi
    fi
}
...
# 加载配置
load_config() {
    # 尝试加载主配置文件（如果存在），以获取 INCUBATOR_BFF_NAMESPACE、INCUBATOR_BFF_UNIFIED_HOST 等环境变量
    # 主配置文件路径：从 deploy-ingress/ -> incubator-bff-ingress/ -> ingress/ -> deploy-incubator-bff/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-incubator-bff.conf"
    ...
    SERVICE_NAME="incubator-app-bff"
    NAMESPACE="${NAMESPACE:-${INCUBATOR_BFF_NAMESPACE:-app-platform-dev}}"
    UNIFIED_HOST="${INCUBATOR_BFF_UNIFIED_HOST:-}"
    NODE_IP="${INCUBATOR_BFF_NODE_IP:-}"
    EXTERNAL_PORT="${INCUBATOR_BFF_EXTERNAL_PORT:-}"
...
deploy_http_route() {
    ...
    log_info "部署 Incubator App BFF HTTP 路由..."
    ...
    # 从主配置文件构建配置（所有默认值应在配置文件中定义）
    SERVICE_NAME="incubator-app-bff"
    NAMESPACE="${INCUBATOR_BFF_NAMESPACE:-}"
    UNIFIED_HOST="${INCUBATOR_BFF_UNIFIED_HOST:-}"
    NODE_IP="${INCUBATOR_BFF_NODE_IP:-}"
    ...
    if [[ ! -f "$INGRESS_FILE" ]]; then
        ...
        local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/incubator-bff-ingress/generate-ingress/generate-ingress.sh"
    ...
    verify_service "$namespace" "$SERVICE_NAME" "true" || {
        log_warning "⚠️ Incubator App BFF 服务尚未创建，Ingress 将在服务部署后自动生效"
    }
    ...
    # 检查路由状态
    log_info "检查 Incubator App BFF HTTP 路由状态..."
    kubectl get ingressroute -n "$namespace" incubator-app-bff-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" incubator-app-bff-stripprefix 2>/dev/null || log_warn "Middleware 尚未创建"
    
    log_success "✅ Incubator App BFF HTTP 路由部署完成！"
    return 0
}

# 检查部署状态
check_deployment_status() {
    log_info "检查 Incubator App BFF HTTP 路由部署状态..."
    
    echo ""
    echo "=== Incubator App BFF HTTP 路由状态 ==="
    kubectl get ingressroute -n "$NAMESPACE" -l component=incubator-app-bff 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=incubator-app-bff 2>/dev/null || echo "Middleware 不存在"
    
    echo ""
    echo "=== Incubator App BFF 服务状态 ==="
    kubectl get svc "$SERVICE_NAME" -n "$NAMESPACE" 2>/dev/null || echo "Incubator App BFF 服务不存在"
...
delete_http_route() {
    ...
    log_info "删除 Incubator App BFF HTTP 路由..."
    log_info "命名空间: $namespace"
...
# 显示帮助信息
show_help() {
    echo "Incubator App BFF HTTP 路由部署脚本"
    echo ""
    echo "用法: $0 [命令] [项目ID] [命名空间] [环境]"
    echo ""
    echo "命令:"
    echo "  deploy <project_id> [namespace] [environment]    部署 Incubator App BFF HTTP 路由"
    echo "  uninstall <project_id> [namespace] [environment] 卸载 Incubator App BFF HTTP 路由"
    echo "  status    检查部署状态"
    echo "  help      显示帮助信息"
    echo ""
    echo "参数:"
...
main() {
    ...
    case "$action" in
        deploy)
            ...
            log_info "开始部署 Incubator App BFF HTTP 路由..."
        ...
        uninstall)
            ...
            log_info "开始删除 Incubator App BFF HTTP 路由..."
            log_info "项目: $project_id"
            log_info "命名空间: $namespace"
            log_info "环境: $environment"
