...
def check_deployment_handbook(template: Path, errors: list[str]) -> None:
    """The restored handbook is documentation, not the retired deploy tool."""
    handbook = template / "dev-to-prod-deploy"
    if handbook.is_symlink():
        errors.append("tpl: deployment handbook must not be a symlink")
        return
    if not handbook.exists():
        return
    if not handbook.is_dir():
        errors.append("tpl: deployment handbook must be a directory")
        return
    for path in handbook.rglob("*"):
        if path.is_symlink() or (
            not path.is_dir()
            and (not path.is_file() or path.suffix != ".md" or path.stat().st_mode & 0o111)
        ):
            errors.append(f"tpl: non-documentation legacy deploy asset: {path.relative_to(template)}")
...
def main() -> int:
    ...
    check_deployment_handbook(template, errors)
