...
/**
 * Strict-TLS, real-Casdoor pairing verification for P0-009B.
...
const origin =
  process.env.P0_009B_ORIGIN ||
  'https://info-admin-p0-009b.sunmoonai.com:30443'
...
function stage(value) {
  process.stderr.write(`P0_009B_ADMIN_BROWSER_STAGE=${value}\n`)
}
...
function prepareStrictTlsHome() {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'sunmoonai-p0-009b-'))
...
async function submitCasdoorLogin(page, identity) {
  ...
  try {
    ...
    const diagnosticPath = '/tmp/p0-009b-casdoor-login.png'
...
async function main() {
  ...
  const frontend = deployment('info-admin-frontend-p0-009b', 3000)
  const backend = deployment('info-admin-backend-p0-009b', 8000)
  const identity = {
  ...
  execFileSync(
    ...
    [
      ...
      '-n',
      namespace,
      'deployment/info-admin-frontend-p0-009b',
      '--',
      'sh',
      '-lc',
      'test "$(id -u)" = 1001 && command -v node >/dev/null && ! touch /app/.p0-009b-write-probe',
    ],
    { stdio: 'ignore' },
  )
