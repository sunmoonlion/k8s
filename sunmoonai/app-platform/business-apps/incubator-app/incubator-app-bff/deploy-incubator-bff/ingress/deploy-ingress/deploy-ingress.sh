...
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../.." && pwd)/deploy-incubator-bff.conf"
...
status() {
  ...
  kubectl get ingressroute -n "$NAMESPACE" -l component=incubator-app-bff 2>/dev/null || true
  log_info "Middleware:"
  kubectl get middleware -n "$NAMESPACE" -l component=incubator-app-bff 2>/dev/null || true
}
