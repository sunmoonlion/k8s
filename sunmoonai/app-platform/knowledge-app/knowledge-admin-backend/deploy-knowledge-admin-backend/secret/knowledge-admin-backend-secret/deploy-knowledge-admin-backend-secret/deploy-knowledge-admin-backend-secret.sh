...
# 从 deploy-knowledge-admin-backend-secret/ -> knowledge-admin-backend-secret/ -> secret/ -> deploy-knowledge-admin-backend/ -> knowledge-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/knowledge-admin-backend-secret/generate-knowledge-admin-backend-secret/knowledge-admin-backend-secret-generated.yaml"
...
main() {
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/secret/knowledge-admin-backend-secret/generate-knowledge-admin-backend-secret/generate-knowledge-admin-backend-secret.sh"
    ...
    case "$action" in
        ...
        uninstall)
            ...
            ;;
        status)
            kubectl get secret "knowledge-admin-backend-secret" -n "$NAMESPACE" 2>/dev/null \
                || log_warn "Secret 不存在: knowledge-admin-backend-secret"
            ;;
        generate)
