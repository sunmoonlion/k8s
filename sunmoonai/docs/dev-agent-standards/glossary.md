...
| turn | 一问一答：一次派工和它的交回。目录名为「本地号-执行环境 id」，如 `01-01k8f3m2qz`；本地号按派出顺序编，不跳、不复用；交回即冻结。行文引用只用本地号 | 轮、轮次、回合 |
...
| 并行尝试 | 同一问同时发给几个执行者：任务书只有一份，各家各占 turn 目录下的一个尝试子目录 `a-…`、`b-…`，引用写作 03a、03b | — |
