...
# 从 deploy-ingress/ -> auth-app-bff-ingress/ -> ingress/ -> deploy-auth-app-bff/ -> auth-app-bff/
APP_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
RESOURCES_DIR="${APP_ROOT}/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"

# 导入统一部署模板（建立远程 k8s 连接）
# 从应用根目录（auth-app-bff/）到 k8s/ 需要向上 4 级
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/auth-app-bff-ingress/generate-ingress/auth-app-bff-ingress-generated.yaml"
...
load_config() {
    ...
    # 主配置文件路径：从 deploy-ingress/ -> auth-app-bff-ingress/ -> ingress/ -> deploy-auth-app-bff/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-auth-app-bff.conf"
...
deploy_http_route() {
    ...
    if [[ ! -f "$INGRESS_FILE" ]]; then
        ...
        local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/auth-app-bff-ingress/generate-ingress/generate-ingress.sh"
    ...
    kubectl get ingressroute -n "$namespace" auth-app-bff-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" auth-app-bff-stripprefix 2>/dev/null || log_warn "Middleware 尚未创建"
...
check_deployment_status() {
    ...
    kubectl get ingressroute -n "$NAMESPACE" -l component=auth-app-bff 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=auth-app-bff 2>/dev/null || echo "Middleware 不存在"
