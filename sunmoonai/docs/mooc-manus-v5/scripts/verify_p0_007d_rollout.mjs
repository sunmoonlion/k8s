...
/**
 * P0-007D React Router/Nginx rolling rollback gate.
 *
...
function parseArguments(argv) {
  const value = {
    ...
    rollbackImage: '',
    backendImage: '',
    acceptedDeploymentId: 'p0-007d-v2',
    rollbackDeploymentId: 'p0-007d-v1',
  }
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--accepted-image') value.acceptedImage = next
      ...
      process.stdout.write(
        'Usage: verify_p0_007d_rollout.mjs ' +
...
function stage(value) {
  process.stderr.write(`P0_007D_ROLLOUT_STAGE=${value}\n`)
}
...
function runFullVerifier(stageName) {
  ...
  execFileSync('node', [fullVerifier], {
    env: {
      ...
      P0_007D_NAMESPACE: namespace,
      P0_007D_ORIGIN: origin,
      P0_007D_CA_CERT: caCertificate,
    },
    stdio: 'inherit',
  })
}
...
async function main() {
  ...
  try {
    ...
    const backend = deploymentIdentity('tpl-admin-backend-p0-007d')
    if (
