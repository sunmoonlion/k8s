...
> 深读时间：2026-08-11；拓扑复核：2026-08-13。仓库路径 `/home/zymun/tpl-app`。
...
三个活动 git 子模块 + 父仓治理文件。默认三组件（当前锁定的 commit）：
...
React Router Admin、Vue Admin 与 NestJS Backend 已移至独立 `repo-backup`，只保证源码可恢复，
不再是 tpl-app 子模块，也不参与当前配对验收。`frontend-pairing-matrix.json` 只保留两个 DEFAULT
pair：Next Admin↔FastAPI 与 Next Web↔FastAPI。历史 release manifest 中的旧路径仅作冻结审计。
`frontend-capability-matrix.json` 继续规定 COMMON 能力必须 Admin/Web 各自实现并测试。
