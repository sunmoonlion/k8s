#!/bin/bash
# info-admin-backend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> info-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-info-admin-backend/app/deploy-app/deploy-info-admin-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${INFO_ADMIN_BACKEND_NAMESPACE:-}")
...
export INFO_ADMIN_BACKEND_IMAGE_REGISTRY="${INFO_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export INFO_ADMIN_BACKEND_IMAGE_PROJECT="${INFO_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export INFO_ADMIN_BACKEND_IMAGE="${INFO_ADMIN_BACKEND_IMAGE:-}"
export INFO_ADMIN_BACKEND_TAG="${INFO_ADMIN_BACKEND_TAG:-}"
...
export INFO_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${INFO_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
...
if [[ -n "$INFO_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" ||
      -n "$INFO_ADMIN_BACKEND_REDIS_SECRET_NAME" ||
      -n "$INFO_ADMIN_BACKEND_MONGODB_SECRET_NAME" ]]; then
    if [[ -z "$INFO_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" ||
          -z "$INFO_ADMIN_BACKEND_REDIS_SECRET_NAME" ||
          -z "$INFO_ADMIN_BACKEND_MONGODB_SECRET_NAME" ]]; then
        log_error "PostgreSQL、Redis 和 MongoDB Secret 名称必须同时设置"
        exit 1
    fi
    printf -v INFO_ADMIN_BACKEND_DATABASE_ENV_FROM \
      '        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$INFO_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" \
      "$INFO_ADMIN_BACKEND_REDIS_SECRET_NAME" \
      "$INFO_ADMIN_BACKEND_MONGODB_SECRET_NAME"
    export INFO_ADMIN_BACKEND_DATABASE_ENV_FROM
fi
...
main() {
    log_info "开始生成 info-admin-backend YAML 文件..."
