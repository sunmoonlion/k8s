...
> 最后更新：2026-08-12
>
> 本目录（`k8s/sunmoonai/docs/sunmoonai-architecture`，原名“项目总体提示”，曾位于家目录）是 SunMoonAI App Platform 的提示文档集。**三组结构**：`overall-architecture-summarizations/`（App 之间的结构与摘要）+ `apps-architecture-summarizations/`（各 App 的结构与摘要，均时期内稳定、手动刷新）+ `requests/`（需求输入与五环闭环：request→baseline→plan→证据→handoff）。权威链：代码 > 架构基线 > request。

## 文档清单与阅读顺序

| 文档 | 职责 | 何时读 |
| --- | --- | --- |
| `AGENTS.md` | 协作总入口：权威排序、五环链、进度单面、三约定与编辑自检、requests 评审、演进闭环、Git 纪律 | **任何开发者动手前第一份必读** |
...
| `requests/<请求>/`（v6 开写时创建） | 五环闭环三件套：baseline（做成什么样）+ plan-*（按 App 施工清单）+ handoff（进度游标） | 接手某阶段实施时 |

**阅读原则**：改某个项目前，先读 overall-architecture-summarizations/ 两篇中相关章节 + 该项目摘要；跨项目问题（契约、身份、发布）以 overall-architecture-summarizations/ 为准。

## 协作规则（唯一权威：AGENTS.md）

权威排序、五环链、进度单面、三条维护约定与编辑自检、requests 评审流程、演进闭环、
Git 与推送纪律，全部见 [`AGENTS.md`](./AGENTS.md)；本索引不复述（约定 1）。

## 修订记录

- 2026-08-12：`goals/` 改名 `requests/`（单元 = 一次开发任务请求，前缀 REQ-，一个文件夹内五环闭环）；G-001→REQ-001-v6-重写/request.md；AGENTS.md 明确术语：平台级基线 = 两套 summarizations。
