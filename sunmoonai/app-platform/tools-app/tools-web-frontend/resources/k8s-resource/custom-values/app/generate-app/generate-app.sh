#!/bin/bash
# tools-web-frontend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> tools-web-frontend/
...
export TOOLS_WEB_FRONTEND_IMAGE_REGISTRY="${TOOLS_WEB_FRONTEND_IMAGE_REGISTRY:-}"
export TOOLS_WEB_FRONTEND_IMAGE_PROJECT="${TOOLS_WEB_FRONTEND_IMAGE_PROJECT:-}"
export TOOLS_WEB_FRONTEND_IMAGE="${TOOLS_WEB_FRONTEND_IMAGE:-}"
export TOOLS_WEB_FRONTEND_TAG="${TOOLS_WEB_FRONTEND_TAG:-}"
...
export TOOLS_WEB_FRONTEND_IMAGE_PULL_SECRET_NAME="${TOOLS_WEB_FRONTEND_IMAGE_PULL_SECRET_NAME:-}"
export TOOLS_WEB_FRONTEND_SECRET_NAME="${TOOLS_WEB_FRONTEND_SECRET_NAME:-}"
export TOOLS_WEB_FRONTEND_CONFIGMAP_NAME="${TOOLS_WEB_FRONTEND_CONFIGMAP_NAME:-}"
export TOOLS_WEB_FRONTEND_POSTGRESQL_SECRET_NAME="${TOOLS_WEB_FRONTEND_POSTGRESQL_SECRET_NAME:-}"
...
export TOOLS_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME="${TOOLS_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME:-}"
export TOOLS_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM=""
if [[ -n "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" ||
      -n "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME" ]]; then
    if [[ -z "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" ||
          -z "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME" ]]; then
        log_error "对象存储 ConfigMap 和 Secret 名称必须同时设置"
        exit 1
    fi
    printf -v TOOLS_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM \
      '        - configMapRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" \
      "$TOOLS_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME"
    export TOOLS_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM
fi
export TOOLS_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME="${TOOLS_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME:-}"
export TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME="${TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME:-}"
export TOOLS_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM=""
export TOOLS_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT=""
export TOOLS_WEB_FRONTEND_ELASTICSEARCH_VOLUME=""
if [[ -n "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" ||
      -n "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME" ]]; then
    if [[ -z "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" ||
          -z "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME" ]]; then
        log_error "Elasticsearch ConfigMap 和 Secret 名称必须同时设置"
        exit 1
    fi
    printf -v TOOLS_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM \
      '        - configMapRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" \
      "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME"
    printf -v TOOLS_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT \
      '        - name: elasticsearch-ca\n          mountPath: /var/run/secrets/sunmoonai/elasticsearch\n          readOnly: true'
    printf -v TOOLS_WEB_FRONTEND_ELASTICSEARCH_VOLUME \
      '      - name: elasticsearch-ca\n        secret:\n          secretName: %s\n          items:\n          - key: ca.crt\n            path: ca.crt' \
      "$TOOLS_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME"
    export TOOLS_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM
    export TOOLS_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT
...
main() {
    log_info "开始生成 tools-web-frontend YAML 文件..."
