...
# =============================================================================
# Kaniko Registry Secret 部署脚本
# 文件名: deploy-kaniko-registry-secret.sh
# 用途: 部署Kaniko镜像推送Secret到Kubernetes集群
# =============================================================================
...
# Kaniko Registry Secret 配置文件路径
KANIKO_SECRET_CONFIG_FILE="$SCRIPT_DIR/deploy-kaniko-registry-secret.conf"
...
load_config() {
    ...
    if [[ ! -f "$KANIKO_SECRET_CONFIG_FILE" ]]; then
        log_error "Kaniko Registry Secret 配置文件不存在: $KANIKO_SECRET_CONFIG_FILE"
        exit 1
    fi
    
    source "$KANIKO_SECRET_CONFIG_FILE"
    ...
    log_success "✅ Kaniko Registry Secret 配置加载成功"
}

# 部署 Kaniko Registry Secret
deploy_kaniko_registry_secret() {
    local namespace="${1:-cicd-platform-dev}"
    
    log_info "部署 Kaniko Registry Secret..."
    log_info "命名空间: $namespace"
    log_info "Docker服务器: ${DOCKER_SERVER:-harbor.sunmoonai.com:30443}"
    log_info "用户名: $DOCKER_USERNAME"
    
    # 检查密钥是否已存在
    if kubectl get secret "$SECRET_NAME" -n "$namespace" >/dev/null 2>&1; then
        log_success "✅ Kaniko Registry Secret 已存在: $SECRET_NAME"
        log_info "如需更新，请先执行 uninstall 删除现有 Secret"
        return 0
    fi
    
    log_info "创建 Kaniko Registry Secret..."
    ...
    if "${create_cmd[@]}" 2>/dev/null; then
        log_success "✅ Kaniko Registry Secret 创建成功: $SECRET_NAME"
        log_info "Secret 信息："
        ...
        log_error "❌ Kaniko Registry Secret 创建失败"
        return 1
    fi
...
# 删除 Kaniko Registry Secret
delete_kaniko_registry_secret() {
    local namespace="${1:-cicd-platform-dev}"
    
    log_info "删除 Kaniko Registry Secret..."
    log_info "命名空间: $namespace"
    
    if kubectl delete secret "$SECRET_NAME" -n "$namespace" 2>/dev/null; then
        log_success "✅ Kaniko Registry Secret 删除成功"
    else
        log_warn "⚠️ Kaniko Registry Secret 不存在或删除失败"
    fi
...
check_secret_status() {
    ...
    log_info "检查 Kaniko Registry Secret 状态..."
    log_info "命名空间: $namespace"
    
    if kubectl get secret "$SECRET_NAME" -n "$namespace" >/dev/null 2>&1; then
        log_success "✅ Kaniko Registry Secret 存在: $SECRET_NAME"
        ...
        log_error "❌ Kaniko Registry Secret 不存在: $SECRET_NAME"
        return 1
    fi
...
main() {
    ...
    case "$action" in
        "deploy")
            deploy_kaniko_registry_secret "$namespace"
            ;;
        "uninstall")
            delete_kaniko_registry_secret "$namespace"
            ;;
        "status")
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 Kaniko Registry Secret（默认）"
            echo "  uninstall        删除 Kaniko Registry Secret"
