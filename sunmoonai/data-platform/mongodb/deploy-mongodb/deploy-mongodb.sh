    ...
    case "$environment" in
        "development"|"dev")
            if [[ "$cluster_lower" == "kind" ]]; then
                ...
                local pv_pvc_file="$MONGODB_CUSTOM_VALUES_DIR/mongodb-kind-pv-pvc.yaml"
...
show_mongodb_connection_info() {
    ...
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件:"
    echo "   echo '101.126.151.0 ${MONGODB_EXTERNAL_HOST:-www.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 连接 MongoDB:"
