...
>
> - [`dev-agent-task/components/backend/components/02-intake/agent-dev-guide.md`](../dev-agent-task/components/backend/components/02-intake/agent-dev-guide.md)（6 节）
> - [`dev-agent-task/components/backend/components/04-agent-execution/agent-dev-guide.md`](../dev-agent-task/components/backend/components/04-agent-execution/agent-dev-guide.md)（23 节）
> - [`dev-agent-task/components/backend/components/05-interrupt-resume/agent-dev-guide.md`](../dev-agent-task/components/backend/components/05-interrupt-resume/agent-dev-guide.md)（3 节）
> - [`dev-agent-task/components/backend/components/06-acceptance-commit/agent-dev-guide.md`](../dev-agent-task/components/backend/components/06-acceptance-commit/agent-dev-guide.md)（12 节）
...
> - 第 2 步：「执行者的共同纪律」写进 [dev-process 总则](../dev-process/README.md)；「检查本身也必须接受检查」与「没查什么」的通用部分写进 [`dev-process/05-uat`](../dev-process/05-uat/README.md)
>
...
> 2026-09-14：本节通用的写法（查了 / 没查 / 不能排除，三类盲区）已移至 [dev-process/05-uat](../dev-process/05-uat/README.md)；以下是 `runtime-refact` 轮成文时的覆盖声明，作为历史记录原样保留。
...
| runtime-architecture | ### 3.5 修订工作单元（按 `request-lifecycle.md @ baa28858:627` 修订纪律） | §1.1、§4.3；故意不要该单元，因其前提已被代码证伪 |
