...
deploy_app() {
    ...
    if ! check_harbor_image_exists "$INVESTMENT_ADMIN_BACKEND_FULL_IMAGE_NAME" "$NAMESPACE" "${INVESTMENT_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"; then
        return 1
    fi
    ...
    if ! auto_generate_yaml "$INVESTMENT_ADMIN_BACKEND_PVC_YAML" "$K8S_RESOURCE_DIR"; then
        return 1
    fi
