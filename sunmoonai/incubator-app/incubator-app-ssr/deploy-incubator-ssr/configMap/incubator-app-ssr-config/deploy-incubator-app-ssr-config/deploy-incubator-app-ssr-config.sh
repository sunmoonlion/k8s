...
# 计算项目根目录（应用根目录）
# 从 deploy-incubator-app-ssr-config/ 向上 3 级到达应用根目录
# deploy-incubator-app-ssr-config/ -> incubator-app-ssr-config/ -> secrets/ -> deploy-incubator-ssr/ -> incubator-app-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/incubator-app-ssr-config/generate-incubator-app-ssr-config/incubator-app-ssr-config-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/configMap/incubator-app-ssr-config/generate-incubator-app-ssr-config/generate-incubator-app-ssr-config.sh"
...
main() {
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${INCUBATOR_SSR_CONFIGMAP_NAME:-incubator-app-ssr-config}"
