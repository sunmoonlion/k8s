...
5. **界面先用现有网页前端，外加 runtime 自带的本地窗口；网页不直连 runtime。**网页只和后端通信（提交 Task、看进度、Task 级审查）；只能在本机做的事（工具级审批、key 与模型配置、工作区授权、本地 diff）放在 runtime 的本地窗口里。Electron 一体化桌面应用作为后续选项（第 2 节）。
...
| **本地 runtime** | 主动连后端；接收 Attempt 并驱动 Codex；代后端执行推理请求；向 Codex 提供本地 `escalate` 工具；工具级审批的强制点；模型配置与 key 保管；工作区与沙箱策略；上报事件、用量、副作用意图与回执；本地窗口；系统通知 | 不持有四本账权威副本；不自行决定 Task 状态 |
...
| **后端（supervisor）** | 身份与登录会话、设备注册；Task/Attempt/Interaction/Event/Delivery；四本账；router、orchestrator、validator 与 workflow；skills 库；派发网关；推理请求的发起与限额；审计 | 自己不调用生成式模型；不中转模型请求；不接触用户 key |
...
| # | 两端 | 协议 | 内容 | 安全要点 |
| --- | --- | --- | --- | --- |
| ① | runtime ⇄ 后端 | WSS，runtime 主动向外连 | 派发 Attempt（带签名，1.7）、取消、续约、事件与进度回传、副作用意图与回执、审批结论上报、用量上报、厂商预设下发、"有待审查"提醒、推理请求与结果、`escalate` 转交 | 设备私钥认证；协议里不设 key 字段；推理请求只接受允许的用途，受限额约束（1.5） |
...
| `composition/constraints.md` | A1 改写为「一个执行体，按 Profile 与 workflow 区分任务」；新增设备身份条款（① 的认证、派发签名、设备吊销）与推理请求条款（用途、限额、自报）；A3 的说明补"本地暂存回执不是权威副本"；上 Electron 时 I4、I6 补桌面端条款 |
...
| 新增组成部分 | 本地 runtime（连接、策略、审批、记账拦截、模型配置、推理请求执行、本地 `escalate` 工具、本地窗口、执行 Adapter）；知识服务的 MCP 接口；workflow 清单与分类评测集 |
...
| 新仓：本地 runtime | Python；Codex Python SDK；钥匙串；推理请求的单次调用客户端与限额执行；本地 MCP 工具 `escalate`；本地窗口（React 页面 + 轻量壳）；签名校验；独立工作区与经批准的合并；工具级请求升级；本地转换层（可后做） |
