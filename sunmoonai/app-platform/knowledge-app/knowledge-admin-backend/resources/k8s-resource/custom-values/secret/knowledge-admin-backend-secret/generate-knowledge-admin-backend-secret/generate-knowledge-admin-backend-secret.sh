#!/bin/bash
# knowledge-admin-backend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-knowledge-admin-backend-secret.conf"
# 从 generate-knowledge-admin-backend-secret/ -> knowledge-admin-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-knowledge-admin-backend-secret/ -> ... -> k8s-resource/ -> resources/ -> knowledge-admin-backend/
...
main() {
    log_info "开始生成 knowledge-admin-backend Secret YAML 文件..."
