...
# 计算项目根目录（应用根目录）
# 从 deploy-investment-admin-backend-secret/ 向上 3 级到达应用根目录
# deploy-investment-admin-backend-secret/ -> investment-admin-backend-secret/ -> secret/ -> deploy-investment-admin-backend/ -> investment-admin-backend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/investment-admin-backend-secret/generate-investment-admin-backend-secret/generate-investment-admin-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${INVESTMENT_ADMIN_BACKEND_SECRET_NAME:-investment-admin-backend-secret}"
