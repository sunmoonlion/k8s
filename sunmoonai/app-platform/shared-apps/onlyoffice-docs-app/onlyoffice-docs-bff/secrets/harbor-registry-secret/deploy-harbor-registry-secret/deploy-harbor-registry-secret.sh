...
# =============================================================================
# Harbor Registry Secret 部署脚本（ONLYOFFICE Docs）
...
# 计算项目根目录（k8s目录）
# 从 deploy-harbor-registry-secret/ 向上6级到达 k8s/
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> onlyoffice-docs/ -> app-platform/ -> sunmoonai/ -> k8s/
...
# 尝试加载主配置文件（如果存在），以获取 ONLYOFFICE_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-onlyoffice-docs.conf（相对于当前脚本目录）
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (ONLYOFFICE Docs)..."
    log_info "部署参数："
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (ONLYOFFICE Docs)"
    echo ""
    echo "用法:"
