...
# ============================================================================
# Celery Worker 启动脚本（多后端架构）
# ============================================================================
# 说明：
# - 应用代码通过 Init Containers 挂载到 /app/app
# - 从环境变量读取队列列表和并发数
# - 支持监听多个队列
# ============================================================================

echo "=========================================="
echo "启动 Celery Worker（多后端）"
echo "=========================================="

# 检查必要的环境变量
# 注意：这些环境变量应该在 Kubernetes 部署时通过 multi-celeryworker.yaml 设置
...
# 从环境变量读取队列列表（多个队列用逗号分隔）
# 注意：这些环境变量应该在 Kubernetes 部署时通过 multi-celeryworker.yaml 设置
# 如果没有设置，使用合理的默认值（可以根据实际需求调整）
QUEUES="${CELERY_QUEUES:-default-queue}"
CONCURRENCY="${CELERY_CONCURRENCY:-2}"

# 验证队列配置
if [ -z "$QUEUES" ]; then
    echo "❌ 错误: CELERY_QUEUES 未设置且无默认值"
    echo "   请在 Kubernetes Deployment 中设置 CELERY_QUEUES 环境变量"
    exit 1
fi
...
# 启动 Celery Worker，监听多个队列
# 注意：-Q 参数支持逗号分隔的多个队列
echo "启动 Celery Worker..."
echo "命令: hatch run celery -A app.worker worker -l info -Q $QUEUES -c $CONCURRENCY"
echo ""
