...
# 主配置文件路径：从 deploy-ingress/ -> ingress/ -> deploy-llmops-front/ -> app/deploy-app/
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-llmops-front.conf"

# 计算应用根目录和资源路径（在 source 之前计算，避免 SCRIPT_DIR 被覆盖）
# 从 deploy-ingress/ -> ingress/ -> deploy-llmops-front/ -> llmops-app-front/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/llmops-front-ingress/generate-ingress/llmops-app-front-ingress-generated.yaml"
...
check_namespace(){
    ...
    if [[ $? -ne 0 ]]; then
        if echo "$_ns_err4" | grep -qiE "not.?found|NotFound"; then
            ...
            log_warn "kubectl 连接失败，尝试自动重连后重试（${_ns_err4%%$'\n'*}）"
...
apply_file(){
  ...
  if [[ "$yaml_file" == *"-generated.yaml" ]]; then
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/llmops-front-ingress/generate-ingress/generate-ingress.sh"
...
status(){
  ...
  kubectl get ingressroute -n "$NAMESPACE" -l component=llmops-app-front 2>/dev/null || true
  log_info "Middleware:"
  kubectl get middleware -n "$NAMESPACE" -l component=llmops-app-front 2>/dev/null || true
}
