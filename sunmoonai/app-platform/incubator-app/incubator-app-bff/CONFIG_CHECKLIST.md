...
> 本文档用于验证 `incubator-app-bff` 的 ConfigMap 和 Secret 配置是否完整，作为其他应用的配置基准。
...
### ConfigMap 配置（generate-incubator-bff-config.conf）
...
### Secret 配置（generate-incubator-bff-secret.conf）
...
### ConfigMap 模板（incubator-app-bff-config.yaml）

#### ✅ 已包含的字段
- 所有 ConfigMap 配置中的字段都已正确映射到模板中

### Secret 模板（incubator-app-bff-secret.yaml）
...
### ConfigMap 生成脚本（generate-incubator-bff-config.sh）

#### ✅ 已导出的变量
- 所有 ConfigMap 配置中的字段都已正确导出

### Secret 生成脚本（generate-incubator-bff-secret.sh）
...
### ConfigMap 配置优先级
1. ✅ 环境变量（最高优先级）
2. ✅ 主应用的 deploy-incubator-bff.conf（由生成脚本自动读取）
3. ✅ generate-incubator-bff-config.conf 中的默认值（最低优先级）

### Secret 配置优先级
1. ✅ 环境变量（最高优先级）
2. ✅ 主应用的 deploy-incubator-bff.conf（由生成脚本自动读取）
3. ✅ generate-incubator-bff-secret.conf 中的默认值（最低优先级）
...
`incubator-app-bff` 的配置已完整且规范，可以作为其他应用的配置基准。
