...
# 计算项目根目录（应用根目录）
# 从 deploy-tools-web-backend-secret/ 向上 3 级到达应用根目录
# deploy-tools-web-backend-secret/ -> tools-web-backend-secret/ -> secret/ -> deploy-tools-web-backend/ -> tools-web-backend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/tools-web-backend-secret/generate-tools-web-backend-secret/generate-tools-web-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${TOOLS_WEB_BACKEND_SECRET_NAME:-tools-web-backend-secret}"
