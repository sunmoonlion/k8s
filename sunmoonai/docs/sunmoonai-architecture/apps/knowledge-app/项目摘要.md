...
> 基于全量源码阅读（后端约 7000 行 Python、契约 schema、迁移链、前端结构），2026-08-11。
> 仓库路径：`/home/zymun/knowledge-app`

## 1. 定位与领域边界

knowledge-app 是架构 v2 的**知识库域 App**：消费 info-app 分发的不可变 artifact，校验后入库到检索提供方（RAGFlow），并向下游研究消费方提供受契约约束的检索/引用（retrieval/citation）服务。它是 Info→Knowledge→Investment 数据链的中枢。

> 命名注记（权威架构文档 2026-08-09）：旧 `research-app` 已由 `investment-app` 取代，当前检索契约的实际消费方是 investment-app；本仓 manifest 与 service auth 配置中残留的 `research-app`/`sunmoonai-research-knowledge-retrieve` 名称属历史兼容命名，不代表存在一个与 Investment 并列的活动 research-app。

- 拓扑（`docs/ARCHITECTURE-V2-SOURCE-TOPOLOGY.md`）：`knowledge-admin-frontend + knowledge-web-frontend + unified backend`（knowledge-backend，app_slug=knowledge）。
- 父仓 `contracts/` 是 **Knowledge 摄入与检索 provider 契约的唯一可编辑真源**（Info 仓只有消费锁）。
...
### 3.2 retrieval/v1 检索契约

- manifest：contract=`sunmoonai.knowledge.retrieval`，service_consumers=[research-app]、browser_consumers=[research-web-frontend]（均为历史兼容命名，实际消费方已是 investment-app）；request=exact-fields-v1，response 允许加性可选字段，破坏性变更需新 major；消费方必须钉住 manifest digest 并跑 CDC 测试。
...
### 4.1 摄入链（`knowledge_ingestion_service.py` + `ragflow.py`）
...
### 4.2 检索链（`knowledge_retrieval_service.py`）
...
- 端点：`POST /knowledge/ingestions`（admin 提交，202）、`POST /internal/v1/knowledge/ingestions`（Info 服务身份）、`POST /internal/v1/knowledge/retrievals`（Research 服务身份）、`GET /knowledge/ingestions`（多条件过滤列表）、`GET /knowledge/ragflow/config-check`、`GET /knowledge/ingestions/{id}`、`POST .../status`、`.../dispatch`、`.../retry`。

### 4.4 双关系 Service Identity（`service_auth.py`）
...
- retrieve：`sunmoonai-research-knowledge-retrieve`（config.py 默认值仍是旧身份；investment 出站侧已用 `sunmoonai-investment-knowledge-retrieve`，k8s R5 计划宣告身份正式改名，命名迁移两侧未完全收口，部署时以 subject allowlist/env 覆盖为准）、scope `knowledge:retrieve`；各自独立 discovery/backchannel/audience/subject allowlist。
...
## 6. Knowledge 专属配置（core/config.py 625 行）

- 双资源边界：`internal_auth_*`（Info→Knowledge ingest）与 `retrieval_auth_*`（Research→Knowledge retrieve），各含 casdoor_application/discovery/backchannel/audience/subject_allowlist/required_scope。
