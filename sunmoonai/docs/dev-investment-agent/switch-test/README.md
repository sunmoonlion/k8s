...
| **本地机** | 所有者自己的电脑：Windows 机上的 **WSL**，日常测试都在 WSL 里跑；只有 Windows 专项（原生 exec-server）在同一台机的宿主机上跑，可从 WSL 调 `powershell.exe` | 所有者桌上 |
| **远程助手** | 在远程机上工作的 AI 助手，不论是哪个模型、哪个工具；换模型不换工位 | 远程机 |
| **本地助手** | 在本地机上工作的 AI 助手，不论是哪个模型、哪个工具 | 本地机 |
| **所有者** | 人。两台机都能操作；在两个助手之间传话、批「前提」、决定合并 | 两边 |

分工只有一句：**远程助手写，本地助手跑。**远程助手写脚本、推仓、往 `inbox/` 放待办、读结果；本地助手跑待办、把输出写进 `results/`，**每条做完自己在本地提交**；同步和推回是所有者的两个脚本。这是长期规则，每一轮都这样（所有者 2026-09-26 定）。
两个助手互相看不见对话，**只共享 git 分支**；所以一切请求与结果都进 git，对话只传一句话。

⚠ 本文的「本地」指**所有者的本地机**，不是产品文档里的「用户机器 / 本地代理」。做探针时本地机**扮演**用户机器，远程机**扮演**我们的云端。

本目录：`README.md`（本文，约定）、`human-local.sh`（**所有者在本地机上跑，拉：同步仓与副本，列出待办**）、`human-remote.sh`（**所有者在本地机上跑，推：把本地助手已提交的结果推回远程，顺带补提交漏掉的改动与父仓 gitlink**）、`inbox/`（待办，一个文件一条，远程助手写、本地助手做）、`done/`（已办）、`luna-task.md`（所有者交给本地助手的**开发**任务，分工与规矩写在文件里）。测试脚本不放这里，放被测仓。
...
规矩：**「前提」没满足先问**，不猜着跑；**不删、不改、不截输出**；不改代码、不改判据，脚本有问题报出来由远程改；
**待办里的命令本身写错了（参数、路径、拼写、检查方式明显不对），本地助手可以直接改对了接着做**（所有者 2026-09-26 定）——只改"怎么执行"，不改"怎样算通过"；在结果里写明原命令、改后的命令和原因，远程随后改正待办。门禁、判据、产品代码仍不能自己改；拿不准是不是"命令写错"就停下问；`done/` 里的不动；**git 只做 3c 的本地提交**：不 pull、不 fetch、不 rebase、不推、不改分支（同步与推回是所有者那两步）。
