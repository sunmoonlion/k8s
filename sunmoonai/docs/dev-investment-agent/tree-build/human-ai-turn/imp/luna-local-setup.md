...
3. 跑脚本，整份落文件，整份贴回：
   ...
   几百行以内直接贴；长的（界面日志、联调日志）整份提交到被测仓 `luna` 工位的 `test/results/<日期>-<脚本名>.txt`、推 `luna` 分支，
   对话里贴提交号、路径、`exit=` 那一行和最后 40 行。请求的固定格式、脚本放哪、结论怎么写，都在 `~/switch-test/README.md`（2026-09-23 版）。
