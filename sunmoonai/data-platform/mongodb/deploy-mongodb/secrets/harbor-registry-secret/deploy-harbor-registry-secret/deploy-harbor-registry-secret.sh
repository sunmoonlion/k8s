...
# =============================================================================
# Harbor Registry Secret 部署脚本（MongoDB）
# 文件名: deploy-harbor-registry-secret.sh
# 用途: 生成并部署Harbor镜像拉取Secret到Kubernetes集群
# 说明: 与PostgreSQL共享同一个harbor-registry-secret名称和命名空间
# =============================================================================
...
# 尝试加载主配置文件（如果存在），以获取 MONGODB_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-mongodb.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-mongodb.conf"
...
main() {
    ...
    # 说明：secret-core.sh 的 generate_docker_secret_yaml 使用账号密码生成 dockerconfigjson
    ...
    echo ""
    log_success "Harbor Registry Secret (MongoDB) 部署完成！"
    log_info "部署信息："
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (MongoDB)"
    echo ""
    echo "用法:"
