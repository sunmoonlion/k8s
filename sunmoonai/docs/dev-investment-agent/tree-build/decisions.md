...
| `0005-agent` | **`D5`**（本地上限由谁挡）、`D11`（Windows 上的 exec-server） |
...
| D5 | 本地上限由谁挡：Codex 执行端本地配置能否拒绝 harness 要求的更高权限；不能则本地代理自己包一层 | 第一段第一个探针 | 安全模型能否成立 |
...
- **本地上限**：exec-server 一侧能否限定 harness 要求的沙箱模式、根目录与网络（源码有执行端本地配置层与 managed requirements，行为未验）；
