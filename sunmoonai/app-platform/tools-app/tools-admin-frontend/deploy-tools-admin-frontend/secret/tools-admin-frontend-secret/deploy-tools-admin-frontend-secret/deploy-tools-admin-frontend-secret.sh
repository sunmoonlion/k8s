...
# 计算项目根目录（应用根目录）
# 从 deploy-tools-admin-frontend-secret/ 向上 3 级到达应用根目录
# deploy-tools-admin-frontend-secret/ -> tools-admin-frontend-secret/ -> secret/ -> deploy-tools-admin-frontend/ -> tools-admin-frontend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/tools-admin-frontend-secret/generate-tools-admin-frontend-secret/generate-tools-admin-frontend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${TOOLS_ADMIN_FRONTEND_SECRET_NAME:-tools-admin-frontend-secret}"
