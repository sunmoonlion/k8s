...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"investment-web-frontend-pvc-generated.yaml" ]]; then
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${INVESTMENT_WEB_FRONTEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INVESTMENT_WEB_FRONTEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 investment-web-frontend 子组件..."
...
deploy_app() {
    ...
    log_info "镜像: $INVESTMENT_WEB_FRONTEND_FULL_IMAGE_NAME"
    log_warn "⚠️  请确保该镜像已存在于 Harbor 仓库"

    export NAMESPACE="$NAMESPACE" ENV="$ENV" ENVIRONMENT="$ENVIRONMENT"
    export INVESTMENT_WEB_FRONTEND_FULL_IMAGE_NAME
    ...
    auto_generate_yaml "$INVESTMENT_WEB_FRONTEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$INVESTMENT_WEB_FRONTEND_PVC_YAML" || return 1
