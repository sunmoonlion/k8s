...
| B7m 实例渲染覆盖 | 本地源码/工作区回归通过；未部署 | [模板角色引用保留与最终产物测试](v5-backlog-runtime-rendering-luna.md)；补 Worker 发布注解，串行修三个实例覆盖及 Secret 依赖清单，保留领域配置；模板 8、部署脚本最终 23 项通过，真实身份/切换仍待验 |
