...
# 用途: 从 PostgreSQL Secret 获取密码并创建 ONLYOFFICE Docs 的 PostgreSQL 连接 Secret
# =============================================================================
...
# 计算项目根目录（k8s目录）
# 从 deploy-onlyoffice-postgresql-secret/ 向上6级到达 k8s/
# deploy-onlyoffice-postgresql-secret/ -> onlyoffice-postgresql-secret/ -> secrets/ -> onlyoffice-docs/ -> app-platform/ -> sunmoonai/ -> k8s/
...
main() {
    ...
    # 1. 从配置文件获取密码
    log_info "从配置文件获取 PostgreSQL 密码..."
    ...
    if [[ -z "${POSTGRESQL_PASSWORD:-}" ]]; then
        ...
        log_error ""
        log_error "示例："
        log_error "  POSTGRESQL_PASSWORD=\"your_secure_password\""
        exit 1
    fi
    
    local password="$POSTGRESQL_PASSWORD"
    log_success "已从配置文件获取 PostgreSQL 密码"
    log_warn "⚠️  请确保此密码与 PostgreSQL 部署时使用的密码保持一致！"
    ...
    local secret_yaml="$SECRET_DIR/onlyoffice-postgresql-secret.yaml"
    ...
    echo ""
    log_success "ONLYOFFICE PostgreSQL Secret 部署完成！"
    log_info "部署信息："
