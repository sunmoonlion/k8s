...
| 项 | 实际状态 |
| --- | --- |
| **`RunBudget` 在生产生效** | 未接线，见 §4.5。`budget_exceeded` 生产不可达。**已并入 U3**——现有实现是内存态 pydantic model，随 graph state 传递，进程一死即失，结构上满足不了「跨 run／跨进程仍须正确」的判据；须换 PG 载体，见 [`../../dev-plan/open-issues.md`](../../dev-plan/open-issues.md) O3 |
