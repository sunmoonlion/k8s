...
# research-web-backend 部署脚本
# 用法: ./deploy-research-web-backend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 从 app/deploy-app/ 向上 2 级到达 deploy-research-web-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 向上 1 级到达应用根目录
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

RESEARCH_WEB_BACKEND_SCRIPT_DIR="$SCRIPT_DIR"
...
SCRIPT_DIR="$RESEARCH_WEB_BACKEND_SCRIPT_DIR"
...
RESEARCH_WEB_BACKEND_CONFIG_FILE="$SCRIPT_DIR/deploy-research-web-backend.conf"
if [[ -f "$RESEARCH_WEB_BACKEND_CONFIG_FILE" ]]; then
    source "$RESEARCH_WEB_BACKEND_CONFIG_FILE"
    ...
    log_info "已加载 research-web-backend 配置文件: $RESEARCH_WEB_BACKEND_CONFIG_FILE"
else
    log_warn "未找到 research-web-backend 配置文件: $RESEARCH_WEB_BACKEND_CONFIG_FILE，使用默认配置"
fi

DEFAULT_PROJECT_ID="${RESEARCH_WEB_BACKEND_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${RESEARCH_WEB_BACKEND_NAMESPACE:-}"
DEFAULT_ENVIRONMENT="${ENVIRONMENT:-}"

RESOURCES_DIR="$APP_ROOT/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
RESEARCH_WEB_BACKEND_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/research-web-backend-generated.yaml"
RESEARCH_WEB_BACKEND_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/research-web-backend-pvc/generate-research-web-backend-pvc/research-web-backend-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"research-web-backend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/research-web-backend-pvc/generate-research-web-backend-pvc/generate-research-web-backend-pvc.sh"
    else
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    fi

    if [ -f "$generate_script" ]; then
        export NAMESPACE="${RESEARCH_WEB_BACKEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${RESEARCH_WEB_BACKEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 research-web-backend 子组件..."
    ...
    log_success "✅ research-web-backend 子组件部署完成"
    return 0
}

deploy_app() {
    log_info "开始部署 research-web-backend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    auto_generate_yaml "$RESEARCH_WEB_BACKEND_YAML" "$K8S_RESOURCE_DIR" || exit 1

    export RESEARCH_WEB_BACKEND_IMAGE_REGISTRY="${RESEARCH_WEB_BACKEND_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    export RESEARCH_WEB_BACKEND_IMAGE_PROJECT="${RESEARCH_WEB_BACKEND_IMAGE_PROJECT:-app-images}"
    export RESEARCH_WEB_BACKEND_IMAGE="${RESEARCH_WEB_BACKEND_IMAGE:-research-web-backend}"
    export RESEARCH_WEB_BACKEND_TAG="${RESEARCH_WEB_BACKEND_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-Always}"
    export RESEARCH_WEB_BACKEND_IMAGE_PULL_SECRET_NAME="${RESEARCH_WEB_BACKEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"
    export RESEARCH_WEB_BACKEND_FULL_IMAGE_NAME="${RESEARCH_WEB_BACKEND_IMAGE_REGISTRY}/${RESEARCH_WEB_BACKEND_IMAGE_PROJECT}/${RESEARCH_WEB_BACKEND_IMAGE}:${RESEARCH_WEB_BACKEND_TAG}"

    log_info "镜像: $RESEARCH_WEB_BACKEND_FULL_IMAGE_NAME"
    log_warn "⚠️  请确保该镜像已存在于 Harbor 仓库"

    export NAMESPACE="$NAMESPACE" ENV="$ENV" ENVIRONMENT="$ENVIRONMENT"
    export RESEARCH_WEB_BACKEND_FULL_IMAGE_NAME

    log_info "🚀 阶段1：部署子组件..."
    deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false \
        || { log_error "❌ 子组件部署失败"; return 1; }

    log_info "🚀 阶段2：部署 research-web-backend 核心服务..."
    ...
    auto_generate_yaml "$RESEARCH_WEB_BACKEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$RESEARCH_WEB_BACKEND_PVC_YAML" || return 1
    kubectl apply -f "$RESEARCH_WEB_BACKEND_PVC_YAML" -n "$NAMESPACE" \
        && log_success "PVC 部署完成" \
        || { log_error "PVC 部署失败"; return 1; }

    kubectl apply -f "$RESEARCH_WEB_BACKEND_YAML" -n "$NAMESPACE"

    if [ $? -eq 0 ]; then
        log_success "research-web-backend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=research-web-backend"
        echo "  kubectl logs -n $NAMESPACE -l app=research-web-backend -f"
    else
        log_error "research-web-backend 部署失败"
        exit 1
    fi
}

uninstall_app() {
    log_info "开始卸载 research-web-backend..."

    log_info "🚀 阶段1：卸载核心服务..."
    if [ ! -f "$RESEARCH_WEB_BACKEND_YAML" ]; then
        log_warn "生成的 YAML 不存在，直接删除资源"
        kubectl delete deployment research-web-backend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service research-web-backend -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$RESEARCH_WEB_BACKEND_YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    ...
    log_success "research-web-backend 卸载完成！"
}

show_status() {
    log_info "research-web-backend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=research-web-backend 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    log_info "research-web-backend 部署脚本启动"
