...
CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-bff-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-auth-app-bff-config/ 向上 3 级到达应用根目录
# deploy-auth-app-bff-config/ -> auth-app-bff-config/ -> configMap/ -> deploy-auth-app-bff/ -> auth-app-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/auth-app-bff-config/generate-auth-app-bff-config/auth-app-bff-config-generated.yaml"
...
# 默认配置从配置文件读取（deploy-auth-app-bff-config.conf）
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/configMap/auth-app-bff-config/generate-auth-app-bff-config/generate-auth-app-bff-config.sh"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/configMap/auth-app-bff-config/generate-auth-app-bff-config/generate-auth-app-bff-config.sh"
  if [ -f "$generate_script" ]; then
    if bash "$generate_script"; then
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${AUTH_APP_BFF_CONFIGMAP_NAME:-auth-app-bff-config}"
