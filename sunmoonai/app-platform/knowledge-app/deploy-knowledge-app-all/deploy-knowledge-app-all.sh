...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --cluster)
      [[ "${2:-}" == "KIND" ]] || {
        echo "Architecture v2 Knowledge 当前只验收 KIND；收到: ${2:-}" >&2
        exit 2
      }
      shift 2
      ;;
...
case "$ACTION" in
  ...
  uninstall|cleanup)
    echo "拒绝从默认入口删除正式 Knowledge；回滚/清理由 R7 专用流程执行。" >&2
    exit 2
    ;;
esac
...
if [[ ${#POSITIONAL[@]} -ge 2 && "${POSITIONAL[1]}" != "app-platform-dev" ]]; then
  echo "Knowledge formal release namespace 已锁定为 app-platform-dev；收到: ${POSITIONAL[1]}" >&2
  exit 2
fi
