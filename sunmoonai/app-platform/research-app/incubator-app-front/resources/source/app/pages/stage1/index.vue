...
// 获取所有 stage1 文章
const { data: articles } = await useAsyncData('stage1-list', () =>
  queryContent('/stage1')
    .sort({ publishedAt: -1 })
    .find()
)
...
const getArticleLink = (article: any) => {
  ...
  if (article.slug) {
    return `/stage1/${article.slug}`
  }
  
  const path = article._path || ''
  // 提取文件名（去掉 /stage1/ 前缀和 .md 后缀）
  const fileName = path.replace(/^\/stage1\//, '').replace(/\.md$/, '')
  return `/stage1/${fileName}`
}
</script>
