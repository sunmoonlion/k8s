...
# Flower 中间件总控部署脚本
# 统一部署所有 Flower 中间件组件（stripprefix + policy）
...
FLOWER_MIDDLEWARE_CONFIG_FILE="$SCRIPT_DIR/deploy-middleware-all.conf"

# 加载配置
load_config() {
    if [[ ! -f "$FLOWER_MIDDLEWARE_CONFIG_FILE" ]]; then
        log_error "Flower 中间件总控配置文件不存在: $FLOWER_MIDDLEWARE_CONFIG_FILE"
        exit 1
    fi

    source "$FLOWER_MIDDLEWARE_CONFIG_FILE"
    log_success "✅ Flower 中间件总控配置加载成功"
}
...
deploy_all_middleware() {
    ...
    log_info "开始基于优先级的 Flower 中间件组件部署..."
    log_info "命名空间: $namespace"
    
    local components=(
        "stripprefix:${flower_stripprefix_enabled:-true}:${flower_stripprefix_priority:-200}:Flower StripPrefix:$SCRIPT_DIR/../flower-stripprefix/deploy-flower-stripprefix/deploy-flower-stripprefix.sh"
        "policy:${flower_policy_enabled:-true}:${flower_policy_priority:-100}:Flower Policy:$SCRIPT_DIR/../flower-policy/deploy-flower-policy/deploy-flower-policy.sh"
    )
    ...
    log_success "✅ Flower 所有中间件组件部署完成！"
    return 0
}
...
delete_all_middleware() {
    ...
    log_info "开始删除 Flower 所有中间件组件（按优先级逆序）..."
    log_info "命名空间: $namespace"
    ...
    log_success "✅ Flower 所有中间件组件删除完成！"
    return 0
}
...
check_all_middleware_status() {
    ...
    log_success "✅ Flower 所有中间件组件状态检查完成！"
    return 0
}
...
main() {
    ...
    case "$action" in
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 Flower 所有中间件组件（默认）"
            echo "  uninstall        删除 Flower 所有中间件组件"
