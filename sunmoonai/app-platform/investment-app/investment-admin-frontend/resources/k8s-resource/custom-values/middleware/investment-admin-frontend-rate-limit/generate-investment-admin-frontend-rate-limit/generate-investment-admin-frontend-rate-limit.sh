#!/bin/bash
# Investment Admin Frontend Rate Limit Middleware YAML 生成脚本
# 根据配置生成 Rate Limit Middleware 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-investment-admin-frontend-rate-limit.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-investment-admin-frontend-rate-limit/ -> investment-admin-frontend-rate-limit/ -> middleware/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-investment-admin-frontend-rate-limit/ -> investment-admin-frontend-rate-limit/ -> middleware/ -> custom-values/ -> k8s-resource/ -> resources/ -> investment-admin-frontend/
...
# 生成 YAML
main() {
    log_info "开始生成 Investment Admin Frontend Rate Limit Middleware YAML 文件..."
