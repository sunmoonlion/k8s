...
- A2.5 当前施工包已形成模板实现提交 `tpl-admin-frontend@dabd541d59f553a83b23559ac6564a98dcbff1ca`：Vite 生产构建拒绝 `VITE_AUTH_MODE=demo`，Docker 构建强制 typecheck/lint/unit/build，加入静态生产配置门禁和 reduced-motion/键盘焦点 E2E；39 个 Vitest、7 个 Chromium、typecheck、lint、普通构建及 `BASE_PATH=/admin` 构建通过；从该固定 commit 的 clean-room install、verify:production、build 和 7 个 Playwright 也已通过。A2.5 仍未接受：必须由目标环境补齐 Docker/Nginx smoke、镜像 digest 和 KIND/严格 TLS 证据；不得以本地或 clean-room 结果替代这些证据。矩阵已回填至 `tpl-admin-frontend/docs/vue-react-capability-matrix.md`，父仓指针为 `tpl-app@eac78686e819148e5528f00d55c4852e4d56e252`。
