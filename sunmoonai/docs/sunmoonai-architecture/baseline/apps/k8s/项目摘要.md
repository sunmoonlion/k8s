...
> 深读时间：2026-08-12。基于 `/home/zymun/k8s` 全量阅读：`sunmoonai/` 各平台部署编排、
> `sunmoonai/docs/` 文档体系、三个领域 App 的部署 bundle 与 `release-inputs.json`。
...
| 目录 | 内容 |
| --- | --- |
| `app-platform/` | 全部领域 App 的部署声明：auth-app（Casdoor，不套三仓模板）、info-app、knowledge-app、investment-app；`scripts/`（build-push-app-images.sh、verify-architecture-v2-instance.py 通用门禁）；`docs/`（平台文档与历史记录） |
...
平台职责与依赖方向的权威定义见总体架构文档（`baseline/overall/sunmoonai·-architecture.md`）；
本仓实现遵循同一原则：基础能力向上提供、领域所有权不向下泄漏，部署顺序≠运行时耦合。

## 3. 领域 App 部署 bundle 结构（每 App 一份）

五件套 YAML：`00-prerequisites.yaml`（ServiceAccount/Secret 引用/NetworkPolicy）、
...
加 `release.json`、`render-formal.py`、`deploy-formal.py`。通用门禁：
`app-platform/scripts/verify-architecture-v2-instance.py`。
...
每 App 发布输入 `release-inputs.json`（schema_version 1）：stage、formal_release、模板版本、
三镜像 digest（只收 `repository@sha256`）、rollback_baseline、gate 结果。

流程：源码仓构建不可变镜像 digest → 写入 release-inputs.json → k8s-scaffold-v2 render/deploy
→ 门禁通过后晋级正式发布。禁止为正式版本重新构建再打 tag、禁止用可变 tag 替代 digest。
