...
/**
 * Strict-TLS, real-Casdoor pairing verification for P0-007E.
...
function stage(value) {
  process.stderr.write(`P0_007E_BROWSER_STAGE=${value}\n`)
}
...
function prepareStrictTlsHome() {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'sunmoonai-p0-007e-'))
...
async function submitCasdoorLogin(page, identity) {
  ...
  try {
    ...
    const diagnosticPath = '/tmp/p0-007e-casdoor-login.png'
...
async function main() {
  ...
  const frontend = deployment('tpl-admin-frontend-p0-007e', 3000)
  ...
  execFileSync(
    ...
    [
      ...
      'sh',
      '-lc',
      'test "$(id -u)" = 1001 && command -v node >/dev/null && ! touch /app/.p0-007e-write-probe',
    ],
    { stdio: 'ignore' },
  )
