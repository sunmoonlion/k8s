...
# =============================================================================
# MongoDB Auth Secret 部署脚本
# 文件名: deploy-mongodb-auth-secret.sh
# 用途: 生成并部署MongoDB认证Secret到Kubernetes集群
# 说明: MongoDB Chart要求Secret键名使用破折号（如mongodb-root-password）
#       配置中使用下划线变量名，脚本会自动转换为破折号键名
# =============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SECRET_DIR="$(dirname "$SCRIPT_DIR")"  # mongodb-auth-secret 目录
# 计算项目根目录（k8s目录）
...
CONFIG_FILE="$SCRIPT_DIR/deploy-mongodb-auth-secret.conf"
...
main() {
    ...
    log_info "部署 MongoDB Auth Secret..."
    log_info "部署参数："
    ...
    # 1. 准备Opaque Secret数据（MongoDB Chart要求破折号键名）
    ...
    # 从配置中提取数据键，将下划线变量名转换为破折号键名
    # 例如：mongodb_root_password -> mongodb-root-password
    if [[ -n "${mongodb_root_password:-}" ]]; then
        echo -n "${mongodb_root_password}" > "$temp_data_dir/mongodb-root-password"
        log_info "添加数据键: mongodb-root-password"
    fi
    
    if [[ -n "${mongodb_passwords:-}" ]]; then
        echo -n "${mongodb_passwords}" > "$temp_data_dir/mongodb-passwords"
        log_info "添加数据键: mongodb-passwords"
    fi
    
    if [[ -n "${mongodb_metrics_password:-}" ]]; then
        echo -n "${mongodb_metrics_password}" > "$temp_data_dir/mongodb-metrics-password"
        log_info "添加数据键: mongodb-metrics-password"
    fi
    
    if [[ -n "${mongodb_replica_set_key:-}" ]]; then
        echo -n "${mongodb_replica_set_key}" > "$temp_data_dir/mongodb-replica-set-key"
        log_info "添加数据键: mongodb-replica-set-key"
    fi
    
    # 2. 生成Opaque Secret YAML
    local secret_yaml="$SECRET_DIR/mongodb-auth-secret.yaml"
    ...
    if [[ "$dry_run" != "true" ]]; then
        ...
        if ! kubectl cluster-info >/dev/null 2>&1; then
            log_error "kubectl 连接失败，请检查 Kubernetes 连接"
            log_error "KUBECONFIG: ${KUBECONFIG:-未设置}"
            log_error "脚本应由主部署脚本调用，主脚本会设置 Kubernetes 连接"
            exit 1
        fi
    ...
    echo ""
    log_success "MongoDB Auth Secret 部署完成！"
    log_info "部署信息："
...
show_help() {
    ...
    echo ""
    echo "配置文件: $SCRIPT_DIR/deploy-mongodb-auth-secret.conf"
}
