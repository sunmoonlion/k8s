...
# =============================================================================
# PostgreSQL Auth Secret 部署脚本
# 文件名: deploy-postgresql-auth-secret.sh
# 用途: 生成并部署PostgreSQL认证Secret到Kubernetes集群
# 说明: PostgreSQL Chart要求Secret键名使用小写字母
# =============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SECRET_DIR="$(dirname "$SCRIPT_DIR")"  # postgresql-auth-secret 目录
# 计算项目根目录（k8s目录）
...
CONFIG_FILE="$SCRIPT_DIR/deploy-postgresql-auth-secret.conf"
...
main() {
    ...
    log_info "部署 PostgreSQL Auth Secret..."
    log_info "部署参数："
    ...
    # 1. 准备Opaque Secret数据（PostgreSQL Chart要求小写键名）
    ...
    # 从配置中提取数据键（使用小写键名，符合PostgreSQL Chart要求）
    if [[ -n "${admin_password:-}" ]]; then
        echo -n "${admin_password}" > "$temp_data_dir/admin_password"
        log_info "添加数据键: admin_password"
    fi
    
    if [[ -n "${dev_password:-}" ]]; then
        echo -n "${dev_password}" > "$temp_data_dir/dev_password"
        log_info "添加数据键: dev_password"
    fi
    
    # 2. 生成Opaque Secret YAML
    local secret_yaml="$SECRET_DIR/postgresql-auth-secret.yaml"
    ...
    echo ""
    log_success "PostgreSQL Auth Secret 部署完成！"
    log_info "部署信息："
...
# 显示帮助信息
show_help() {
    echo "PostgreSQL Auth Secret 部署脚本"
    echo ""
    echo "用法:"
    ...
    echo ""
    echo "配置文件: $SCRIPT_DIR/deploy-postgresql-auth-secret.conf"
}
