...
CONFIG_FILE="$SCRIPT_DIR/deploy-dc-backend-ns.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-document-converter-backend-namespace/ -> document-converter-backend-namespace/ -> namespace/ -> deploy-document-converter-backend/ -> document-converter-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
NAMESPACE_YAML="$K8S_RESOURCE_DIR/custom-values/namespace/dc-backend-ns/generate-dc-backend-ns/document-converter-backend-namespace-generated.yaml"
...
# 默认配置从配置文件读取（deploy-document-converter-backend-namespace.conf）
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/namespace/dc-backend-ns/generate-dc-backend-ns/generate-dc-backend-ns.sh"
