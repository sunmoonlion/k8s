...
# 项目根目录（deploy-incubator-ssr 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-incubator-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（incubator-app-ssr 目录）
# 从 deploy-incubator-ssr/ 向上 1 级到达 incubator-app-ssr/
...
# 默认配置从配置文件读取（deploy-incubator-ssr.conf）
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（incubator-app-ssr/）
# app/deploy-app/ -> app/ -> deploy-incubator-ssr/ -> incubator-app-ssr/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
INCUBATOR_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/incubator-app-ssr-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"incubator-app-ssr-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
...
check_env_config() {
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/incubator-app-ssr-config/deploy-incubator-app-ssr-config/deploy-incubator-app-ssr-config.sh"
        local secret_script="$secrets_dir/incubator-app-ssr-secret/deploy-incubator-app-ssr-secret/deploy-incubator-app-ssr-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
