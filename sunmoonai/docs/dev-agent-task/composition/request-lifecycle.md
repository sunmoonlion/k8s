...
> 依据的通用规范：[SDD 规则](../../dev-agent-standards/deliverables/sdd/sdd-rules.md)（本文是此前各任务交回后由人整理的定稿，属本任务的 `composition/`；本任务每次派工的任务书在各 turn 的 `task.md` 里）
...
它必须记录 `要求 ID → 所有者 → 代码位置 → 自动测试 → 当前状态`、迁移映射和待实施工作。任务目录（`turns/`）
