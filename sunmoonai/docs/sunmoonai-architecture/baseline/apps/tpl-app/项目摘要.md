...
> 深读时间：2026-08-11。阅读范围：父仓全部治理文档与脚本（CLAUDE/README/build-image/自动注册/init.sh/manifest/verify/两份矩阵/contracts）、tpl-backend 全部 Python 源码（122 文件 5639 行，含 bootstrap/config/interfaces/application/domain/infrastructure/alembic/tests/工具链）、双 Next.js 前端核心源码、k8s-scaffold-v2 全部脚本与模板。仓库路径 `/home/zymun/tpl-app`。

## 1. 定位

**领域 App 母模板**：info/knowledge/investment（未来 research）都从它实例化。本身无业务领域，提供「一个 FastAPI Backend + 两个 Next.js 前端 + K8s 脚手架 + 发布治理」的完整生产骨架。模板发布由 `template-release-manifest.json` 锁定，当前 `formal_release=false`（正式晋级需过门禁）。
...
manifest schema 2：source commit `09f60f7d`/tree `bed9b0bb`；三组件锁 commit/tree/tracked_files/digest；scaffold tree `b004f548`（10 files），deployment_order = prerequisites-secret-network-policy → migration → runtime → ingress；common_file_manifest（git-tree-v1，差异只许 domain-extension/deployment-config/temporary-compatibility，其余 prohibited-drift）；contract_versions（backend_api v1、migration head `20260801_0002`、bundle 1）；identity_policy（shared_backend + 五分离）；database_policy（一 App 一 owner + 四运行 principal）；release_policy（实例同步顺序 info→knowledge→research 冻结、禁 mutable tag、禁覆盖 1.0.0）。verify 脚本逐项核对：schema/架构/formal_release=false/source tree/组件 HEAD=锁定 commit/tree/文件数/镜像不可变/scaffold tree 与计数/同步顺序/1.0.0 保护。

## 8. 使用要点

1. 新 App：新克隆 tpl-app → 改名目录 → `./init.sh <app> <org>` → 先推三子仓再提父仓 gitlink。
2. 公共缺陷必须先修模板、重建镜像、复验门禁，再按固定顺序（info→knowledge→research）串行同步实例；实例差异必须分类登记，prohibited-drift=0 才能写入。
