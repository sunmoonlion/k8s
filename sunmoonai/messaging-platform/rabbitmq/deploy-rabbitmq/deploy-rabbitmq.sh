...
check_rabbitmq_status() {
    ...
    # 检查 Pod 状态
    local pods=$(kubectl get pods -n "$namespace" -l app.kubernetes.io/name=rabbitmq,app.kubernetes.io/instance="rabbitmq-$project_id" -o jsonpath='{.items[*].status.phase}' 2>/dev/null)
    if [[ -z "$pods" ]]; then
    ...
    if [[ "$all_running" == "true" ]]; then
        log_success "✅ 所有 RabbitMQ Pod 运行正常"
        return 0
    else
        log_error "❌ 部分 RabbitMQ Pod 状态异常"
        return 1
    fi
}
