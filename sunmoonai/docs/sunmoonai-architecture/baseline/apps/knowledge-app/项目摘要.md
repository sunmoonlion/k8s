...
> 基于全量源码阅读（后端约 7000 行 Python、契约 schema、迁移链、前端结构），2026-08-13。
> 仓库路径：`/home/zymun/knowledge-app`

## 1. 定位与领域边界

knowledge-app 是**知识库域 App**：消费 info-app 分发的不可变 artifact，校验后入库到检索提供方（RAGFlow），并向下游投资研究消费方（investment-app）提供受契约约束的检索/引用（retrieval/citation）服务。它是 Info→Knowledge→Investment 数据链的中枢。
