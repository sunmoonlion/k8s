...
- 当前推进哪件事、卡在哪、下一动作，由任务目录（`task.md` 各版本、`turns/`、`composition/`、`components/`）推出，不落在脑子里；
...
⚠ **`task.md` 与 `composition/` 是单写者面。**多名执行者并行时只有派工的一方（协调者）写它们，各执行者只交回到自己的 turn，
进度写在**自己的 worktree** 里。判据不是「内容重不重要」，而是
「同一事实只能有一个权威写入者」（`I13`，[§1.2](../../composition/agent-dev-guide.md)）——**多人各自往同一份 `task.md` 追加，
就是在共享路径上并发写**。
