...
**输出就是你的参与方名，此外没有第二个来源。**

三条硬规矩（`rulings.md` `R5`——已经出过一次事故）：

1. **命令跑不出名字（报错、空、或一串 `.`），就停下报告，不要推理。**
   仓外跑会吐垃圾而不是报错——先 `cd` 到 `~/worktrees/<你的名>/k8s`。
2. **产品名、模型名、你在哪个界面里，全都不是身份证据。**
   本轮有两个参与方来自同一厂商的不同产品，任何形如「我是 X 产品所以我是 X 参与方」的
   推理**一律错**——已经有一家据此把自己判成了另一家。
3. **只写你自己路径下的文件。**往别家 worktree 写 = 污染他家候选 + 你自己判未交付。
