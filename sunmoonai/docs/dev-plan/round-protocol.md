...
> 写在该轮自己的 `round.md`（§2）。
>
> **本文的读者既是人也是 agent。**推进一轮的唯一指令是固定的一句话（§0），
> 收到它的执行者靠本文自行定位，不靠发起人逐轮说明。
>
> 规则来自实跑：2026-09-02 的生命周期文档重写（完整流程）、2026-09-03 的一次失败整合
> （失败原因见 §12）、2026-09-04 的 refact 轮（异议轮缺失被发现，见 §12）。
...
| 2 | 现在是哪一轮 | `ls sunmoonai/docs/dev-plan/rounds/` 里 `status: ACTIVE` 的那一个（§2） |
| 3 | 现在是第几环节 | 跑 §5 的判定命令，**从产物反推，不看任何人的声明** |
| 4 | 我这一环节做什么 | 读 `rounds/<round-id>/call-<当前环节>.md` 里**属于你的那一节**；它自足，不需要额外指令 |

**第 4 问的答案永远在一个路径可推导的文件里，这就是「固定指令」能成立的原因。**
...
声明会和事实漂移。判据一律是 §5 的命令，对照 git 提交，不看工作区、不看谁说了什么。
...
2026-09-03 那次失败整合正是这么丢掉了基座候选独有的锚点（§12）。
...
**不得由 §0 那句固定指令推进**，也不得由任何 agent 代行。人确认前不得写入共享最终路径。
...
- **引用本协议按标题，不按章节号。**章节会移动，标题不会。本仓已有同源约定：
  ...
  起草本节时插入一节导致 §4 之后全部后移，当场就有两处内部引用失效，
