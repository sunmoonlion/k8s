...
# Investment Web Backend 部署脚本
# 用法: ./deploy-investment-web-backend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-investment-web-backend 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-investment-web-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（investment-web-backend 目录）
# 从 deploy-investment-web-backend/ 向上 1 级到达 investment-web-backend/
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

# 保存 Investment Web Backend 脚本的目录路径（统一部署模板会重新定义 SCRIPT_DIR）
INVESTMENT_WEB_BACKEND_SCRIPT_DIR="$SCRIPT_DIR"
...
# 加载部署配置文件
INVESTMENT_WEB_BACKEND_CONFIG_FILE="$SCRIPT_DIR/deploy-investment-web-backend.conf"
if [[ -f "$INVESTMENT_WEB_BACKEND_CONFIG_FILE" ]]; then
    source "$INVESTMENT_WEB_BACKEND_CONFIG_FILE"
    ...
    log_info "已加载 Investment Web Backend 配置文件: $INVESTMENT_WEB_BACKEND_CONFIG_FILE"
else
    log_warn "未找到 Investment Web Backend 配置文件: $INVESTMENT_WEB_BACKEND_CONFIG_FILE，使用默认配置"
fi

# 默认配置从配置文件读取（deploy-investment-web-backend.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${INVESTMENT_WEB_BACKEND_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${INVESTMENT_WEB_BACKEND_NAMESPACE:-}"
...
deploy_sub_components() {
    ...
    log_info "开始部署 Investment Web Backend 子组件..."
    ...
    log_success "✅ Investment Web Backend 子组件部署完成"
    return 0
}

# 镜像配置
INVESTMENT_WEB_BACKEND_IMAGE="${INVESTMENT_WEB_BACKEND_IMAGE:-}"
...
INVESTMENT_WEB_BACKEND_CONFIGMAP="${TEMPLATES_DIR}/configMap/investment-web-backend-config.yaml"
INVESTMENT_WEB_BACKEND_SECRET="${TEMPLATES_DIR}/secret/investment-web-backend-secret.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"investment-web-backend-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"investment-web-backend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/investment-web-backend-pvc/generate-investment-web-backend-pvc/generate-investment-web-backend-pvc.sh"
    else
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${INVESTMENT_WEB_BACKEND_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INVESTMENT_WEB_BACKEND_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    if ! auto_generate_yaml "$INVESTMENT_WEB_BACKEND_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/investment-web-backend-config/deploy-investment-web-backend-config/deploy-investment-web-backend-config.sh"
        local secret_script="$secrets_dir/investment-web-backend-secret/deploy-investment-web-backend-secret/deploy-investment-web-backend-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
run_db_access_bootstrap() {
    if [[ "${INVESTMENT_WEB_BACKEND_DB_BOOTSTRAP_ENABLED:-true}" != "true" ]]; then
        log_info "⏭️  跳过 Investment Web Backend DB bootstrap (已禁用)"
        return 0
    fi

    local source_root="${INVESTMENT_WEB_BACKEND_SOURCE_ROOT:-${HOME}/investment-app/investment-web-backend}"
    ...
    if [[ ! -x "$bootstrap_script" ]]; then
        ...
        log_error "请设置 INVESTMENT_WEB_BACKEND_SOURCE_ROOT 指向 investment-web-backend 源码目录"
        return 1
    fi

    log_info "开始执行 Investment Web Backend DB bootstrap (namespace=${NAMESPACE})..."
    ...
    "$bootstrap_script"
    log_success "✅ Investment Web Backend DB bootstrap 完成"
}

# 部署 Investment Web Backend
deploy_app() {
    log_info "开始部署 Investment Web Backend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    check_env_config

    export INVESTMENT_WEB_BACKEND_IMAGE_REGISTRY="${INVESTMENT_WEB_BACKEND_IMAGE_REGISTRY:-harbor.sunmoonai.com}"
    export INVESTMENT_WEB_BACKEND_IMAGE_PROJECT="${INVESTMENT_WEB_BACKEND_IMAGE_PROJECT:-app-images}"
    export INVESTMENT_WEB_BACKEND_IMAGE="${INVESTMENT_WEB_BACKEND_IMAGE:-investment-web-backend}"
    export INVESTMENT_WEB_BACKEND_TAG="${INVESTMENT_WEB_BACKEND_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-Always}"
    export INVESTMENT_WEB_BACKEND_IMAGE_PULL_SECRET_NAME="${INVESTMENT_WEB_BACKEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"

    INVESTMENT_WEB_BACKEND_FULL_IMAGE_NAME="${INVESTMENT_WEB_BACKEND_IMAGE_REGISTRY}/${INVESTMENT_WEB_BACKEND_IMAGE_PROJECT}/${INVESTMENT_WEB_BACKEND_IMAGE}:${INVESTMENT_WEB_BACKEND_TAG}"
    log_info "使用 Harbor 镜像部署: $INVESTMENT_WEB_BACKEND_FULL_IMAGE_NAME"
    ...
    export INVESTMENT_WEB_BACKEND_FULL_IMAGE_NAME="$INVESTMENT_WEB_BACKEND_FULL_IMAGE_NAME"

    if ! run_db_access_bootstrap; then
        log_error "❌ Investment Web Backend DB bootstrap 失败"
        return 1
    fi

    log_info "🚀 阶段1：部署 Investment Web Backend 子级组件..."
    if ! deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_error "❌ Investment Web Backend 子级组件部署失败！"
        return 1
    fi
    ...
    if ! auto_generate_yaml "$INVESTMENT_WEB_BACKEND_PVC_YAML" "$K8S_RESOURCE_DIR"; then
        return 1
    fi

    if [ -f "$INVESTMENT_WEB_BACKEND_PVC_YAML" ]; then
        log_info "部署 PVC..."
        kubectl apply -f "$INVESTMENT_WEB_BACKEND_PVC_YAML" -n "$NAMESPACE"
        if [ $? -eq 0 ]; then
    ...
    kubectl apply -f "$INVESTMENT_WEB_BACKEND_YAML" -n "$NAMESPACE"

    if [ $? -eq 0 ]; then
        log_success "Investment Web Backend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=investment-web-backend"
        echo "  kubectl get svc -n $NAMESPACE -l app=investment-web-backend"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=investment-web-backend -f"
    else
        log_error "Investment Web Backend 部署失败"
        exit 1
    fi
}

# 卸载 Investment Web Backend
uninstall_app() {
    log_info "开始卸载 Investment Web Backend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    check_env_config

    log_info "🚀 阶段1：卸载 Investment Web Backend 核心服务..."
    if [ ! -f "$INVESTMENT_WEB_BACKEND_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $INVESTMENT_WEB_BACKEND_YAML，尝试直接删除资源"
        kubectl delete deployment investment-web-backend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service investment-web-backend -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$INVESTMENT_WEB_BACKEND_YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    log_success "✅ Investment Web Backend 核心服务卸载完成"

    log_info "🚀 阶段2：卸载 Investment Web Backend 子级组件..."
    if ! uninstall_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_warn "⚠️ Investment Web Backend 子级组件卸载部分失败，继续..."
    fi
    log_success "✅ Investment Web Backend 子级组件卸载完成"

    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        log_warn "⚠️  注意：卸载 Namespace 将删除命名空间 $NAMESPACE 及其中的所有资源！"
        log_info "如需卸载 Namespace，请手动执行："
        log_info "  cd deploy-investment-web-backend/namespace/investment-web-backend-namespace/deploy-investment-web-backend-namespace"
        log_info "  ./deploy-investment-web-backend-namespace.sh uninstall $PROJECT_ID $NAMESPACE $ENVIRONMENT"
    fi

    log_success "Investment Web Backend 卸载完成！"
}
...
uninstall_sub_components() {
    ...
    log_info "开始卸载 Investment Web Backend 子组件..."

    local sub_components=(
        "investment_web_backend_middleware:${middleware_enabled:-false}:${middleware_priority:-1000}:Investment Web Backend 中间件:$PROJECT_ROOT/middleware/deploy-middleware-all/deploy-middleware-all.sh"
        "investment_web_backend_ingress:${ingress_enabled:-false}:${ingress_priority:-100}:Investment Web Backend HTTP 路由:$PROJECT_ROOT/ingress/deploy-ingress/deploy-ingress.sh"
    )
    ...
    log_success "✅ Investment Web Backend 子组件卸载完成"
    return 0
}

# 显示状态
show_status() {
    log_info "Investment Web Backend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=investment-web-backend 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    log_info "Investment Web Backend 部署脚本启动"
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "操作:"
            echo "  deploy     部署 Investment Web Backend"
            echo "  uninstall  卸载 Investment Web Backend"
            echo "  status     查看 Investment Web Backend 状态"
            echo ""
            echo "示例:"
