...
| `baseline/apps/k8s/项目摘要.md` | 部署编排仓：各平台部署声明、领域 App 部署 bundle、发布输入与验收门禁 | 涉及部署、门禁、发布时 |
...
| `requests/REQ-<编号>-<短名>/` | 闭环产物（按需产生）：baseline.md（目标态）+ plan-*（开发任务）+ handoff.md（进度游标） | 接手某请求实施时 |
