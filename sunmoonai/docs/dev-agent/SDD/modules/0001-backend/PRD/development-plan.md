...
旧 v5 计划及实施计划已退出当前目录；完整原文见
[历史 Git 索引](../../../../../legacy-backlog/verification-index.md)。它们仅是历史设计输入。

理由是它的重心不对：主体是前后端对接（§10 消息/事件/投影/流式 200 行，
而 §14 多智能体架构只有 23 行；实施计划 17 个前端任务对 14 个 Runtime 任务，
多智能体只有一个"薄切"且 `NOT_STARTED`）。

其中需延续的交互目标已收拢到 [旧任务待接收清单](../../../../../legacy-backlog/README.md)，
待新架构讨论后逐项接收，不据旧计划直接派工，也不以其作为智能体架构依据。
...
| | 通用部分 | 专用部分 |
| --- | --- | --- |
| 内容 | 执行编排：多方竞争、隔离、中断恢复、审批、工具协议 | 领域：提示、工具绑定、权限、memory policy、领域约束 |
| 载体 | 执行运行时 + [`competition-protocol.md`](../../../../../dev-human/protocol/competition-protocol.md) | `AgentProfile` |
