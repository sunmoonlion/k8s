...
> 深读时间：2026-08-13。仓库路径 `/home/zymun/tpl-app`。

## 1. 定位

**领域 App 母模板**：info/knowledge/investment（未来 research）都从它实例化。本身无业务领域，提供「一个 FastAPI Backend + 两个 Next.js 前端 + K8s 部署脚手架 + 发布治理」的完整生产骨架。模板发布由 `template-release-manifest.json` 锁定，正式版 **2.0.0**（`status=FORMAL_RELEASE`、`formal_release=true`，晋级方式 exact-digest-alias，观察窗口关闭）。
...
2. 公共缺陷必须先修模板、重建镜像、复验门禁，再按固定顺序（info→knowledge→investment）串行同步实例；实例差异必须分类登记，prohibited-drift=0 才能写入。
3. 部署：scaffold.py 渲染（三镜像 digest）→ deploy.py apply（secret env 文件）→ release.json 登记 → Migration Job 成功后才 rollout API/Worker。
4. 本地验证：backend `uv sync --frozen && ruff && pyright && pytest`；scaffold `python3 -m unittest discover -s k8s-deployment/tests` + `python3 verify_template_release.py`；前端 `pnpm check`（typecheck+lint+i18n+test+build）。
