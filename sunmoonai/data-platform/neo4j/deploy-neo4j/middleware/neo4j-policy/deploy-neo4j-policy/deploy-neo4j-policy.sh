...
NEO4J_POLICY_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# 计算项目根目录（k8s目录）
# 从 deploy-neo4j-policy/ -> neo4j-policy/ -> middleware/ -> deploy-neo4j/ -> neo4j/ -> data-platform/ -> sunmoonai/ -> k8s/
PROJECT_ROOT="$(cd "$NEO4J_POLICY_SCRIPT_DIR/../../../../../../.." && pwd)"

# 导入统一部署模板（提供日志函数和 Kubernetes 连接函数）
source "$PROJECT_ROOT/utils/unified-deployment-template.sh"

# 恢复 Neo4j 策略脚本的目录路径
SCRIPT_DIR="$NEO4J_POLICY_SCRIPT_DIR"

# 配置文件路径
NEO4J_POLICY_CONFIG_FILE="$NEO4J_POLICY_SCRIPT_DIR/deploy-neo4j-policy.conf"
...
load_config() {
    # 使用保存的脚本目录确保路径正确
    local config_file="${NEO4J_POLICY_CONFIG_FILE:-$NEO4J_POLICY_SCRIPT_DIR/deploy-neo4j-policy.conf}"
    
    if [[ ! -f "$config_file" ]]; then
        log_error "Neo4j 策略配置文件不存在: $config_file"
        exit 1
    fi
    ...
    if [[ -n "${NEO4J_POLICY_FILE_RELATIVE:-}" ]]; then
        # 相对路径转换为绝对路径（相对于脚本目录）
        NEO4J_POLICY_FILE="$(cd "$NEO4J_POLICY_SCRIPT_DIR" && cd "$(dirname "$NEO4J_POLICY_FILE_RELATIVE")" && pwd)/$(basename "$NEO4J_POLICY_FILE_RELATIVE")"
    elif [[ -z "${NEO4J_POLICY_FILE:-}" ]]; then
        # 如果没有配置，使用默认路径
        NEO4J_POLICY_FILE="$(cd "$NEO4J_POLICY_SCRIPT_DIR/.." && pwd)/neo4j-policy.yaml"
    fi
    # 展开 ~ 符号
    NEO4J_POLICY_FILE="${NEO4J_POLICY_FILE/#\~/$HOME}"
...
delete_neo4j_policy() {
    ...
    if kubectl delete -f "$policy_file" 2>/dev/null; then
        log_success "✅ Neo4j 策略配置删除成功"
    else
        log_warn "⚠️ Neo4j 策略配置不存在或删除失败"
    fi
    
    log_success "✅ Neo4j 策略中间件删除完成！"
    return 0
}
...
main() {
    ...
    case "$action" in
        "deploy")
            deploy_neo4j_policy "$namespace"
            ;;
        "uninstall")
            delete_neo4j_policy "$namespace"
            ;;
        "status")
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 Neo4j 策略中间件（默认）"
            echo "  uninstall        删除 Neo4j 策略中间件"
            echo "  status           检查中间件状态"
            echo "  help             显示此帮助信息"
            echo ""
            echo "参数:"
