...
process_flower_values() {
    ...
    if [[ -f "$env_values_file" ]]; then
        ...
        sed -i "s/{{FLOWER_UNIFIED_HOST}}/${FLOWER_UNIFIED_HOST:-www.sunmoonai.com}/g" "$flower_values_file"
...
show_flower_connection_info() {
    ...
    echo "=== Flower 连接信息 ==="
    echo "命名空间: $namespace"
    echo "外部访问: https://${FLOWER_UNIFIED_HOST:-www.sunmoonai.com}/flower"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件（如果需要）:"
    echo "   echo '${FLOWER_NODE_IP:-115.190.153.150} ${FLOWER_UNIFIED_HOST:-www.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 访问 Flower:"
    echo "   https://${FLOWER_UNIFIED_HOST:-www.sunmoonai.com}/flower"
    echo ""
    echo "3. 查看服务状态:"
