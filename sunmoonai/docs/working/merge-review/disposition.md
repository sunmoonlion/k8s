...
## kimi 复核与 luna 最终声明的处置

### kimi 复核（[`kimi-followup.md`](kimi-followup.md)）

kimi fetch 后重跑 A1 五项，**全部证伪自己**，撤回 A1 与 A2，"不可合并"的阻断解除。
它对自身验收表的回跑 8 条中 7 条通过，第 4 条标"失效，处置可接受"，
并留了一行观察：**路径存在性检查从此无机制兜底、只剩纪律**。该观察成立，已并入下面 3.1 的处置。

kimi 给流程的建议已采纳：**跨仓改动的"了结"声明必须带「仓 + 提交号」**，
否则评审方只能猜取证对象。已写进 `../../dev-plan/constraints.md`。

### luna 最终声明（[`luna-final-statement.md`](luna-final-statement.md)）四个证据面

| # | 意见 | 处置 |
| --- | --- | --- |
| 3.1 | 删脚本的推导过头：不同工作区产生不同结果不反常，**不报告输入却给确定结论**才不可信 | **接受批评，仍选删除**，但按其选项 2 明确声明——见下 |
| 3.2 | 组件 hash 成立，但缺父仓 → gitlink → 组件的链条 | **采纳**，已补链条表并逐条验证 |
| 3.3 | 「登记下一轮」写在会被删的目录里等于没登记 | **属实，已改**：移入 `../../dev-plan/handoff.md`「文档面待办」D1/D2，带验收条件 |
| 3.4 | 表格公布的是验收**状态**，不是"三份已通过" | **采纳**，标题改为「验收状态」并给总判定 |

#### 3.1 的明确声明

luna 说得对：我把"结论取决于工作区状态"直接推成"不可信"，跳过了中间一步——
真正的问题是**旧脚本没把前置条件建模**（父仓在否、子模块初始化否、是否 fetch），
于是把"无法扫描"误译成"路径不存在"，或在根本没扫描时输出"全部通过"。

**仍然选择删除，并按 luna 的选项 2 声明清楚：**

> **本文档集当前没有自动文档完整性检查。**坏链、空链接、代码路径存在性、
> 取证时点保鲜，全部依赖人工与评审。

理由不是"检查没用"，而是**要人记得跑、又没接进任何会自动执行的工作流的脚本，
不构成门禁**（`../doc-conventions.md` §4）。恢复一个三态检查器只有在它能接进
自动执行的位置时才值得——目前没有那个位置。

kimi 的观察一并记下：**这意味着路径存在性从此只剩纪律兜底**。

#### 3.2 交付链

| 父仓 | `origin/opus` | backend gitlink | 更新该 gitlink 的父仓提交 |
| --- | --- | --- | --- |
| tpl-app | `0dec444` | `4c93f9a` | `0dec444` |
| info-app | `fadeeeb` | `333b9f3` | `fadeeeb` |
| knowledge-app | `120cc68` | `e6d9d33` | `120cc68` |
| investment-app | `2cab3c0` | `4bac58e` | `2cab3c0` |

上表所有证据 hash **均已被对应 gitlink 锁定**（11 项逐条验证）：

```bash
git -C <app>-app/<app>-backend merge-base --is-ancestor <证据hash> \
  $(git -C <app>-app rev-parse origin/opus:<app>-backend) && echo 已锁定
```
