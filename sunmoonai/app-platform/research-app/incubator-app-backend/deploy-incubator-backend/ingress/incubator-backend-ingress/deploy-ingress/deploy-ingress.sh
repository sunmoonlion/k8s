...
# 从 deploy-ingress/ -> incubator-backend-ingress/ -> ingress/ -> deploy-incubator-backend/ -> incubator-app-backend/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/incubator-backend-ingress/generate-ingress/incubator-app-backend-ingress-generated.yaml"
...
load_config() {
    ...
    # 主配置文件路径：从 deploy-ingress/ -> incubator-backend-ingress/ -> ingress/ -> deploy-incubator-backend/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-incubator-backend.conf"
    ...
    SERVICE_NAME="incubator-app-backend"
...
deploy_http_route() {
    ...
    if [[ ! -f "$INGRESS_FILE" ]]; then
        ...
        local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/incubator-backend-ingress/generate-ingress/generate-ingress.sh"
    ...
    kubectl get ingressroute -n "$namespace" incubator-app-backend-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" incubator-app-backend-stripprefix 2>/dev/null || log_warn "Middleware 尚未创建"
...
check_deployment_status() {
    ...
    kubectl get ingressroute -n "$NAMESPACE" -l component=incubator-app-backend 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=incubator-app-backend 2>/dev/null || echo "Middleware 不存在"
