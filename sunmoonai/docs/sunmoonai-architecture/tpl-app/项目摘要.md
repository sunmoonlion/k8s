...
> 全量重读后重写，2026-08-11。阅读范围：父仓全部治理文档与脚本（CLAUDE/README/build-image/自动注册/init.sh/manifest/verify/两份矩阵/contracts）、tpl-backend 全部 Python 源码（122 文件 5639 行，含 bootstrap/config/interfaces/application/domain/infrastructure/alembic/tests/工具链）、双 Next.js 前端核心源码、k8s-scaffold-v2 全部脚本与模板。仓库路径 `/home/zymun/tpl-app`。

## 1. 定位

Architecture v2 的**领域 App 母模板**：info/knowledge/investment（未来 research）都从它实例化。本身无业务领域，提供「一个 FastAPI Backend + 两个 Next.js 前端 + K8s 脚手架 + 发布治理」的完整生产骨架。模板 release = `architecture-v2-r3.2-20260808`，状态 `R3_REVALIDATED_R4_SOURCE`，`formal_release=false`（R7 前不晋级 2.0.0）。

## 2. 仓库拓扑与配对矩阵

六个 git 子模块 + 父仓治理文件。默认三组件（当前锁定的 commit）：

| 组件 | commit | 镜像 digest | 说明 |
| --- | --- | --- | --- |
| tpl-backend | `8997e9e9`（146 files） | `sha256:131793e2...` | FastAPI 3.12，uv 管理，version 2.0.0.dev0 |
...
参考件不进默认链：tpl-admin-frontend-react/vue（REFERENCE_ONLY）、tpl-web-backend-nest（OPTIONAL）；旧 FastAPI tpl-web-backend 已解除活动拓扑（manifest 记为 ROLLBACK_ONLY `289f2c46`）。`frontend-pairing-matrix.json` 五个 pair（两个 DEFAULT、两个 REFERENCE_ONLY、一个 OPTIONAL）；`frontend-capability-matrix.json` 规定 COMMON 能力必须 Admin/Web 各自独立实现并各自测试（identity-session-csrf / api-error-correlation / query-mutation-boundary / table-form / audited-action-upload-download / safe-rich-chart-progress / theme-locale），ADMIN_ONLY=shell 菜单，WEB_ONLY=public-seo-ssr-cache 与 run-stream-hitl-citation，electron/pwa DEFERRED。

## 3. init.sh 实例化（一次性原地转换）

拒绝在名为 `tpl-app` 的目录或脏工作树执行；APP_NAME 必须 `^[a-z][a-z0-9-]*$` 且不为 tpl。步骤：三活动组件全文本替换 tpl→app 标识并改名路径 → react/vue/nest `submodule deinit + git rm` → 重写 `.gitmodules` 指向 `github.com/{org}/{app}-*` → `git mv` 三组件 → uv.lock 仅改 package name（不做通用扫描）→ 修复 `.git/modules` 目录与 config → 配置 submodule url。推送顺序永远是子仓先推、父仓后提交 gitlink。公共能力后续变更必须先修模板过门禁再串行同步，禁止重跑 init.sh 覆盖领域代码。

## 4. tpl-backend：四角色一镜像
...
### 4.4 Web 交互契约（web-interaction v1）
...
## 5. 双前端（Next.js 16.2 / React 19 / pnpm 10.24 / node ≥24.18）
...
> 注记：权威架构文档 app-platform-architecture.md（原名 overall-architecture.md）曾把 admin-frontend 描述为 Ant Design，已于 2026-08-12 更正为 shadcn/@base-ui；技术栈细节始终以代码为准。
...
## 6. k8s-scaffold-v2（238+266 行两脚本 + 五模板）

`scaffold.py`：渲染五件套（00-prerequisites/10-migration/20-runtime/30-network-policies/40-ingress）。校验：镜像必须 `repo@sha256:64hex`、DNS label、strict origin、app+release-id 生成的 Job 名 ≤63、副本 1-20、输出目录必须为空、渲染后拒绝残留 `__TOKEN__`。默认值：client_id `sunmoonai-{app}-{surface}`、casdoor namespace `app-platform-dev`、image-pull `harbor-registry-secret`、ingress namespace `kube-system`（traefik pod label）。产物：required-secret-keys.txt 十键（ADMIN/WEB_CASDOOR_CLIENT_SECRET、API/WORKER/SCHEDULER_CELERY_BROKER_URL、API/MIGRATION/SCHEDULER/WORKER_DATABASE_URL、REDIS_PASSWORD）+ optional 两键（WORKER_CELERY_RESULT_BACKEND、WORKER_DOWNSTREAM_CLIENT_SECRET）+ release.json（schema 1、secret `{app}-backend-runtime`、每文件 sha256、formal_release:false）。`deploy.py`：plan/apply/cleanup；apply 强制 `--secret-env-file`（0600 env 注入 Secret）；cleanup 按 selector `sunmoonai.com/managed-by=architecture-v2`。runtime 模板：非 root 1001、seccomp RuntimeDefault、automountServiceAccountToken:false、topologySpread、RollingUpdate maxUnavailable:0；API 只拿 API_* Secret 键，角色间 Secret 键隔离。

## 7. 发布治理（template-release-manifest.json + verify_template_release.py）

manifest schema 2：source commit `09f60f7d`/tree `bed9b0bb`（branch architecture-v2，pre-refactor tag 20260801）；三组件锁 commit/tree/tracked_files/digest；scaffold tree `b004f548`（10 files），deployment_order = prerequisites-secret-network-policy → migration → runtime → ingress；common_file_manifest（git-tree-v1，差异只许 domain-extension/deployment-config/temporary-compatibility，其余 prohibited-drift）；contract_versions（backend_api v1、migration head `20260801_0002`、bundle 1）；identity_policy（shared_backend + 五分离）；database_policy（一 App 一 owner + 四运行 principal）；release_policy（R4 顺序 info→knowledge→research 冻结、禁 mutable tag、禁覆盖 1.0.0、R7 前不晋级）。verify 脚本逐项核对：schema/架构/formal_release=false/source tree/组件 HEAD=锁定 commit/tree/文件数/镜像不可变/scaffold tree 与计数/R4 顺序/1.0.0 保护。

## 8. 使用要点

1. 新 App：新克隆 tpl-app（architecture-v2 分支）→ 改名目录 → `./init.sh <app> <org>` → 先推三子仓再提父仓 gitlink。
2. 公共缺陷必须先修模板、重建镜像、复验门禁，再按 R4 串行同步实例；实例差异必须分类登记，prohibited-drift=0 才能写入。
3. 部署：scaffold.py 渲染（三镜像 digest）→ deploy.py apply（secret env 文件）→ release.json 登记 → Migration Job 成功后才 rollout API/Worker。
4. 本地验证：backend `uv sync --frozen && ruff && pyright && pytest`；scaffold `python3 -m unittest discover -s k8s-scaffold-v2/tests` + `python3 verify_template_release.py`；前端 `pnpm check`（typecheck+lint+i18n+test+build）。
