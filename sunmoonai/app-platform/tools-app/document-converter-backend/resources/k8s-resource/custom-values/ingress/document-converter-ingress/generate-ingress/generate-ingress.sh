#!/bin/bash
# Document Converter BFF Ingress YAML 生成脚本
...
# 从 generate-ingress/ -> document-converter-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-ingress/ -> document-converter-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/ -> resources/ -> document-converter-backend/
...
# 生成 YAML
main() {
    log_info "开始生成 Document Converter BFF Ingress YAML 文件..."
    ...
    if [ "${USE_STRIP_PREFIX:-false}" = "true" ]; then
        # 使用 stripprefix 中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: document-converter-stripprefix' "$full_output_path"
    elif [ "${USE_RATE_LIMIT:-true}" = "true" ]; then
        # 使用限流中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: document-converter-rate-limit' "$full_output_path"
    else
