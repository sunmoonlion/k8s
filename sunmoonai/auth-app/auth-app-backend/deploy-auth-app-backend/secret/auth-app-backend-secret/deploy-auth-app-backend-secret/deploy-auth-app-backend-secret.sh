...
CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-backend-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-auth-app-backend-secret/ 向上 3 级到达应用根目录
# deploy-auth-app-backend-secret/ -> auth-app-backend-secret/ -> secret/ -> deploy-auth-app-backend/ -> auth-app-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/auth-app-backend-secret/generate-auth-app-backend-secret/auth-app-backend-secret-generated.yaml"
...
# 默认配置从配置文件读取（deploy-auth-app-backend-secret.conf）
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/secret/auth-app-backend-secret/generate-auth-app-backend-secret/generate-auth-app-backend-secret.sh"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/auth-app-backend-secret/generate-auth-app-backend-secret/generate-auth-app-backend-secret.sh"
  if [ -f "$generate_script" ]; then
    if bash "$generate_script"; then
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${AUTH_APP_BFF_SECRET_NAME:-auth-app-backend-secret}"
