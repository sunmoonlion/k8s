...
# Redisinsight 中间件总控部署脚本
# 统一部署所有 Redisinsight 中间件组件（stripprefix + policy）
...
REDISINSIGHT_MIDDLEWARE_CONFIG_FILE="$SCRIPT_DIR/deploy-middleware-all.conf"

# 加载配置
load_config() {
    if [[ ! -f "$REDISINSIGHT_MIDDLEWARE_CONFIG_FILE" ]]; then
        log_error "Redisinsight 中间件总控配置文件不存在: $REDISINSIGHT_MIDDLEWARE_CONFIG_FILE"
        exit 1
    fi

    source "$REDISINSIGHT_MIDDLEWARE_CONFIG_FILE"
    log_success "✅ Redisinsight 中间件总控配置加载成功"
}
...
deploy_all_middleware() {
    ...
    log_info "开始基于优先级的 Redisinsight 中间件组件部署..."
    log_info "命名空间: $namespace"
    
    # 使用 --base-path=/redisinsight 时，不需要任何中间件
    # RedisInsight 自己会处理所有路径，Traefik 直接转发即可
    local components=()
    ...
    if [[ ${#enabled[@]} -eq 0 ]]; then
        log_info "ℹ️ 当前配置不需要部署任何中间件（使用 --base-path 模式）"
        return 0
    fi
    ...
    log_success "✅ Redisinsight 所有中间件组件部署完成！"
    return 0
}
...
delete_all_middleware() {
    ...
    log_info "开始删除 Redisinsight 所有中间件组件（按优先级逆序）..."
    log_info "命名空间: $namespace"
    ...
    # 按逆序删除
    if [[ ${#enabled[@]} -eq 0 ]]; then
        log_info "ℹ️ 当前配置不需要删除任何中间件（使用 --base-path 模式）"
        return 0
    fi
    ...
    log_success "✅ Redisinsight 所有中间件组件删除完成！"
    return 0
}
...
check_all_middleware_status() {
    ...
    log_info "检查 Redisinsight 所有中间件组件状态..."
    log_info "命名空间: $namespace"
    ...
    if [[ ${#components[@]} -eq 0 ]]; then
        log_info "ℹ️ 当前配置不需要检查任何中间件（使用 --base-path 模式）"
        return 0
    fi
    ...
    log_success "✅ Redisinsight 所有中间件组件状态检查完成！"
    return 0
}
...
main() {
    ...
    case "$action" in
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 Redisinsight 所有中间件组件（默认）"
            echo "  uninstall        删除 Redisinsight 所有中间件组件"
