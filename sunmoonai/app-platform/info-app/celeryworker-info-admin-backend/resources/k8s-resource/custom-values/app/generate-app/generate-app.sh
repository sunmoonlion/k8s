#!/bin/bash
# celeryworker-info-admin-backend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-info-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-celeryworker-info-admin-backend/app/deploy-app/deploy-celeryworker-info-admin-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${CELERYWORKER_INFO_ADMIN_BACKEND_NAMESPACE:-}")
...
export CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_REGISTRY="${CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_PROJECT="${CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE="${CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE:-}"
export CELERYWORKER_INFO_ADMIN_BACKEND_TAG="${CELERYWORKER_INFO_ADMIN_BACKEND_TAG:-}"
...
export CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
export CELERYWORKER_INFO_ADMIN_BACKEND_SECRET_NAME="${CELERYWORKER_INFO_ADMIN_BACKEND_SECRET_NAME:-}"
...
main() {
    log_info "开始生成 celeryworker-info-admin-backend YAML 文件..."
