...
| D21 | 租户怎么落。Casdoor 的"组织"只是账号分组（这人是谁、能登哪个应用），我们的"租户"是付费客户单位（数据、预算、权限归谁，同租户可共享、跨租户隔离），两者不是一回事。现状：Casdoor 只有一个组织 `sunmoonai`；工作台 `tenant` 一律 `default`，隔离按个人（`owner_actor_id`）。做团队协作时二选一：甲、Casdoor 保持一个组织，工作台自建客户单位表（成员、角色），委托、底稿、预算挂租户；乙、每个客户单位一个 Casdoor 组织，应用设为跨组织共享——Casdoor 3.42 源码里有三条限制：只有 `built-in` 组织下的应用能设共享（投资网页得从 `sunmoonai` 挪到 Casdoor 自己的管理组织）；每个客户组织要把它设为默认应用，登录时先在请求的组织找人、找不到再逐个找这些组织；访问名带组织（`应用名-org-组织名`），登录跳转与回调随之改；登录、注册、邀请码也按组织分别配 | 甲；乙只留给机构私有化、客户要求账号完全独立时 | 团队协作、机构客户、运维量 |
