...
CONFIG_FILE="$SCRIPT_DIR/deploy-llm-web-frontend-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llm-web-frontend-config/ 向上 3 级到达应用根目录
# deploy-llm-web-frontend-config/ -> llm-web-frontend-config/ -> configMap/ -> deploy-llm-web-frontend/ -> llm-web-frontend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/llm-web-frontend-config/generate-llm-web-frontend-config/llm-web-frontend-config-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/configMap/llm-web-frontend-config/generate-llm-web-frontend-config/generate-llm-web-frontend-config.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${LLM_WEB_FRONTEND_CONFIGMAP_NAME:-llm-web-frontend-config}"
