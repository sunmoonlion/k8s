...
#### 已记录的例外：开发期间由 agent 直接推送（含代推主线）

| 项 | 内容 |
| --- | --- |
| **裁定** | 所有者授权（2026-08 口头，2026-09-23 续作）：**第一期开发期间**，托管方的分支保护与凭据分离暂不执行；agent 用本机现有身份直接推自己的分支，所有者说「推 master」时由 agent 代推 |
| **方向** | **朝省事**——因此按「方向不对称」必须有人确认并记录，两样都有 |
| **理由** | 上面三条前置在托管方，本机做不完；开发期间只有所有者一个人在合，逐条手动合的成本大于风险 |
| **边界** | **只限第一期开发，到展示级产品验收为止**。不得延用到有外部参与方的竞争，也不得据此推断其他不可逆动作可代做。没说推 master 就不推；不改写别人的分支 |
| **不变的部分** | 定稿、批准、验收仍是所有者的动作：这条例外只覆盖「把已定稿的东西搬上主线」，不覆盖「决定它可以上主线」。⑥ 的不可伪造性在此期间**不成立**，文档不得宣称已落成 |
| **失效条件** | 分支保护与凭据两件事做完之日，或第一期展示级产品验收之日，**二者先到者为准**；届时删除本条，回到上表的规则 |
