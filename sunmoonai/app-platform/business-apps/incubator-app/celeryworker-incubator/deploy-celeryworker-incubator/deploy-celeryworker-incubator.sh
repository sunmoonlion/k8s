    ...
    spec:
      ...
      - name: extract-incubator-code
        ...
        - name: CODE_EXTRACT_DIRS
          value: "${CODE_EXTRACT_DIRS:-worker core services db models}"
      ...
      - name: celeryworker
        ...
        # 注意：请根据实际使用的数据库 Secret 名称修改
        - name: DATABASE_URL
          valueFrom:
            secretKeyRef:
              name: postgresql-incubator-db-secret  # 请确认实际的 Secret 名称
              key: DB_URL
        resources:
          requests:
            memory: "512Mi"
...
deploy_celeryworker() {
    ...
    export CODE_EXTRACT_DIRS="${CODE_EXTRACT_DIRS:-worker core services db models}"
