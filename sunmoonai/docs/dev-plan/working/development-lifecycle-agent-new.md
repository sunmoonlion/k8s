...
| 层 | 判定依据 | 机制在哪 |
| --- | --- | --- |
| **产品侧**（Attempt 写回、副作用、终态提交） | Task/Attempt 状态、租约与 fencing token | [`request-lifecycle.md`](request-lifecycle.md)。本文只引用，不重新定义，也不复制其状态名 |
| **开发侧**（候选、评审、改进、整合） | 冻结 commit、候选状态（§7.4）、冻结时间戳 | 本文。**开发侧没有租约机制**：整合方在吸收前必须显式核对每个待吸收 commit 仍在有效分支上、提出方未撤回、基线未被 supersede |

两层都不依据分支头或“看起来更新”。开发侧的迟到靠 Git 冻结语义加整合方核对，靠的是
可验证凭据而不是执行者自律；把它写成租约会让人误以为有一个会自动拒绝迟到写入的运行时。

取消先持久化意图，再停止调度、撤销租约、终止执行、盘点副作用、固定需保留的 commit 和
Artifact，最后回收 worktree/sandbox。取消与完成只能一个终态胜出。过期执行者对产品面的
写入由 `request-lifecycle.md` 的 fencing 拒绝；对开发面（分支、worktree、发布路径）的写入
...
有效权限 = 请求方授权
         ...
         ∩ sandbox / tool policy
         ∩ 当前批准
         ∩ 有效租约        ← 产品侧机制，语义见 request-lifecycle.md
```
...
1. **人那份必须自足。**它当前有多处「见 agent 那份」的引用；在本文删除之前，这些引用
   必须解除——把共同内核完整写进人那份，而不是留一个会悬空的链接。这是本文之外单独的
   一件工作，不由本文代劳，但本文的删除以它完成为前提。
...
上一节的反模式是机制描述。本节记录**本项目已经发生过的**失败，以及尚未实跑因而只能称为
设计风险的条目。两者不是同等级证据，不得混用：
...
| 失败 | 证据性质 | 控制 |
| --- | --- | --- |
| **多执行者把同一相对路径的未提交文件写进共享工作区** | **项目实例（2026-09-02）**：多路并行重写同一 `*-new.md`；写进共享 checkout 的未提交文件后写覆盖先写，Git 不报冲突、不留历史；写在各自 worktree 的得以幸存。三份不带后缀的同名文件在三棵 worktree 中安然共存，证明**隔离来自 worktree + 命名分支，文件名后缀既非必要也非充分条件** | §0.4、§5.1 写入前门禁、§7.5、§7.6 |
| 以工作区最后一版代替 commit 做交卷或比选 | 与上行同一实例：磁盘上的「当前文件」没有作者归属，也不能证明谁先写完 | §7.5、§7.6 |
...
写入本表的项目实例只记**可核对的事实**。2026-09-02 一例中，共享路径上的现场在事后已被
清走，写入顺序不可复核，因此本表不记「谁覆盖了谁」——按 §7.8 第 4 步，mtime 与「最后
一版」都只是线索，不足以单独证明写入顺序或责任人。
