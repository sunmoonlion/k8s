...
# =============================================================================
# RedisInsight Secrets 总控部署脚本
# 文件名: deploy-secrets-all.sh
# 用途: 统一部署RedisInsight的所有Secret
# =============================================================================
...
deploy_sub_components_by_priority() {
    ...
    # 注意：RedisInsight 不需要认证 Secret，认证在应用层面处理
    local components=(
...
main() {
    ...
    log_info "🚀 开始部署 RedisInsight Secrets..."
    ...
    if deploy_sub_components_by_priority "$project_id" "$namespace" "$environment" "$dry_run"; then
        log_success "✅ RedisInsight Secrets 部署完成！"
        return 0
    else
        log_error "❌ RedisInsight Secrets 部署失败"
        return 1
    fi
}
...
# 显示帮助信息
show_help() {
    echo "RedisInsight Secrets 总控部署脚本"
    echo ""
    echo "用法:"
