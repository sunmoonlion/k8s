...
| ⑦ 清理发布 | 指定单写者按 [§3.15](../../components/backend/components/06-acceptance-commit/agent-dev-guide.md)–[§3.16](../../components/backend/components/06-acceptance-commit/agent-dev-guide.md) 收存证据、清理本次私有产物，再发布 | 目标版本比较通过、批准仍有效；只清理获准对象，不清空他家工作区 |
