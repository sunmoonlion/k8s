# 待定决策与未验证事项

> **这里只放"还没定"和"还没验"。**定了就写进它该在的定稿并从本表移除；验过了就把结论写进相应文档并从本表移除。
> 引用用 ID：`D*` 是决策点，⚠ 条目按首句引用。本树重新编号；旧树的 `D2`–`D28` 见 `../tree-build-v1/decisions.md`，已定的已写进正文，其余按下表重列。

## 按块查：哪几条卡着你

| 要动哪一块 | 卡着它的 |
| --- | --- |
| `0001-workbench` | `D4`（方向盘交接的粒度）、`D8`（Task 级审批按次整包还是逐步）、`D21`（租户怎么落：团队协作时） |
| `0003-sandbox` | `D18`（PVC 里的会话记录备不备；`D9` 已定：按需拉起、PVC，见 0003「实现状态」） |
| 公网边缘与 frp 隧道（`topology.md`「边缘到内网」） | `D19`（边缘放哪、域名与证书）、`D15`（内网上行） |
| 工程（`SDD/architecture/engineering.md`「走向企业级」） | `D22`（持续集成与交付用什么）、`D23`（高可用范围与告警通道） |
| `0004-relay` | `D16`（仓位置）；`D10` 已定：ES256 JWT、长期、按 jti/用户吊销推送，见 0004「实现状态」与 `security.md`「令牌」 |
| `0005-agent` | `D5` 已定（代理自己挡）、`D11` 已定（Windows 可做，装机要一次提权初始化）；**Windows 提为第一期首个用户平台（2026-09-26）**；卡着它的是两条未验证事项：无管理员权限的办公机、Windows 外沙箱 |
| `0006-knowledge` | `D2`（数据盘点）、`D7`（用户资料的量与同步） |
| `0007-eval` | `D6`（负责人、预算行、时间表） |
| 整个产品 | **`D1`** 法律边界、**`D3`** 客户画像 |

## 待定决策

| # | 问题 | 建议 | 影响 |
| --- | --- | --- | --- |
| D1 | 定位的法律边界：底稿里已算 DCF 而结论栏空着算不算咨询；自有数据经 MCP 卖出的再分发许可 ⚠ 关键路径 | 法务确认；数据盘点与之绑定 | 能否上线、产品边界 |
| D2 | 数据盘点：有什么、从哪来、使用权、历史跨度、和 Wind 差在哪 | 一页纸，先于第一层定价 | 第一层能不能卖 |
| D3 | 设计对象（非开发者投资经理）与第一层付钱对象（会配 key 的个人与小私募）是否同一群人 | U0 访谈五个目标用户，含一个机构 IT、一个合规 | 第一层卖给谁、网页的重量 |
| D4 | 方向盘交接的粒度：整个 Task 期间顾问独占，还是按步交替 | 第一期整段独占，交回时四项契约 | 状态机、网页交互 |
| D6 | 评测集的负责人、预算行、时间表 | 所有者定 | 三元组能否成立 |
| D7 | 用户资料入库的量与同步：显式上传之外要不要目录同步 | 第一期只显式上传 | 知识服务体积、带宽 |
| D8 | Task 级审批的粒度：按次授予整包步骤的驾驶权，还是逐步确认 | 按次整包；扩大外发范围、加预算、外部副作用、改目标时再授权 | 打扰率、安全 |
| D12 | 计费：第一层订阅、第二层按次还是按包 | 两层各一种；不做 token 包干 | 收入结构 |
| D13 | 离线模式（云端不可达时循环在本地跑）什么时候做 | 第二期 | 可用性 |
| D14 | `runtime` 仓是否开源 | "让用户能验证"已退役，开源变成单纯的工程选择；未定 | 分发与信誉 |
| D21 | 租户怎么落。三个概念：Casdoor 的**组织**是账号大分区（一个应用只属于一个组织，登录只在该组织找人）；Casdoor 的**群组**是组织内的成员分组（可分层，一人可在多个群组，登录令牌带 `groups`，邀请码可指定"注册入组"，可按群组授权）；我们的**租户**是付费客户单位（数据、预算、底稿归谁，同租户可共享、跨租户隔离）。现状：只有组织 `sunmoonai`，所有应用都挂在它下面，账号在各应用间通用；工作台 `tenant` 一律 `default`，隔离按个人（`owner_actor_id`）。**做法（2026-09-26 所有者定）：租户 = `sunmoonai` 组织下的一个顶层群组**——成员关系由 Casdoor 群组管（发邀请码时指定入组，注册即入组；换组、退组在 Casdoor 后台），工作台登录时读令牌的 `groups`，映射到自己的租户表（群组 id → 租户），委托、底稿、预算挂租户并按租户隔离；群组下的子群组（部门）第一期不参与隔离。不走"每个客户一个组织"：一个应用只能属于一个组织，跨组织要设共享应用，3.42 只允许 `built-in` 组织的应用共享（共享应用个数不限，但都得挪进 Casdoor 自己的管理组织）；登录时 Casdoor 先在请求指定的组织找人，只有没指定组织时才退到"默认应用是它"的组织里找，而每个组织只有一个默认应用，所以这条退路只够一个共享应用用，要多个应用共享就得在每次登录时指明组织（访问名带 `-org-组织名` 后缀或请求里带组织），登录跳转与回调按客户拼名；每个客户组织还要单独配登录、注册、邀请码——只留给机构私有化、客户要求账号完全独立时。待做（做团队协作时）：工作台读 `groups` 并建租户表；一个用户在多个顶层群组时如何选当前租户；客户单位的管理员自己拉人（给 Casdoor 后台受限权限，或由工作台调 Casdoor 接口代办） | 已定做法见左；余下三项待做时定 | 团队协作、机构客户、运维量 |
| D22 | 持续集成与交付用什么：沿用仓里已有的 Jenkins + Kaniko（+ 规划中的 Argo CD）跑在自己集群里，还是用 GitHub Actions 等托管服务。另有一处冲突要定：现有方案写"Gitee 全链路"，所有者 2026-09-26 说 GitHub 为主、Gitee 只是镜像 | 自建（Jenkins + Kaniko 构建、Argo CD 部署），代码源以 GitHub 为准；构建机在国内时拉 GitHub 慢，再考虑从 Gitee 镜像拉 | 发版效率、门禁能否原样搬进流水线 |
| D23 | 高可用做到什么程度、告警发到哪：单区多副本还是跨区；告警走邮件、企业微信还是短信；谁值班 | 第一步单区多副本 + 数据层主从 + 告警发企业微信与邮件；跨区等有付费用户再做 | 上线门、运维成本 |
| D19 | 公网边缘放哪、域名与证书：云厂商与区域（必须与用户同区域，实测跨境单跳约 420 ms）、域名、证书来源（ACME 自动签还是自备） | 国内同区域云 VM；ACME（Traefik 自带）；一个主域名 | 上云时间、延迟、备案 |
| D15 | 内网站点的上行链路与冗余：专线、UPS、双路 | 上线前的硬门，与监控告警一起 | 可用性 |
| D18 | 沙箱会话记录（PVC 里的 Codex thread）要不要备份、备多久 | 第一期不备：会话可丢，账不可丢。已做的一半：回收沙箱默认保留 PVC（`?purge=1` 才删） | 用户换沙箱后能否接着聊 |
| D17 | 会合点要不要兼容 Codex 自带的 rendezvous 协议（`exec-server --remote`：HTTP 注册 + protobuf 多路复用帧 + 序号续传 + noise 端到端加密） | 第一期不兼容，自研哑透传；续传需求出现时再评估；加密下协议过滤失效是硬伤 | 会合点复杂度、过滤层 |
| D16 | 会合点放哪个仓：新仓 `relay`，还是 k8s 仓里一个平台 | 已按平台惯例放 `k8s/sunmoonai/relay-platform/`（沙箱池同理 `sandbox-platform/`），与边缘部署清单同仓；独立发布节奏时再拆 | 仓数、发布 |

## 未验证事项 ⚠

- **OpenAI 的 API key 路径**：所有者只有订阅没有 key，未验；BYOK 与国产直连已由 Kimi 验过（`runtime/probe/REPORT-2026-09-23-byok-kimi.md`）；其他国产厂商（DeepSeek、Qwen、GLM）的 responses 兼容度未验；
- **macOS 上外沙箱的嵌套**：sandbox-exec 外层 + Codex seatbelt 内层，预期可叠加，未验；Linux 已验（`runtime/probe/REPORT-2026-09-24-outer-sandbox.md`）；
- **Windows 上本地上限的外沙箱怎么做**（Windows 已提为第一期，这条是上线门）：exec-server 本身在 Windows 上已验可用（`runtime/probe/REPORT-2026-09-24-windows-exec-server.md`，需 `codex sandbox setup --elevated` 一次性提权初始化）；能否把 exec-server 进程包进同一套受限令牌机制，Windows 开工前先探；
- **没有管理员权限的 Windows 办公机能不能装**（上线门）：Codex 非提权沙箱模式能否在执行端挡住白名单外写入，未验；挡得住给免提权安装路径，挡不住就写明"需要 IT 装一次"，不降级放行（见 0005「平台」）；
- **断线恢复已验**（`runtime/probe/REPORT-2026-09-23-reconnect.md`）：25 秒内重连接回原会话、进程与输出无损；超窗与代理重启会话丢，但环境回来后 app-server 自动重连，新 turn 正常。未验：真实公网抖动时长分布；
- **一个 exec-server 服务多个 app-server 会话**；
- **stdio 型 MCP 在远端环境下跑在哪一端**：HTTP 型已验由编排端从执行端配置读取并生效；skills 已验（`runtime/probe/REPORT-2026-09-24-skills-mcp-resolution.md`）：项目 `.agents/skills` 与沙箱 `CODEX_HOME/skills` 可见，用户机器上的全局 `~/.codex/skills` 不可见；
- **换步后 MCP 工具列表是否重取**：现在的设计不依赖它（服务端按 token 范围逐调用鉴权），若可靠重取可再做按步换清单；
- **会合点透传的公网延迟**：回环已验透传成立、开销可忽略（`runtime/probe/REPORT-2026-09-23-relay-passthrough.md`）；每 turn 约 45 个串行来回（九成是 `fs/getMetadata`），公网 RTT × 45 才是真实数字，要在 luna 本地与这台远程之间测；`--concurrent-requests` 与 metadata 缓存能否减少来回未验；
- **Codex 实验性接口的稳定性**：`environment/*` 与 exec-server 均标 experimental；升版必须重跑探针；
- **沙箱池的内存与冷启动**：每用户 app-server 的常驻成本；
- 对比评测所需的领域评测集尚不存在，"本产品优于原版 Codex"目前只是目标；
- `investment-app` 目前没有业务数据表，问数类 Profile 的前置未解决。

⚠ 这一节的条目不得当成已知事实用。按 `P6`，未核前提只能标假设并写明验证办法。

## 已定（近期，细节在正文）

- **D20 注册邮件从哪发**（2026-09-26 所有者定）：暂用腾讯云等云厂商的邮件推送（SMTP），企业邮箱留同一接口；见 `security.md`「账号与注册」。
- **手机号**（2026-09-26 所有者定）：不是必需，只留接口（注册项隐藏，开关 `SIGNUP_PHONE_ENABLED`）。
