...
{{- define "harbor.redisForCore" -}}
  ...
  {{- if not (include "harbor.redis.sentinel.enabled" $args)  -}}
    ...
    {{- else -}}
      {{- printf "%s://:SunMoonAI_Redis_2024!@%s:%s/%s" (include "harbor.redis.scheme" $args) (include "harbor.redis.host" $args) (include "harbor.redis.port" $args) (include "harbor.redis.coreDatabaseIndex" . ) -}}
    {{- end -}}
  {{- else -}}
    {{- if (include "harbor.redis.escapedRawPassword" $args) -}}
    ...
    {{- else -}}
      {{- printf "%s://:SunMoonAI_Redis_2024!@%s:%s/%s/%s" (include "harbor.redis.scheme" $args) (include "harbor.redis.host" $args) (include "harbor.redis.port" $args) (include "harbor.redis.sentinel.masterSet" $args) (include "harbor.redis.coreDatabaseIndex" . ) -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
