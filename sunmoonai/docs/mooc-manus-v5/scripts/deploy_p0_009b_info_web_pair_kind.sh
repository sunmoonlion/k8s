...
HOST="info-web-p0-009b.sunmoonai.com"
PUBLIC_PORT="30443"
BACKEND_IMAGE=""
FRONTEND_IMAGE=""
DEPLOYMENT_ID="p0-009b-v1"
IDENTITY_SECRET="sunmoonai-p0-009b-web-identity"
RUNTIME_SECRET="sunmoonai-p0-009b-web-runtime"
MODE="plan"
...
usage() {
  cat <<'EOF'
Usage: deploy_p0_009b_info_web_pair_kind.sh [--apply|--cleanup] [options]
...
cleanup() {
  k delete ingressroute info-web-p0-009b -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete pdb info-web-backend-p0-009b info-web-frontend-p0-009b \
    -n "$NAMESPACE" --ignore-not-found=true >/dev/null
  k delete service info-web-backend-p0-009b info-web-frontend-p0-009b \
    p0-009b-web-postgresql p0-009b-web-redis -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete deployment info-web-backend-p0-009b info-web-frontend-p0-009b \
    p0-009b-web-postgresql p0-009b-web-redis -n "$NAMESPACE" \
    --ignore-not-found=true --wait=true >/dev/null
  k delete job p0-009b-web-migration -n "$NAMESPACE" \
    --ignore-not-found=true --wait=true >/dev/null
  k delete configmap info-web-p0-009b-config -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete secret "$RUNTIME_SECRET" -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  for _attempt in $(seq 1 60); do
    if ! k get pod -n "$NAMESPACE" -o name | grep -Eq '(info-web.*p0-009b|p0-009b-web)'; then
  ...
  k get pod -n "$NAMESPACE" -o name | grep -E '(info-web.*p0-009b|p0-009b-web)' >&2 || true
  return 1
}
...
k label secret "$RUNTIME_SECRET" -n "$NAMESPACE" \
  sunmoonai.com/task=v5-p0-009b-web --overwrite >/dev/null
...
kind: ConfigMap
metadata:
  name: info-web-p0-009b-config
  namespace: ${NAMESPACE}
  labels:
    sunmoonai.com/task: v5-p0-009b-web
data:
  ...
  ENV: test
  LOG_LEVEL: INFO
  SERVICE_NAME: info-web-backend-p0-009b
  APP_SLUG: info
  SURFACE: web
  REDIS_HOST: p0-009b-web-redis
  REDIS_PORT: "6379"
  REDIS_DB: "0"
  ...
  CASDOOR_APPLICATION: sunmoonai-info-web-p0-009b
  CASDOOR_VERIFY_SSL: "true"
  AUTH_POLICY_VERSION: info-web-p0-009b-v1
  ...
  ALLOWED_HOSTS: "${HOST},info-web-backend-p0-009b"
  SESSION_COOKIE_SECURE: "true"
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: p0-009b-web-postgresql
  namespace: ${NAMESPACE}
  labels:
    sunmoonai.com/task: v5-p0-009b-web
spec:
...
k rollout status deployment/p0-009b-web-postgresql -n "$NAMESPACE" --timeout=180s
k rollout status deployment/p0-009b-web-redis -n "$NAMESPACE" --timeout=180s
          ...
          args:
            - |
              for attempt in \$(seq 1 30); do
                .venv/bin/python -c "import socket; socket.create_connection(('p0-009b-web-postgresql', 5432), 2).close()" && break
            ...
            - name: POSTGRES_PASSWORD
              valueFrom:
                secretKeyRef:
                  ...
                  key: POSTGRES_PASSWORD
            - name: DATABASE_URL
              value: postgresql+asyncpg://tpl:\$(POSTGRES_PASSWORD)@p0-009b-web-postgresql:5432/tpl
          securityContext:
            allowPrivilegeEscalation: false
            capabilities:
              drop: ["ALL"]
EOF
k wait --for=condition=complete job/p0-009b-web-migration \
  -n "$NAMESPACE" --timeout=180s
k logs job/p0-009b-web-migration -n "$NAMESPACE" --tail=20

cat <<EOF | k apply -f - >/dev/null
apiVersion: apps/v1
kind: Deployment
metadata:
  name: info-web-backend-p0-009b
  namespace: ${NAMESPACE}
  labels:
    app: info-web-backend-p0-009b
    sunmoonai.com/task: v5-p0-009b-web
spec:
  replicas: 2
...
kind: Deployment
metadata:
  name: info-web-frontend-p0-009b
  namespace: ${NAMESPACE}
  labels:
    app: info-web-frontend-p0-009b
    sunmoonai.com/task: v5-p0-009b-web
spec:
  replicas: 2
  revisionHistoryLimit: 3
        ...
        - name: frontend
          ...
          env:
            ...
            - name: WEB_BACKEND_INTERNAL_URL
              value: http://info-web-backend-p0-009b:8000
            - name: DEPLOYMENT_ID
              value: "${DEPLOYMENT_ID}"
            # Isolated P0-009B only: enable the deterministic reference workspace UI.
...
k rollout status deployment/info-web-backend-p0-009b \
  -n "$NAMESPACE" --timeout=240s
k rollout status deployment/info-web-frontend-p0-009b \
  -n "$NAMESPACE" --timeout=240s

backend_ready="$(k get deployment info-web-backend-p0-009b -n "$NAMESPACE" \
  -o jsonpath='{.status.readyReplicas}')"
frontend_ready="$(k get deployment info-web-frontend-p0-009b -n "$NAMESPACE" \
  -o jsonpath='{.status.readyReplicas}')"
