...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"tools-admin-backend-pvc-generated.yaml" ]]; then
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${TOOLS_ADMIN_BACKEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${TOOLS_ADMIN_BACKEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 tools-admin-backend 子组件..."
...
deploy_app() {
    ...
    auto_generate_yaml "$TOOLS_ADMIN_BACKEND_YAML" "$K8S_RESOURCE_DIR" || exit 1

    export TOOLS_ADMIN_BACKEND_IMAGE_REGISTRY="${TOOLS_ADMIN_BACKEND_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    ...
    export TOOLS_ADMIN_BACKEND_FULL_IMAGE_NAME="${TOOLS_ADMIN_BACKEND_IMAGE_REGISTRY}/${TOOLS_ADMIN_BACKEND_IMAGE_PROJECT}/${TOOLS_ADMIN_BACKEND_IMAGE}:${TOOLS_ADMIN_BACKEND_TAG}"

    log_info "镜像: $TOOLS_ADMIN_BACKEND_FULL_IMAGE_NAME"
    log_warn "⚠️  请确保该镜像已存在于 Harbor 仓库"

    export NAMESPACE="$NAMESPACE" ENV="$ENV" ENVIRONMENT="$ENVIRONMENT"
    export TOOLS_ADMIN_BACKEND_FULL_IMAGE_NAME
