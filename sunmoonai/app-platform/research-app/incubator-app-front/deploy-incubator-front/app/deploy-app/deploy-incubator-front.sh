...
# 用法: ./deploy-incubator-front.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-incubator-front 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-incubator-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（incubator-app-front 目录）
# 从 deploy-incubator-front/ 向上 1 级到达 incubator-app-front/
...
# 加载部署配置文件
INCUBATOR_SSR_CONFIG_FILE="$SCRIPT_DIR/deploy-incubator-front.conf"
...
# 默认配置从配置文件读取（deploy-incubator-front.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${INCUBATOR_SSR_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${INCUBATOR_SSR_NAMESPACE:-}"
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（incubator-app-front/）
# app/deploy-app/ -> app/ -> deploy-incubator-front/ -> incubator-app-front/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
INCUBATOR_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/incubator-app-front-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"incubator-app-front-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
    ...
    if [ -f "$generate_script" ]; then
        ...
        export NAMESPACE="${INCUBATOR_SSR_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INCUBATOR_SSR_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    # 自动生成 YAML 文件（如果不存在）
    if ! auto_generate_yaml "$INCUBATOR_SSR_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/incubator-app-front-config/deploy-incubator-app-front-config/deploy-incubator-app-front-config.sh"
        local secret_script="$secrets_dir/incubator-app-front-secret/deploy-incubator-app-front-secret/deploy-incubator-app-front-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export INCUBATOR_SSR_IMAGE="${INCUBATOR_SSR_IMAGE:-incubator-app-front}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=incubator-app-front"
        echo "  kubectl get svc -n $NAMESPACE -l app=incubator-app-front"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=incubator-app-front -f"
    else
...
uninstall_app() {
    ...
    if [ ! -f "$INCUBATOR_SSR_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $INCUBATOR_SSR_YAML，尝试自动生成..."
        if ! auto_generate_yaml "$INCUBATOR_SSR_YAML" "$K8S_RESOURCE_DIR"; then
            log_warn "YAML 生成失败，尝试直接删除资源"
            kubectl delete deployment incubator-app-front -n "$NAMESPACE" --ignore-not-found=true
            kubectl delete service incubator-app-front-service -n "$NAMESPACE" --ignore-not-found=true
        fi
    fi
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=incubator-app-front 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=incubator-app-front 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=incubator-app-front 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=incubator-app-front 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=incubator-app-front 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-incubator-front"
            echo "  ./deploy-incubator-front.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
