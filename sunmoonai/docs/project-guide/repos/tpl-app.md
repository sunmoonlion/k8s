...
[v5 历史索引中的 B7b](../topics/v5-history.md#被收拢的-24-份旧账报告)。
本机 KIND 固定候选已完成角色切换；后续 release 仍须重新验收。
...
[v5 历史索引中的 B7d](../topics/v5-history.md#被收拢的-24-份旧账报告)。
...
[v5 历史索引中的 B7j](../topics/v5-history.md#被收拢的-24-份旧账报告)；
当前是否生效仍按具体 release 和运行探针判断。

Worker 消费配置探针 `python -m app.cli.worker_readiness`（B7e）定向本 POD_NAME，
...
历史验证见 [v5 索引中的 B7e](../topics/v5-history.md#被收拢的-24-份旧账报告)。

Scheduler bootstrap 已选用继承 PersistentScheduler 的活动观察类（B7i）。
`python -m app.cli.scheduler_activity --schedule <同一文件> --max-age <秒>`
...
实现和验证边界见 [v5 索引中的 B7a](../topics/v5-history.md#被收拢的-24-份旧账报告)；
不由源码存在推定任一环境生效。
...
**改模板、同步实例前**，先读 [`../../dev-agent/rules/constraints.md`](../../dev-agent/rules/constraints.md)「发布」——本页只写现状。
