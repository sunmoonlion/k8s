...
info-app 是架构 v2 的**资讯采集与内容治理域 App**：负责从外部信息源（RSS/API/网页/上传）采集内容，完成抽取、去重、人工治理，再把可分发制品（artifact）经持久投递链路分发给 knowledge-app。它是 knowledge-app 的唯一内容上游。
...
- 当前权威拓扑（`docs/ARCHITECTURE-V2-SOURCE-TOPOLOGY.md`，2026-08-09）：**admin-frontend + web-frontend + unified backend**；旧的独立 web-backend 仓库只存在于 P0-009 冻结标签（`p0-009a-pre-20260729`）里，是迁移审计材料，不得从中恢复源码。
...
├── docs/
│   ├── ARCHITECTURE-V2-SOURCE-TOPOLOGY.md
│   ├── INSTANCE_FOUNDATION_ALIGNED.json   # P0-009E 四组件对齐快照（历史材料）
│   ├── P0-009-DOMAIN-RECOVERY.md          # 冻结标签恢复规程
│   └── history/SPIDER_MVP_HANDOFF_20260710.md
├── info-backend/              # 后端组件仓
...
- `INSTANCE_FOUNDATION_ALIGNED.json`：P0-009E 任务快照，四组件 commit + 四个 Harbor digest 镜像 pair 验证 passed，`traffic_cutover:false`、`remote_git_push:false`（历史材料）。
- P0-009 冻结标签 `p0-009a-pre-20260729` 是迁移前唯一权威归档；恢复只能走隔离 worktree，禁止把旧树拷回。
- R4 同步顺序：info → knowledge → research（见 tpl 的 template-release-manifest）。
