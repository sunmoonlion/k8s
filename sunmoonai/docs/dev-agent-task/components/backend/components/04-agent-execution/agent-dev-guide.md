...
只在工单判为 T2 时使用；T0/T1 按 [§3.4](../../composition/agent-dev-guide.md) 执行。协议仍是流程真源，
...
| ⑦ 清理发布 | 指定单写者按 [§3.15](../06-acceptance-commit/agent-dev-guide.md)–[§3.16](../06-acceptance-commit/agent-dev-guide.md) 收存证据、清理本轮私有产物，再发布 | 目标版本比较通过、批准仍有效；只清理获准对象，不清空他家工作区 |
...
「同一事实只能有一个权威写入者」（`I13`，[§1.2](../../composition/agent-dev-guide.md)）——**多人各自往同一份 handoff 追加，
就是在共享路径上并发写**。
