...
# Llm Web Frontend 部署脚本
# 用法: ./deploy-llm-web-frontend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-llm-web-frontend 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-llm-web-frontend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（llm-web-frontend 目录）
# 从 deploy-llm-web-frontend/ 向上 1 级到达 llm-web-frontend/
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

# 保存 Llm Web Frontend 脚本的目录路径（统一部署模板会重新定义 SCRIPT_DIR）
LLM_WEB_FRONTEND_SCRIPT_DIR="$SCRIPT_DIR"
...
# 恢复 Llm Web Frontend 脚本的目录路径
SCRIPT_DIR="$LLM_WEB_FRONTEND_SCRIPT_DIR"
...
# 加载部署配置文件
LLM_WEB_FRONTEND_CONFIG_FILE="$SCRIPT_DIR/deploy-llm-web-frontend.conf"
if [[ -f "$LLM_WEB_FRONTEND_CONFIG_FILE" ]]; then
    source "$LLM_WEB_FRONTEND_CONFIG_FILE"
    ...
    log_info "已加载 Llm Web Frontend 配置文件: $LLM_WEB_FRONTEND_CONFIG_FILE"
else
    log_warn "未找到 Llm Web Frontend 配置文件: $LLM_WEB_FRONTEND_CONFIG_FILE，使用默认配置"
fi

# 默认配置从配置文件读取（deploy-llm-web-frontend.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${LLM_WEB_FRONTEND_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${LLM_WEB_FRONTEND_NAMESPACE:-}"
...
deploy_sub_components() {
    ...
    log_info "开始部署 Llm Web Frontend 子组件..."
    ...
    log_success "✅ Llm Web Frontend 子组件部署完成"
    return 0
}

# 镜像配置
LLM_WEB_FRONTEND_IMAGE="${LLM_WEB_FRONTEND_IMAGE:-}"
LLM_WEB_FRONTEND_TAG="${LLM_WEB_FRONTEND_TAG:-}"

# 资源文件路径
RESOURCES_DIR="$APP_ROOT/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
LLM_WEB_FRONTEND_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/llm-web-frontend-generated.yaml"
LLM_WEB_FRONTEND_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/llm-web-frontend-pvc/generate-llm-web-frontend-pvc/llm-web-frontend-pvc-generated.yaml"
TEMPLATES_DIR="${K8S_RESOURCE_DIR}/templates"
LLM_WEB_FRONTEND_CONFIGMAP="${TEMPLATES_DIR}/configMap/llm-web-frontend-config.yaml"
LLM_WEB_FRONTEND_SECRET="${TEMPLATES_DIR}/secret/llm-web-frontend-secret.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"llm-web-frontend-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"llm-web-frontend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/llm-web-frontend-pvc/generate-llm-web-frontend-pvc/generate-llm-web-frontend-pvc.sh"
    else
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${LLM_WEB_FRONTEND_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${LLM_WEB_FRONTEND_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    if ! auto_generate_yaml "$LLM_WEB_FRONTEND_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/llm-web-frontend-config/deploy-llm-web-frontend-config/deploy-llm-web-frontend-config.sh"
        local secret_script="$secrets_dir/llm-web-frontend-secret/deploy-llm-web-frontend-secret/deploy-llm-web-frontend-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
# 部署 Llm Web Frontend
deploy_app() {
    log_info "开始部署 Llm Web Frontend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    check_env_config

    export LLM_WEB_FRONTEND_IMAGE_REGISTRY="${LLM_WEB_FRONTEND_IMAGE_REGISTRY:-harbor.sunmoonai.com}"
    export LLM_WEB_FRONTEND_IMAGE_PROJECT="${LLM_WEB_FRONTEND_IMAGE_PROJECT:-app-images}"
    export LLM_WEB_FRONTEND_IMAGE="${LLM_WEB_FRONTEND_IMAGE:-llm-web-frontend}"
    export LLM_WEB_FRONTEND_TAG="${LLM_WEB_FRONTEND_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-Always}"
    export LLM_WEB_FRONTEND_IMAGE_PULL_SECRET_NAME="${LLM_WEB_FRONTEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"

    LLM_WEB_FRONTEND_FULL_IMAGE_NAME="${LLM_WEB_FRONTEND_IMAGE_REGISTRY}/${LLM_WEB_FRONTEND_IMAGE_PROJECT}/${LLM_WEB_FRONTEND_IMAGE}:${LLM_WEB_FRONTEND_TAG}"
    log_info "使用 Harbor 镜像部署: $LLM_WEB_FRONTEND_FULL_IMAGE_NAME"
    ...
    export LLM_WEB_FRONTEND_FULL_IMAGE_NAME="$LLM_WEB_FRONTEND_FULL_IMAGE_NAME"

    log_info "🚀 阶段1：部署 Llm Web Frontend 子级组件..."
    if ! deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_error "❌ Llm Web Frontend 子级组件部署失败！"
        return 1
    fi
    log_success "✅ Llm Web Frontend 子级组件部署完成"

    if ! check_harbor_image_exists "$LLM_WEB_FRONTEND_FULL_IMAGE_NAME" "$NAMESPACE" "${LLM_WEB_FRONTEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"; then
        return 1
    fi

    log_info "🚀 阶段2：部署 Llm Web Frontend 核心服务..."
    log_info "部署 Llm Web Frontend (环境: $ENVIRONMENT, 镜像: $LLM_WEB_FRONTEND_FULL_IMAGE_NAME, 拉取策略: ${IMAGE_PULL_POLICY:-Always}, 命名空间: $NAMESPACE)..."
    ...
    if ! auto_generate_yaml "$LLM_WEB_FRONTEND_PVC_YAML" "$K8S_RESOURCE_DIR"; then
        return 1
    fi

    if [ -f "$LLM_WEB_FRONTEND_PVC_YAML" ]; then
        log_info "部署 PVC..."
        kubectl apply -f "$LLM_WEB_FRONTEND_PVC_YAML" -n "$NAMESPACE"
        if [ $? -eq 0 ]; then
    ...
    kubectl apply -f "$LLM_WEB_FRONTEND_YAML" -n "$NAMESPACE"

    if [ $? -eq 0 ]; then
        log_success "Llm Web Frontend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=llm-web-frontend"
        echo "  kubectl get svc -n $NAMESPACE -l app=llm-web-frontend"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=llm-web-frontend -f"
    else
        log_error "Llm Web Frontend 部署失败"
        exit 1
    fi
}
...
# 卸载 Llm Web Frontend
uninstall_app() {
    log_info "开始卸载 Llm Web Frontend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    check_env_config

    log_info "🚀 阶段1：卸载 Llm Web Frontend 核心服务..."
    if [ ! -f "$LLM_WEB_FRONTEND_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $LLM_WEB_FRONTEND_YAML，尝试直接删除资源"
        kubectl delete deployment llm-web-frontend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service llm-web-frontend -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$LLM_WEB_FRONTEND_YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    log_success "✅ Llm Web Frontend 核心服务卸载完成"

    log_info "🚀 阶段2：卸载 Llm Web Frontend 子级组件..."
    if ! uninstall_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_warn "⚠️ Llm Web Frontend 子级组件卸载部分失败，继续..."
    fi
    log_success "✅ Llm Web Frontend 子级组件卸载完成"

    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        log_warn "⚠️  注意：卸载 Namespace 将删除命名空间 $NAMESPACE 及其中的所有资源！"
        log_info "如需卸载 Namespace，请手动执行："
        log_info "  cd deploy-llm-web-frontend/namespace/llm-web-frontend-namespace/deploy-llm-web-frontend-namespace"
        log_info "  ./deploy-llm-web-frontend-namespace.sh uninstall $PROJECT_ID $NAMESPACE $ENVIRONMENT"
    fi

    log_success "Llm Web Frontend 卸载完成！"
}
...
uninstall_sub_components() {
    ...
    log_info "开始卸载 Llm Web Frontend 子组件..."

    local sub_components=(
        "llm_web_frontend_middleware:${middleware_enabled:-false}:${middleware_priority:-1000}:Llm Web Frontend 中间件:$PROJECT_ROOT/middleware/deploy-middleware-all/deploy-middleware-all.sh"
        "llm_web_frontend_ingress:${ingress_enabled:-false}:${ingress_priority:-100}:Llm Web Frontend HTTP 路由:$PROJECT_ROOT/ingress/deploy-ingress/deploy-ingress.sh"
    )
    ...
    log_success "✅ Llm Web Frontend 子组件卸载完成"
    return 0
}

# 显示状态
show_status() {
    log_info "Llm Web Frontend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=llm-web-frontend 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    log_info "Llm Web Frontend 部署脚本启动"
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "操作:"
            echo "  deploy     部署 Llm Web Frontend"
            echo "  uninstall  卸载 Llm Web Frontend"
            echo "  status     查看 Llm Web Frontend 状态"
            echo ""
            echo "示例:"
