...
# Celery Worker API 接口 HTTP 路由部署脚本
...
# 计算项目根目录（k8s目录）
# 从 deploy-ingress/ -> ingress/ -> deploy-celeryworker-bff/ -> celeryworker-bff/ -> celeryworker-app/ -> shared-apps/ -> app-platform/ -> sunmoonai/ -> k8s/
...
# 验证 Celery Worker 服务是否存在（警告模式，不阻止部署）
verify_service() {
    local namespace="$1"
    ...
    if kubectl get service -n "$namespace" "$service_name" >/dev/null 2>&1; then
        log_success "✅ Celery Worker 服务存在: $service_name"
        return 0
    else
        if [[ "$warn_only" == "true" ]]; then
            log_warning "⚠️ Celery Worker 服务不存在: $service_name（将在服务部署后自动生效）"
            return 0  # 返回成功，允许继续部署
        else
            log_error "❌ Celery Worker 服务不存在: $service_name"
            return 1
        fi
    fi
}
...
load_config() {
    ...
    # 主配置文件路径：../../deploy-multi-celeryworker.conf（相对于当前脚本目录）
    local main_config_file="$(cd "$SCRIPT_DIR/../.." && pwd)/deploy-multi-celeryworker.conf"
    ...
    # 从主配置文件构建配置
    SERVICE_NAME="celeryworker-multi-service"
    NAMESPACE="${CELERY_WORKER_NAMESPACE:-app-platform-dev}"
    UNIFIED_HOST="${CELERY_WORKER_UNIFIED_HOST:-celeryworker.sunmoonai.com}"
    ...
    if type get_service_port >/dev/null 2>&1; then
        ...
        if [[ -n "$service_port" ]]; then
            ...
            log_warn "⚠️  无法从 Service 获取端口，使用默认端口 5555"
            SERVICE_PORT="5555"
        fi
    else
        log_warn "⚠️  get_service_port 函数不可用，使用默认端口 5555"
        SERVICE_PORT="5555"
    fi
...
deploy_http_route() {
    ...
    log_info "部署 Celery Worker API 接口 HTTP 路由..."
    ...
    verify_service "$namespace" "$SERVICE_NAME" "true" || {
        log_warning "⚠️ Celery Worker 服务尚未创建，Ingress 将在服务部署后自动生效"
    }
    ...
    log_info "应用 Celery Worker API 接口 HTTP 路由配置..."
    if kubectl apply -f "$temp_file"; then
        log_success "✅ Celery Worker API 接口 HTTP 路由配置应用成功"
        rm -f "$temp_file"
    else
        log_error "❌ Celery Worker API 接口 HTTP 路由配置应用失败"
        rm -f "$temp_file"
        return 1
    fi
    ...
    kubectl get ingressroute -n "$namespace" celeryworker-multi-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" celeryworker-multi-stripprefix 2>/dev/null || log_warn "Middleware 尚未创建"
...
check_deployment_status() {
    log_info "检查 Celery Worker API 接口 HTTP 路由部署状态..."
    
    echo ""
    echo "=== Celery Worker API 接口 HTTP 路由状态 ==="
    kubectl get ingressroute -n "$NAMESPACE" -l component=celeryworker-multi 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=celeryworker-multi 2>/dev/null || echo "Middleware 不存在"
    
    echo ""
    echo "=== Celery Worker 服务状态 ==="
    kubectl get svc "$SERVICE_NAME" -n "$NAMESPACE" 2>/dev/null || echo "Celery Worker 服务不存在"
    
    echo ""
    echo "=== 访问信息 ==="
    echo "统一域名访问: https://${UNIFIED_HOST}/api/v1/celeryworker"
    echo "节点 IP 访问: https://${NODE_IP}:${EXTERNAL_PORT}/api/v1/celeryworker"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 通过 curl 访问 Celery Worker API 接口:"
    echo "   curl -k https://${UNIFIED_HOST}/api/v1/celeryworker/health"
    echo "   curl -k https://${NODE_IP}:${EXTERNAL_PORT}/api/v1/celeryworker/health"
    echo ""
...
delete_http_route() {
    ...
    log_info "删除 Celery Worker API 接口 HTTP 路由..."
    log_info "命名空间: $namespace"
    ...
    # 使用 sed 替换模板变量
    sed -i "s/{{NAMESPACE}}/$namespace/g" "$temp_file"
    sed -i "s/{{SERVICE_NAME}}/celeryworker-multi-service/g" "$temp_file"
    sed -i "s/{{SERVICE_PORT}}/5555/g" "$temp_file"
    sed -i "s/{{UNIFIED_HOST}}/celeryworker.sunmoonai.com/g" "$temp_file"
    sed -i "s/{{NODE_IP}}/101.126.151.0/g" "$temp_file"
    
    if kubectl delete -f "$temp_file" 2>/dev/null; then
        log_success "✅ Celery Worker API 接口 HTTP 路由配置删除成功"
    else
        log_warn "⚠️ Celery Worker API 接口 HTTP 路由配置不存在或删除失败"
    fi
    
    rm -f "$temp_file"
    log_success "✅ Celery Worker API 接口 HTTP 路由删除完成！"
    return 0
}

# 显示帮助信息
show_help() {
    echo "Celery Worker API 接口 HTTP 路由部署脚本"
    echo ""
    echo "用法: $0 [命令] [项目ID] [命名空间] [环境]"
    echo ""
    echo "命令:"
    echo "  deploy <project_id> [namespace] [environment]    部署 Celery Worker API 接口 HTTP 路由"
    echo "  uninstall <project_id> [namespace] [environment] 卸载 Celery Worker API 接口 HTTP 路由"
    echo "  status    检查部署状态"
    echo "  help      显示帮助信息"
    echo ""
    echo "参数:"
...
main() {
    ...
    case "$action" in
        deploy)
            ...
            log_info "开始部署 Celery Worker API 接口 HTTP 路由..."
        ...
        uninstall)
            ...
            log_info "开始删除 Celery Worker API 接口 HTTP 路由..."
            log_info "项目: $project_id"
            log_info "命名空间: $namespace"
            log_info "环境: $environment"
