...
人驱动每一步：写任务书、发出去、读 response、定稿、决定下一段问什么。没有 supervisor 程序，人兼这个角色。

| 步骤 | 人做什么 |
| --- | --- |
| 开一段 thread | 定这一段是哪个阶段（BRD、PRD、SDD、SDP、UAT），建目录、写 `user-message.md` |
| 派工 | 把任务书发给 AI 助手；要补的材料放 `others/` |
...
| 建子任务 | 按 `SDD/modules/` 的拆分建目录，并写好各自第 `0001` 个 turn 的任务书 |
...
- **任务书里没写的，模型不会知道**：要它交回什么、能改哪些文件、哪些动作要先问，都写进任务书。
- **验收标准在派工前定**，出题与答题分开；事后改题等于白问。
