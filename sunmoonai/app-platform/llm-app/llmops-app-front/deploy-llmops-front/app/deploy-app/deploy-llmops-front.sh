...
# LLMOps App SSR 部署脚本
# 用法: ./deploy-llmops-front.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-llmops-front 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-llmops-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（llmops-app-front 目录）
# 从 deploy-llmops-front/ 向上 1 级到达 llmops-app-front/
...
# 加载部署配置文件
LLMOPS_SSR_CONFIG_FILE="$SCRIPT_DIR/deploy-llmops-front.conf"
...
# 默认配置从配置文件读取（deploy-llmops-front.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${LLMOPS_SSR_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${LLMOPS_SSR_NAMESPACE:-}"
...
deploy_sub_components() {
    ...
    log_info "开始部署 LLMOps App SSR 子组件..."
    ...
    log_success "✅ LLMOps App SSR 子组件部署完成"
    return 0
}

# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（llmops-app-front/）
# app/deploy-app/ -> app/ -> deploy-llmops-front/ -> llmops-app-front/
RESOURCES_DIR="$APP_ROOT/resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
LLMOPS_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/llmops-app-front-generated.yaml"
LLMOPS_SSR_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-front-pvc/generate-llmops-front-pvc/llmops-app-front-pvc-generated.yaml"
# 模板文件路径（在 resources/k8s-resource/templates/）
TEMPLATES_DIR="${K8S_RESOURCE_DIR}/templates"
LLMOPS_SSR_CONFIGMAP="${TEMPLATES_DIR}/configmap/llmops-app-front-config.yaml"
LLMOPS_SSR_SECRET="${TEMPLATES_DIR}/secret/llmops-app-front-secret.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"llmops-app-front-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"llmops-app-front-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-front-pvc/generate-llmops-front-pvc/generate-llmops-front-pvc.sh"
    else
    ...
    if [ -f "$generate_script" ]; then
        ...
        export NAMESPACE="${LLMOPS_SSR_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${LLMOPS_SSR_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    # 自动生成 YAML 文件（如果不存在）
    if ! auto_generate_yaml "$LLMOPS_SSR_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/llmops-app-front-config/deploy-llmops-app-front-config/deploy-llmops-app-front-config.sh"
        local secret_script="$secrets_dir/llmops-app-front-secret/deploy-llmops-app-front-secret/deploy-llmops-app-front-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export LLMOPS_SSR_IMAGE_REGISTRY="${LLMOPS_SSR_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    export LLMOPS_SSR_IMAGE_PROJECT="${LLMOPS_SSR_IMAGE_PROJECT:-k8s-images}"
    export LLMOPS_SSR_IMAGE="${LLMOPS_SSR_IMAGE:-llmops-app-front}"
    export LLMOPS_SSR_TAG="${LLMOPS_SSR_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-Always}"
    export LLMOPS_SSR_IMAGE_PULL_SECRET_NAME="${LLMOPS_SSR_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"
    ...
    # ============================================================
    log_info "🚀 阶段1：部署 LLMOps App SSR 子级组件..."
    if ! deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_error "❌ LLMOps App SSR 子级组件部署失败！"
        return 1
    fi
    ...
    # ============================================================
    log_info "🚀 阶段2：部署 LLMOps App SSR 核心服务..."
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=llmops-app-front"
        echo "  kubectl get svc -n $NAMESPACE -l app=llmops-app-front"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=llmops-app-front -f"
    else
...
uninstall_app() {
    ...
    # ============================================================
    log_info "🚀 阶段1：卸载 LLMOps App SSR 核心服务..."
    # 检查生成的 YAML 文件是否存在
    # 如果 YAML 不存在，尝试自动生成（用于卸载）
    if [ ! -f "$LLMOPS_SSR_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $LLMOPS_SSR_YAML，尝试自动生成..."
        if ! auto_generate_yaml "$LLMOPS_SSR_YAML" "$K8S_RESOURCE_DIR"; then
            log_warn "YAML 生成失败，尝试直接删除资源"
            kubectl delete deployment llmops-app-front -n "$NAMESPACE" --ignore-not-found=true
            kubectl delete service llmops-app-front-service -n "$NAMESPACE" --ignore-not-found=true
        fi
    fi
    ...
    log_success "✅ LLMOps App SSR 核心服务卸载完成"
    
    # ============================================================
    # 阶段2：卸载子级组件（按优先级，逆序卸载）
    # ============================================================
    log_info "🚀 阶段2：卸载 LLMOps App SSR 子级组件..."
    if ! uninstall_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_warn "⚠️ LLMOps App SSR 子级组件卸载部分失败，继续..."
    fi
    log_success "✅ LLMOps App SSR 子级组件卸载完成"
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=llmops-app-front 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=llmops-app-front 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=llmops-app-front 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=llmops-app-front 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=llmops-app-front 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-llmops-front"
            echo "  ./deploy-llmops-front.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
...
uninstall_sub_components() {
    ...
    log_info "开始卸载 LLMOps App SSR 子组件..."
    
    # 使用动态扫描函数卸载组件（逆序：先卸载 Ingress，最后卸载 Namespace）
    # Ingress 组件
    if [[ "${ingress_enabled:-true}" == "true" ]]; then
        if ! scan_and_deploy_components "ingress" "$project_id" "$namespace" "$environment" "uninstall"; then
            log_warn "⚠️ Ingress 组件卸载失败（继续）"
        fi
    fi
    
    # Middleware 组件
    if [[ "${middleware_enabled:-false}" == "true" ]]; then
        if ! scan_and_deploy_components "middleware" "$project_id" "$namespace" "$environment" "uninstall"; then
            log_warn "⚠️ Middleware 组件卸载失败（继续）"
        fi
    fi
    
    # ConfigMap 组件
    if [[ "${configmap_enabled:-true}" == "true" ]]; then
        if ! scan_and_deploy_components "configMap" "$project_id" "$namespace" "$environment" "uninstall"; then
            log_warn "⚠️ ConfigMap 组件卸载失败（继续）"
        fi
    fi
    
    # Secret 组件
    if [[ "${secrets_enabled:-true}" == "true" ]]; then
        if ! scan_and_deploy_components "secret" "$project_id" "$namespace" "$environment" "uninstall"; then
            log_warn "⚠️ Secret 组件卸载失败（继续）"
        fi
    fi
    
    # Namespace 组件（最后卸载）
    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        if ! scan_and_deploy_components "namespace" "$project_id" "$namespace" "$environment" "uninstall"; then
            log_warn "⚠️ Namespace 组件卸载失败（继续）"
        fi
    fi
    
    log_success "✅ LLMOps App SSR 子组件卸载完成"
    return 0
}
