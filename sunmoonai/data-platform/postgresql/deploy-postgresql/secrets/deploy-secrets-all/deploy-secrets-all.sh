...
deploy_sub_components_by_priority() {
    ...
    # 注意：如果组件是TLS Secret，会先检查/生成服务器证书
    local components=(
        "postgresql_auth_secret:${postgresql_auth_secret_enabled:-true}:${postgresql_auth_secret_priority:-1000}:PostgreSQL 认证密钥:$SCRIPT_DIR/../postgresql-auth-secret/deploy-postgresql-auth-secret/deploy-postgresql-auth-secret.sh"
        ...
        # "postgresql_mydb_secret:${postgresql_mydb_secret_enabled:-true}:${postgresql_mydb_secret_priority:-600}:PostgreSQL 数据库连接密钥:$SCRIPT_DIR/../postgresql-mydb-secret/deploy-postgresql-mydb-secret/deploy-postgresql-mydb-secret.sh"  # ❌ 已删除：使用服务级别的 Secret
        "postgresql_authservice_db_secret:${postgresql_authservice_db_secret_enabled:-true}:${postgresql_authservice_db_secret_priority:-500}:Auth Service 数据库连接密钥:$SCRIPT_DIR/../postgresql-authservice-db-secret/deploy-postgresql-authservice-db-secret/deploy-postgresql-authservice-db-secret.sh"
        "postgresql_llmopsservice_db_secret:${postgresql_llmopsservice_db_secret_enabled:-true}:${postgresql_llmopsservice_db_secret_priority:-500}:LLMOps Service 数据库连接密钥:$SCRIPT_DIR/../postgresql-llmopsservice-db-secret/deploy-postgresql-llmopsservice-db-secret/deploy-postgresql-llmopsservice-db-secret.sh"
    )
...
show_help() {
    ...
    echo "示例:"
    echo "  $0                                    # 使用默认参数"
    echo "  $0 sunmoonai postgresql development      # 指定参数"
    echo "  $0 sunmoonai postgresql development true # 试运行模式"
    echo ""
...
main() {
    ...
    log_info "🚀 开始部署 PostgreSQL Secrets..."
    log_info "  项目ID: $project_id"
    log_info "  命名空间: $namespace"
    log_info "  环境: $environment"
    log_info "  试运行: $dry_run"
    ...
    if deploy_secrets "$project_id" "$namespace" "$environment" "$dry_run"; then
        log_success "✅ PostgreSQL Secrets 部署完成！"
        return 0
    else
        log_error "❌ PostgreSQL Secrets 部署失败"
        return 1
    fi
}
