...
# =============================================================================
# Harbor Registry Secret 部署脚本（Investment Web Backend）
...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secret/ -> deploy-investment-web-backend/ -> investment-web-backend/
...
# 尝试加载主配置文件
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-investment-web-backend.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (Investment Web Backend)..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    echo ""
    log_success "Harbor Registry Secret (Investment Web Backend) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (Investment Web Backend)"
    echo ""
    echo "用法:"
