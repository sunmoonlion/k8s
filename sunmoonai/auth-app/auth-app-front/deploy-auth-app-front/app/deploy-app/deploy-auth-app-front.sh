...
# Auth App SSR 部署脚本
# 用法: ./deploy-auth-app-front.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
# 注意：资源 YAML 位置由 RESOURCES_DIR/AUTH_APP_SSR_YAML 指定，resources 目录不改动

set -e

# 脚本目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 项目根目录（deploy-auth-app-front 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-auth-app-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（auth-app-front 目录）
# 从 deploy-auth-app-front/ 向上 1 级到达 auth-app-front/
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

# 保存 Auth App SSR 脚本的目录路径（统一部署模板会重新定义 SCRIPT_DIR）
AUTH_APP_SSR_SCRIPT_DIR="$SCRIPT_DIR"

# 导入统一部署模板
# 从应用根目录（auth-app-front/）到 k8s/ 需要向上 3 级
# auth-app-front/ -> auth-app/ -> sunmoonai/ -> k8s/
...
# 加载部署配置文件
AUTH_APP_SSR_CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-front.conf"
if [[ -f "$AUTH_APP_SSR_CONFIG_FILE" ]]; then
    source "$AUTH_APP_SSR_CONFIG_FILE"
    ...
    log_info "已加载 Auth App SSR 配置文件: $AUTH_APP_SSR_CONFIG_FILE"
else
    log_warn "未找到 Auth App SSR 配置文件: $AUTH_APP_SSR_CONFIG_FILE，使用默认配置"
fi

# 默认配置从配置文件读取（deploy-auth-app-front.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${AUTH_APP_SSR_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${AUTH_APP_SSR_NAMESPACE:-}"
...
deploy_sub_components() {
    ...
    log_info "开始部署 Auth App SSR 子组件..."
    ...
    log_success "✅ Auth App SSR 子组件部署完成"
    return 0
}

# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（auth-app-front/）
# app/deploy-app/ -> app/ -> deploy-auth-app-front/ -> auth-app-front/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
AUTH_APP_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/auth-app-front-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"auth-app-front-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
    ...
    if [ -f "$generate_script" ]; then
        ...
        export NAMESPACE="${AUTH_APP_SSR_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${AUTH_APP_SSR_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    # 自动生成 YAML 文件（如果不存在）
    if ! auto_generate_yaml "$AUTH_APP_SSR_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/auth-app-front-config/deploy-auth-app-front-config/deploy-auth-app-front-config.sh"
        local secret_script="$secrets_dir/auth-app-front-secret/deploy-auth-app-front-secret/deploy-auth-app-front-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_secrets_config() {
    ...
    tmp_sec=$(mktemp)
    # 处理 ${VAR:-default} 语法
    sed -e 's/\${\([^:}]*\):-[^}]*}/\${\1}/g' "$AUTH_APP_SSR_CONFIGMAP" | envsubst > "$tmp_cm"
    sed -e 's/\${\([^:}]*\):-[^}]*}/\${\1}/g' "$AUTH_APP_SSR_SECRET" | envsubst > "$tmp_sec"
...
deploy_app() {
    ...
    export AUTH_APP_SSR_IMAGE_REGISTRY="${AUTH_APP_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com:30443}"
    export AUTH_APP_SSR_IMAGE_PROJECT="${AUTH_APP_SSR_IMAGE_PROJECT:-k8s-images}"
    export AUTH_APP_SSR_IMAGE="${AUTH_APP_SSR_IMAGE:-auth-app-front}"
    export AUTH_APP_SSR_TAG="${AUTH_APP_SSR_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-IfNotPresent}"
    
    AUTH_APP_SSR_FULL_IMAGE_NAME="${AUTH_APP_SSR_IMAGE_REGISTRY}/${AUTH_APP_SSR_IMAGE_PROJECT}/${AUTH_APP_SSR_IMAGE}:${AUTH_APP_SSR_TAG}"
    log_info "使用 Harbor 镜像部署: $AUTH_APP_SSR_FULL_IMAGE_NAME"
    ...
    export AUTH_APP_SSR_FULL_IMAGE_NAME="$AUTH_APP_SSR_FULL_IMAGE_NAME"
    
    # ============================================================
    # 阶段1：部署子级组件（按优先级，先部署依赖项）
    # ============================================================
    log_info "🚀 阶段1：部署 Auth App SSR 子级组件..."
    if ! deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_error "❌ Auth App SSR 子级组件部署失败！"
        return 1
    fi
    ...
    # ============================================================
    log_info "🚀 阶段2：部署 Auth App SSR 核心服务..."
    log_info "部署 Auth App SSR (环境: $ENVIRONMENT, 镜像: $AUTH_APP_SSR_FULL_IMAGE_NAME, 拉取策略: ${IMAGE_PULL_POLICY:-IfNotPresent}, 命名空间: $NAMESPACE)..."
    ...
    kubectl apply -f "$AUTH_APP_SSR_YAML" -n "$NAMESPACE"
    
    if [ $? -eq 0 ]; then
        log_success "Auth App SSR 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=auth-app-front"
        echo "  kubectl get svc -n $NAMESPACE -l app=auth-app-front"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=auth-app-front -f"
    else
        log_error "Auth App SSR 部署失败"
        exit 1
    fi
}
...
uninstall_app() {
    ...
    # ============================================================
    log_info "🚀 阶段1：卸载 Auth App SSR 核心服务..."
    # 检查生成的 YAML 文件是否存在
    # 注意：卸载时不需要重新生成 YAML，直接删除资源即可
    # 如果 YAML 不存在，尝试自动生成（用于卸载）
    if [ ! -f "$AUTH_APP_SSR_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $AUTH_APP_SSR_YAML，尝试自动生成..."
        if ! auto_generate_yaml "$AUTH_APP_SSR_YAML" "$K8S_RESOURCE_DIR"; then
            log_warn "YAML 生成失败，尝试直接删除资源"
            kubectl delete deployment auth-app-front -n "$NAMESPACE" --ignore-not-found=true
            kubectl delete service auth-app-front-service -n "$NAMESPACE" --ignore-not-found=true
        fi
    fi
    
    if [ -f "$AUTH_APP_SSR_YAML" ]; then
        kubectl delete -f "$AUTH_APP_SSR_YAML" -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete deployment auth-app-front -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service auth-app-front-service -n "$NAMESPACE" --ignore-not-found=true
    fi
    log_success "✅ Auth App SSR 核心服务卸载完成"
    
    # ============================================================
    # 阶段2：卸载子级组件（按优先级，逆序卸载）
    # ============================================================
    log_info "🚀 阶段2：卸载 Auth App SSR 子级组件..."
    if ! uninstall_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false; then
        log_warn "⚠️ Auth App SSR 子级组件卸载部分失败，继续..."
    fi
    log_success "✅ Auth App SSR 子级组件卸载完成"
    
    log_success "Auth App SSR 卸载完成！"
}
...
uninstall_sub_components() {
    ...
    log_info "开始卸载 Auth App SSR 子组件..."
    
    # 定义子组件卸载顺序（按优先级排序，逆序卸载）
    local sub_components=(
        "AUTH_APP_SSR_middleware:${middleware_enabled:-false}:${middleware_priority:-1000}:Auth App SSR 中间件:$SCRIPT_DIR/middleware/deploy-middleware-all/deploy-middleware-all.sh"
        "AUTH_APP_SSR_ingress:${ingress_enabled:-false}:${ingress_priority:-100}:Auth App SSR HTTP 路由:$SCRIPT_DIR/ingress/deploy-ingress/deploy-ingress.sh"
    )
    ...
    for component_info in "${sub_components[@]}"; do
        ...
        if [[ "$enabled" == "true" ]]; then
            ...
            if [[ -f "$script_path" ]]; then
                ...
                if [[ "$name" == "AUTH_APP_SSR_ingress" ]]; then
    ...
    log_success "✅ Auth App SSR 子组件卸载完成"
    return 0
}

# 显示状态
show_status() {
    log_info "Auth App SSR 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=auth-app-front 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=auth-app-front 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=auth-app-front 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=auth-app-front 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=auth-app-front 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "操作:"
            echo "  deploy     部署 Auth App SSR"
            echo "  uninstall  卸载 Auth App SSR"
            echo "  status     查看 Auth App SSR 状态"
            echo ""
            echo "参数说明:"
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-auth-app-front"
            echo "  ./deploy-auth-app-front.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
