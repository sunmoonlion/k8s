...
**第五段「运营层」进行中（2026-09-25）**：已做——KIND 开发包的 runner 角色与只换镜像的升级门、丢失身份准备的现场重建、沙箱按需拉起（供给器 + 会合点管理通道 + 设置页）、知识 MCP 数据集从对象存储钉版取。本地 KIND 部署由 switch-test inbox 06 至 09 串行推进。未做见各模块「实现状态」：D10 令牌签发、runner 重启恢复与分片、质量信号、CODEX_HOME 备份。
