...
# 项目根目录（deploy-incubator-bff 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-incubator-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（incubator-app-bff 目录）
# 从 deploy-incubator-bff/ 向上 1 级到达 incubator-app-bff/
...
# 导入统一部署模板
# 从应用根目录（incubator-app-bff/）到 k8s/ 需要向上 4 级
# incubator-app-bff/ -> incubator-app/ -> sunmoonai/ -> k8s/
...
# 默认配置从配置文件读取（deploy-incubator-bff.conf）
...
scan_and_deploy_components() {
    ...
    for subdir in "$base_dir"/*/; do
        ...
        # 将目录名转换为变量名（将 - 替换为 _）
        # 特殊处理：incubator-bff-* -> incubator_bff_*
        # 例如：harbor-registry-secret -> harbor_registry_secret
        #      incubator-bff-secret -> incubator_bff_secret
        #      incubator-bff-config -> incubator_bff_config
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（incubator-app-bff/）
# app/deploy-app/ -> app/ -> deploy-incubator-bff/ -> incubator-app-bff/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
INCUBATOR_BFF_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/incubator-app-bff-generated.yaml"
INCUBATOR_BFF_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/incubator-bff-pvc/generate-incubator-bff-pvc/incubator-app-bff-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"incubator-app-bff-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"incubator-app-bff-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/incubator-bff-pvc/generate-incubator-bff-pvc/generate-incubator-bff-pvc.sh"
    else
...
check_env_config() {
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/incubator-bff-config/deploy-incubator-bff-config/deploy-incubator-bff-config.sh"
        local secret_script="$secrets_dir/incubator-bff-secret/deploy-incubator-bff-secret/deploy-incubator-bff-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
uninstall_app() {
    ...
    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        ...
        log_info "  cd deploy-incubator-bff/namespace/incubator-bff-namespace/deploy-incubator-bff-namespace"
        log_info "  ./deploy-incubator-bff-namespace.sh uninstall $PROJECT_ID $NAMESPACE $ENVIRONMENT"
    fi
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "参数说明:"
            echo "  project_id   项目标识符（从配置文件 deploy-incubator-bff.conf 读取）"
            echo "  namespace    命名空间（从配置文件 deploy-incubator-bff.conf 读取）"
            echo "  environment  环境（从配置文件 deploy-incubator-bff.conf 读取）"
            echo ""
            echo "操作说明:"
