#!/bin/bash
# Auth App Front Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-auth-app-front-secret.conf"
# 从 generate-auth-app-front-secret/ -> auth-app-front-secret/ -> secret/ -> custom-values/ -> k8s-resource/
...
main() {
    log_info "开始生成 Auth App Front Secret YAML..."
