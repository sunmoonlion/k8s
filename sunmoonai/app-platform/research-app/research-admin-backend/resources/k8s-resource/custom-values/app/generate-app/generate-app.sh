#!/bin/bash
# research-admin-backend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> research-admin-backend/
...
export RESEARCH_ADMIN_BACKEND_IMAGE_REGISTRY="${RESEARCH_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export RESEARCH_ADMIN_BACKEND_IMAGE_PROJECT="${RESEARCH_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export RESEARCH_ADMIN_BACKEND_IMAGE="${RESEARCH_ADMIN_BACKEND_IMAGE:-}"
export RESEARCH_ADMIN_BACKEND_TAG="${RESEARCH_ADMIN_BACKEND_TAG:-}"
...
export RESEARCH_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${RESEARCH_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
...
if [[ -n "$RESEARCH_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" ||
      -n "$RESEARCH_ADMIN_BACKEND_REDIS_SECRET_NAME" ||
      -n "$RESEARCH_ADMIN_BACKEND_MONGODB_SECRET_NAME" ]]; then
    if [[ -z "$RESEARCH_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" ||
          -z "$RESEARCH_ADMIN_BACKEND_REDIS_SECRET_NAME" ||
          -z "$RESEARCH_ADMIN_BACKEND_MONGODB_SECRET_NAME" ]]; then
        log_error "PostgreSQL、Redis 和 MongoDB Secret 名称必须同时设置"
        exit 1
    fi
    printf -v RESEARCH_ADMIN_BACKEND_DATABASE_ENV_FROM \
      '        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$RESEARCH_ADMIN_BACKEND_POSTGRESQL_SECRET_NAME" \
      "$RESEARCH_ADMIN_BACKEND_REDIS_SECRET_NAME" \
      "$RESEARCH_ADMIN_BACKEND_MONGODB_SECRET_NAME"
    export RESEARCH_ADMIN_BACKEND_DATABASE_ENV_FROM
fi
...
main() {
    log_info "开始生成 research-admin-backend YAML 文件..."
