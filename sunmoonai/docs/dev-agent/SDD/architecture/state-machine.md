...
workflow **不得自己定义状态**（`C-P0`：任何场景不得新增状态词）——否则每个领域方法都会长出一套
「已取数」「已分析」「待复核」，状态词立刻失控。
...
| `QUEUED` | 已可执行，**等调度**——不需要任何外部事件，预期自行推进 | 完成契约、执行策略、预算、路由决定已持久化 |
...
### `QUEUED` 与 `WAITING` 的分界

**判据只有一条：要不要一个外部事件来解除。**

| | `QUEUED` | `WAITING(<原因>)` |
| --- | --- | --- |
| 在等什么 | 只等调度器轮到它 | 一个**可命名的外部条件** |
| 会自行推进吗 | 会 | **不会**，条件不解除就一直等 |
| 必须有什么 | 无 | 原因码、条件、恢复方式、超时策略（见上表「进入门禁」） |

**同一个事实不得两边都能投影。**逐情形判定：

| 情形 | 进哪个 | 为什么 |
| --- | --- | --- |
| 尚未绑定设备 | `QUEUED` | 选设备是调度器的正常工作，不需要外部事件 |
| 已绑定，但那台设备离线 | `WAITING(DEVICE)` | 要等那台机器上线——外部事件，且用户需要知道 |
| 已绑定，那台设备满载 | `WAITING(RESOURCE)` | 要等它腾出容量——外部事件 |
| 未绑定，设备池满载 | `QUEUED` | 调度器可以选别的设备或等容量释放，仍是正常排队 |
| 配额不足 | `WAITING(RESOURCE)` | 要用户加额度或等周期重置——外部事件 |
| 可靠投递待发送 | `QUEUED` | 正在发，投递层自己会重试 |

⚠ **`QUEUED` 的语义里不再有「等待设备、资源」。**留着那句话，
同一个「设备离线」既满足 `QUEUED` 也满足 `WAITING(DEVICE)`，
客户端、调度器与超时扫描器会各判各的。
