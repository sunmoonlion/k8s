...
CONFIG_FILE="$SCRIPT_DIR/deploy-document-converter-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-document-converter-config/ 向上 3 级到达应用根目录
# deploy-document-converter-config/ -> document-converter-config/ -> configMap/ -> deploy-document-converter-bff/ -> document-converter-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/document-converter-config/generate-document-converter-config/document-converter-config-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/configMap/document-converter-config/generate-document-converter-config/generate-document-converter-config.sh"
