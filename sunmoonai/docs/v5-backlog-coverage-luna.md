...
| 指标/告警 / M1-503、M1-105 | [B7d](v5-backlog-delivery-observation-luna.md) 补只读 gauge/CLI；[B7h](v5-backlog-metrics-http-luna.md) 补受保护 HTTP；[B7j](v5-backlog-worker-progress-luna.md) 补已提交回执聚合及故障恢复 | 观测/受保护入口源码已补；实际监控部署、采集/新鲜度/告警到达与 broker queue 接线已按所有者决定交未来 [N4-OPS-01](dev-agent-task/composition/implementation-plan.md)，尚未实施；新 retrieval/run/SSE 产品指标仍 N4，不算整套观测完成 |
