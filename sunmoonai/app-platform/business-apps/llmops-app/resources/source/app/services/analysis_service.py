...
# -*- coding: utf-8 -*-
"""
LLMOps Analysis Service - 从 imooc-llmops 迁移
已转换为异步版本
...
class AnalysisService(BaseService):
    ...
    async def get_app_analysis(
        ...
        try:
            from app.utils.redis_helper import redis_exists, redis_get
            exists = await redis_exists(self.redis_client, cache_key)
            if exists:
                # 5.解析数据并将数据返回
                app_analysis_str = await redis_get(self.redis_client, cache_key)
                if app_analysis_str:
                    return json.loads(app_analysis_str)
        except Exception:
            # 6.如果出错则什么都不处理，重新计算数据并更新缓存
            pass

        # 7.查询消息表最近7天，以及最近14-7天的数据
        seven_days_messages = await self.get_messages_by_time_range(
            db, app, seven_days_ago, today_midnight
        )
        fourteen_days_messages = await self.get_messages_by_time_range(
            db, app, fourteen_days_ago, seven_days_ago
        )
    ...
    @classmethod
    def calculate_overview_indicators_by_messages(cls, messages: list) -> dict[str, Any]:
        """根据传递的消息列表计算概览指标"""
        # 1.计算全部会话数
        total_messages = len(messages)

        # 2.计算激活用户数
        active_accounts = len({message.user_id for message in messages})
        ...
        # 4.Token输出速度
        token_output_rate = 0
        latency_sum = sum(message.latency for message in messages if message.latency)
        if latency_sum != 0:
            token_output_rate = sum(
                message.total_token_count for message in messages if message.total_token_count
            ) / latency_sum

        # 5.计算费用消耗
        cost_consumption = sum(
            message.total_price for message in messages if message.total_price
        )
    ...
    def calculate_trend_by_messages(
        ...
        for day in range(days_ago):
            ...
            cost_consumption_trend_y_axis = sum(
                ...
                if message.total_price and trend_start_at <= message.created_at < trend_end_at
            )
