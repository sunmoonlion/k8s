...
只在工单判为 T2 时使用；T0/T1 按 （已随标签 `backend-rules-archive` 归档） 执行。协议仍是流程真源，
...
| ⑦ 清理发布 | 指定单写者按 （已随标签 `backend-rules-archive` 归档）–（已随标签 `backend-rules-archive` 归档） 收存证据、清理本次私有产物，再发布 | 目标版本比较通过、批准仍有效；只清理获准对象，不清空他家工作区 |
