...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-kibana/ -> kibana/ -> data-platform/ -> sunmoonai/ -> k8s/
...
# 尝试加载主配置文件（如果存在），以获取 KIBANA_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-kibana.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-kibana.conf"
