...
# 从 generate-harbor-registry-secret/ -> harbor-registry-secret/ -> secret/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-llmops/
...
# ============================================================================
# 设置环境变量（用于模板替换）
# 注意：所有配置都从 generate-harbor-registry-secret.conf 读取，不在脚本中设置硬编码默认值
# ============================================================================
# 基础配置（从 generate-harbor-registry-secret.conf 中读取，所有默认值在 conf 文件中定义）
export NAMESPACE="${NAMESPACE:-}"
export ENVIRONMENT="${ENVIRONMENT:-}"
export ENV="${ENV:-}"

# Harbor Secret 数据内容（从 generate-harbor-registry-secret.conf 中读取，所有默认值在 conf 文件中定义）
HARBOR_DOCKER_SERVER="${HARBOR_REGISTRY_URL:-}"
HARBOR_DOCKER_USERNAME="${HARBOR_REGISTRY_USERNAME:-}"
HARBOR_DOCKER_PASSWORD="${HARBOR_REGISTRY_PASSWORD:-}"
...
export HARBOR_REGISTRY_SECRET_NAME="${HARBOR_REGISTRY_SECRET_NAME:-}"
