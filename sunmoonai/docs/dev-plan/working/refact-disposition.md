...
| ~~D-Q6~~ → **D-K8** | ~~qwen~~ → **kimi** | 自由文本分类器的默认通用兜底（kimi 候选 §15.2） | **拒绝**：与「上层必须确定性代码」冲突（luna 评审 D-2 同判）。**出处更正见下** |

> **D-Q6 出处更正（2026-09-04，采纳 qwen 异议）**：该主张原误挂在 qwen 名下，实为 kimi 候选
> §15.2「自由文本走受限分类器（闭集标签 + 置信度阈值 + **兜底通用档**）」，重编号为 **D-K8**。
> 三证：qwen 候选对「自由文本／分类器／通用兜底」grep **零命中**；kimi 候选该段逐字命中；
> luna 评审 D 块第 2 条本就归给「kimi，§15.2」——**裁决方转录时串了行**。
> 拒绝结论本身不变，只改归属。
>
> **作废的判断不删除**：原条目保留划线，因为它记录了当时的信息状态。
> 这与本轮 D-C5（`constraints.md` I12 错引）是同一类错误的镜像——**出处错挂会污染整合审计链**，
> 而处置记录正是 ⑤ 验收要拿来对账的东西。
...
**2026-09-04 已更换**，依 `round-protocol.md`「换角色只有一个合法理由：该家不可用」：

| 项 | 内容 |
| --- | --- |
| 原指定 | kimi（采纳 4 条） |
| 新指定 | **qwen**（采纳 5 条） |
| 原因 | kimi 经裁定 R1 免除参与，属「不可用」——**不是对结果不满意** |
| 顺位计算 | 排除裁决方 opus、基座作者 luna；`kimi(4) → qwen(5) → cursor(12)`，顺位已在 ④ 通知中预先声明 |
| 记录 | `refact-rulings.md` R1、R2 |
