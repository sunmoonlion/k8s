#!/bin/bash
# Llm Admin Backend Namespace YAML 生成脚本
# 根据配置生成 Namespace 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-llm-admin-backend-namespace.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-llm-admin-backend-namespace/ -> llm-admin-backend-namespace/ -> namespace/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-llm-admin-backend-namespace/ -> llm-admin-backend-namespace/ -> namespace/ -> custom-values/ -> k8s-resource/ -> resources/ -> llm-admin-backend/
...
# 生成 YAML
main() {
    log_info "开始生成 Llm Admin Backend Namespace YAML 文件..."
