#!/bin/bash
# research-admin-frontend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-research-admin-frontend-secret.conf"
# 从 generate-research-admin-frontend-secret/ -> research-admin-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-research-admin-frontend-secret/ -> ... -> k8s-resource/ -> resources/ -> research-admin-frontend/
...
main() {
    log_info "开始生成 research-admin-frontend Secret YAML 文件..."
