...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> llm-web-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

# 尝试读取主应用的 deploy-*.conf（作为基础配置的默认值源）
MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-llm-web-backend/app/deploy-app/deploy-llm-web-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${LLM_WEB_BACKEND_NAMESPACE:-}")
...
export LLM_WEB_BACKEND_IMAGE_REGISTRY="${LLM_WEB_BACKEND_IMAGE_REGISTRY:-}"
export LLM_WEB_BACKEND_IMAGE_PROJECT="${LLM_WEB_BACKEND_IMAGE_PROJECT:-}"
export LLM_WEB_BACKEND_IMAGE="${LLM_WEB_BACKEND_IMAGE:-}"
export LLM_WEB_BACKEND_TAG="${LLM_WEB_BACKEND_TAG:-}"
export LLM_WEB_BACKEND_FULL_IMAGE_NAME="${LLM_WEB_BACKEND_IMAGE_REGISTRY}/${LLM_WEB_BACKEND_IMAGE_PROJECT}/${LLM_WEB_BACKEND_IMAGE}:${LLM_WEB_BACKEND_TAG}"
export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-}"
export LLM_WEB_BACKEND_IMAGE_PULL_SECRET_NAME="${LLM_WEB_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
...
export LLM_WEB_BACKEND_QUEUE_REDIS_SECRET_NAME="${LLM_WEB_BACKEND_QUEUE_REDIS_SECRET_NAME:-}"
...
# 生成 YAML
main() {
    log_info "开始生成 Llm Web Backend YAML 文件..."
