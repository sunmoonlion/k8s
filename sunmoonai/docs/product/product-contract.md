...
| 事实 | 落点 |
| --- | --- |
| 用户的资料与研究成果**不经过我们**——存在用户电脑上，发给哪家模型由用户自选、自付 | 本地知识库（§2）；结果端到端加密（[`invariants.md`](../dev-agent/SDD/architecture/invariants.md)「问题侧明文，资料侧加密」） |
...
**让用户能验证**：runtime 应该开源；二进制必须签名，应该可复现构建；必须公开 runtime 与 Codex 会连接的全部域名，以及 ① 与 ⑤ 的协议文档，证明**不存在把 key 与资料上传到我们这一侧的途径**（⚠ 不是「不存在上传途径」——key 与被读取的内容会由本机直发用户自选的模型厂商，见 [数据流与信任边界](../dev-agent/SDD/architecture/trust.md)）。后端对仍能看到的内容（查询、元数据）以服务条款、隐私政策与数据处理协议约束。
