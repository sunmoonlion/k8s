#!/bin/bash
# toutiao-app-backend Namespace YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-toutiao-app-backend-namespace.conf"
# 从 generate-toutiao-app-backend-namespace/ -> toutiao-app-backend-namespace/ -> namespace/ -> custom-values/ -> k8s-resource/
...
source "$CONFIG_FILE"
[ "${ENABLED:-true}" != "true" ] && log_info "跳过资源生成: namespace (已禁用)" && exit 0
...
main() {
    log_info "开始生成 toutiao-app-backend Namespace YAML..."
