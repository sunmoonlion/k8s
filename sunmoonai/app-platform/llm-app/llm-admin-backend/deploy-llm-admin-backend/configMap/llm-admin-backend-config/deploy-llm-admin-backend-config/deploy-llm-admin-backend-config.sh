...
CONFIG_FILE="$SCRIPT_DIR/deploy-llm-admin-backend-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llm-admin-backend-config/ 向上 3 级到达应用根目录
# deploy-llm-admin-backend-config/ -> llm-admin-backend-config/ -> configMap/ -> deploy-llm-admin-backend/ -> llm-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/llm-admin-backend-config/generate-llm-admin-backend-config/llm-admin-backend-config-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/configMap/llm-admin-backend-config/generate-llm-admin-backend-config/generate-llm-admin-backend-config.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${LLM_ADMIN_BACKEND_CONFIGMAP_NAME:-llm-admin-backend-config}"
