...
knowledge-app 是**知识库域 App**：消费 info-app 分发的不可变 artifact，校验后入库到检索提供方（RAGFlow），并向下游研究消费方提供受契约约束的检索/引用（retrieval/citation）服务。它是 Info→Knowledge→Investment 数据链的中枢。

> 命名注记：旧 `research-app` 已由 `investment-app` 取代，当前检索契约的实际消费方是 investment-app；本仓 manifest 与 service auth 配置中残留的 `research-app`/`sunmoonai-research-knowledge-retrieve` 名称属历史兼容命名，不代表存在一个与 Investment 并列的活动 research-app。

- 拓扑：`knowledge-admin-frontend + knowledge-web-frontend + unified backend`（knowledge-backend，app_slug=knowledge）。
...
- retrieve：`sunmoonai-research-knowledge-retrieve`（config.py 默认值仍是旧身份；investment 出站侧已用 `sunmoonai-investment-knowledge-retrieve`，命名迁移两侧未完全收口，部署时以 subject allowlist/env 覆盖为准）、scope `knowledge:retrieve`；各自独立 discovery/backchannel/audience/subject allowlist。
