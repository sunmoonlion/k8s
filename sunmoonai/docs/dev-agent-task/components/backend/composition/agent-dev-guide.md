...
角色分离以 round-protocol“⑤ 验收 与 ⑥ 确认”为准，本文 [§3.19](../../../../dev-agent-standards/protocol/round-operations.md) 给出操作算法。
...
——最后四项是 [§3.6](../components/04-agent-execution/agent-dev-guide.md) 命名空间纪律在工单上的落点。
...
产物命名、候选冻结、处置表和验收方算法见 [§3.19](../../../../dev-agent-standards/protocol/round-operations.md)–[§3.22](../components/04-agent-execution/agent-dev-guide.md)，来源为现行 round-protocol。状态脚本从 commit 反推，工作区
...
但**必须可数**，因为成本上界（[§6.2](../components/02-intake/agent-dev-guide.md)）和绕过口径（[§6.3](../components/02-intake/agent-dev-guide.md)）都要数它。
...
这是 [§12](../components/06-acceptance-commit/agent-dev-guide.md)「拿休眠代码当能力证据」那条失败的词汇层防线；配套的可运行验证动作是
**回跑 dormant 测试**并同时确认「锚点仍在」与「能力仍未接线」两个方向（[§2.6](../components/04-agent-execution/agent-dev-guide.md)）。
...
| # | 独立观察 | 结论与落点 |
| --- | --- | --- |
| K1 | `rg 'interrupt\(|Command\(resume=' investment-backend/app/app`；实际锚见 [§4.3](../components/05-interrupt-resume/agent-dev-guide.md) | 出向和入向原语都接受业务值；删去平台自造的固定修订 schema，直接绑定产品 Interaction |
...
| 人自己写下一版就必须换 Attempt | **不采用这个触发条件**。人的内容归属保留；执行是否延续按 [§4.3](../components/05-interrupt-resume/agent-dev-guide.md) checkpoint 和终态判，不按作者类型判 |
