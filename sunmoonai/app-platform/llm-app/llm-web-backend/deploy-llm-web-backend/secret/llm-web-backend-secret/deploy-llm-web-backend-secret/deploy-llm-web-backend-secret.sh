...
CONFIG_FILE="$SCRIPT_DIR/deploy-llm-web-backend-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llm-web-backend-secret/ 向上 3 级到达应用根目录
# deploy-llm-web-backend-secret/ -> llm-web-backend-secret/ -> secret/ -> deploy-llm-web-backend/ -> llm-web-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/llm-web-backend-secret/generate-llm-web-backend-secret/llm-web-backend-secret-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/llm-web-backend-secret/generate-llm-web-backend-secret/generate-llm-web-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${LLM_WEB_BACKEND_SECRET_NAME:-llm-web-backend-secret}"
