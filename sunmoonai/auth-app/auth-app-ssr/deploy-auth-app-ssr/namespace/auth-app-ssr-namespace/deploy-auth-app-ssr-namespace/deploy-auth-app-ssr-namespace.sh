...
CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-ssr-namespace.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-auth-app-ssr-namespace/ -> auth-app-ssr-namespace/ -> namespace/ -> deploy-auth-app-ssr/ -> auth-app-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
NAMESPACE_YAML="$K8S_RESOURCE_DIR/custom-values/namespace/auth-app-ssr-namespace/generate-auth-app-ssr-namespace/auth-app-ssr-namespace-generated.yaml"
...
# 默认配置从配置文件读取（deploy-auth-app-ssr-namespace.conf）
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/namespace/auth-app-ssr-namespace/generate-auth-app-ssr-namespace/generate-auth-app-ssr-namespace.sh"
