...
- 未触碰其他 Harbor 仓库。
- Harbor v2.13.2 GC dry-run（任务 `16`）在
  `delete_untagged=false` 下确认可安全回收约 2.164 GiB。
- 真实 GC（任务 `17`）执行成功：删除 595 个无引用 blob、368 个无引用
  manifest，回收 `2,269,180,564` 字节。
- GC 后 registry 所在存储的空闲量由 `699,953,111,040` 增至
  `702,226,534,400` 字节。
- Harbor quota API 已刷新：`app-images` 使用 `7,569,316,700` 字节，
  `k8s-images` 使用 `7,939,650,115` 字节，合计约 14.44 GiB。
