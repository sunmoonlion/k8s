...
# 计算项目根目录（应用根目录）
# 从 deploy-investment-web-frontend-config/ 向上 3 级到达应用根目录
# deploy-investment-web-frontend-config/ -> investment-web-frontend-config/ -> configMap/ -> deploy-investment-web-frontend/ -> investment-web-frontend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/investment-web-frontend-config/generate-investment-web-frontend-config/investment-web-frontend-config-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/configMap/investment-web-frontend-config/generate-investment-web-frontend-config/generate-investment-web-frontend-config.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${INVESTMENT_WEB_FRONTEND_CONFIGMAP_NAME:-investment-web-frontend-config}"
