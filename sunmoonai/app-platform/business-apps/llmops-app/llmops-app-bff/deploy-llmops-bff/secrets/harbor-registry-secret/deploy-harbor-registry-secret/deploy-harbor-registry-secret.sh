...
# =============================================================================
# Harbor Registry Secret 部署脚本（LLMOPS BFF）
...
# 计算项目根目录（k8s 目录）
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-llmops-bff/ -> llmops-app-bff/ -> llmops-app/ -> business-apps/ -> app-platform/ -> sunmoonai/ -> k8s/
...
# 主配置文件：../../deploy-llmops-bff.conf
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-llmops-bff.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (LLMOPS BFF)..."
    log_info "部署参数："
    ...
    echo ""
    log_success "Harbor Registry Secret (LLMOPS BFF) 部署完成！"
    log_info "部署信息："
...
show_help() {
    echo "Harbor Registry Secret 部署脚本 (LLMOPS BFF)"
    echo ""
    echo "用法:"
