...
# 用法: ./deploy-incubator-backend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-incubator-backend 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-incubator-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（incubator-app-backend 目录）
# 从 deploy-incubator-backend/ 向上 1 级到达 incubator-app-backend/
...
# 加载部署配置文件
INCUBATOR_BFF_CONFIG_FILE="$SCRIPT_DIR/deploy-incubator-backend.conf"
...
# 默认配置从配置文件读取（deploy-incubator-backend.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${INCUBATOR_BFF_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${INCUBATOR_BFF_NAMESPACE:-}"
...
scan_and_deploy_components() {
    ...
    for subdir in "$base_dir"/*/; do
        ...
        # 将目录名转换为变量名（将 - 替换为 _）
        # 特殊处理：incubator-backend-* -> incubator_bff_*
        # 例如：harbor-registry-secret -> harbor_registry_secret
        #      incubator-backend-secret -> incubator_bff_secret
        #      incubator-backend-config -> incubator_bff_config
        local var_base=$(echo "$dirname" | tr '-' '_')
        # 处理 incubator_app_bff_* 简化为 incubator_bff_*
...
INCUBATOR_BFF_IMAGE="${INCUBATOR_BFF_IMAGE:-}"
INCUBATOR_BFF_TAG="${INCUBATOR_BFF_TAG:-}"

# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（incubator-app-backend/）
# app/deploy-app/ -> app/ -> deploy-incubator-backend/ -> incubator-app-backend/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
INCUBATOR_BFF_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/incubator-app-backend-generated.yaml"
INCUBATOR_BFF_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/incubator-backend-pvc/generate-incubator-backend-pvc/incubator-app-backend-pvc-generated.yaml"
# 模板文件路径（在 resources/k8s-resource/templates/）
TEMPLATES_DIR="${K8S_RESOURCE_DIR}/templates"
INCUBATOR_BFF_CONFIGMAP="${TEMPLATES_DIR}/configmap/incubator-app-backend-config.yaml"
INCUBATOR_BFF_SECRET="${TEMPLATES_DIR}/secret/incubator-app-backend-secret.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"incubator-app-backend-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"incubator-app-backend-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/incubator-backend-pvc/generate-incubator-backend-pvc/generate-incubator-backend-pvc.sh"
    else
    ...
    if [ -f "$generate_script" ]; then
        ...
        export NAMESPACE="${INCUBATOR_BFF_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INCUBATOR_BFF_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    # 自动生成 YAML 文件（如果不存在）
    if ! auto_generate_yaml "$INCUBATOR_BFF_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/incubator-backend-config/deploy-incubator-backend-config/deploy-incubator-backend-config.sh"
        local secret_script="$secrets_dir/incubator-backend-secret/deploy-incubator-backend-secret/deploy-incubator-backend-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export INCUBATOR_BFF_IMAGE="${INCUBATOR_BFF_IMAGE:-incubator-app-backend}"
    ...
    # 部署 PVC（如果存在）
    if [ -f "$INCUBATOR_BFF_PVC_YAML" ]; then
        log_info "部署 PVC..."
        kubectl apply -f "$INCUBATOR_BFF_PVC_YAML" -n "$NAMESPACE"
        if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=incubator-app-backend"
        echo "  kubectl get svc -n $NAMESPACE -l app=incubator-app-backend"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=incubator-app-backend -f"
    else
...
uninstall_app() {
    ...
    if [ ! -f "$INCUBATOR_BFF_YAML" ]; then
        ...
        kubectl delete deployment incubator-app-backend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service incubator-app-backend-service -n "$NAMESPACE" --ignore-not-found=true
    else
    ...
    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        ...
        log_info "  cd deploy-incubator-backend/namespace/incubator-backend-namespace/deploy-incubator-backend-namespace"
        log_info "  ./deploy-incubator-backend-namespace.sh uninstall $PROJECT_ID $NAMESPACE $ENVIRONMENT"
    fi
...
uninstall_sub_components() {
    ...
    local sub_components=(
        "incubator_bff_middleware:${middleware_enabled:-false}:${middleware_priority:-1000}:Incubator App BFF 中间件:$PROJECT_ROOT/middleware/deploy-middleware-all/deploy-middleware-all.sh"
        "incubator_bff_ingress:${ingress_enabled:-false}:${ingress_priority:-100}:Incubator App BFF HTTP 路由:$PROJECT_ROOT/ingress/deploy-ingress/deploy-ingress.sh"
    )
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=incubator-app-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=incubator-app-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=incubator-app-backend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=incubator-app-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=incubator-app-backend 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "参数说明:"
            echo "  project_id   项目标识符（从配置文件 deploy-incubator-backend.conf 读取）"
            echo "  namespace    命名空间（从配置文件 deploy-incubator-backend.conf 读取）"
            echo "  environment  环境（从配置文件 deploy-incubator-backend.conf 读取）"
            echo ""
            echo "操作说明:"
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-incubator-backend"
            echo "  ./deploy-incubator-backend.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
