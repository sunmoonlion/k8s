...
（长期记忆、用户级配置文件）的与分支无关；在仓库内的只在共享分支 master 提交
一次、全体助手共享，功能分支从 master 开出时天然继承，功能分支上不建不改
（属治理文件，见 §7）——新助手启动时若当前在功能分支，只读不建，切回 master
再创建（或将配置放仓库外）。

**多助手互不干扰**：仓库内指针文件按“每工具一个槽位”而非每助手一份：文件名由
工具约定固定（仓库根 AGENTS.md、CLAUDE.md、`.cursor/rules/` 等），内容为同一行
...
| Claude Code | 项目 CLAUDE.md 一行 | 会话启动自动读取 |
| Codex | 仓库根 AGENTS.md 一行 | 原生自动读取 |
