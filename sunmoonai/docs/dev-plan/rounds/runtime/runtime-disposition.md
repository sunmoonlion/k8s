...
| E-12 | 删 `refact-fable.md` 时只验了「锚点能否从 git 历史复跑」，没验「人能否读到」；而该文 §3.13.2–3.13.5（回执仓对象模型与回执 schema，**S1 直接依赖**）、§6 实施路线 R0–R5、§3.4 / §3.6 / §3.7 在活跃文档里**没有第二个落点** | **所有者**（「读的人不可能像你一样去 git 历史啊」） | 删除后这些规范内容对人不可达。且我对它用了比它自己 §5.3「删除条件」更低的标准——那条要求先迁出零缺口再删 | 已恢复，并加「被推翻 / 仍有效 / 删除条件」抬头。**内容迁出到独立文档之前不得再删**。T0 的其余部分（锚点钉 commit、§7 移出、`anchor-gate`）仍然成立 |

**元观察**：本轮 `observations-opus.md` §G 记过「裁决方犯的错多于执行者」。②之后这条仍然成立：
E-1…E-12 十二条全部是裁决方的，而五家候选的实质缺陷主要集中在 qwen 与 kimi 的两处样例。
