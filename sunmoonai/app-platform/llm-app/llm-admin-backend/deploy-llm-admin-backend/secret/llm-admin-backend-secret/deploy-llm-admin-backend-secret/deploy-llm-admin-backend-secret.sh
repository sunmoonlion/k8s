...
CONFIG_FILE="$SCRIPT_DIR/deploy-llm-admin-backend-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llm-admin-backend-secret/ 向上 3 级到达应用根目录
# deploy-llm-admin-backend-secret/ -> llm-admin-backend-secret/ -> secret/ -> deploy-llm-admin-backend/ -> llm-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/llm-admin-backend-secret/generate-llm-admin-backend-secret/llm-admin-backend-secret-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/llm-admin-backend-secret/generate-llm-admin-backend-secret/generate-llm-admin-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${LLM_ADMIN_BACKEND_SECRET_NAME:-llm-admin-backend-secret}"
