...
**读本节任何一条都要带着这个前提，不得把目标态写成现状**——这正是 [§5.2](../../composition/agent-dev-guide.md) 推论 2
...
这是执行器 spike，**不是 [§7.1](../../composition/agent-dev-guide.md) 的 G0 协议修复步骤**。
...
⚠ **注意这条与 [§4.3](../05-interrupt-resume/agent-dev-guide.md) 的关系**：`dev.change` 走的是 LangGraph checkpointer，**原地 resume 可用**
（[§4.3](../05-interrupt-resume/agent-dev-guide.md) 已实测）。本节说的是 Harness 腿——**两条腿的恢复语义不同，不可互相外推**。
...
当前批准共同决定（[§4.5](../../composition/agent-dev-guide.md) 的交集公式）。**上一 Task 的工作区不得在未重新受理、授权和物化的
情况下复用给下一 Task。**
...
⚠ **「已派工」「全部返回」「多数一致」都不等于完成**（[§3.12](../06-acceptance-commit/agent-dev-guide.md)、§11）。
...
这落实 [§4.6](../../composition/agent-dev-guide.md) 的 Scope 与 Policy；**只写“两类 worker 互斥”而不管子进程继承，边界仍会漏**。
...
- 当前推进哪件事、卡在哪、下一动作，由任务目录（`turns/`、`composition/`、`components/`）推出，不落在脑子里；
- **「不能倒退的输入」必须写下来**——已经定过的事下次不重新讨论，写进定稿（[`composition/`](../../../../composition/README.md)），派工时写进 turn 的 `task.md`；
- 停下来之前，先让任务目录能回答：「接手的人一分钟内要知道什么？」

⚠ **交接投影不是第二真源**，不得在 `task.md` 或 turn 里复制整张覆盖矩阵。
checkpoint 是恢复输入，不是第二份代码真源，**也不保存凭据**。

⚠ **`composition/` 与各 turn 的 `task.md` 是单写者面。**多名执行者并行时只有派工的一方（协调者）写它们，各执行者只交回到自己的 turn，
进度写在**自己的 worktree** 里。判据不是「内容重不重要」，而是
「同一事实只能有一个权威写入者」（`I13`，[§1.2](../../composition/agent-dev-guide.md)）——**多人各自往同一份定稿 追加，
就是在共享路径上并发写**。
...
⚠ **§2.6–§2.10 描述的执行层，在本清单清空之前一律按 `defined` 对待**（[§5.8](../../composition/agent-dev-guide.md) 四级词典）。
...
| 每个执行者都改共享的 `task.md` 或 `composition/` | 单写者面被互相覆盖，定稿失真 |
