...
- 旧 Research 四组件运行态保留为**回滚面**，删除需单独评估；历史材料只在冻结标签里，不得复制回活动架构。
...
发布规则：三子仓各自过质量门禁 → 不可变候选 tag 推 `harbor.sunmoonai.com:30443/app-images` → 记录 manifest digest 写入 K8s 仓库 Investment `release-inputs.json` → 用 `tpl-app/k8s-scaffold-v2` 部署 → **门禁通过后才能晋级正式 `2.0.0`，禁止重新构建再打 tag**；K8s 清单必须引用 `repository@sha256:...`。
...
| 旧 Research 运行态保留为回滚面 | CLAUDE.md / README.md |
