...
CONFIG_FILE="$SCRIPT_DIR/deploy-celeryworker-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-celeryworker-secret/ 向上 3 级到达应用根目录
# deploy-celeryworker-secret/ -> celeryworker-secret/ -> secret/ -> deploy-celeryworker-incubator/ -> celeryworker-incubator/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/celeryworker-secret/generate-celeryworker-secret/celeryworker-secret-generated.yaml"
...
# 默认配置从配置文件读取（deploy-celeryworker-secret.conf）
...
check_sensitive_config() {
  ...
  if [[ "${POSTGRES_PASSWORD:-zym123}" == "zym123" ]]; then
    log_warn "⚠️  POSTGRES_PASSWORD 仍使用默认值，请修改为实际密码"
    warnings=$((warnings + 1))
  fi
  
  if [[ "${NEO4J_PASSWORD:-zym123}" == "zym123" ]]; then
    log_warn "⚠️  NEO4J_PASSWORD 仍使用默认值，请修改为实际密码"
    warnings=$((warnings + 1))
  fi
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/celeryworker-secret/generate-celeryworker-secret/generate-celeryworker-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="celeryworker-secret"
