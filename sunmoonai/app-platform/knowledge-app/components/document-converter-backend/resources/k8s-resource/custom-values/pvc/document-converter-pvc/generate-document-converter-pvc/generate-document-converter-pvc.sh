#!/bin/bash
# Document Converter BFF PVC YAML 生成脚本
# 根据配置生成 PVC 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-document-converter-pvc.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-document-converter-pvc/ -> document-converter-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-document-converter-pvc/ -> document-converter-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/ -> resources/ -> document-converter-backend/
...
# ============================================================================
# 设置环境变量（用于模板替换）
# 注意：所有配置都从 generate-document-converter-pvc.conf 读取，不在脚本中设置硬编码默认值
# ============================================================================
# 基础配置（从 generate-document-converter-pvc.conf 中读取，所有默认值在 conf 文件中定义）
export NAMESPACE="${NAMESPACE:-}"
export ENVIRONMENT="${ENVIRONMENT:-}"
export ENV="${ENV:-}"

# PVC 数据内容（从 generate-document-converter-pvc.conf 中读取，所有默认值在 conf 文件中定义）
...
# 生成 YAML
main() {
    log_info "开始生成 Document Converter BFF PVC YAML 文件..."
