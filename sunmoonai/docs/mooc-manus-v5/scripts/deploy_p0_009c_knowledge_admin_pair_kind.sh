...
HOST="knowledge-admin-p0-009c.sunmoonai.com"
PUBLIC_PORT="30443"
BACKEND_IMAGE=""
FRONTEND_IMAGE=""
DEPLOYMENT_ID="p0-009c-v1"
IDENTITY_SECRET="sunmoonai-p0-009c-admin-identity"
RUNTIME_SECRET="sunmoonai-p0-009c-admin-runtime"
MODE="plan"
...
usage() {
  cat <<'EOF'
Usage: deploy_p0_009c_knowledge_admin_pair_kind.sh [--apply|--cleanup] [options]
...
cleanup() {
  k delete ingressroute knowledge-admin-p0-009c -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete pdb knowledge-admin-backend-p0-009c knowledge-admin-frontend-p0-009c \
    -n "$NAMESPACE" --ignore-not-found=true >/dev/null
  k delete service knowledge-admin-backend-p0-009c knowledge-admin-frontend-p0-009c \
    p0-009c-admin-postgresql p0-009c-admin-redis -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete deployment knowledge-admin-backend-p0-009c knowledge-admin-frontend-p0-009c \
    p0-009c-admin-postgresql p0-009c-admin-redis -n "$NAMESPACE" \
    --ignore-not-found=true --wait=true >/dev/null
  k delete job p0-009c-admin-migration -n "$NAMESPACE" \
    --ignore-not-found=true --wait=true >/dev/null
  k delete configmap knowledge-admin-p0-009c-config -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  k delete secret "$RUNTIME_SECRET" -n "$NAMESPACE" \
    --ignore-not-found=true >/dev/null
  for _attempt in $(seq 1 60); do
    if ! k get pod -n "$NAMESPACE" -o name | grep -Eq '(knowledge-admin.*p0-009c|p0-009c-admin)'; then
  ...
  k get pod -n "$NAMESPACE" -o name | grep -E '(knowledge-admin.*p0-009c|p0-009c-admin)' >&2 || true
  return 1
}
...
k label secret "$RUNTIME_SECRET" -n "$NAMESPACE" \
  sunmoonai.com/task=v5-p0-009c --overwrite >/dev/null
...
kind: ConfigMap
metadata:
  name: knowledge-admin-p0-009c-config
  namespace: ${NAMESPACE}
  labels:
    sunmoonai.com/task: v5-p0-009c-admin
data:
  ENV: production
  LOG_LEVEL: INFO
  SERVICE_NAME: knowledge-admin-backend-p0-009c
  APP_SLUG: knowledge
  SURFACE: admin
  REDIS_HOST: p0-009c-admin-redis
  REDIS_PORT: "6379"
  REDIS_DB: "0"
  ...
  CASDOOR_APPLICATION: sunmoonai-knowledge-admin-p0-009c
  CASDOOR_VERIFY_SSL: "true"
  AUTH_POLICY_VERSION: knowledge-admin-p0-009c-v1
  AUTH_ROLE_ALLOWLIST: admin,operator
  AUTH_SCOPE_ALLOWLIST: knowledge:admin
  FRONTEND_BASE_URL: "${origin}"
  FRONTEND_ALLOWED_ORIGINS: "${origin}"
  ALLOWED_HOSTS: "${HOST},knowledge-admin-backend-p0-009c"
  SESSION_COOKIE_SECURE: "true"
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: p0-009c-admin-postgresql
  namespace: ${NAMESPACE}
  labels:
    sunmoonai.com/task: v5-p0-009c-admin
spec:
...
k rollout status deployment/p0-009c-admin-postgresql -n "$NAMESPACE" --timeout=180s
k rollout status deployment/p0-009c-admin-redis -n "$NAMESPACE" --timeout=180s
          ...
          args:
            - |
              for attempt in \$(seq 1 30); do
                .venv/bin/python -c "import socket; socket.create_connection(('p0-009c-admin-postgresql', 5432), 2).close()" && break
            ...
            - name: POSTGRES_PASSWORD
              valueFrom:
                secretKeyRef:
                  ...
                  key: POSTGRES_PASSWORD
            - name: DATABASE_URL
              value: postgresql+asyncpg://tpl:\$(POSTGRES_PASSWORD)@p0-009c-admin-postgresql:5432/tpl
          securityContext:
            allowPrivilegeEscalation: false
            capabilities:
              drop: ["ALL"]
EOF
k wait --for=condition=complete job/p0-009c-admin-migration \
  -n "$NAMESPACE" --timeout=180s
k logs job/p0-009c-admin-migration -n "$NAMESPACE" --tail=20

cat <<EOF | k apply -f - >/dev/null
apiVersion: apps/v1
kind: Deployment
metadata:
  name: knowledge-admin-backend-p0-009c
  namespace: ${NAMESPACE}
  labels:
    app: knowledge-admin-backend-p0-009c
    sunmoonai.com/task: v5-p0-009c-admin
spec:
  replicas: 2
...
kind: Deployment
metadata:
  name: knowledge-admin-frontend-p0-009c
  namespace: ${NAMESPACE}
  labels:
    app: knowledge-admin-frontend-p0-009c
    sunmoonai.com/task: v5-p0-009c-admin
spec:
  replicas: 2
  revisionHistoryLimit: 3
        ...
        - name: frontend
          ...
          env:
            ...
            - name: ADMIN_BACKEND_INTERNAL_URL
              value: http://knowledge-admin-backend-p0-009c:8000
            - name: DEPLOYMENT_ID
              value: "${DEPLOYMENT_ID}"
          readinessProbe:
...
k rollout status deployment/knowledge-admin-backend-p0-009c \
  -n "$NAMESPACE" --timeout=240s
k rollout status deployment/knowledge-admin-frontend-p0-009c \
  -n "$NAMESPACE" --timeout=240s

backend_ready="$(k get deployment knowledge-admin-backend-p0-009c -n "$NAMESPACE" \
  -o jsonpath='{.status.readyReplicas}')"
frontend_ready="$(k get deployment knowledge-admin-frontend-p0-009c -n "$NAMESPACE" \
  -o jsonpath='{.status.readyReplicas}')"
