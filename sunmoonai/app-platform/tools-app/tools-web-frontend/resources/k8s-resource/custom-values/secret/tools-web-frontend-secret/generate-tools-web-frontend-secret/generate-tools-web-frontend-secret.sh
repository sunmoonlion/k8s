...
CONFIG_FILE="$SCRIPT_DIR/generate-tools-web-frontend-secret.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-tools-web-frontend-secret/ -> tools-web-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-tools-web-frontend-secret/ -> tools-web-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/ -> resources/ -> tools-web-frontend/
