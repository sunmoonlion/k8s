...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-incubator-front/ -> incubator-app-front/
...
# 主配置文件路径：../../deploy-incubator-front.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-incubator-front.conf"
...
main() {
    ...
    echo ""
    log_success "Harbor Registry Secret (Incubator SSR) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
