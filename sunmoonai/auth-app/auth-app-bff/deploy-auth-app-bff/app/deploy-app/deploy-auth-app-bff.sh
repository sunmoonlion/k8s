...
# Auth App BFF 部署脚本
# 用法: ./deploy-auth-app-bff.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-auth-app-bff 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-auth-app-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（auth-app-bff 目录）
# 从 deploy-auth-app-bff/ 向上 1 级到达 auth-app-bff/
...
# 导入统一部署模板
# 从应用根目录（auth-app-bff/）到 k8s/ 需要向上 3 级
# auth-app-bff/ -> auth-app/ -> sunmoonai/ -> k8s/
...
# 加载部署配置文件
AUTH_APP_BFF_CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-bff.conf"
...
# 默认配置从配置文件读取（deploy-auth-app-bff.conf）
...
scan_and_deploy_components() {
    ...
    for subdir in "$base_dir"/*/; do
        ...
        # 将目录名转换为变量名（将 - 替换为 _）
        # 特殊处理：auth-app-bff-* -> auth_app_bff_*
        # 例如：harbor-registry-secret -> harbor_registry_secret
        #      auth-app-bff-secret -> auth_app_bff_secret
        #      auth-app-bff-config -> auth_app_bff_config
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（auth-app-bff/）
# app/deploy-app/ -> app/ -> deploy-auth-app-bff/ -> auth-app-bff/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
AUTH_APP_BFF_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/auth-app-bff-generated.yaml"
AUTH_APP_BFF_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/auth-app-bff-pvc/generate-auth-app-bff-pvc/auth-app-bff-pvc-generated.yaml"
# 模板文件路径（在 resources/k8s-resource/templates/）
TEMPLATES_DIR="${K8S_RESOURCE_DIR}/templates"
AUTH_APP_BFF_CONFIGMAP="${TEMPLATES_DIR}/configmap/auth-app-bff-config.yaml"
AUTH_APP_BFF_SECRET="${TEMPLATES_DIR}/secret/auth-app-bff-secret.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"auth-app-bff-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"auth-app-bff-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/auth-app-bff-pvc/generate-auth-app-bff-pvc/generate-auth-app-bff-pvc.sh"
    else
...
check_env_config() {
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/auth-app-bff-config/deploy-auth-app-bff-config/deploy-auth-app-bff-config.sh"
        local secret_script="$secrets_dir/auth-app-bff-secret/deploy-auth-app-bff-secret/deploy-auth-app-bff-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export AUTH_APP_BFF_IMAGE="${AUTH_APP_BFF_IMAGE:-auth-app-bff}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=auth-app-bff"
        echo "  kubectl get svc -n $NAMESPACE -l app=auth-app-bff"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=auth-app-bff -f"
    else
...
uninstall_app() {
    ...
    if [ ! -f "$AUTH_APP_BFF_YAML" ]; then
        ...
        kubectl delete deployment auth-app-bff -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service auth-app-bff -n "$NAMESPACE" --ignore-not-found=true
    else
    ...
    if [[ "${namespace_enabled:-true}" == "true" ]]; then
        ...
        log_info "  cd deploy-auth-app-bff/namespace/auth-app-bff-namespace/deploy-auth-app-bff-namespace"
        log_info "  ./deploy-auth-app-bff-namespace.sh uninstall $PROJECT_ID $NAMESPACE $ENVIRONMENT"
    fi
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=auth-app-bff 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "参数说明:"
            echo "  project_id   项目标识符（从配置文件 deploy-auth-app-bff.conf 读取）"
            echo "  namespace    命名空间（从配置文件 deploy-auth-app-bff.conf 读取）"
            echo "  environment  环境（从配置文件 deploy-auth-app-bff.conf 读取）"
            echo ""
            echo "操作说明:"
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-auth-app-bff"
            echo "  ./deploy-auth-app-bff.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
