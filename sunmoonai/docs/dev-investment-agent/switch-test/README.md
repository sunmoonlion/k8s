...
`~/switch-test/` 是本目录在两台机上的副本，`human-do.sh` 每次跑完自动刷新它；以仓里为准。本地机要装什么见「六、固定事实」。
...
| 2 | **所有者** | 在本地机跑 `bash ~/switch-test/human-do.sh`：同步五仓、子仓、runtime，刷新副本，列出待办。然后对本地助手说：**「请看 inbox」** |
