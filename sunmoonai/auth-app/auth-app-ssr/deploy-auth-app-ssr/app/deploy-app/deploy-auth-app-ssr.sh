...
# Auth App SSR 部署脚本
# 用法: ./deploy-auth-app-ssr.sh <deploy|uninstall|status> [project_id] [namespace] [environment]
...
# 项目根目录（deploy-auth-app-ssr 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-auth-app-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（auth-app-ssr 目录）
# 从 deploy-auth-app-ssr/ 向上 1 级到达 auth-app-ssr/
...
# 导入统一部署模板
# 从应用根目录（auth-app-ssr/）到 k8s/ 需要向上 3 级
# auth-app-ssr/ -> auth-app/ -> sunmoonai/ -> k8s/
...
# 加载部署配置文件
AUTH_APP_SSR_CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-ssr.conf"
...
# 默认配置从配置文件读取（deploy-auth-app-ssr.conf）
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（auth-app-ssr/）
# app/deploy-app/ -> app/ -> deploy-auth-app-ssr/ -> auth-app-ssr/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
AUTH_APP_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/auth-app-ssr-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"auth-app-ssr-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
...
check_env_config() {
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/auth-app-ssr-config/deploy-auth-app-ssr-config/deploy-auth-app-ssr-config.sh"
        local secret_script="$secrets_dir/auth-app-ssr-secret/deploy-auth-app-ssr-secret/deploy-auth-app-ssr-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export AUTH_APP_SSR_IMAGE="${AUTH_APP_SSR_IMAGE:-auth-app-ssr}"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=auth-app-ssr"
        echo "  kubectl get svc -n $NAMESPACE -l app=auth-app-ssr"
        echo ""
        log_info "查看 Pod 日志:"
        echo "  kubectl logs -n $NAMESPACE -l app=auth-app-ssr -f"
    else
...
uninstall_app() {
    ...
    if [ ! -f "$AUTH_APP_SSR_YAML" ]; then
        ...
        if ! auto_generate_yaml "$AUTH_APP_SSR_YAML" "$K8S_RESOURCE_DIR"; then
            ...
            kubectl delete deployment auth-app-ssr -n "$NAMESPACE" --ignore-not-found=true
            kubectl delete service auth-app-ssr-service -n "$NAMESPACE" --ignore-not-found=true
        fi
    fi
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=auth-app-ssr 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=auth-app-ssr 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=auth-app-ssr 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=auth-app-ssr 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=auth-app-ssr 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../deploy-auth-app-ssr"
            echo "  ./deploy-auth-app-ssr.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
