...
# =============================================================================
# PostgreSQL Auth Service DB Secret 部署脚本
# 文件名: deploy-postgresql-authservice-db-secret.sh
...
SECRET_DIR="$(dirname "$SCRIPT_DIR")"  # postgresql-authservice-db-secret 目录
# 计算项目根目录（k8s目录）
...
CONFIG_FILE="$SCRIPT_DIR/deploy-postgresql-authservice-db-secret.conf"
...
main() {
    ...
    log_info "部署 PostgreSQL Auth Service DB Secret..."
    log_info "部署参数："
    ...
    local secret_yaml="$SECRET_DIR/postgresql-authservice-db-secret.yaml"
    ...
    echo ""
    log_success "PostgreSQL Auth Service DB Secret 部署完成！"
    log_info "部署信息："
...
# 显示帮助信息
show_help() {
    echo "PostgreSQL Auth Service DB Secret 部署脚本"
    echo ""
    echo "用法:"
    ...
    echo ""
    echo "配置文件: $SCRIPT_DIR/deploy-postgresql-authservice-db-secret.conf"
}
