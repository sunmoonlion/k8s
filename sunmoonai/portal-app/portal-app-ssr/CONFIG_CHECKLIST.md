...
- [ ] `PORTAL_BFF_URL=http://portal-app-bff:3000` - 业务 BFF 地址（备用）
...
- [ ] `SERVER_NAME=portal-ssr.sunmoonai.com` - 服务器名称
- [ ] `SERVER_HOST=https://portal-ssr.sunmoonai.com` - 服务器地址

## 部署前检查

1. ✅ 确认 `BFF_URL` 的端口与 `portal-app-bff` 的 `PORT` 一致（3000）
2. ✅ 确认 `API_PREFIX` 与 `portal-app-bff` 的 `PREFIX` 一致（`/api`）
3. ✅ 确认 `API_VERSION` 与 `portal-app-bff` 的 `VERSION` 一致（`v1`）
