...
每个执行额外占用一个数据库连接。源码候选与实际部署分开验收；当前运行边界见
...
本机 KIND 已在停写、备份和隔离恢复后升级到 `20260913_0009`；该事实不自动覆盖其他环境。
...
未接周期任务或自动回收；实际存储核查、保留策略和权限仍须按[数据与迁移](../topics/data.md)另验。
...
最新边界与续作入口见[发布与门禁](../topics/release.md)。
