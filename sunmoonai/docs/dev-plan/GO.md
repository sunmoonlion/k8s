...
> ⚠ **只有本节分通道，其余五节两条通道通用。**
>
> **你是被人在窗口里叫起来的**（交互）——照本节做。
>
> **你是被一次性命令行叫起来的**（`codex exec` / `agent -p` / `qoder -p`，
> 没有人在看你的输出）——**本节的「说出来」在你这条路上必然失效**：
> stdout 没有人在听，说完进程就退出了。你要做的是**换一种交卷**：
>
> 1. **照样交**。把能做的部分做完并提交，不要因为一处卡住就整个不交。
> 2. **把卡住的地方写进产物**，标 ⚠，写清「我需要什么才能继续」。
>    ——你的疑问变成一件 artifact，人下一轮就能看见；而停下来说，等于没发生过。
> 3. **绝不猜着往下走**。猜出来的东西混进产物，比缺一块更贵。
>
> ⚠ 通道能力差异有实测：`rounds/executor-adapter/task.md` §1.0
> （同一个 `cursor`，`-p` 下三次都不提交，交互式跑一次即提交）。
