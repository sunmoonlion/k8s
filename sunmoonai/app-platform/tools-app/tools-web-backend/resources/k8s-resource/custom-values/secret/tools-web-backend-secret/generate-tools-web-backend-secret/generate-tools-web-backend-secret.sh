#!/bin/bash
# Tools Web Backend Secret YAML 生成脚本
# 根据配置生成 Secret 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-tools-web-backend-secret.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-tools-web-backend-secret/ -> tools-web-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-tools-web-backend-secret/ -> tools-web-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/ -> resources/ -> tools-web-backend/
...
# 生成 YAML
main() {
    log_info "开始生成 Tools Web Backend Secret YAML 文件..."
