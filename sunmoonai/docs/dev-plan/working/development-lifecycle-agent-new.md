...
### 7.5 Git commit、分支与 worktree 的不同语义

| 对象 | 能证明什么 | 不能证明什么 |
| --- | --- | --- |
| commit | 一组文件内容及父历史不可变 | 谁批准、测试是否通过、外部副作用状态 |
| branch | 当前指向哪个 commit，便于运输和续接 | 稳定评审对象；分支头会移动 |
| worktree | 某执行者当前可写目录和所检出分支 | 长期证据；目录可以删除 |
| tag/ref | 使对象可达并表达冻结点 | 自动构成验收或发布 |
| patch | 可传输的差异 | 完整父基线、未跟踪文件或外部状态 |
...
建立新 Work Unit 或 Task。迟到依据状态、租约/fencing、commit 和冻结时间判断，不依据分支
头或“看起来更新”。

取消先持久化意图，再停止调度、撤销租约、终止执行、盘点副作用、固定需保留的 commit 和
Artifact，最后回收 worktree/sandbox。取消与完成只能一个终态胜出；过期执行者的写入由
fencing 拒绝，不能只靠自律。
