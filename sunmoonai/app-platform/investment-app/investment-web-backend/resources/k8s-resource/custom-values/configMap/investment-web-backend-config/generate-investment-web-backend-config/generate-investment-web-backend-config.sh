#!/bin/bash
# Investment Web Backend ConfigMap YAML 生成脚本
# 根据配置生成 ConfigMap 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-investment-web-backend-config.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-investment-web-backend-config/ -> investment-web-backend-config/ -> configMap/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-investment-web-backend-config/ -> investment-web-backend-config/ -> configMap/ -> custom-values/ -> k8s-resource/ -> resources/ -> investment-web-backend/
...
export PORT="${PORT:-}"
export QUEUE_ENABLED="${QUEUE_ENABLED:-}"
export QUEUE_ON="${QUEUE_ON:-}"
export TENANT_MODE="${TENANT_MODE:-}"
export TENANT_DB_TYPE="${TENANT_DB_TYPE:-}"
export REDIS_TYPE="${REDIS_TYPE:-}"
...
# 生成 YAML
main() {
    log_info "开始生成 Investment Web Backend ConfigMap YAML 文件..."
