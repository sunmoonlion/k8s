...
部署通过（见 `k8s/sunmoonai/scripts/results/kind-sandbox-relay.20260925-07b.md`）。第 9 步浏览器实时验证**没做**：所有者决定跳过（测试账号密码已不在手边）。修法有单元测试覆盖，浏览器行为未实测；下次任何一轮在浏览器里发消息时顺带看一眼「过程」是否不刷新就出现。
