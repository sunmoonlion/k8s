...
> 由产品合同搬入。谁来编排见 [控制面与执行体](control-plane.md)。
...
**工具怎么到 Codex**：Codex 不直接连后端，所以走代理——本步工具清单随派发下发（通道 ①）→
runtime 注册成本地 MCP 的代理工具（通道 ②）→ Codex 调用 → runtime 经 ① 转回后端 →
orchestrator **二次校验**（是不是这个 Attempt、是不是这一步的清单、租约还有效吗）→ 执行并返回。
**清单随换步失效**；runtime 只代理、不缓存、不实现。二次校验不可省——runtime 在用户电脑上，
清单可能被改，这与三道门里「工具网关收到伪造名称再 deny 一次」同理。
