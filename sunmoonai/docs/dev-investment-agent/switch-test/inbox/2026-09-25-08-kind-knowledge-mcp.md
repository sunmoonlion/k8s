...
4. MCP 令牌 Secret（不进 git 不回传）：
   ...
   ```
5. knowledge 的准备记录重建：`cd ~/worktrees/fable/k8s/sunmoonai/app-platform/scripts && ~/worktrees/fable/investment-app/investment-backend/app/.venv/bin/python kind_identity_recover.py --app knowledge --kubeconfig ~/.kube/kind-config --cluster-uid $(kubectl get ns kube-system -o jsonpath='{.metadata.uid}') --prepared-release-id kind-b7-20260919 --output ~/private/knowledge-identity-recovered-20260925`。记下 `plan_sha256`。
