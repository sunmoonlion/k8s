...
# 计算项目根目录（应用根目录）
# 从 deploy-harbor-registry-secret/ 向上 4 级到达应用根目录
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-document-converter-bff/ -> document-converter-bff/
...
main() {
    ...
    echo ""
    log_success "Harbor Registry Secret (Document Converter) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
