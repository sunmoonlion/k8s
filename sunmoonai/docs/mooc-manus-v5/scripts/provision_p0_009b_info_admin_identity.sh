...
# Reconcile the isolated Casdoor application used by P0-009B. Credentials stay
...
APPLICATION="sunmoonai-info-admin-p0-009b"
REDIRECT_URI="https://info-admin-p0-009b.sunmoonai.com:30443/api/auth/callback"
MODE="plan"
...
Usage: provision_p0_007e_identity.sh [--apply|--cleanup] [options]
  ...
  --redirect-uri URL      Exact P0-009B OIDC callback URI
EOF
}
...
delete_application() {
  local job="p0-009b-identity-delete"
          ...
          args:
            - |
              ...
              psql -h "\$DB_HOST" -p "\$DB_PORT" -U "\$APP_DB_USER" -d "\$APP_DB_NAME" \
                ...
                >/dev/null
              printf 'P0-009B Casdoor application removed\n'
EOF
  wait_job "$job"
}
...
if [[ "$MODE" == "cleanup" ]]; then
  ...
  printf 'V5-P0-009B identity cleanup passed\n'
  exit 0
fi
...
[[ "$REDIRECT_URI" =~ ^https://info-admin-p0-009b\.sunmoonai\.com(:[0-9]+)?/api/auth/callback$ ]] || {
  printf 'redirect URI is outside the P0-009B trust boundary\n' >&2
  exit 1
}
k get secret "$CASDOOR_DATABASE_SECRET" -n "$NAMESPACE" >/dev/null
...
job="p0-009b-identity-provision"
k delete job "$job" -n "$NAMESPACE" --ignore-not-found=true --wait=true >/dev/null
cat <<EOF | k apply -f - >/dev/null
apiVersion: batch/v1
kind: Job
          ...
          args:
            - |
              ...
              INSERT INTO application (
                ...
                'admin', '${APPLICATION}', NOW()::text, 'Template Admin P0-009B',
              ...
              SQL
              printf 'P0-009B Casdoor application reconciled\n'
EOF
wait_job "$job"
printf 'V5-P0-009B identity provisioning passed\n'
