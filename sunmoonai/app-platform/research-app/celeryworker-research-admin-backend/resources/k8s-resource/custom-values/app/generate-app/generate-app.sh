#!/bin/bash
# celeryworker-research-admin-backend Deployment YAML 生成脚本
...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-research-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-celeryworker-research-admin-backend/app/deploy-app/deploy-celeryworker-research-admin-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${CELERYWORKER_RESEARCH_ADMIN_BACKEND_NAMESPACE:-}")
...
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_REGISTRY="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_PROJECT="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE:-}"
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_TAG="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_TAG:-}"
...
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
export CELERYWORKER_RESEARCH_ADMIN_BACKEND_SECRET_NAME="${CELERYWORKER_RESEARCH_ADMIN_BACKEND_SECRET_NAME:-}"
...
main() {
    log_info "开始生成 celeryworker-research-admin-backend YAML 文件..."
