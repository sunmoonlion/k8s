...
CONFIG_FILE="$SCRIPT_DIR/deploy-llmops-app-front-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llmops-app-front-config/ 向上 3 级到达应用根目录
# deploy-llmops-app-front-config/ -> llmops-app-front-config/ -> secrets/ -> deploy-llmops-front/ -> llmops-app-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/llmops-app-front-config/generate-llmops-app-front-config/llmops-app-front-config-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/configMap/llmops-app-front-config/generate-llmops-app-front-config/generate-llmops-app-front-config.sh"
...
main() {
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${LLMOPS_SSR_CONFIGMAP_NAME:-llmops-app-front-config}"
