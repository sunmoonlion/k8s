...
SECRET_CONFIG="../../deploy-llmops-front/secrets/llmops-app-front-secret/deploy-llmops-app-front-secret/deploy-llmops-app-front-secret.conf"
...
# 设置默认环境变量
export NAMESPACE="${NAMESPACE:-${LLMOPS_SSR_NAMESPACE:-app-platform-dev}}"
export ENVIRONMENT="${ENVIRONMENT:-development}"
export ENV="${ENV:-dev}"

# 准备 LLMOps App SSR 相关的环境变量
export LLMOPS_SSR_IMAGE_REGISTRY="${LLMOPS_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}"
...
export LLMOPS_SSR_FULL_IMAGE_NAME="${LLMOPS_SSR_IMAGE_REGISTRY}/${LLMOPS_SSR_IMAGE_PROJECT}/${LLMOPS_SSR_IMAGE}:${LLMOPS_SSR_TAG}"
...
export LLMOPS_SSR_UNIFIED_HOST="${LLMOPS_SSR_UNIFIED_HOST:-llmops-front.sunmoonai.com}"
export LLMOPS_SSR_NODE_IP="${LLMOPS_SSR_NODE_IP:-101.126.151.0}"
export LLMOPS_SSR_EXTERNAL_PORT="${LLMOPS_SSR_EXTERNAL_PORT:-30443}"

# Harbor Docker 认证配置（用于 harbor-registry-secret）
HARBOR_DOCKER_SERVER="${HARBOR_DOCKER_SERVER:-${LLMOPS_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com}}"
...
# Ingress 相关变量（用于模板替换）
export SERVICE_NAME="${SERVICE_NAME:-llmops-app-front}"
export SERVICE_PORT="${SERVICE_PORT:-3000}"
export UNIFIED_HOST="${UNIFIED_HOST:-${LLMOPS_SSR_UNIFIED_HOST}}"
export NODE_IP="${NODE_IP:-${LLMOPS_SSR_NODE_IP}}"

# PVC 相关变量（用于模板替换，从部署配置文件中读取）
# 注意：这些变量的默认值在 deploy-llmops-front.conf 中配置
export PVC_NAME="${PVC_NAME:-llmops-app-front-pvc}"
...
# 主函数
main() {
    log_info "开始生成 LLMOps App SSR YAML 文件..."
