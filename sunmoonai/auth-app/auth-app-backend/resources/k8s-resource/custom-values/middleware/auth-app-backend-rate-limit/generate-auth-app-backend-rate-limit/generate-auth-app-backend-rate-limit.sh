#!/bin/bash
# Auth App BFF Rate Limit Middleware YAML 生成脚本
# 根据配置生成 Rate Limit Middleware 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-auth-app-backend-rate-limit.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-auth-app-backend-rate-limit/ -> auth-app-backend-rate-limit/ -> middleware/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-auth-app-backend-rate-limit/ -> auth-app-backend-rate-limit/ -> middleware/ -> custom-values/ -> k8s-resource/ -> resources/ -> auth-app-backend/
...
if [ "${ENABLED:-true}" != "true" ]; then
    log_info "跳过资源生成: middleware (已禁用)"
    exit 0
fi

# ============================================================================
# 设置环境变量（用于模板替换）
# 注意：所有配置都从 generate-auth-app-backend-rate-limit.conf 读取，不在脚本中设置硬编码默认值
# ============================================================================
# 基础配置（从 generate-auth-app-backend-rate-limit.conf 中读取，所有默认值在 conf 文件中定义）
export NAMESPACE="${NAMESPACE:-}"
export ENVIRONMENT="${ENVIRONMENT:-}"
export ENV="${ENV:-}"

# Rate Limit 数据内容（从 generate-auth-app-backend-rate-limit.conf 中读取，所有默认值在 conf 文件中定义）
export RATE_LIMIT_AVERAGE="${RATE_LIMIT_AVERAGE:-}"
export RATE_LIMIT_BURST="${RATE_LIMIT_BURST:-}"
export RATE_LIMIT_PERIOD="${RATE_LIMIT_PERIOD:-}"
...
# 生成 YAML
main() {
    log_info "开始生成 Auth App BFF Rate Limit Middleware YAML 文件..."
    ...
    log_info "生成 middleware: $OUTPUT_FILE"
    ...
    log_success "✅ middleware 生成完成: $OUTPUT_FILE"
    return 0
}
