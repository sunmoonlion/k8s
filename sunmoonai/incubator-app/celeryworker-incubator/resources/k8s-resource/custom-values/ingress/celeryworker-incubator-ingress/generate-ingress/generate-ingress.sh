#!/bin/bash
# Celery Worker (Incubator) Ingress YAML 生成脚本
# 根据配置生成 Ingress 的 YAML 文件
# 注意：Celery Worker 是纯后台任务处理，通常不需要 Ingress
...
# 从 generate-ingress/ -> celeryworker-incubator-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-ingress/ -> celeryworker-incubator-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-incubator/
...
if [ "${ENABLED:-false}" != "true" ]; then
    log_info "跳过资源生成: ingress (已禁用，Celery Worker 不需要 HTTP 接口)"
    exit 0
fi
...
# Ingress 数据内容（从 generate-ingress.conf 中读取，所有默认值在 conf 文件中定义）
export SERVICE_NAME="${SERVICE_NAME:-}"
export SERVICE_PORT="${SERVICE_PORT:-}"
export UNIFIED_HOST="${UNIFIED_HOST:-}"
export NODE_IP="${NODE_IP:-}"

# Middleware 配置（从 generate-ingress.conf 中读取）
...
# 生成 YAML
main() {
    log_info "开始生成 Celery Worker (Incubator) Ingress YAML 文件..."
    ...
    # 处理条件中间件：
    # 1. 如果 USE_STRIP_PREFIX=false，移除 stripprefix 中间件定义
    # 2. 如果 USE_STRIP_PREFIX=true，保留 stripprefix 中间件定义
    local temp_file=$(mktemp)
    if [ "${USE_STRIP_PREFIX:-false}" = "true" ]; then
        # 保留 stripprefix 中间件定义（移除注释标记）
    ...
    # 处理中间件列表：根据配置生成中间件列表
    # 查找所有包含 #MIDDLEWARE_LIST_START 和 #MIDDLEWARE_LIST_END 标记的位置
    if [ "${USE_STRIP_PREFIX:-false}" = "true" ]; then
        # 使用 stripprefix 中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: celeryworker-incubator-stripprefix' "$full_output_path"
    elif [ "${USE_RATE_LIMIT:-false}" = "true" ]; then
        # 使用限流中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: celeryworker-incubator-rate-limit' "$full_output_path"
    else
        # 不使用任何中间件（移除 middlewares 部分）
        sed -i '/middlewares:/,/^[[:space:]]*#MIDDLEWARE_LIST_END/d' "$full_output_path"
    fi
    
    # 移除标记行（如果还存在）
    sed -i '/#MIDDLEWARE_LIST_START/d; /#MIDDLEWARE_LIST_END/d' "$full_output_path"
