...
| 3 用 `F-*`/`I*`/`AT-*`/A-R 锚定 | ✅ **用于锚定的 `AT-*` 11 个**（全文 13 个不同编号，含头部 `AT-01`…`AT-22` 一处范围引用，不计为锚定）；`F-EXEC/F-INTERACT` 10 个全覆盖 |
| 4 双腿映射表 | ✅ §8.1，另补三档对应表 |
...
⚠ **本节是 ③ 收口时点的快照。**④ 异议轮又加了 10 行（O-L1…O-L5 与 D-K8），
冻结稿为 **1603 行**（`c501c723`，sha256 `a602b344…`）。按时间戳读两者不矛盾。


## 10. ④ 异议轮处置

参与：luna、cursor、qwen 三家（kimi 经裁定 R1 免除，见 `refact-rulings.md`）。

| 家 | 异议 commit | 行 | 结论 |
| --- | --- | ---: | --- |
| luna | `2e1e4a53` | 172 | 4 条语义异议 + 1 条结构异议 |
| cursor | `6db57869` | 65 | **无异议**（逐条核对 12 条处置的落点忠实度后给出） |
| qwen | `10267649` | 87 | 7 条无异议 + 1 条据证异议（出处误挂） |

**六条异议全部采纳，无一驳回。**

| 异议 | 主张 | 裁定 | 落点 |
| --- | --- | --- | --- |
| O-L1 | `F-EXEC-02` dsh 腿改判仍分类错误：义务是 Attempt 级，无 Turn 级子义务；协议未显式否定 Attempt 级关联，不该映射到「当前缺失」 | **接受** | `f4ca8fd1` 再改判 `implicit_fallback`（需补法），逐 Turn correlation 单列为上游门禁 G3 |
| O-L2 | §0.3 一面要求确定性代码，一面仍允许模型「在合法候选中选择」 | **接受** | `7db445a8` 模型输出降为不具决策权的建议与证据 |
| O-L3 | Gate 0 第 3 条「独立运行角色但不进 worker」在拓扑词典里无合法落点 | **接受** | `2ee77993` 改为按 constraints 的拆分门槛拆专业 Worker |
| O-L4 | 「超时独立一态」会被读成新增状态，与验收标准 8 冲突 | **接受** | `44863162` 改称「独立的审计结果与 reason code」，显式声明不新增状态 |
| O-L5 | §4 标题已覆盖不住 §4.5–§4.11 | **接受** | `2ee77993` 只改标题，不搬内容、不动锚点 |
| qwen-1 | D-Q6 出处误挂，实为 kimi §15.2 | **接受** | `cf57cb68` 改挂并重编号 D-K8，原条目划线保留 |

附带修正：dsh teardown 锚点的省略号路径展开为可复跑路径（`2ee77993`）。

### 10.1 异议轮的实际收益

**裁决方在 ③ 犯了两类错，都由 ④ 逮住：**

1. **分类错误**（O-L1）：裁决方在 ③ 推翻了 kimi 与 luna 两家的判定，自己给出的第三种判法
   **与自己同一提交里刚加的三档对应表相矛盾**。基座作者在异议轮里**同时撤回了自己的原判**，
   三方判定至此才收敛。
2. **转录错误**（qwen-1）：把 luna 评审里明写「kimi，§15.2」的一条串行记到 qwen 名下，
   并据此写成「拒绝 qwen 的主张」。这与本轮 D-C5 是同一类错误的镜像。

**这两条都不是验收能发现的。**验收按冻结标准判「稿子达标了吗」，
判不到「主张被误读」与「出处记错」——那需要来源方视角。
`round-protocol.md` 缺失异议轮的那个洞，本轮实测到了它的代价。

### 10.2 R1 的实际代价，已兑现

`refact-rulings.md` R1 免除 kimi 参加 ④ 时登记的风险是「移除了对 F-EXEC-02 改判的唯一直接检查」。

**该风险由 luna 意外补上了**——luna 以基座作者身份对同一条提出 O-L1，并撤回自己的原判。
但**另一处代价已经兑现**：D-K8（原 D-Q6）是 kimi 的主张被拒绝，
而因为出处误挂 + kimi 被免除，**kimi 从未被告知它的这条主张被拒了**。

处置：本记录与裁定记录均已载明；kimi 依 `round-protocol.md`「裁量权 · 推翻」
仍可随时就 D-K8 或 `F-EXEC-02` 提异议，**不设时限**。

## 11. ⑤ 验收结果

验收方 **qwen**（裁定 R2 改派），验收稿 `qwen` `1012b76a`，162 行，
sha256 `ef45a998a3876749…`。

**判定：通过。**11 条验收标准全数通过，五项独立复核全数通过，**无异议**。
无一条判为「标准本身过期」，故无标准腐坏项交裁决方处置。

裁决方另跑了一遍机械验收（`round-status.py --verify`），与验收方结论一致：

| 复核项 | 机械验收 | 验收方 | 一致 |
| --- | --- | --- | --- |
| 冻结区逐字节（7 节） | ✅ | ✅ 分段 sha256 逐一 IDENTICAL | ✅ |
| 处置记录 ↔ 提交双向对账 | ✅ 17 个触及裁决稿的提交全部登记 | ✅ 25 提交构成拆解、三查通过 | ✅ |
| 外部仓锚点可达 | ✅ 17 处 | ✅ 抽验 24 处全 OK | ✅ |
| 无 Profile 字段表 | ✅ 零命中 | ✅ 零命中 | ✅ |
| Node 误判不得复活 | ✅ 零命中 | ✅ 仅两处否定式 | ✅ |
| **锚点语义是否支持断言** | 🔶 判不了 | ✅ 抽验（自陈非全量） | — |
| **编号出处逐处** | 🔶 判不了 | ✅ 抽验，未见重犯 | — |
| **11 条标准的判断题部分** | 🔶 不覆盖 | ✅ 逐条给证据 | — |

**这张表就是下一轮 ⑤ 该保留的范围**：机器判掉的五项不必再交人；
标 🔶 的三项才是验收方不可替代的工作。按此，下一轮 ⑤ 的工作量约为本轮的三分之一。

### 11.1 验收方的两条非异议观察，已采纳

| 观察 | 处置 |
| --- | --- |
| §9 自查「`AT-*` 13 个」口径松 | **接受**。13 是全文不同编号数，含头部 `AT-01`…`AT-22` 一处范围引用；用于锚定的是 **11 个**。已改为分开表述（`d13d1656`） |
| §9 自查「1593 行」是 ③ 收口时点 | **接受**。④ 又加 10 行至 1603，已补时点标注（`d13d1656`） |

### 11.2 验收方自陈的利益关联，裁决方知悉并记录

qwen 在盲区第 5 条自陈：它是候选作者之一，且是本轮两处取证被纠正的家，
「由被纠正者验收纠正了自己的稿」本身是一层利益关联。

裁决方认定该关联**已被 R2 的处置覆盖且未造成偏差**，依据三条：

1. R2 指定它的判据是**既得利益最小**（采纳 5 条，仅次于被免除的 kimi），不是质量；
2. 验收通知把它的两类弱项**显式列为重点复核项**，它在 §2.4 正面接住并做了定向回扫；
3. 裁决方的机械验收独立复算了五项，与它的结论逐项一致（见上表）——
   若它放水，机械项会与之冲突。

**但这一层关联仍记录在案**：本轮验收方是从「不可用」顺位递补而来，
而非无关第三方。这属于 R1/R2 的连带代价，已在裁定记录登记。
