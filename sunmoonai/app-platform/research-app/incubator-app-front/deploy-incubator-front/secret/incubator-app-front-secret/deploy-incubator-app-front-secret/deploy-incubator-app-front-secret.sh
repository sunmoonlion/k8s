...
CONFIG_FILE="$SCRIPT_DIR/deploy-incubator-app-front-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-incubator-app-front-secret/ 向上 3 级到达应用根目录
# deploy-incubator-app-front-secret/ -> incubator-app-front-secret/ -> secrets/ -> deploy-incubator-front/ -> incubator-app-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/incubator-app-front-secret/generate-incubator-app-front-secret/incubator-app-front-secret-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/secret/incubator-app-front-secret/generate-incubator-app-front-secret/generate-incubator-app-front-secret.sh"
...
main() {
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${INCUBATOR_SSR_SECRET_NAME:-incubator-app-front-secret}"
