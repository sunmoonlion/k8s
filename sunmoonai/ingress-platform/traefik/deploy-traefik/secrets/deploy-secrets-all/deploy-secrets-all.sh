...
# =============================================================================
# Traefik Secrets 部署脚本
# - 部署 Traefik 相关的 Secrets
# - 支持多集群配置
# =============================================================================
...
log_success() { green "[SUCCESS] $(date '+%Y-%m-%d %H:%M:%S') $*"; }
log_warn() { yellow "[WARN] $(date '+%Y-%m-%d %H:%M:%S') $*"; }
log_error() { red "[ERROR] $(date '+%Y-%m-%d %H:%M:%S') $*"; }

# 脚本目录配置
THIS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCRIPT_DIR="$THIS_DIR"
PROJECT_ROOT="$(dirname "$(dirname "$THIS_DIR")")"  # 向上两层到 traefik
...
if [[ -z "$K8S_ROOT_DIR" ]]; then
    log_error "无法定位 k8s 根目录（未找到 utils/cluster-arg-parser.sh），THIS_DIR=$THIS_DIR"
    exit 1
fi
...
deploy_sub_components_by_priority() {
    ...
    local components=(
        "traefik_tls_secret:${traefik_tls_secret_enabled:-true}:${traefik_tls_secret_priority:-2000}:Traefik TLS Secret:$SCRIPT_DIR/../traefik-tls-secret/deploy-traefik-tls-secret/deploy-traefik-tls-secret.sh"
    )
...
deploy_secrets() {
    ...
    log_info "🚀 开始部署 Traefik Secrets 递归架构..."
    log_info "📋 部署参数："
    ...
    echo ""
    log_success "🎯 Traefik Secrets 部署成功完成！"
}

# 显示帮助信息
show_help() {
    echo "Traefik Secrets All 递归部署脚本"
    echo ""
    echo "用法:"
