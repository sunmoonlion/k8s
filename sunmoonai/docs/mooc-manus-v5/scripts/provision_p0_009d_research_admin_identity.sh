...
# Reconcile the isolated Casdoor application used by P0-009D. Credentials stay
...
APPLICATION="sunmoonai-research-admin-p0-009d"
REDIRECT_URI="https://research-admin-p0-009d.sunmoonai.com:30443/api/auth/callback"
MODE="plan"
...
Usage: provision_p0_007e_identity.sh [--apply|--cleanup] [options]
  ...
  --redirect-uri URL      Exact P0-009D OIDC callback URI
EOF
}
...
delete_application() {
  local job="p0-009d-identity-delete"
          ...
          args:
            - |
              ...
              psql -h "\$DB_HOST" -p "\$DB_PORT" -U "\$APP_DB_USER" -d "\$APP_DB_NAME" \
                ...
                >/dev/null
              printf 'P0-009D Casdoor application removed\n'
EOF
  wait_job "$job"
}
...
if [[ "$MODE" == "cleanup" ]]; then
  ...
  printf 'V5-P0-009D identity cleanup passed\n'
  exit 0
fi
...
[[ "$REDIRECT_URI" =~ ^https://research-admin-p0-009d\.sunmoonai\.com(:[0-9]+)?/api/auth/callback$ ]] || {
  printf 'redirect URI is outside the P0-009D trust boundary\n' >&2
  exit 1
}
k get secret "$CASDOOR_DATABASE_SECRET" -n "$NAMESPACE" >/dev/null
...
job="p0-009d-identity-provision"
k delete job "$job" -n "$NAMESPACE" --ignore-not-found=true --wait=true >/dev/null
cat <<EOF | k apply -f - >/dev/null
apiVersion: batch/v1
kind: Job
          ...
          args:
            - |
              ...
              INSERT INTO application (
                ...
                'admin', '${APPLICATION}', NOW()::text, 'Template Admin P0-009D',
              ...
              SQL
              printf 'P0-009D Casdoor application reconciled\n'
EOF
wait_job "$job"
printf 'V5-P0-009D identity provisioning passed\n'
