...
write_k8s_out() {
  ...
  if bool_true "${ENABLE_REDIS:-false}" && [[ -f "${REDIS_CONFIG}" ]]; then
    ...
    replace_env_value "${OUT}" "REDIS_URI" \
      "redis://${APP_DB_USER}:${APP_DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${REDIS_DB_INDEX}"
  fi

  if bool_true "${ENABLE_NODEBULL_REDIS:-false}" && [[ -f "${NODEBULL_REDIS_CONFIG}" ]]; then
    runtime_config="$(k8s_bootstrap_prepare_config "${NODEBULL_REDIS_CONFIG}")"
    # shellcheck disable=SC1090
    source "${runtime_config}"
    rm -f "${runtime_config}"
    replace_env_value "${OUT}" "QUEUE_REDIS_HOST"     "${DB_HOST}"
    replace_env_value "${OUT}" "QUEUE_REDIS_PORT"     "${DB_PORT}"
    replace_env_value "${OUT}" "QUEUE_REDIS_DB"       "${REDIS_DB_INDEX}"
    replace_env_value "${OUT}" "QUEUE_REDIS_USERNAME" "${APP_DB_USER}"
    replace_env_value "${OUT}" "QUEUE_REDIS_PASSWORD" "${APP_DB_PASSWORD}"
    replace_env_value "${OUT}" "QUEUE_REDIS_PREFIX"   "${QUEUE_REDIS_PREFIX}"
  fi
...
main() {
  ...
  bool_true "${ENABLE_NODEBULL_REDIS:-false}" && require_file "${NODEBULL_REDIS_CONFIG}" && k8s_bootstrap_run_dbctl "${DBCTL_BIN}" "${NODEBULL_REDIS_CONFIG}"
