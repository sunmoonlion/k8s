...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-llmops-front/ -> llmops-app-front/
...
# 尝试加载主配置文件（如果存在），以获取 LLMOPS_SSR_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-llmops-front.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-llmops-front.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (LLMOps SSR)..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    echo ""
    log_success "Harbor Registry Secret (LLMOps SSR) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (LLMOps SSR)"
    echo ""
    echo "用法:"
