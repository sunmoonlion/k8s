...
# =============================================================================
# Harbor Registry Secret 部署脚本（Celery Worker LLMOps）
...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-celeryworker-llmops/ -> celeryworker-llmops/
...
# 尝试加载主配置文件（如果存在），以获取 CELERYWORKER_LLMOPS_IMAGE_REGISTRY 等环境变量
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (Celery Worker LLMOps)..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    echo ""
    log_success "Harbor Registry Secret (Celery Worker LLMOps) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (Celery Worker LLMOps)"
    echo ""
    echo "用法:"
