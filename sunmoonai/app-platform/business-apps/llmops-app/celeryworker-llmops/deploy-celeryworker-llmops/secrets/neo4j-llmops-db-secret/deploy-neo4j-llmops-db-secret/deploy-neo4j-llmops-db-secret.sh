...
# 计算项目根目录（k8s目录）
# 从 deploy-neo4j-llmops-db-secret/ -> neo4j-llmops-db-secret/ -> secrets/ -> deploy-celeryworker-llmops/ -> celeryworker-llmops/ -> llmops-app/ -> business-apps/ -> app-platform/ -> sunmoonai/ -> k8s/
...
main() {
    ...
    local secret_yaml="$SECRET_DIR/neo4j-llmops-db-secret.yaml"
    ...
    echo ""
    log_success "Neo4j LLMOps DB Secret 部署完成！"
    log_info "部署信息："
