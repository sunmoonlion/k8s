...
不得伪造事前批准；其与 I1 的兼容性仍为 [§7.4](../../../../composition/agent-dev-guide.md) 未决。追认比例是流程成本线索，不自动降低权限。

## 14. 开发 Task 持久记录模板

> 依据的通用规范：[任务持久记录模板](../../../../../dev-agent-standards/task/task-rules.md)
...
⚠ ⑧ 中的改判字段对应 [§4.8](../../composition/agent-dev-guide.md) 的改判三要素——**「原判断错处」栏写「当时信息不全」不算填**，
要写出具体哪一步推错了。
