...
| `turn.md` | 文档 turn 的回执：`status`、`branch`、`commit` 三个字段，**就这三个**（`branch` 的旧名是 `worktree`，只在已冻结的原件里保留）；交回时写，写完冻结。没有它就是还没交回。**验收结论不进回执**，写在那条提交的说明里 | 状态回执、交回回执 |
...
**不要和产品侧的「任务类别」（通用 / 专业）混用**：那是[任务类别判定与路由](../tree-build/SDD/architecture/routing.md)里客户任务的分类，与 thread 的种类无关。
...
| 进度 | 从 `thread/`、`PRD/`、`SDD/` 及它们的提交推出的投影；**任务目录里**不另写记录或进度文件 | 交接文件、状态文件 |
