...
| 文档 thread | 任务目录 `thread/` 下的一个目录，对应一个运行时 thread；名为「本地号-运行时 thread id」。运行时换了 thread 就新开一个 | 会话、turns、rounds |
...
| 派工 | 发出一个 turn | — |
| 交回物 | 某个 turn 实际交回来的那一份 SDD、SDP 或 UAT（改代码时还有改动所在的提交），放在 turn 目录的 `sdd/`、`sdp/`、`uat/` 里 | 交付物（指这一份时） |
