...
doc-gate 另查任务目录下的 turn：`thread/` 下正好两级（文档 thread、文档 turn），目录名合乎「四位本地号-运行时 id」、编号连续不复用、同一个运行时 id 不出现在两处、交回物只在 `sdd/`、`sdp/`、`uat/` 里且与任务书声明的相符、`user-message.md` 与 `turn.md` 的字段齐全合法，交回即冻结（提交与合并时查）。
