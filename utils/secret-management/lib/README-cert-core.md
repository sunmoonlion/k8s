...
⚠️ **重要**：此模块只提供服务器证书生成，**不提供 CA 生成功能**。CA 的生成和管理请使用 `~/k8s/utils/ca-management/` 模块。
...
```bash
cd ~/k8s/utils/ca-management

# 自动读取 ca-management/ca-management.conf 中的 ROOT_CA_LOCAL_DIR
./generate-ca.sh

# 或者手动指定路径（会覆盖配置文件）
./generate-ca.sh --output-dir ~/k8s/certs/ca/shared
```

CA会生成到：`~/k8s/certs/ca/shared/ca.crt` 和 `ca.key`
...
generate_server_cert_from_ca \
    ...
    --output-dir ~/k8s/certs/server/postgresql \
    --days 365 \
    --key-size 2048
```
...
2. 重新运行 `~/k8s/utils/ca-management/generate-ca.sh`（会在新路径生成CA）
3. 确保所有组件都从配置文件读取路径
