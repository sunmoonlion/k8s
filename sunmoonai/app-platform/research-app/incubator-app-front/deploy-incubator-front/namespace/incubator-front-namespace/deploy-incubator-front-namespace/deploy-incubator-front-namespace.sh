#!/bin/bash
# Incubator App SSR Namespace 部署脚本
# 根据配置部署 Namespace 到 Kubernetes 集群

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/deploy-incubator-front-namespace.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-incubator-front-namespace/ -> incubator-front-namespace/ -> namespace/ -> deploy-incubator-front/ -> incubator-app-front/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
NAMESPACE_YAML="$K8S_RESOURCE_DIR/custom-values/namespace/incubator-front-namespace/generate-incubator-front-namespace/incubator-front-namespace-generated.yaml"
...
# 默认配置从配置文件读取（deploy-incubator-front-namespace.conf）
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/namespace/incubator-front-namespace/generate-incubator-front-namespace/generate-incubator-front-namespace.sh"
  ...
  case "$action" in
    deploy)
      log_info "部署 Incubator App SSR Namespace..."
    ...
    uninstall)
      log_info "卸载 Incubator App SSR Namespace..."
      log_info "命名空间: $namespace"
    ...
    status)
      log_info "检查 Incubator App SSR Namespace 状态..."
      log_info "命名空间: $namespace"
