...
**阶段**有五个：BRD、PRD、SDD、SDP、UAT，规则见 [BRD](brd/brd-rules.md)、[PRD](prd/prd-rules.md)、[SDD](sdd/sdd-rules.md)、[SDP](sdp/sdp-rules.md)、[UAT](uat/uat-rules.md)。每一问的用户消息怎么写，见各阶段规则的「这一段的用户消息」与 [任务的生命周期](lifecycle.md)「用户消息写多严」。
