...
CONFIG_FILE="$SCRIPT_DIR/generate-incubator-backend-config.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-incubator-backend-config/ -> incubator-backend-config/ -> configMap/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-incubator-backend-config/ -> incubator-backend-config/ -> configMap/ -> custom-values/ -> k8s-resource/ -> resources/ -> incubator-app-backend/
...
# ============================================================================
# 设置环境变量（用于模板替换）
# 注意：所有配置都从 generate-incubator-backend-config.conf 读取，不在脚本中设置硬编码默认值
# ============================================================================
# 基础配置（从 generate-incubator-backend-config.conf 中读取，所有默认值在 conf 文件中定义）
export NAMESPACE="${NAMESPACE:-}"
export ENVIRONMENT="${ENVIRONMENT:-}"
export ENV="${ENV:-}"

# ConfigMap 数据内容（从 generate-incubator-backend-config.conf 中读取，所有默认值在 conf 文件中定义）
...
# SMTP 配置（从 generate-incubator-backend-config.conf 中读取，所有默认值在 conf 文件中定义）
...
# Sentry 监控配置（从 generate-incubator-backend-config.conf 中读取，所有默认值在 conf 文件中定义）
...
# 认证服务配置（从 generate-incubator-backend-config.conf 中读取，所有默认值在 conf 文件中定义）
export AUTH_SERVICE_URL="${AUTH_SERVICE_URL:-}"

# 基础配置已经在 generate-incubator-backend-config.conf 中定义，上面已导出
