#!/bin/bash
# research-web-backend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-research-web-backend-secret.conf"
# 从 generate-research-web-backend-secret/ -> research-web-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-research-web-backend-secret/ -> ... -> k8s-resource/ -> resources/ -> research-web-backend/
...
main() {
    log_info "开始生成 research-web-backend Secret YAML 文件..."
