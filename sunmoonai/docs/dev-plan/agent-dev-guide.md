...
> 类型：操作手册（共用执行附件），S0–S6 都用。流程本身见 [pipeline.md](pipeline.md)。

这份手册回答一件事：**接到一个工作单元之后，每一步具体怎么做。**它按动作的先后排：
受理并留持久记录 → 开工前自检、准备工作区 → 在私有工作区执行、出错时怎么退 →
交独立评审、请人批准具体对象 → 发布与保留 → 停下后怎么续接、需要改内核时怎么提。

它不讲系统怎么搭。组件和接口在 [运行时总体设计](design/runtime-tld.md)，执行器、授权、证据分别在
[执行器设计](design/executor-sdd.md)、[授权设计](design/authority-sdd.md)、[证据与观测设计](design/evidence-sdd.md)；
完成判据与验证层次在 [验证与验收规程](delivery/verification.md)；代码必须遵守的规则在 [constraints.md](constraints.md)。
同一条规则只在一处展开，这里用到时给链接。

## 受理与持久任务记录

对应 pipeline 的 S0–S1。先守住所有执行者共用的八条纪律，再把原始请求可靠受理、冻结边界与验收，
最后写成一条不会丢的持久记录。实际业务请求的原始档案仍放 `working/request-baseline/`。
...
⚠ 第 3 条是 [证据等级与采信规则](design/evidence-sdd.md) 和 [常见失败方式与项目实例](delivery/verification.md) 里三级证据分档的**前提**：分不清事实与推断，
后面两处的分级就无从谈起。
...
[实施计划](implementation-plan.md) 里的每条任务固定这几栏，**缺栏视为未定义，不开工**。
任务条目说明要做什么；下一节的持久记录跟着一次执行走，记下实际发生了什么。
...
| 测试 | 适用的测试层次（见 [测试层次](delivery/verification.md)），**不能只写"补测试"** |
| 验收 | 可判定的条件。做完能一条条对着勾 |
| 回滚 | 出问题怎么退回去 |
...
对应 pipeline S3 的开工门（DoR）。先对照硬约束和反模式自检，再建一个能交付的工作区，
并确认 Git 真的能提交。拿到一个可写路径不等于拿到授权。
...
没有失败方式的条目不该进表。它与运行时总体设计里的 [本轮核查裁定](design/runtime-tld.md) 不同：那里记的是本项目**已被推翻的设计**，
本节是**任何项目都会踩的做法**。
...
对应 pipeline S4 的实施部分。只在自己的工作区里写，每次落笔前过写入前门禁；遇到并发、覆盖、
迟到、取消或超时，按下面各节的顺序处理。这里的任何动作都不附带发布。
...
当前批准共同决定（[权限公式与只有 principal 能做的动作](design/authority-sdd.md) 的交集公式）。**上一 Task 的工作区不得在未重新受理、授权和物化的
情况下复用给下一 Task。**
...
档位（[T0/T1/T2 不是三套状态机](design/runtime-tld.md)）定 guard，**执行形态**定这一次实际怎么排人：
...
换执行器、扩大权限或增加总预算（[Attempt 内的三条硬禁令](design/authority-sdd.md)）。
...
⚠ **「已派工」「全部返回」「多数一致」都不等于完成**（[完成判据](delivery/verification.md)、「反模式」）。
...
对应 pipeline S4 的独立验证与 S5 的批准。人批准的永远是一个具体、可重取的对象，人这一侧也有义务。
需要多家比方案时调用 T2 协作子流程。权力表本身在 [授权设计](design/authority-sdd.md)，这里不改。
...
这与 [`dev.change/1` 工单](design/runtime-tld.md) 保存 route_proposal 不冲突：展示建议不等于提前填好批准。
...
| 并行提案时**不泄露其他方案** | 独立信号退化成改写（[派工契约、角色补充与隔离的诚实边界](design/runtime-tld.md)） |
...
这与 [没查什么](delivery/verification.md)「如实写『不能排除』不扣分，隐瞒才扣」是同一条规则的两侧——
一侧约束写的人，一侧约束读的人。

**委派转移的是执行，不是最终责任。**派活时必须给出范围、权限、预算、停止条件和可验收
输出；对超范围、追加成本、不可逆动作和规范修改及时批准或拒绝；**亲自验收，或指定未参与
实施的验收方**（[权限公式与只有 principal 能做的动作](design/authority-sdd.md) 责任归属表）。
...
这几类无论多小都走完整流程（与 [路由只读可判字段](design/runtime-tld.md) 的 T2 触发条件一致）。
...
本节是 [round-protocol.md](protocol/round-protocol.md) 的操作摘要，**两处不一致时以协议为准**。
只在工单判为 T2 时使用；T0/T1 按 [T0/T1/T2 不是三套状态机](design/runtime-tld.md) 执行。协议仍是流程真源，
...
对应 pipeline S5。交卷、发布、清理、删除是四个不同的动作，不能混成一步：先守交付规则，
再分清三个路径，清理失败单独处理，垃圾回收是显式阶段。

### 交付规则

**分支与提交**——父仓不得出现悬空 gitlink（constraints T4）。五仓同步用
...
**单一权威**——架构语义以 [运行时总体设计](design/runtime-tld.md) 与三份 SDD 为准，
[实施计划](implementation-plan.md) 只描述执行增量、依赖与验收；API/Schema 以各仓 `contracts/` 发布物为准，
计划和本手册里出现的字段只用于解释，不作为机器契约。
...
跑 final gate，再形成 final commit。是否推主线仍受 [权力表](design/authority-sdd.md) H5 约束。
...
对应 pipeline S6 与下一次需求。停下来时要让别人只凭持久载体接着做；删除旧文档要过删除门；
需要改产品内核时，提一个明确的修订工作单元。
...
「同一事实只能有一个权威写入者」（`I13`，[四本账与单一权威写入面](design/runtime-tld.md)）——**多人各自往同一份 handoff 追加，
就是在共享路径上并发写**。
...
现场快照的 digest 与 runtime/Profile 兼容性按 [双 runtime 的部署、进程与恢复](design/executor-sdd.md) 校验；只能在新获准可写面恢复，
...
3. **五份**源稿全部非代码块内 `#`–`####` 标题在 [五份历史正文及目录说明的逐节处置](records/development-history.md) 恰有一个处置；目录说明亦单列，保留内容有正文落点，未采用内容有理由；
...
而**一张只验到标题级的表就能满足它**——于是六条可以全绿，同时 [两份 lifecycle 的吸收轮（2026-09-07）](records/development-history.md) 自报的
...
**由此得出 `archive/` 的存续条件**：只要 [五份历史正文及目录说明的逐节处置](records/development-history.md) 还有任何一行的落点没验到正文级，
`archive/` 下对应的源稿**就不能删**——它是那一行**唯一可抽查的对象**。
[五份历史正文及目录说明的逐节处置](records/development-history.md) 的开头写着「核它的办法是抽查落点、以正文为准」；把被抽查的对象删掉，
这张表就从「可证伪」变成「不可证伪」，而不可证伪的覆盖声明比没有更差。
⚠ **git 历史里有，不算数**：「交付、清理和恢复」 已立「Git 历史可恢复不等于新读者能找到」。

**迁移内容不是删除授权。**日常操作不再依赖 archive，历史源稿可保留供审计。
...
本手册由 [dev-plan-architecture.md](dev-plan-architecture.md) 第八节安置的源节编辑而成。冻结原文用 architecture 的 `show <ID>` 取；源节在冻结基线 `baa28858` 的位置见其第八节。

| 栏目 | 节 | 源单元 |
| --- | --- | --- |
| 受理与持久任务记录 | （原章标题「一次开发 Task 怎样执行」，无正文） | `I02-028` |
| 受理与持久任务记录 | 执行者的共同纪律 | `I02-012` |
| 受理与持久任务记录 | 受理与冻结 | `I02-029` |
| 受理与持久任务记录 | 任务条目格式 | `I07-002` |
| 受理与持久任务记录 | 开发 Task 持久记录模板 | `I02-108` |
| 开工自检与工作区供给 | Agent 硬约束自检 | `I02-010` |
| 开工自检与工作区供给 | 反模式 | `I02-103` |
| 开工自检与工作区供给 | 工作区供给 | `I02-030` |
| 开工自检与工作区供给 | 建立 worktree 的细则 | `I02-042` |
| 开工自检与工作区供给 | 工作区能写，不代表 Git 能提交 | `I02-048` |
| 私有执行与失败退出 | 私有地产生，单写者发布 | `I02-034` |
| 私有执行与失败退出 | 物化门禁与写入前门禁 | `I02-038` |
| 私有执行与失败退出 | 并发场景处置表 | `I02-035` |
| 私有执行与失败退出 | 覆盖或来源不明时的事故规程 | `I02-036` |
| 私有执行与失败退出 | 冻结、迟到与取消 | `I02-037` |
| 私有执行与失败退出 | 执行形态、停止规则与成本 | `I02-041` |
| 私有执行与失败退出 | 停止、超时与回退不能省略 | `I02-050` |
| 独立评审与具体批准对象 | 人的收件箱：让批准具体、可读、可重取 | `I02-063` |
| 独立评审与具体批准对象 | 人这一侧的义务 | `I02-061` |
| 独立评审与具体批准对象 | principal 的裁量权与改判纪律 | `I02-059` |
| 独立评审与具体批准对象 | T2 七环节的操作闭环 | `I02-047` |
| 独立评审与具体批准对象 | 通知、取件与人的检视面 | `I02-049` |
| 发布与保留 | 交付规则 | `I07-004` |
| 发布与保留 | 发布协议：三个路径不是一个 | `I02-043` |
| 发布与保留 | 交付、清理和恢复 | `I02-033` |
| 发布与保留 | 保留与垃圾回收 | `I02-044` |
| 续接及下一次修订 | 跨会话续接 | `I02-088` |
| 续接及下一次修订 | 删除与迁移门 | `I02-086` |
| 续接及下一次修订 | 需要改内核时，提交明确的修订工作单元 | `I02-090` |
