#!/bin/bash
# Incubator App BFF Rate Limit Middleware 部署脚本
# 根据配置部署 Rate Limit Middleware 到 Kubernetes 集群

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/deploy-incubator-backend-rate-limit.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-incubator-backend-rate-limit/ -> incubator-backend-rate-limit/ -> middleware/ -> deploy-incubator-backend/ -> incubator-app-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
MIDDLEWARE_YAML="$K8S_RESOURCE_DIR/custom-values/middleware/incubator-backend-rate-limit/generate-incubator-backend-rate-limit/incubator-app-backend-rate-limit-generated.yaml"
...
# 默认配置从配置文件读取（deploy-incubator-backend-rate-limit.conf）
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/middleware/incubator-backend-rate-limit/generate-incubator-backend-rate-limit/generate-incubator-backend-rate-limit.sh"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/middleware/incubator-backend-rate-limit/generate-incubator-backend-rate-limit/generate-incubator-backend-rate-limit.sh"
  if [ -f "$generate_script" ]; then
    if bash "$generate_script"; then
  ...
  case "$action" in
    deploy)
      log_info "部署 Incubator App BFF Rate Limit Middleware..."
    ...
    uninstall)
      log_info "卸载 Incubator App BFF Rate Limit Middleware..."
      log_info "命名空间: $namespace"
    ...
    status)
      log_info "检查 Incubator App BFF Rate Limit Middleware 状态..."
      log_info "命名空间: $namespace"
      
      if kubectl get middleware incubator-app-backend-rate-limit -n "$namespace" 2>/dev/null; then
