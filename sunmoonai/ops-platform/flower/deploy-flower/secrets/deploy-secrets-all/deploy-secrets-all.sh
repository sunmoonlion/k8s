...
# =============================================================================
# Flower Secrets 总控部署脚本
# 文件名: deploy-secrets-all.sh
# 用途: 统一部署Flower的所有Secret
# =============================================================================
...
deploy_sub_components_by_priority() {
    ...
    local components=(
        ...
        "flower_secrets:${flower_secrets_enabled:-true}:${flower_secrets_priority:-1000}:Flower 认证密钥:$SCRIPT_DIR/../flower-secrets/deploy-flower-secrets/deploy-flower-secrets.sh"
    )
...
main() {
    ...
    if deploy_sub_components_by_priority "$project_id" "$namespace" "$environment" "$dry_run"; then
        log_success "✅ Flower Secrets 部署完成！"
        return 0
    else
