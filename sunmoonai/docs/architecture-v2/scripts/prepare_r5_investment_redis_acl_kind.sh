...
SOURCE_SECRET="research-admin-backend-redis-conn"
...
PY
  ...
  k get secret "$SOURCE_SECRET" -n "$APP_NAMESPACE" -o json >"$tmpdir/source.json"
  chmod 600 "$tmpdir/source.json"

  # Build the Secret without decoding or printing the inherited connection fields.
  python3 - "$tmpdir/source.json" "$APP_NAMESPACE" "$TARGET_SECRET" "$USER_NAME" "$password" <<'PY' >"$tmpdir/target.json"
import base64,json,sys
source_path,namespace,name,user,password=sys.argv[1:]
source=json.load(open(source_path))["data"]
data={k:source[k] for k in ("REDIS_HOST","REDIS_PORT","REDIS_DB")}
...
job=architecture-v2-investment-redis-acl
    ...
    spec:
      ...
      - name: reconcile
        ...
        - |
          ...
          redis-cli -e -h '${REDIS_HOST}' --user "\$APP_REDIS_USER" SET 'investment:architecture-v2:acl-test' ok EX 30 >/dev/null
          test "\$(redis-cli -e -h '${REDIS_HOST}' --user "\$APP_REDIS_USER" GET 'investment:architecture-v2:acl-test')" = ok
          redis-cli -e -h '${REDIS_HOST}' --user "\$APP_REDIS_USER" DEL 'investment:architecture-v2:acl-test' >/dev/null
          if redis-cli -e -h '${REDIS_HOST}' --user "\$APP_REDIS_USER" SET 'research:architecture-v2:forbidden' denied EX 30 >/dev/null 2>&1; then exit 1; fi
