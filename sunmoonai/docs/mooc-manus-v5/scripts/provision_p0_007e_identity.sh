...
# Reconcile the isolated Casdoor application used by P0-007E. Credentials stay
...
APPLICATION="sunmoonai-tpl-admin-p0-007e"
REDIRECT_URI="https://tpl-admin-p0-007e.sunmoonai.com:30443/api/auth/callback"
MODE="plan"
...
Usage: provision_p0_007e_identity.sh [--apply|--cleanup] [options]
  ...
  --redirect-uri URL      Exact P0-007E OIDC callback URI
EOF
}
...
delete_application() {
  local job="p0-007e-identity-delete"
          ...
          args:
            - |
              ...
              psql -h "\$DB_HOST" -p "\$DB_PORT" -U "\$APP_DB_USER" -d "\$APP_DB_NAME" \
                ...
                >/dev/null
              printf 'P0-007E Casdoor application removed\n'
EOF
  wait_job "$job"
}
...
if [[ "$MODE" == "cleanup" ]]; then
  ...
  printf 'V5-P0-007E identity cleanup passed\n'
  exit 0
fi
...
[[ "$REDIRECT_URI" =~ ^https://tpl-admin-p0-007e\.sunmoonai\.com(:[0-9]+)?/api/auth/callback$ ]] || {
  printf 'redirect URI is outside the P0-007E trust boundary\n' >&2
  exit 1
}
k get secret "$CASDOOR_DATABASE_SECRET" -n "$NAMESPACE" >/dev/null
...
job="p0-007e-identity-provision"
k delete job "$job" -n "$NAMESPACE" --ignore-not-found=true --wait=true >/dev/null
cat <<EOF | k apply -f - >/dev/null
apiVersion: batch/v1
kind: Job
          ...
          args:
            - |
              ...
              INSERT INTO application (
                ...
                'admin', '${APPLICATION}', NOW()::text, 'Template Admin P0-007E',
              ...
              SQL
              printf 'P0-007E Casdoor application reconciled\n'
EOF
wait_job "$job"
printf 'V5-P0-007E identity provisioning passed\n'
