...
# 计算项目根目录（应用根目录）
# 从 deploy-postgresql-llmops-db-secret/ 向上 3 级到达应用根目录
# deploy-postgresql-llmops-db-secret/ -> postgresql-llmops-db-secret/ -> secrets/ -> deploy-celeryworker-llmops/ -> celeryworker-llmops/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由 resources/custom-values/generate.sh 生成）
CUSTOM_VALUES_DIR="$PROJECT_ROOT/resources/custom-values"
SECRET_YAML="$CUSTOM_VALUES_DIR/postgresql-llmops-db-secret-generated.yaml"
...
main() {
    ...
    case "$action" in
        deploy)
            ...
            if kubectl apply -f "$SECRET_YAML" -n "$namespace"; then
                log_success "Secret 已部署: ${SECRET_NAME:-postgresql-llmops-db-secret} (命名空间: $namespace)"
            else
        ...
        status)
            ...
            local secret_name="${SECRET_NAME:-postgresql-llmops-db-secret}"
    ...
    echo ""
    log_success "PostgreSQL LLMOps DB Secret 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
    log_info "  - Secret 名称: ${SECRET_NAME:-postgresql-llmops-db-secret}"
    log_info "  - 命名空间: $namespace"
