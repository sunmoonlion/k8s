...
CONFIG_FILE="$SCRIPT_DIR/deploy-dc-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-document-converter-secret/ 向上 3 级到达应用根目录
# deploy-document-converter-secret/ -> document-converter-secret/ -> secret/ -> deploy-document-converter-backend/ -> document-converter-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/dc-secret/generate-dc-secret/document-converter-secret-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/dc-secret/generate-dc-secret/generate-dc-secret.sh"
