#!/bin/bash
# toutiao-app-backend Ingress YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-ingress.conf"
# 从 generate-ingress/ -> toutiao-app-backend-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/
...
source "$CONFIG_FILE"
[ "${ENABLED:-true}" != "true" ] && log_info "跳过资源生成: ingress (已禁用)" && exit 0
...
main() {
    log_info "开始生成 toutiao-app-backend Ingress YAML..."
