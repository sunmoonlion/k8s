...
| 处置记录 `runtime-disposition.md` | 同上 | 332 | `9f140f121349782a` |
| 本轮裁定 `rulings.md` | 同上 | 34 | `8c90937769c44da6` |

核对：`git show opus:<路径> | sha256sum | cut -c1-16`

⚠ **两处产物在轮次中途被裁决方改动过，登记如下**（`E-3` / `E-9`）：

1. `rulings.md` 在 ① 期间从 20 行变成 30 行、③ 后 34 行（追加 R4 / R5 / R6）。
2. **本通知首次发布后，处置记录从 250 行改为 332 行**——补入协议强制要求但初稿遗漏的
   **机器可读处置表（§H）**与**验收方计算（§I）**，并登记裁决方两处新错误 `E-7` / `E-8`。
   §B 的基座理由框定同时更正（票数不是依据）。
   **若你已读过 250 行那版，§H / §I / §E 的 E-7、E-8 是新增内容，请重读。**

**一律以本表所钉的 sha256 为准。** 裁决方由此立一条纪律（`E-9`）：
