...
> （失败原因见「角色不分会怎样」）、2026-09-04 的 refact 竞争（异议环节缺失被发现，同节）。
...
| 3 | 现在是哪一次竞争 | `ls sunmoonai/docs/dev-plan/rounds/` 里 `status: ACTIVE` 的那一个（见「本次定义」） |
...
这与 [`doc-gate.py`](../../../dev-plan/doc-gate.py) 同源：结论不取决于工作区状态。
...
`competition-status.py` 每次都算出来并打印：
...
[`doc-gate.py`](../../../dev-plan/doc-gate.py) 按**当前分支的 git 索引**判定链接目标是否存在——
...
2026-09-06 实测：已发布的 `runtime` 竞争被自己的判据报成「当前环节 ①，缺五家」。
...
| 2026-09-04 `protocol/competition-status.py --verify` 首版 | 锚点正则只认 `~/repo/` 全路径，短路径形式的一大类**一处未验**，却报「17 处全部可达」 |
| 2026-09-04 refact 竞争的清理扫描 | 正则漏了一类文件名，扫出「0 份残留」，实际还剩一份 |
...
它由 2026-09-04 refact 竞争裁决方的密封判据升级而来——那份文件按其自身规定
...
`competition-status.py` 在 `sunmoonai/docs/dev-plan/protocol/` 下（与本文同一目录，目录说明见该处 `README.md`）。
在仓内任一 worktree 的任意目录跑都可以，路径由脚本自己解析。

| 命令 | 作用 |
| --- | --- |
| `protocol/competition-status.py` | 自动找唯一 `status=ACTIVE` 的竞争，算当前环节、本环节通知在哪、组织者是谁 |
| `protocol/competition-status.py --competition runtime` | 指定哪次竞争；值是 `rounds/` 下的目录名（历史目录名，不改；旧参数名 `--round` 仍可用） |
| `protocol/competition-status.py --json` | 机器可读输出，含 `current` 与 `conflict` |
| `protocol/competition-status.py --competition runtime --verify` | 机械验收：把 ⑤ 里机器能判的判掉，判不了的标「人判」 |

`competition-status.py` 不接受「指定环节」参数：它的职责是**算出**在第几环，接受指定等于把结论交回给调用者。

**投喂不用脚本。**所有者在各家窗口说一句话即可；说哪一句、为什么路径要写死，见同目录 `README.md` 的「投喂」一节。

### 退出码

| 码 | `competition-status.py` |
| --- | --- |
| 0 | 正常；`--verify` 时表示机械判定零失败 |
| 1 | `--verify` 有失败项（标「人判」的不计入） |
| 2 | 用法错误：找不到该次竞争、没有 ACTIVE、有多个 ACTIVE、`round.md` 缺字段；或 ACTIVE 竞争的角色配置不一致 |
...
这行出现时，二者必有一错：要么产物没归档到判据找得到的地方，要么这次竞争其实没走完。
**不许靠改 `status` 让它消失。**

## 8c. 组织者的两条纪律

这两条都是 2026-09-07 实跑撞出来的，写在这里因为它们**跨竞争生效**，
不属于任何一次竞争的裁定记录。
...
⚠ 2026-09-05 的 `runtime` 竞争违反过这一条：工单 §3.2 把发起方的结论
...
**这份协议自己也漏过一次**：异议环节在 2026-09-02 那次竞争实际走过，却没被写进环节表，
