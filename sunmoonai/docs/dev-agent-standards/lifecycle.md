...
- **一个文档 thread 对一个运行时 thread**：运行时换了 thread（resume、fork、上下文压缩后重开），就新开一个文档 thread，接着做什么写在它第一个 turn 的任务书里。
...
任务书就是发出去的那一问，存成 `user-message.md`。**模型只知道任务书里写了的东西**：要它交回什么、能改哪些文件、
...
- **任务书由人写**；由上层定稿拆出的任务，任务书来自定稿的相应一节，随定稿一起生效，人在定稿前可以修改。
...
- 哪些动作需要人批准，见 [人的批准点](detailed-rules/approvals.md)；写任务书的人要把它们写进任务书。人这一侧的其余义务见同一份的「人这一侧的义务」。
