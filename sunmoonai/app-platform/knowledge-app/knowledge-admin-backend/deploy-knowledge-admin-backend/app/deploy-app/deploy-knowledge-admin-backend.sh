...
# knowledge-admin-backend 部署脚本
# 用法: ./deploy-knowledge-admin-backend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 从 app/deploy-app/ 向上 2 级到达 deploy-knowledge-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 向上 1 级到达应用根目录
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

KNOWLEDGE_ADMIN_BACKEND_SCRIPT_DIR="$SCRIPT_DIR"
...
KNOWLEDGE_ADMIN_BACKEND_CONFIG_FILE="$SCRIPT_DIR/deploy-knowledge-admin-backend.conf"
if [[ -f "$KNOWLEDGE_ADMIN_BACKEND_CONFIG_FILE" ]]; then
    source "$KNOWLEDGE_ADMIN_BACKEND_CONFIG_FILE"
    ...
    log_info "已加载 knowledge-admin-backend 配置文件: $KNOWLEDGE_ADMIN_BACKEND_CONFIG_FILE"
else
    log_warn "未找到 knowledge-admin-backend 配置文件: $KNOWLEDGE_ADMIN_BACKEND_CONFIG_FILE，使用默认配置"
fi

DEFAULT_PROJECT_ID="${KNOWLEDGE_ADMIN_BACKEND_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${KNOWLEDGE_ADMIN_BACKEND_NAMESPACE:-}"
DEFAULT_ENVIRONMENT="${ENVIRONMENT:-}"

RESOURCES_DIR="$APP_ROOT/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
KNOWLEDGE_ADMIN_BACKEND_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/knowledge-admin-backend-generated.yaml"
KNOWLEDGE_ADMIN_BACKEND_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/knowledge-admin-backend-pvc/generate-knowledge-admin-backend-pvc/knowledge-admin-backend-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"knowledge-admin-backend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/knowledge-admin-backend-pvc/generate-knowledge-admin-backend-pvc/generate-knowledge-admin-backend-pvc.sh"
    else
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    fi

    if [ -f "$generate_script" ]; then
        export NAMESPACE="${KNOWLEDGE_ADMIN_BACKEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${KNOWLEDGE_ADMIN_BACKEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 knowledge-admin-backend 子组件..."
    ...
    log_success "✅ knowledge-admin-backend 子组件部署完成"
    return 0
}

deploy_app() {
    log_info "开始部署 knowledge-admin-backend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    auto_generate_yaml "$KNOWLEDGE_ADMIN_BACKEND_YAML" "$K8S_RESOURCE_DIR" || exit 1

    export KNOWLEDGE_ADMIN_BACKEND_IMAGE_REGISTRY="${KNOWLEDGE_ADMIN_BACKEND_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    ...
    export KNOWLEDGE_ADMIN_BACKEND_FULL_IMAGE_NAME

    log_info "🚀 阶段1：部署子组件..."
    deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false \
        || { log_error "❌ 子组件部署失败"; return 1; }

    log_info "🚀 阶段2：部署 knowledge-admin-backend 核心服务..."
    ...
    auto_generate_yaml "$KNOWLEDGE_ADMIN_BACKEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$KNOWLEDGE_ADMIN_BACKEND_PVC_YAML" || return 1
    ...
    if [ $? -eq 0 ]; then
        log_success "knowledge-admin-backend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=knowledge-admin-backend"
        echo "  kubectl logs -n $NAMESPACE -l app=knowledge-admin-backend -f"
    else
        log_error "knowledge-admin-backend 部署失败"
        exit 1
    fi
}

uninstall_app() {
    log_info "开始卸载 knowledge-admin-backend..."

    log_info "🚀 阶段1：卸载核心服务..."
    if [ ! -f "$KNOWLEDGE_ADMIN_BACKEND_YAML" ]; then
        log_warn "生成的 YAML 不存在，直接删除资源"
        kubectl delete deployment knowledge-admin-backend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service knowledge-admin-backend -n "$NAMESPACE" --ignore-not-found=true
    else
    ...
    log_success "knowledge-admin-backend 卸载完成！"
}

show_status() {
    log_info "knowledge-admin-backend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=knowledge-admin-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=knowledge-admin-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=knowledge-admin-backend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=knowledge-admin-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=knowledge-admin-backend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
...
main() {
    ...
    log_info "knowledge-admin-backend 部署脚本启动"
