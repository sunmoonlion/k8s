...
检查点恢复、事件流与副作用记账的仓——[`dev-agent/PRD/requirement.md`](../../dev-agent/PRD/requirement.md)
§7 的映射表以本仓为对象。
...
| 项 | 实际状态 |
| --- | --- |
| **`RunBudget` 在生产生效** | 未接线，见 §4.5。`budget_exceeded` 生产不可达。现有实现是内存态 pydantic model、随 graph state 传递，**进程一死即失**，结构上满足不了「跨 run／跨进程仍须正确」的判据。载体须换 PG——见 [`constraints.md`](../../dev-agent/rules/constraints.md)「智能体」`C-A3`（四本账必须落 PostgreSQL），不是单独接线 |
