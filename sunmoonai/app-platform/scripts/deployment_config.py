...
BASE_KEYS = {
    "CONFIG_VERSION", "APP", "DEFAULT_PROFILE", "BUNDLE_DIR", "DEPLOY_SCRIPT",
    "NAMESPACE", "RELEASE_ID", "BACKEND_IMAGE", "ADMIN_IMAGE", "WEB_IMAGE",
...
def _decode_value(raw: str, *, path: Path, line: int) -> str:
    ...
    if "#" in value or UNSAFE_UNQUOTED.search(value):
        raise ConfigError(f"{path}:{line}: quote values containing whitespace or shell syntax")
    return value


def load_conf(path: Path, *, allowed: set[str], required: Iterable[str]) -> dict[str, str]:
    if not path.is_file():
...
def load_profile(path: Path) -> dict[str, str]:
    config = load_conf(path, allowed=PROFILE_KEYS, required={"PROFILE_VERSION", "PROFILE_ENABLED"})
...
def validate_release(config: dict[str, str], release: dict[str, Any]) -> None:
    expected = {
        ...
        "NAMESPACE": release.get("namespace"), "RELEASE_ID": release.get("release_id"),
    ...
    if mismatches:
        raise ConfigError(
            "deployment config differs from the immutable release; render and gate a new release instead of overriding it at runtime: "
