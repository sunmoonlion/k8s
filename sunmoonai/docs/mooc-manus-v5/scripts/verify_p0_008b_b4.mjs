...
/**
 * Strict-TLS, real-Casdoor verification for V5-P0-008B/B4.
 *
...
function businessDeploymentSnapshot() {
  ...
  return Object.fromEntries(
    value.items
      .filter((item) => !item.metadata.name.endsWith('-b4'))
      .map((item) => [
        item.metadata.name,
        {
...
function prepareStrictTlsHome() {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'sunmoonai-b4-browser-'))
...
async function verifySessionOnEveryBackendPod(cookieHeader) {
  ...
  if (pods.length !== 2) throw new Error('expected exactly two B4 backend pods')
...
async function main() {
  ...
  try {
    ...
    summary.backend = assertDeployment('tpl-web-backend-b4', 8000)
    summary.frontend = assertDeployment('tpl-web-frontend-b4', 3000)
    ...
    browser = await chromium.launch({
      headless: true,
      args: [
        `--host-resolver-rules=MAP tpl-web-b4.sunmoonai.com 127.0.0.1, MAP ${providerHost} 127.0.0.1`,
      ],
    ...
    await page.getByText('B4 real identity interaction').waitFor({ timeout: 30000 })
    ...
    if (accepted.status !== 201 || accepted.json().status !== 'succeeded') {
    ...
    if (source.status !== 200 || source.json().evidence_id !== evidenceId) {
    ...
    if (JSON.stringify(businessBefore) !== JSON.stringify(businessDeploymentSnapshot())) {
      throw new Error('a non-B4 business deployment changed during isolated verification')
    }
    summary.http = {
      home: home.status,
    ...
    summary.error = error instanceof Error ? error.message : 'unknown B4 verification failure'
