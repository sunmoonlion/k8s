#!/bin/bash
# research-admin-backend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-research-admin-backend-secret.conf"
# 从 generate-research-admin-backend-secret/ -> research-admin-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-research-admin-backend-secret/ -> ... -> k8s-resource/ -> resources/ -> research-admin-backend/
...
main() {
    log_info "开始生成 research-admin-backend Secret YAML 文件..."
