...
# =============================================================================
# Harbor Registry Secret 部署脚本（auth BFF）
...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secret/ -> deploy-auth-app-backend/ -> auth-app-backend/
...
# 尝试加载主配置文件（如果存在），以获取 auth_APP_BFF_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-auth-app-backend.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-auth-app-backend.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (auth BFF)..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    echo ""
    log_success "Harbor Registry Secret (auth BFF) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (auth BFF)"
    echo ""
    echo "用法:"
