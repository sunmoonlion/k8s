...
SECRET_CONFIG="../../deploy-auth-app-bff/secrets/auth-app-bff-secret/deploy-auth-app-bff-secret/deploy-auth-app-bff-secret.conf"
...
# 设置默认环境变量
export NAMESPACE="${NAMESPACE:-${AUTH_APP_BFF_NAMESPACE:-app-platform-dev}}"
export ENVIRONMENT="${ENVIRONMENT:-development}"
export ENV="${ENV:-dev}"

# 准备 Auth App BFF 相关的环境变量
...
export AUTH_APP_BFF_FULL_IMAGE_NAME="${AUTH_APP_BFF_IMAGE_REGISTRY}/${AUTH_APP_BFF_IMAGE_PROJECT}/${AUTH_APP_BFF_IMAGE}:${AUTH_APP_BFF_TAG}"
...
export AUTH_APP_BFF_SECRET_NAME="${AUTH_APP_BFF_SECRET_NAME:-auth-app-bff-secret}"
export AUTH_APP_BFF_CONFIGMAP_NAME="${AUTH_APP_BFF_CONFIGMAP_NAME:-auth-app-bff-config}"
export AUTH_APP_BFF_UNIFIED_HOST="${AUTH_APP_BFF_UNIFIED_HOST:-auth.sunmoonai.com}"
export AUTH_APP_BFF_NODE_IP="${AUTH_APP_BFF_NODE_IP:-101.126.151.0}"
export AUTH_APP_BFF_EXTERNAL_PORT="${AUTH_APP_BFF_EXTERNAL_PORT:-30443}"
export AUTH_APP_BFF_TLS_ENABLED="${AUTH_APP_BFF_TLS_ENABLED:-false}"

# Harbor Docker 认证配置（用于 harbor-registry-secret）
HARBOR_DOCKER_SERVER="${HARBOR_DOCKER_SERVER:-${AUTH_APP_BFF_IMAGE_REGISTRY:-harbor.sunmoonai.com:30443}}"
...
# 注意：这些变量的默认值在 deploy-auth-app-bff.conf 中配置
export PVC_NAME="${PVC_NAME:-auth-app-bff-pvc}"
...
# 主函数
main() {
    log_info "开始生成 Auth App BFF YAML 文件..."
