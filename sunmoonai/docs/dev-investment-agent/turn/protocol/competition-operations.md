...
> ⚠ **这一份是「操作」，不是「规则」。**它写的是**派发这一格现在由人做**时怎么办——
> 人投喂、人组织、人确认。规则在 [`competition-protocol.md`](competition-protocol.md)，
> 与谁做无关。哪一格现在谁做见 [九站](../../pipeline.md)。
> **这一格脚本化之后，本文退役，不是被另一份取代。**
...
只在工单判为 T2 时使用；T0/T1 按 [`competition.md`](../competition.md)「要不要开」那张表执行。协议仍是流程真源，
...
| ⑦ 清理发布 | 指定单写者按 [协议](competition-protocol.md)「⑦ 清理与发布」收存证据、清理本次私有产物，再发布 | 目标版本比较通过、批准仍有效；只清理获准对象，不清空他家工作区 |
