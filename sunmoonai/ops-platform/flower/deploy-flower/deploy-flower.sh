...
# 保存 Flower 脚本的目录路径（统一部署模板会重新定义 SCRIPT_DIR）
FLOWER_SCRIPT_DIR="$SCRIPT_DIR"

# 导入统一部署模板
source "$PROJECT_ROOT/../../../utils/unified-deployment-template.sh"

# 恢复 Flower 脚本的目录路径
SCRIPT_DIR="$FLOWER_SCRIPT_DIR"
...
FLOWER_CONFIG_FILE="$SCRIPT_DIR/deploy-flower.conf"
if [[ -f "$FLOWER_CONFIG_FILE" ]]; then
    source "$FLOWER_CONFIG_FILE"
    ...
    log_info "已加载 Flower 配置文件: $FLOWER_CONFIG_FILE"
else
    log_error "缺少 Flower 配置文件: $FLOWER_CONFIG_FILE"
    exit 1
fi
...
# Flower 资源目录（固定路径）
FLOWER_CHART_DIR="$SCRIPT_DIR/../resources/flower"
FLOWER_CUSTOM_VALUES_DIR="$SCRIPT_DIR/../resources/custom-values"
...
process_flower_values() {
    ...
    case "$environment" in
        "production")
            env_values_file="$FLOWER_CUSTOM_VALUES_DIR/prod-values.yaml"
            ;;
        "development")
            env_values_file="$FLOWER_CUSTOM_VALUES_DIR/dev-values.yaml"
            ;;
        *)
            env_values_file="$FLOWER_CUSTOM_VALUES_DIR/dev-values.yaml"
            ;;
    ...
    if [[ -f "$env_values_file" ]]; then
        ...
        # 创建临时 values 文件
        local flower_values_file=$(mktemp)
        cp "$env_values_file" "$flower_values_file"
        
        # 替换基础变量
        local created_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
        
        # 基础变量替换
        sed -i "s/{{PROJECT_ID}}/$project_id/g" "$flower_values_file"
        sed -i "s/{{NAMESPACE}}/$namespace/g" "$flower_values_file"
        sed -i "s/{{ENVIRONMENT}}/$environment/g" "$flower_values_file"
        sed -i "s/{{COMPONENT_NAME}}/flower/g" "$flower_values_file"
        sed -i "s/{{CREATED_AT}}/$created_at/g" "$flower_values_file"
        
        # Flower 特定变量替换（即使变量为空也替换，使用空字符串）
        sed -i "s/{{FLOWER_PROJECT_ID}}/${FLOWER_PROJECT_ID:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_NAMESPACE}}/${FLOWER_NAMESPACE:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_TLS_ENABLED}}/${FLOWER_TLS_ENABLED:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_AUTH_SECRET_NAME}}/${FLOWER_AUTH_SECRET_NAME:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_AUTH_SECRET_PASSWORD_KEY}}/${FLOWER_AUTH_SECRET_PASSWORD_KEY:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_BROKER_URL_KEY}}/${FLOWER_BROKER_URL_KEY:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_BROKER_API_URL_KEY}}/${FLOWER_BROKER_API_URL_KEY:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_BASIC_AUTH_USER_KEY}}/${FLOWER_BASIC_AUTH_USER_KEY:-basic-auth-user}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_IMAGE_VERSION}}/${FLOWER_IMAGE_VERSION:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_IMAGE_REGISTRY}}/${FLOWER_IMAGE_REGISTRY:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_IMAGE_PROJECT}}/${FLOWER_IMAGE_PROJECT:-k8s-images}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_IMAGE_PULL_SECRET_NAME}}/${FLOWER_IMAGE_PULL_SECRET_NAME:-}/g" "$flower_values_file"
        sed -i "s/{{FLOWER_UNIFIED_HOST}}/${FLOWER_UNIFIED_HOST:-llmops.sunmoonai.com}/g" "$flower_values_file"
        sed -i "s|{{FLOWER_URL_PREFIX}}|${FLOWER_URL_PREFIX:-/flower}|g" "$flower_values_file"
        
        # 输出处理后的文件路径
        echo "$flower_values_file"
        return 0
    else
...
# 执行 Flower 部署
execute_flower_deployment() {
    local project_id="$1"
    local namespace="$2"
    local environment="$3"
    local dry_run="$4"
    
    log_info "开始执行 Flower 部署..."
    log_info "项目: $project_id"
    log_info "命名空间: $namespace"
    log_info "环境: $environment"
    log_info "离线模式: $FLOWER_FORCE_OFFLINE"
    
    # 检查 Chart 目录是否存在
    if [[ ! -d "$FLOWER_CHART_DIR" ]]; then
        log_error "Flower Chart 目录不存在: $FLOWER_CHART_DIR"
        return 1
    fi
    
    # 处理 Flower 特定的 values 文件（模板变量替换）
    local values_file=$(process_flower_values "$project_id" "$namespace" "$environment" 2>/dev/null)
    ...
    if [[ $process_exit_code -ne 0 ]] || [[ -z "$values_file" ]]; then
        ...
        log_info "使用 Flower 特定配置: $values_file"
    fi
    ...
    if [[ "$dry_run" == "true" ]]; then
        ...
        log_info "执行 Flower 部署（试运行模式）..."
    else
        log_info "执行 Flower 部署..."
    fi
    
    # 执行部署
    if eval "$helm_cmd"; then
        if [[ "$dry_run" == "true" ]]; then
            log_success "✅ Flower 部署试运行完成"
        else
            log_success "✅ Flower 部署完成"
        fi
        return 0
    else
        log_error "❌ Flower 部署失败"
        return 1
    fi
}
...
test_flower_connection() {
    ...
    log_info "Flower 服务: $service_name"
    log_success "✅ Flower 连接测试完成"
}

# 显示 Flower 连接信息
show_flower_connection_info() {
    local namespace="$1"
    
    echo ""
    echo "=== Flower 连接信息 ==="
    echo "命名空间: $namespace"
    echo "外部访问: https://${FLOWER_UNIFIED_HOST:-llmops.sunmoonai.com}/flower"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件（如果需要）:"
    echo "   echo '${FLOWER_NODE_IP:-115.190.153.150} ${FLOWER_UNIFIED_HOST:-llmops.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 访问 Flower:"
    ...
    echo "   支持 Redis、RabbitMQ、Amazon SQS 等"
    echo ""
}
...
backup_flower_data() {
    local namespace="$1"
    local backup_name="flower-backup-$(date +%Y%m%d-%H%M%S)"
    
    log_info "开始备份 Flower 数据..."
    log_info "备份名称: $backup_name"
    
    # 这里可以实现具体的备份逻辑
    # 例如使用 Flower 备份工具或 Kubernetes 备份工具
    log_success "✅ Flower 数据备份完成: $backup_name"
}

# 创建 Flower 密钥（如果需要）
create_flower_secrets_if_needed() {
    local namespace="$1"
    ...
    case "$action" in
        "deploy")
            ...
            if [[ "${middleware_enabled:-true}" == "true" ]]; then
                log_info "🚀 部署 Flower Middleware 子组件..."
                ...
                if DISABLE_AUTO_CLEANUP=true bash "$SCRIPT_DIR/middleware/deploy-middleware-all/deploy-middleware-all.sh" deploy "$project_id" "$namespace" "$environment" "$dry_run"; then
                    log_success "✅ Flower Middleware 部署成功"
                else
                    log_error "❌ Flower Middleware 部署失败"
                    return 1
                fi
            else
                log_info "跳过 Flower Middleware (enabled=false)"
            fi
            ...
            if [[ "${PUSH_IMAGES_BEFORE_DEPLOY:-false}" == "true" ]]; then
                if [[ -x "/home/zym/k8s/utils/registry-push-management/loadimage.sh" ]]; then
                    log_info "按清单推送 Flower 镜像到 Registry..."
                    "/home/zym/k8s/utils/registry-push-management/loadimage.sh" push-from-list \
                        "/home/zym/k8s/utils/components-images/flower-images.txt" || log_warn "Flower 镜像推送失败或部分失败"
                fi
            else
                log_info "跳过 Flower 镜像推送（PUSH_IMAGES_BEFORE_DEPLOY=false）"
            fi
            
            # 部署核心组件（Flower Helm Chart，创建 Service 和 Pod）
            if ! execute_flower_deployment "$project_id" "$namespace" "$environment" "$dry_run"; then
                log_error "❌ Flower 核心组件部署失败"
                return 1
            fi
            
            # 部署 Ingress 子组件（依赖 Service 存在，必须在核心组件之后部署）
            if [[ "${ingress_enabled:-true}" == "true" ]]; then
                log_info "🚀 部署 Flower Ingress 子组件..."
                if bash "$SCRIPT_DIR/ingress/deploy-ingress/deploy-ingress.sh" deploy "$project_id" "$namespace" "$environment"; then
                    log_success "✅ Flower Ingress 部署成功"
                else
                    log_error "❌ Flower Ingress 部署失败"
                    return 1
                fi
            else
        ...
        "uninstall")
            ...
            if [[ -n "$existing_release" ]]; then
                log_info "发现 Flower release: $release_name"
                if helm uninstall "$release_name" -n "$namespace" --wait --timeout 5m; then
                    log_success "✅ Flower Helm release 卸载成功！"
                else
                    log_error "❌ Flower Helm release 卸载失败！"
                fi
            else
                log_warn "Flower release '$release_name' 未安装或已卸载"
            fi
            ...
            kubectl delete ingressroute -n "$namespace" flower-web-route 2>/dev/null || true
            kubectl delete middleware -n "$namespace" flower-stripprefix flower-policy 2>/dev/null || true
            
            # 最终验证（排除 Terminating 状态的 Pod）
            local remaining
            remaining=$(kubectl get all,ingressroute,middleware -n "$namespace" 2>/dev/null | grep -i "flower" | grep -v "Terminating" || true)
            if [[ -n "$remaining" ]]; then
                log_warn "⚠️  仍有残留资源:"
                echo "$remaining"
                log_info "💡 提示: 如果 Pod 处于 Terminating 状态，这是正常的，Kubernetes 会继续清理"
            else
                log_success "✅ Flower 完全卸载完成！"
            fi
            ;;
        "clean")
            log_info "开始清理 Flower..."
        ...
        *)
            ...
            echo ""
            echo "操作:"
            echo "  deploy     部署 Flower（默认）"
            echo "  upgrade    升级 Flower"
            echo "  uninstall  卸载 Flower"
            echo "  clean      清理 Flower（包括数据）"
            echo "  status     检查 Flower 状态"
            echo "  logs       显示 Flower 日志"
            echo "  backup     备份 Flower 数据"
            echo "  connect    显示连接信息"
            echo ""
            echo "参数:"
