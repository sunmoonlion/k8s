...
# 计算项目根目录（应用根目录）
# 从 deploy-investment-web-frontend-secret/ 向上 3 级到达应用根目录
# deploy-investment-web-frontend-secret/ -> investment-web-frontend-secret/ -> secret/ -> deploy-investment-web-frontend/ -> investment-web-frontend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/investment-web-frontend-secret/generate-investment-web-frontend-secret/generate-investment-web-frontend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${INVESTMENT_WEB_FRONTEND_SECRET_NAME:-investment-web-frontend-secret}"
