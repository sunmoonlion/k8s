...
# 从 deploy-ingress/ -> ingress/ -> deploy-incubator-front/ -> incubator-app-front/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/incubator-front-ingress/generate-ingress/incubator-app-front-ingress-generated.yaml"
...
load_config() {
    ...
    # 主配置文件路径：从 deploy-ingress/ -> ingress/ -> deploy-incubator-front/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-incubator-front.conf"
    ...
    # 从主配置文件构建配置
    SERVICE_NAME="incubator-app-front"
    NAMESPACE="${INCUBATOR_SSR_NAMESPACE:-app-platform-dev}"
    UNIFIED_HOST="${INCUBATOR_SSR_UNIFIED_HOST:-incubator-front.sunmoonai.com}"
...
deploy_http_route() {
    ...
    kubectl get ingressroute -n "$namespace" incubator-app-front-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" incubator-app-front-headers 2>/dev/null || log_warn "Middleware 尚未创建"
...
check_deployment_status() {
    ...
    kubectl get ingressroute -n "$NAMESPACE" -l component=incubator-app-front 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=incubator-app-front 2>/dev/null || echo "Middleware 不存在"
