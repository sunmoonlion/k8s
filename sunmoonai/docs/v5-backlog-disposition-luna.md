...
| B6 解析调度 | B6a 源码已集成；B6b 待实施 | 公共持久定时及四仓消费到期门禁已通过并同步两端 master/Luna；[本包证据](v5-backlog-scheduling-luna.md)。Knowledge 单次轮询、deadline/backoff、回执及重试代次仍待接入，不销掉 M1-203 |
