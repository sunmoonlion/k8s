...
# 尝试加载主配置文件（如果存在），以获取 pgadmin_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-pgadmin.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-pgadmin.conf"
...
main() {
    ...
    if [[ "$dry_run" != "true" ]]; then
        ...
        # 3. 可选：重启相关组件（如果配置了）
        if [[ "${RESTART_pgadminS:-false}" == "true" && -n "${RESTART_pgadminS_LIST:-}" ]]; then
            log_info "重启使用该Secret的组件..."
            IFS=',' read -ra pgadminS <<< "${RESTART_pgadminS_LIST}"
            for component in "${pgadminS[@]}"; do
