...
/**
 * Strict-TLS, real-Casdoor verification for V5-P0-009B Info Web.
 *
...
const { chromium } = require(
  '/home/zymun/master/tpl-app/tpl-web-frontend/app/node_modules/@playwright/test',
)
...
const taskId = 'V5-P0-009B-web-pair'
const namespace = process.env.P0_009B_NAMESPACE || 'app-platform-dev'
const origin =
  process.env.P0_009B_ORIGIN || 'https://info-web-p0-009b.sunmoonai.com:30443'
const providerHost = 'casdoor.sunmoonai.com'
const identitySecret = 'sunmoonai-p0-005-browser-identity'
const caCertificate =
  process.env.P0_009B_CA_CERT ||
...
function stage(value) {
  console.error(`P0_009B_WEB_STAGE=${value}`)
}
...
function businessDeploymentSnapshot() {
  ...
  return Object.fromEntries(
    value.items
      .filter((item) => !item.metadata.name.endsWith('-p0-009b'))
      .map((item) => [
        item.metadata.name,
        {
...
async function verifySessionOnEveryBackendPod(cookieHeader) {
  ...
  if (pods.length !== 2) throw new Error('expected exactly two P0-009B backend pods')
...
async function main() {
  ...
  try {
    ...
    summary.backend = assertDeployment('info-web-backend-p0-009b', 8000)
    summary.frontend = assertDeployment('info-web-frontend-p0-009b', 3000)
    ...
    browser = await chromium.launch({
      headless: true,
      args: [
        `--host-resolver-rules=MAP info-web-p0-009b.sunmoonai.com 127.0.0.1, MAP ${providerHost} 127.0.0.1`,
      ],
    ...
    if (JSON.stringify(businessBefore) !== JSON.stringify(businessDeploymentSnapshot())) {
      throw new Error('a business deployment changed during isolated P0-009B verification')
    }
    summary.http = {
      home: home.status,
    ...
    summary.error =
      error instanceof Error ? error.message : 'unknown P0-009B Web verification failure'
