...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> llm-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
OUTPUT_DIR="$SCRIPT_DIR"

# 尝试读取主应用的 deploy-*.conf（作为基础配置的默认值源）
MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-llm-admin-backend/app/deploy-app/deploy-llm-admin-backend.conf"
if [ -f "$MAIN_DEPLOY_CONFIG" ]; then
    _temp_namespace=$(source "$MAIN_DEPLOY_CONFIG" 2>/dev/null && echo "${LLM_ADMIN_BACKEND_NAMESPACE:-}")
...
export LLM_ADMIN_BACKEND_IMAGE_REGISTRY="${LLM_ADMIN_BACKEND_IMAGE_REGISTRY:-}"
export LLM_ADMIN_BACKEND_IMAGE_PROJECT="${LLM_ADMIN_BACKEND_IMAGE_PROJECT:-}"
export LLM_ADMIN_BACKEND_IMAGE="${LLM_ADMIN_BACKEND_IMAGE:-}"
export LLM_ADMIN_BACKEND_TAG="${LLM_ADMIN_BACKEND_TAG:-}"
export LLM_ADMIN_BACKEND_FULL_IMAGE_NAME="${LLM_ADMIN_BACKEND_IMAGE_REGISTRY}/${LLM_ADMIN_BACKEND_IMAGE_PROJECT}/${LLM_ADMIN_BACKEND_IMAGE}:${LLM_ADMIN_BACKEND_TAG}"
export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-}"
export LLM_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME="${LLM_ADMIN_BACKEND_IMAGE_PULL_SECRET_NAME:-}"
...
# 生成 YAML
main() {
    log_info "开始生成 Llm Admin Backend YAML 文件..."
