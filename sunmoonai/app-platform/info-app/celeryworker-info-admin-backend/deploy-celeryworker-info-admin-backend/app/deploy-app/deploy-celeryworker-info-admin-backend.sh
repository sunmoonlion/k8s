...
# celeryworker-info-admin-backend 部署脚本
# 用法: ./deploy-celeryworker-info-admin-backend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 从 app/deploy-app/ 向上 2 级到达 deploy-celeryworker-info-admin-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 向上 1 级到达应用根目录
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

CELERYWORKER_INFO_ADMIN_BACKEND_SCRIPT_DIR="$SCRIPT_DIR"
...
CELERYWORKER_INFO_ADMIN_BACKEND_CONFIG_FILE="$SCRIPT_DIR/deploy-celeryworker-info-admin-backend.conf"
if [[ -f "$CELERYWORKER_INFO_ADMIN_BACKEND_CONFIG_FILE" ]]; then
    source "$CELERYWORKER_INFO_ADMIN_BACKEND_CONFIG_FILE"
    ...
    log_info "已加载 celeryworker-info-admin-backend 配置文件: $CELERYWORKER_INFO_ADMIN_BACKEND_CONFIG_FILE"
else
    log_warn "未找到 celeryworker-info-admin-backend 配置文件: $CELERYWORKER_INFO_ADMIN_BACKEND_CONFIG_FILE，使用默认配置"
fi

DEFAULT_PROJECT_ID="${CELERYWORKER_INFO_ADMIN_BACKEND_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${CELERYWORKER_INFO_ADMIN_BACKEND_NAMESPACE:-}"
DEFAULT_ENVIRONMENT="${ENVIRONMENT:-}"

RESOURCES_DIR="$APP_ROOT/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
CELERYWORKER_INFO_ADMIN_BACKEND_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/celeryworker-info-admin-backend-generated.yaml"
CELERYWORKER_INFO_ADMIN_BACKEND_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/celeryworker-info-admin-backend-pvc/generate-celeryworker-info-admin-backend-pvc/celeryworker-info-admin-backend-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"celeryworker-info-admin-backend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/celeryworker-info-admin-backend-pvc/generate-celeryworker-info-admin-backend-pvc/generate-celeryworker-info-admin-backend-pvc.sh"
    else
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    fi

    if [ -f "$generate_script" ]; then
        export NAMESPACE="${CELERYWORKER_INFO_ADMIN_BACKEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${CELERYWORKER_INFO_ADMIN_BACKEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 celeryworker-info-admin-backend 子组件..."
    ...
    log_success "✅ celeryworker-info-admin-backend 子组件部署完成"
    return 0
}

deploy_app() {
    log_info "开始部署 celeryworker-info-admin-backend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    auto_generate_yaml "$CELERYWORKER_INFO_ADMIN_BACKEND_YAML" "$K8S_RESOURCE_DIR" || exit 1

    export CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_REGISTRY="${CELERYWORKER_INFO_ADMIN_BACKEND_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    ...
    export CELERYWORKER_INFO_ADMIN_BACKEND_FULL_IMAGE_NAME

    log_info "🚀 阶段1：部署子组件..."
    deploy_sub_components "$PROJECT_ID" "$NAMESPACE" "$ENVIRONMENT" false \
        || { log_error "❌ 子组件部署失败"; return 1; }

    log_info "🚀 阶段2：部署 celeryworker-info-admin-backend 核心服务..."
    ...
    if [ $? -eq 0 ]; then
        log_success "celeryworker-info-admin-backend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=celeryworker-info-admin-backend"
        echo "  kubectl logs -n $NAMESPACE -l app=celeryworker-info-admin-backend -f"
    else
        log_error "celeryworker-info-admin-backend 部署失败"
        exit 1
    fi
}

uninstall_app() {
    log_info "开始卸载 celeryworker-info-admin-backend..."

    log_info "🚀 阶段1：卸载核心服务..."
    if [ ! -f "$CELERYWORKER_INFO_ADMIN_BACKEND_YAML" ]; then
        log_warn "生成的 YAML 不存在，直接删除资源"
        kubectl delete deployment celeryworker-info-admin-backend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service celeryworker-info-admin-backend -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$CELERYWORKER_INFO_ADMIN_BACKEND_YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    ...
    log_success "celeryworker-info-admin-backend 卸载完成！"
}

show_status() {
    log_info "celeryworker-info-admin-backend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=celeryworker-info-admin-backend 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    log_info "celeryworker-info-admin-backend 部署脚本启动"
