...
#### 已记录的例外：本轮 refact 期间由 agent 代推主线

| 项 | 内容 |
| --- | --- |
| **裁定** | 所有者口头授权：本轮 refact 期间，由 agent 代做 `master` 的合并与推送 |
| **方向** | **朝省事**——因此按「方向不对称」必须有人确认并记录，两样都有 |
| **理由** | 上面三条前置在托管方，本机做不完；不授权则每一条改动都要人手动合，本轮改动量大 |
| **边界** | **只限本轮 refact**。本轮结束即失效，不得延用到下一次竞争，也不得据此推断其他不可逆动作可代做 |
| **不变的部分** | ⑥ 确认本身**仍不可代行**：这条例外只覆盖「把已定稿的东西搬上主线」，不覆盖「决定它可以上主线」 |
| **失效条件** | 分支保护与凭据两件事做完之日，本例外一并作废，回到上表的规则 |
