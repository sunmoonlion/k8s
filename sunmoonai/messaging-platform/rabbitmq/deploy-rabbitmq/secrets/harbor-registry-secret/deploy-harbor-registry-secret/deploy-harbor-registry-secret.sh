...
# 尝试加载主配置文件（如果存在），以获取 RABBITMQ_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-rabbitmq.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-rabbitmq.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (RabbitMQ)..."
    ...
    # 读取 .dockerconfigjson 文件并 base64 编码
    local docker_config_b64=$(base64 -w 0 "$temp_data_dir/.dockerconfigjson")
    ...
    # 生成 YAML
    {
        ...
        echo "data:"
        echo "  .dockerconfigjson: ${docker_config_b64}"
    } > "$secret_yaml"
    ...
    log_success "Harbor Registry Secret (RabbitMQ) 部署完成！"
}
