...
**新起点原则**：大规模重构（自 sumova 重构起）视为新起点——文档中不写历史演进
叙事（“原来是 v5…”“重写自…”、版本演进对比），仿佛一开始就是如此；演进历史全部
由 git 承担。request 的 ① 原始需求除外（原话证据，允许含旧词）。品牌改名等奠基
变更属于起点之前的前置工作，不立 REQ；请求序列自改名成功的时刻开始。
...
- request 文件夹：`REQ-<三位序号>-<短名>`，如 `REQ-001-sumova-重构`。
...
- 品牌前缀：项目文档统一用 **sumova** 前缀（REQ 短名、任务 ID `SUMOVA-XXX`、
  文档标题与产物名），弃用旧 mooc-manus/v6 命名；文件夹内固定名文件（baseline.md
  等）不重复前缀，唯一性由文件夹名保证。
