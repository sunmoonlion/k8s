...
RESTORE_DATABASE="${RESTORE_DATABASE:-info_r5_restore_roundtrip_20260809}"
SOURCE_MIGRATION_SECRET="${SOURCE_MIGRATION_SECRET:-info-admin-backend-migration-postgresql-conn}"
ROUNDTRIP_SECRET="r5-info-migration-roundtrip-db"
IMAGE_PULL_SECRET="${IMAGE_PULL_SECRET:-harbor-registry-secret}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="$(mktemp -d /tmp/r5-info-roundtrip.XXXXXX)"
...
cleanup() {
  ...
  kubectl_cmd delete job \
    -n "$APP_NAMESPACE" \
    -l architecture.sunmoonai.com/r5-info-roundtrip=true \
    --ignore-not-found \
    --wait=true >/dev/null 2>&1 || true
  kubectl_cmd delete secret "$ROUNDTRIP_SECRET" \
    -n "$APP_NAMESPACE" \
...
run_migration_job() {
  local suffix="$1"
  shift
  local job_name="r5-info-migration-$suffix"
  local command_json

  command_json="$(printf '%s\n' "$@" | jq -R . | jq -sc .)"
  kubectl_cmd delete job "$job_name" \
    -n "$APP_NAMESPACE" \
    --ignore-not-found \
    --wait=true >/dev/null
  ...
  jq -n \
    ...
    '{
      ...
      metadata: {
        namespace: $namespace,
        name: $name,
        labels: {"architecture.sunmoonai.com/r5-info-roundtrip": "true"}
      },
      spec: {
        backoffLimit: 0,
        template: {
...
KEEP_RESTORE_DATABASE=true \
  ...
  "$SCRIPT_DIR/restore_r5_info_database_kind.sh" \
  "$BACKUP_FILE" \
  "$RESTORE_DATABASE" >"$WORK_DIR/baseline.txt"

source_url_b64="$(
  kubectl_cmd get secret "$SOURCE_MIGRATION_SECRET" \
    -n "$APP_NAMESPACE" \
...
run_migration_job upgrade \
  ...
  upgrade \
  head >"$WORK_DIR/upgrade.log"
"$SCRIPT_DIR/audit_r5_info_database_kind.sh" "$RESTORE_DATABASE" \
...
run_migration_job downgrade \
  ...
  /app/alembic.ini \
  downgrade \
  20260714_0004 >"$WORK_DIR/downgrade.log"
"$SCRIPT_DIR/audit_r5_info_database_kind.sh" "$RESTORE_DATABASE" \
...
business_filter='del(.outbox_message, .inbox_message)'
...
test "$(extract_section_json "$WORK_DIR/after-upgrade.txt" alembic_heads)" = '["20260809_0005"]'
test "$(extract_section_json "$WORK_DIR/after-downgrade.txt" alembic_heads)" = '["20260714_0004"]'
test "$(extract_section_json "$WORK_DIR/after-reupgrade.txt" alembic_heads)" = '["20260809_0005"]'

for phase in after-upgrade after-downgrade after-reupgrade; do
  invariants="$(extract_section_json "$WORK_DIR/$phase.txt" info_invariants)"
  test "$(printf '%s' "$invariants" | jq '[.[]] | add')" = "0"
done

echo "section=result"
jq -n \
  --arg task "R5-INFO-MIGRATION-ROUNDTRIP" \
  --arg result "passed" \
  --arg image "$IMAGE" \
  ...
  '{
    ...
    heads: {
      baseline: "20260714_0004",
      upgraded: "20260809_0005",
      downgraded: "20260714_0004",
      reupgraded: "20260809_0005"
    },
    counts: {
...
jq -n \
  ...
  --arg jobs "$(kubectl_cmd get job -n "$APP_NAMESPACE" -l architecture.sunmoonai.com/r5-info-roundtrip=true -o name)" \
  '{
