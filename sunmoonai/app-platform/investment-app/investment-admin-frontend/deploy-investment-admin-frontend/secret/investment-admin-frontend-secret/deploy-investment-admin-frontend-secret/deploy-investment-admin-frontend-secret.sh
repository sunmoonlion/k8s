...
# 从 deploy-investment-admin-frontend-secret/ -> investment-admin-frontend-secret/ -> secret/ -> deploy-investment-admin-frontend/ -> investment-admin-frontend/
...
main() {
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/secret/investment-admin-frontend-secret/generate-investment-admin-frontend-secret/generate-investment-admin-frontend-secret.sh"
    ...
    case "$action" in
        deploy)
            ...
            log_success "investment-admin-frontend Secret 部署完成"
            ;;
        uninstall)
            kubectl delete -f "$SECRET_YAML" -n "$NAMESPACE" --ignore-not-found
            log_success "investment-admin-frontend Secret 卸载完成"
            ;;
        status)
            kubectl get secret "investment-admin-frontend-secret" -n "$NAMESPACE" 2>/dev/null \
                || log_warn "Secret 不存在: investment-admin-frontend-secret"
            ;;
        generate)
