...
本块覆盖**全部 4 份候选**（luna / kimi / **cursor（本家）** / qwen），opus 基座不参赛。`round-protocol.md` §4-C 要求指出每份的强项与缺陷；利益冲突声明正是为自评而设——本家与其余三家同一套标准、同一张排序，不另开豁免。

图例：通过 / 部分 / 缺口。比较依据写到节号或表，不靠票数。排序见 C 末。
...
**cursor（本家，与上同一套尺子）**  
强：七块目录与任务书 §8 一一对应；§6.9 可执行；「熟路问数主链编排权不交给聊天循环」是别人没有的分层；Port 签名含 `submit_result` / `probe`；§23.4 ⚠ 清单；钉 SDK commit。  
缺：见下一小节，不在这里缩成一句。自评不享受「强项多写、缺陷一笔带过」。
...
下列缺陷按是否挡住「当基座」排列。利益冲突的正确用法是**对本家更严**，不把结构问题说成风格问题。

1. **阅读顺序不合格（当基座的硬伤）。** 新章节接在附录 B 之后。一份可发布的开发指南应在附录处结束；读者读完词汇表和持久记录模板后不应再遇到 §15–§23。这是提案期为了冻结节 `git diff` 干净而做的取舍，评审时判定为失败：当基座会迫使整合方先搬家，09-03 就是在搬家时丢掉锚点的。luna / kimi / qwen 都把新内容放在附录之前，本家独有此病。
2. **C.3 未过「上次栽过的那条」。** 新增正文几乎不引 AT-05…AT-21，全文只有 AT-01、AT-22 两个编号——等于声明了验收矩阵的两端，中间门禁/恢复/预算条目全部写成散文指针。luna 有 11 个 AT 编号；本家与 qwen 并列最差。按任务书 §12.3，这不是文风，是验收缺口。
3. **映射表过宽。** Codex × F-EXEC-05 标「已支持」，再补「仍须写 Postgres」。义务是业务 checkpoint 不依赖进程内记忆（基座 §5.5、A3），SDK `thread_resume` 必要非充分。luna 的 `implicit_fallback` / kimi 的「需补法」更贴。本家这条会被读成门禁已过。
4. **两套 I 编号未分写。** 正文混用产品 I3/I12 与 constraints 的 A/C/D/T，没有 luna 那种「constraints I8、产品 I12」。自己没写成 qwen 那样的错误引用，但给整合方留下再撞车的空间。
5. **取证覆盖面窄于 kimi/qwen。** 生产无 agent 写了 domain 定义行号，但没引 `test_dormant_capabilities.py`；部署写了 bundle，但没写 Celery 12 进程 OOM 史；OpenClaw 停在 docs，没有 luna 的 `resolve-route.ts`。七块「都在」不等于每块都是四家里最硬的锚点。
6. **§11 与 §23 重复。** 存续改判拆到两处，读者要拼起来才知道删除条件。luna 一处改完；kimi 的「暂不改」评估表比本家的双份清理清单更好用。
7. **附录 A 未收新词。** TaskRouter / AgentSupervisor 只活在 §0.5，不许动的词汇表没有对应行（追加本可以接在附录 A 后、§15 前）。与第 1 条同源：把增量全堆到文末。

这些缺陷足以把本家从「基座」里排除。D 节列出的本家主张（§6.9、熟路/生路、Port `submit_result`、⚠ 清单、超时分态）仍然值得吸收——那是落选稿的用法，不是把整份升成基座。

### 四份排序

| 名次 | 候选 | 当基座 | 一句话 |
| --- | ---: | --- | --- |
| 1 | **luna** | **选它** | C.3 独到、七块在正文该在的位置、映射表与存续开句都干净 |
| 2 | kimi | 不选 | 对基座最礼貌、可直接当附录前长节并入；F-EXEC-02 过宽与 D-1…D-4 撞号必须先改 |
| 3 | **cursor（本家）** | **不选** | 七块目录最齐、有独有主张，但附录后落点 + AT 缺口不能当发布基座 |
| 4 | qwen | 不选 | 单点取证往往最硬，但出现错误引用（`constraints.md` I12）和存续开句自相矛盾——当基座会把正确性缺陷写成默认正文 |

第 3 / 第 4 的次序按「当基座的可靠性」排，不是按「可吸收金句数量」排。qwen 的休眠测试、OOM、`subagent-codex` 仍应进最终稿（见 D）；cursor 的 §6.9 等同样进 D。金句多不能抬名次：错误引用比结构差更不能当正文默认。

若只问「谁的独有锚点最多」：qwen ≈ kimi > cursor > luna（生产代码 / 休眠登记）。那是 D 的输入，不是 C 的基座排序。

### 该选谁当基座，以及理由

**选第 1 名 luna（`f8bc48e3` / SHA-256 `3749a0c00bfe6258525f809ec7cc4cb396e4b9c7cc6877d88abfc10f27a5e3e2`）。**
...
**不选第 3 名本家 cursor。** 利益冲突之外，上面 7 条缺陷里第 1、第 2 条单独成立即不合格：附录后追加不是可发布形态；AT 锚定与 09-03 失败模式同类。本家排在 kimi 之后、qwen 之前，是因为没有 qwen 那种事实性错引，同时也没有 kimi 那种「只追加、附录前」的可并入形态。

不选第 2 名 kimi：对基座最礼貌，但 F-EXEC-02 过宽是事实错误，编号撞车会把下一轮整合再次写成散文。先改这两处再并节，不要整份替换 luna。

不选第 4 名 qwen：取证往往最硬，但错误引用 `constraints.md` I12、存续开句未改、F-EXEC-03 过宽，当基座会把这些缺陷变成正文默认。

luna 当基座之后，**必须**按 D 节吸收第 2–4 名，否则会重演 09-03「弄丢落选稿里的锚点」。
