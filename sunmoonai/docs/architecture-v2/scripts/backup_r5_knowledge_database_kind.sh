...
DATABASE_NAME="${DATABASE_NAME:-knowledge_admin}"
STATE_ROOT="${R5_STATE_ROOT:-$HOME/.local/state/sunmoonai/architecture-v2/r5-knowledge}"
...
if [[ "$DATABASE_NAME" != "knowledge_admin" ]]; then
  echo "R5 Knowledge backup only accepts DATABASE_NAME=knowledge_admin" >&2
  exit 2
fi
