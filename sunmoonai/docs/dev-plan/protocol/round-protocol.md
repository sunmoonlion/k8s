...
| **未启动** | 进程存在但 **CPU 时间为 0**（`ps -o etime,time`）→ 它没在算，是挂着 |
...
- **零产出要先分「没起来」还是「在跑」，别急着判逾期。**字节数判据在这里无能——
  ...
  **`ps -o etime,time` 的 CPU 时间就是那个「已开始」信号**：
  2026-09-07 实测，某家进程存活 17 分 29 秒而 **CPU 时间 `00:00:00`**——
  它卡在读 stdin，一步没跑。按字节判据会判成「在推进，不催」，白等一轮。
  ⚠ 反过来**不成立**：CPU 时间为 0 只证明「此刻没在算」，
