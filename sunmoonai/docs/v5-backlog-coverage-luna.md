...
| 服务身份 / M1-002 | `infrastructure/security/service_identity.py`、`core/config.py` 的 subject/scope 精确绑定；[B7l](v5-backlog-runtime-preflight-luna.md) 身份共用；[B7m](v5-backlog-runtime-rendering-luna.md) 修角色键；[B7n](v5-backlog-runtime-permissions-luna.md) 实查 ACL/broker；[B7o](v5-backlog-template-database-policy-luna.md) 模板 18 PG；[B7p](v5-backlog-info-database-policy-luna.md) Info 25 PG；[B7q](v5-backlog-knowledge-database-policy-luna.md) Knowledge 27 PG；[B7r](v5-backlog-investment-database-policy-luna.md) Investment 真断点/控制/回执及拒绝 24 PG 通过 | 真实集群仍旧身份、版本表 CRUD 与 broker 共用仍未切换；模板及三领域加法候选不收敛旧账号，broker、历史权限/供给/撤销/轮换继续受控验收；数据库列 ACL 不代替行级/租户/工具/人的审批，不同键名不等于不同凭据 |
