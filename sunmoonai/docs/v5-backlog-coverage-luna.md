...
| 服务身份 / M1-002 | `infrastructure/security/service_identity.py`、`core/config.py` 的 subject/scope 精确绑定；[B7l](v5-backlog-runtime-preflight-luna.md) 确认运行身份共用；[B7m](v5-backlog-runtime-rendering-luna.md) 修角色键覆盖；[B7n](v5-backlog-runtime-permissions-luna.md) 实查九角色 ACL/继承/default ACL 与 broker 三 vhost 权限，新增只读工具 | 角色引用源码已修，真实集群仍旧身份；全表 CRUD 包含版本表，broker 三角色同用户；独立 principal/精确 ACL/撤销/轮换仍需隔离试验及受控切换，不同键名不等于不同凭据 |
