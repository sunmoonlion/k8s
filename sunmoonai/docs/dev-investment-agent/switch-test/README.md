...
远程开发机（3.6 GB 内存、2 核）只写代码和文档；要真跑的测试在所有者本地机器上跑，结果进 git 回来。两个角色：

| 角色 | 谁 | 做什么 |
| --- | --- | --- |
| 远程助手 | 在远程机上写代码的助手 | 写脚本、推仓、往 `inbox/` 放待办、读结果 |
| 本地助手 | 在所有者本地机器上的助手（现在是 luna） | 同步、跑 `inbox/` 里的待办、把输出推回 |

本目录：`README.md`（本文，约定）、`inbox/`（**待办，一个文件一条**）、`done/`（已办）。测试脚本不放这里，放被测仓。
`~/switch-test/` 是本目录的副本，以仓里为准。一次性环境配置见 [`luna-local-setup.md`](../tree-build/human-ai-turn/imp/luna-local-setup.md)。
...
远程的用词纪律：在远程跑不了的东西一律写「**这一条我没跑过**」，不写「已验证」；远程跑通的写「已跑通，输出如下」。
...
远程能跑的，本地可以再跑一遍作独立复核，那是验收的事，不是必须。

## 六、固定事实

- 工位名 `fable`（工位名 = 目录名 = 分支名）；换模型不换工位，改名时所有者会说；
