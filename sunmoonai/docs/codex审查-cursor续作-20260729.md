...
9. 发布前树审计进一步发现三个 Admin Backend 长期追踪 `app/.env` 与
   `app/.env.k8s`，其中包含非占位数据库、Redis 和 Casdoor 凭据。文件虽早已进入远端历史，
   且被 `.dockerignore` 排除在镜像之外，但继续追踪不符合凭据治理要求。三个正式分支已停止
   追踪这些文件并补齐 ignore；既有历史中的相关凭据必须在 P0-008C 首个真实环境步骤前轮换，
   不能因“已删除当前文件”而视为泄露已消除。
...
- Cursor 续作文档已在审查收敛后删除，本地 `cursor-1` 分支也已删除；历史判断只以本审查
  记录、正式 Git 提交、冻结标签和权威门禁证据为准。
...
- P0-008C 发现的通用缺陷先回流 `tpl-app`，再按受控传播流程同步实例；不得直接制造新的
  三仓漂移。
- P0-008C 的第一项环境前置是轮换曾进入 Git 历史的数据库、Redis、Casdoor 凭据，并以
  secret scan 和旧凭据拒绝矩阵留证；轮换完成前不得启动真实 Provider/身份试点。
