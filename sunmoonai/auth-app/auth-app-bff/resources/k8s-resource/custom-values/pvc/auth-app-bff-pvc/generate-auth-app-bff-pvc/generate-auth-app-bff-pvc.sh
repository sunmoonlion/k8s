...
CONFIG_FILE="$SCRIPT_DIR/generate-auth-app-bff-pvc.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-auth-app-bff-pvc/ -> auth-app-bff-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-auth-app-bff-pvc/ -> auth-app-bff-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/ -> resources/ -> auth-app-bff/
...
# ============================================================================
# 设置环境变量（用于模板替换）
# 注意：所有配置都从 generate-auth-app-bff-pvc.conf 读取，不在脚本中设置硬编码默认值
# ============================================================================
# 基础配置（从 generate-auth-app-bff-pvc.conf 中读取，所有默认值在 conf 文件中定义）
export NAMESPACE="${NAMESPACE:-}"
export ENVIRONMENT="${ENVIRONMENT:-}"
export ENV="${ENV:-}"

# PVC 数据内容（从 generate-auth-app-bff-pvc.conf 中读取，所有默认值在 conf 文件中定义）
