...
# 项目根目录（deploy-llmops-bff 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-llmops-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（llmops-app-bff 目录）
# 从 deploy-llmops-bff/ 向上 1 级到达 llmops-app-bff/
...
# 导入统一部署模板
# 从应用根目录（llmops-app-bff/）到 k8s/ 需要向上 4 级
# llmops-app-bff/ -> llmops-app/ -> sunmoonai/ -> k8s/
...
# 默认配置从配置文件读取（deploy-llmops-bff.conf）
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（llmops-app-bff/）
# app/deploy-app/ -> app/ -> deploy-llmops-bff/ -> llmops-app-bff/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
LLMOPS_BFF_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/llmops-app-bff-generated.yaml"
LLMOPS_BFF_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-bff-pvc/generate-llmops-bff-pvc/llmops-app-bff-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"llmops-app-bff-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"llmops-app-bff-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-bff-pvc/generate-llmops-bff-pvc/generate-llmops-bff-pvc.sh"
    else
