...
INGRESS_YAML="$K8S_RESOURCE_DIR/custom-values/ingress/investment-web-frontend-ingress/generate-ingress/investment-web-frontend-ingress-generated.yaml"
...
main() {
    ...
    case "$action" in
        deploy)
            ...
            log_success "investment-web-frontend Ingress 部署完成"
            ;;
        uninstall)
            kubectl delete -f "$INGRESS_YAML" -n "$NAMESPACE" --ignore-not-found
            log_success "investment-web-frontend Ingress 卸载完成"
            ;;
        status)
            kubectl get ingressroute "investment-web-frontend-ingress" -n "$NAMESPACE" 2>/dev/null \
                || log_warn "IngressRoute 不存在: investment-web-frontend-ingress"
            ;;
        generate)
