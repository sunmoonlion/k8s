...
/**
 * Strict-TLS, real-Casdoor pairing verification for P0-009C.
...
const origin =
  process.env.P0_009C_ORIGIN ||
  'https://knowledge-admin-p0-009c.sunmoonai.com:30443'
...
function stage(value) {
  process.stderr.write(`P0_009C_ADMIN_BROWSER_STAGE=${value}\n`)
}
...
function prepareStrictTlsHome() {
  ...
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'sunmoonai-p0-009c-'))
...
async function submitCasdoorLogin(page, identity) {
  ...
  try {
    ...
    const diagnosticPath = '/tmp/p0-009c-casdoor-login.png'
...
async function main() {
  ...
  const frontend = deployment('knowledge-admin-frontend-p0-009c', 3000)
  const backend = deployment('knowledge-admin-backend-p0-009c', 8000)
  const identity = {
  ...
  execFileSync(
    ...
    [
      ...
      '-n',
      namespace,
      'deployment/knowledge-admin-frontend-p0-009c',
      '--',
      'sh',
      '-lc',
      'test "$(id -u)" = 1001 && command -v node >/dev/null && ! touch /app/.p0-009c-write-probe',
    ],
    { stdio: 'ignore' },
  )
