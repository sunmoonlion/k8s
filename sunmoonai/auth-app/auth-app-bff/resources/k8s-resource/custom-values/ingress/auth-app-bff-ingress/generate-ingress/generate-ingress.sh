...
# 从 generate-ingress/ -> auth-app-bff-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-ingress/ -> auth-app-bff-ingress/ -> ingress/ -> custom-values/ -> k8s-resource/ -> resources/ -> auth-app-bff/
...
MAIN_DEPLOY_CONFIG="$PROJECT_ROOT/deploy-auth-app-bff/app/deploy-app/deploy-auth-app-bff.conf"
...
main() {
    ...
    if [ "${USE_STRIP_PREFIX:-false}" = "true" ]; then
        # 使用 stripprefix 中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: auth-app-bff-stripprefix' "$full_output_path"
    elif [ "${USE_RATE_LIMIT:-true}" = "true" ]; then
        # 使用限流中间件
        sed -i '/#MIDDLEWARE_LIST_START/,/#MIDDLEWARE_LIST_END/c\        - name: auth-app-bff-rate-limit' "$full_output_path"
    else
