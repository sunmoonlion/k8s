#!/bin/bash
# Auth App Front PVC YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-auth-app-front-pvc.conf"
# 从 generate-auth-app-front-pvc/ -> auth-app-front-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/
...
main() {
    log_info "开始生成 Auth App Front PVC YAML..."
