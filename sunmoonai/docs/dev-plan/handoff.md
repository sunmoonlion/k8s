...
| 轮 | 状态 |
| --- | --- |
| R0 修脚本↔协议不一致 + 合并 `protocol-v2` | **已做**（2026-09-06）。主线 `round-protocol.md` = 七环节版；三个脚本进主线；四处脚本↔协议不一致已修（产物路径、调用方式、退出码、已完成轮次被判成刚开始）。判据与结果见 `rounds/_r0/criteria.md`。⚠ 该轮**产出方 = 验收方**，待外部复验 |
