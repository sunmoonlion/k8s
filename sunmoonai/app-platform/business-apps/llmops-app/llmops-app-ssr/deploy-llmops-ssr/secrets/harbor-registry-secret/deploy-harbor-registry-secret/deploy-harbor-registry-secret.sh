...
# 计算项目根目录（k8s 目录）
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secrets/ -> deploy-llmops-ssr/ -> llmops-app-ssr/ -> llmops-app/ -> business-apps/ -> app-platform/ -> sunmoonai/ -> k8s/
...
main() {
    ...
    echo ""
    log_success "Harbor Registry Secret (LLMOPS SSR) 部署完成！"
    log_info "部署信息："
