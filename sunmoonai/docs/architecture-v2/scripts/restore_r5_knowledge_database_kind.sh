...
BACKUP_FILE="$1"
RESTORE_DATABASE="${2:-knowledge_r5_restore_$(date -u +%Y%m%d%H%M%S)}"
...
if [[ ! "$RESTORE_DATABASE" =~ ^knowledge_r5_restore_[a-zA-Z0-9_]+$ ]]; then
...
drop_restore_database
postgres_sql "CREATE DATABASE \"$RESTORE_DATABASE\" OWNER knowledge_backend_user_migration;" >/dev/null
