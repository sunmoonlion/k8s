...
> 基于全量源码阅读（后端约 15600 行 Python 含测试/脚本、契约锁、迁移链、两个前端、父仓文档），2026-08-11。
> 仓库路径：`/home/zymun/investment-app`

## 1. 定位与改名背景

investment-app 是架构 v2 的**投资研究与智能体域 App**，由原 `research-app` 以 Git 历史为迁移主体**原地改名 + R3.2 模板同步**而来（K8s 仓库 `sunmoonai/docs/investment清理和改名.md` 是阶段权威）。

- 领域所有权：Session/Thread/Run/Attempt/Invocation、Graph/Runtime 与 Agent 产品状态；**不接管 Info/Knowledge 领域事实**。
- 合法领域概念可保留 `research` 命名（如 `research_web_pilot` graph、前端 research 组件），但应用/基础设施身份必须逐项迁为 `investment-*`。
- 旧 Research 四组件运行态是**回滚面**，R7 前禁止删除；历史材料只在冻结标签 `p0-009a-pre-20260729`，不得复制回活动架构。
- 未来 research-app 边界（权威架构文档）：未来通用研究 App 必须从届时已验收的 tpl-app 全新实例化（独立仓库/身份/数据库/对象空间/消息资源/契约），不继承旧 Research 身份，也不得把 Investment 的研究数据自动归属给它；当前拓扑中的“研究能力”默认全部属于 investment-app。
...
├── contracts/
│   ├── README.md                              # 消费锁治理规则
│   └── knowledge-retrieval-provider-lock.json # retrieval/v1 消费锁
├── docs/（改名/恢复/对齐说明，均只读历史性质）
├── investment-backend/（app/ 包根 + db-provisioner + 三个 access-bootstrap + mybuild）
├── investment-admin-frontend/app/
└── investment-web-frontend/app/
```

发布规则：三子仓各自过质量门禁 → 不可变候选 tag 推 `harbor.sunmoonai.com:30443/app-images` → 记录 manifest digest 写入 K8s 仓库 Investment `release-inputs.json` → 用 `tpl-app/k8s-scaffold-v2` 部署 → **R7 通过才能晋级正式 `2.0.0`，禁止重新构建再打 tag**；K8s 清单必须引用 `repository@sha256:...`。

## 3. 契约消费侧（retrieval/v1 锁）
...
## 5. LangGraph 图族（infrastructure/graph）
...
## 9. Investment 专属配置（core/config.py 647 行）
...
- 认证表面沿用模板：cookie `sunmoonai_investment_{surface}_sid`、admin 强制 scope `investment:admin`、Web 可开自助注册（`GET /api/auth/web/signup`，Casdoor signup endpoint + 授权码 PKCE，Admin 关闭注册）。前端 AUTH_APP 枚举在模板 `tpl|info|knowledge|research` 基础上领域扩展加入 `investment`（默认值），是 R4 domain-extension 分类的实例。
...
| R7 前 Research 运行态保留为回滚面 | CLAUDE.md / README.md |
