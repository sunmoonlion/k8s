...
| `F-SBX-08` | 全局沙箱上限（配置项）：在跑的沙箱数到上限时，新拉起请求明确拒绝并提示稍后再试，不排挤已有用户；上限与当前数在运维面可见（自助注册的配套闸，见 `security.md`「账号与注册」）。**已做（2026-09-26）**：配置 `WORKBENCH_SANDBOX_GLOBAL_LIMIT`（默认 20，0 = 不设上限），只挡新增，已有沙箱的用户更新不受限；满了回 503 `sandbox_capacity_full`，设置页提示"云端沙箱暂时满了" |
