...
# 尝试加载主配置文件（如果存在），以获取 mongo-express_IMAGE_REGISTRY 等环境变量
# 主配置文件路径：../../deploy-mongo-express.conf（相对于当前脚本目录）
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-mongo-express.conf"
...
main() {
    ...
    if [[ "$dry_run" != "true" ]]; then
        ...
        # 3. 可选：重启相关组件（如果配置了）
        if [[ "${RESTART_mongo-expressS:-false}" == "true" && -n "${RESTART_mongo-expressS_LIST:-}" ]]; then
            log_info "重启使用该Secret的组件..."
            IFS=',' read -ra mongo-expressS <<< "${RESTART_mongo-expressS_LIST}"
            for component in "${mongo-expressS[@]}"; do
