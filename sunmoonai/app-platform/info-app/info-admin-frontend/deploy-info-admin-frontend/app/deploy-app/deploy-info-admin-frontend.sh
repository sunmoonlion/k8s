...
# info-admin-frontend 部署脚本
# 用法: ./deploy-info-admin-frontend.sh <deploy|uninstall|status> [project_id] [namespace] [environment]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 从 app/deploy-app/ 向上 2 级到达 deploy-info-admin-frontend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 向上 1 级到达应用根目录
APP_ROOT="$(cd "$PROJECT_ROOT/.." && pwd)"

INFO_ADMIN_FRONTEND_SCRIPT_DIR="$SCRIPT_DIR"
...
SCRIPT_DIR="$INFO_ADMIN_FRONTEND_SCRIPT_DIR"
...
INFO_ADMIN_FRONTEND_CONFIG_FILE="$SCRIPT_DIR/deploy-info-admin-frontend.conf"
if [[ -f "$INFO_ADMIN_FRONTEND_CONFIG_FILE" ]]; then
    source "$INFO_ADMIN_FRONTEND_CONFIG_FILE"
    ...
    log_info "已加载 info-admin-frontend 配置文件: $INFO_ADMIN_FRONTEND_CONFIG_FILE"
else
    log_warn "未找到 info-admin-frontend 配置文件: $INFO_ADMIN_FRONTEND_CONFIG_FILE，使用默认配置"
fi

DEFAULT_PROJECT_ID="${INFO_ADMIN_FRONTEND_PROJECT_ID:-}"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"info-admin-frontend-pvc-generated.yaml" ]]; then
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/info-admin-frontend-pvc/generate-info-admin-frontend-pvc/generate-info-admin-frontend-pvc.sh"
    else
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    fi

    if [ -f "$generate_script" ]; then
        export NAMESPACE="${INFO_ADMIN_FRONTEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INFO_ADMIN_FRONTEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 info-admin-frontend 子组件..."
    ...
    log_success "✅ info-admin-frontend 子组件部署完成"
    return 0
}

deploy_app() {
    log_info "开始部署 info-admin-frontend..."
    log_info "环境: $ENVIRONMENT, 命名空间: $NAMESPACE"

    auto_generate_yaml "$INFO_ADMIN_FRONTEND_YAML" "$K8S_RESOURCE_DIR" || exit 1

    export INFO_ADMIN_FRONTEND_IMAGE_REGISTRY="${INFO_ADMIN_FRONTEND_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    export INFO_ADMIN_FRONTEND_IMAGE_PROJECT="${INFO_ADMIN_FRONTEND_IMAGE_PROJECT:-app-images}"
    export INFO_ADMIN_FRONTEND_IMAGE="${INFO_ADMIN_FRONTEND_IMAGE:-info-admin-frontend}"
    export INFO_ADMIN_FRONTEND_TAG="${INFO_ADMIN_FRONTEND_TAG:-1.0.0}"
    export IMAGE_PULL_POLICY="${IMAGE_PULL_POLICY:-Always}"
    export INFO_ADMIN_FRONTEND_IMAGE_PULL_SECRET_NAME="${INFO_ADMIN_FRONTEND_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"
    ...
    auto_generate_yaml "$INFO_ADMIN_FRONTEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$INFO_ADMIN_FRONTEND_PVC_YAML" || return 1
    kubectl apply -f "$INFO_ADMIN_FRONTEND_PVC_YAML" -n "$NAMESPACE" \
        && log_success "PVC 部署完成" \
        || { log_error "PVC 部署失败"; return 1; }

    kubectl apply -f "$INFO_ADMIN_FRONTEND_YAML" -n "$NAMESPACE"

    if [ $? -eq 0 ]; then
        log_success "info-admin-frontend 部署完成！"
        echo ""
        log_info "检查部署状态:"
        echo "  kubectl get pods -n $NAMESPACE -l app=info-admin-frontend"
        echo "  kubectl logs -n $NAMESPACE -l app=info-admin-frontend -f"
    else
        log_error "info-admin-frontend 部署失败"
        exit 1
    fi
}

uninstall_app() {
    log_info "开始卸载 info-admin-frontend..."

    log_info "🚀 阶段1：卸载核心服务..."
    if [ ! -f "$INFO_ADMIN_FRONTEND_YAML" ]; then
        log_warn "生成的 YAML 不存在，直接删除资源"
        kubectl delete deployment info-admin-frontend -n "$NAMESPACE" --ignore-not-found=true
        kubectl delete service info-admin-frontend -n "$NAMESPACE" --ignore-not-found=true
    else
        kubectl delete -f "$INFO_ADMIN_FRONTEND_YAML" -n "$NAMESPACE" --ignore-not-found=true
    fi
    ...
    log_success "info-admin-frontend 卸载完成！"
}

show_status() {
    log_info "info-admin-frontend 状态:"
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 Deployments:"
    kubectl get deployment -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 Deployment"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 Secret"
    echo ""
    echo "📦 PVCs:"
    kubectl get pvc -n "$NAMESPACE" -l app=info-admin-frontend 2>/dev/null || echo "  无 PVC"
}
...
main() {
    ...
    log_info "info-admin-frontend 部署脚本启动"
