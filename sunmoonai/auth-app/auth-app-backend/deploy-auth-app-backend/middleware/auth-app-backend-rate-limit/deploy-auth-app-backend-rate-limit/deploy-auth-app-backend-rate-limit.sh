...
CONFIG_FILE="$SCRIPT_DIR/deploy-auth-app-backend-rate-limit.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-auth-app-backend-rate-limit/ -> auth-app-backend-rate-limit/ -> middleware/ -> deploy-auth-app-backend/ -> auth-app-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
MIDDLEWARE_YAML="$K8S_RESOURCE_DIR/custom-values/middleware/auth-app-backend-rate-limit/generate-auth-app-backend-rate-limit/auth-app-backend-rate-limit-generated.yaml"
...
# 默认配置从配置文件读取（deploy-auth-app-backend-rate-limit.conf）
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/middleware/auth-app-backend-rate-limit/generate-auth-app-backend-rate-limit/generate-auth-app-backend-rate-limit.sh"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/middleware/auth-app-backend-rate-limit/generate-auth-app-backend-rate-limit/generate-auth-app-backend-rate-limit.sh"
  if [ -f "$generate_script" ]; then
    if bash "$generate_script"; then
  ...
  case "$action" in
    ...
    status)
      ...
      if kubectl get middleware auth-app-backend-rate-limit -n "$namespace" 2>/dev/null; then
