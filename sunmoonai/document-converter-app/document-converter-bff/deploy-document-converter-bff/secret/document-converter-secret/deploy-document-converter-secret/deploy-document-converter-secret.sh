...
CONFIG_FILE="$SCRIPT_DIR/deploy-document-converter-secret.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-document-converter-secret/ 向上 3 级到达应用根目录
# deploy-document-converter-secret/ -> document-converter-secret/ -> secret/ -> deploy-document-converter-bff/ -> document-converter-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/document-converter-secret/generate-document-converter-secret/document-converter-secret-generated.yaml"
...
# 默认配置从配置文件读取（deploy-document-converter-secret.conf）
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/document-converter-secret/generate-document-converter-secret/generate-document-converter-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="document-converter-secret"
