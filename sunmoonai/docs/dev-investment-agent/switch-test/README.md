...
| **远程助手** | 在远程机上工作的 AI 助手，工位 `fable`。现在是 Claude（Fable）；换模型不换工位 | 远程机 |
| **本地助手** | 在本地机上工作的 AI 助手。现在是 luna | 本地机 |
...
`~/switch-test/` 是本目录在两台机上的副本，以仓里为准。本地机的一次性环境配置见 [`luna-local-setup.md`](../tree-build/human-ai-turn/imp/luna-local-setup.md)。
...
- 两台机上工位名都是 `fable`（工位名 = 目录名 = 分支名，`~/worktrees/fable/<仓>`）；换模型不换工位，改名时所有者会说；
