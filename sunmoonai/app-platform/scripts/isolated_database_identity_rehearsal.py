...
def exercise(app, baseline, old_secret, container, admin_password):
    ...
    # Exercise the actual live activation inventory/compiler/atomic guard, not
    # merely a separate approximation of its SQL. This still has no live access.
    raw = common.docker_pg(container,admin_password,db,None,"psql","-h","127.0.0.1","-U","postgres",
        "-d",db,"-X","-qAt","-v","ON_ERROR_STOP=1",data=("SET search_path=pg_catalog;\n"+activation.inventory_sql(app)).encode()).decode().strip()
    inventory = json.loads(raw)
    common.private_write(common.ERROR_DIR / ("activation-inventory-" + secrets.token_hex(6) + ".private.json"), raw)
    sql,tables = activation.compile_activation(app,inventory["revisions"][0],inventory,passwords)
    guarded = activation.guarded_transaction(app,raw,sql,tables)
    # Two actual PostgreSQL failures, not string assertions: stale inventory
    # before mutation, and a late failure after all role/grant statements.
    # Both must leave no runtime role and an identical permission catalog.
    rejected = ((activation.guarded_transaction(app, raw + " ", sql, tables), "activation inventory changed"),
                (guarded.replace("COMMIT;", "DO $fault$ BEGIN RAISE EXCEPTION 'synthetic activation rollback'; END $fault$;\nCOMMIT;"),
                 "synthetic activation rollback"))
    for transaction, expected in rejected:
        result = subprocess.run(["docker", "exec", "-i", "-e", "PGPASSWORD", container,
            "psql", "-h", "127.0.0.1", "-U", "postgres", "-d", db, "-X", "-qAt", "-v", "ON_ERROR_STOP=1"],
            input=transaction.encode(), capture_output=True, env={**os.environ, "PGPASSWORD": admin_password}, timeout=40)
        if result.returncode == 0 or ("ERROR:  " + expected).encode() not in result.stderr:
            raise common.RehearsalError("expected_activation_rejection_missing")
        after = common.docker_pg(container,admin_password,db,None,"psql","-h","127.0.0.1","-U","postgres",
            "-d",db,"-X","-qAt","-v","ON_ERROR_STOP=1",data=("SET search_path=pg_catalog;\n"+activation.inventory_sql(app)).encode()).decode().strip()
        if after != raw:
            raise common.RehearsalError("rejected_activation_did_not_rollback_catalog")
    common.docker_pg(container,admin_password,db,guarded)
    probe_count = 1
    ...
    return {"fresh_role_login_and_permissions":True,"creator_defaults_closed":True,
            "stale_inventory_rejected":True,"late_failure_rolled_back_complete_catalog":True,
...
def main():
    ...
    sources = [Path(__file__),Path(common.__file__),Path(cutover.__file__),Path(activation.__file__),Path(activation.validation.__file__),
...
if __name__ == "__main__":
    try:
        ...
        print(common.encoded({"status":"failed","reason":str(exc) if isinstance(exc,(common.RehearsalError, activation.validation.PolicyError)) else type(exc).__name__}))
        raise SystemExit(1)
