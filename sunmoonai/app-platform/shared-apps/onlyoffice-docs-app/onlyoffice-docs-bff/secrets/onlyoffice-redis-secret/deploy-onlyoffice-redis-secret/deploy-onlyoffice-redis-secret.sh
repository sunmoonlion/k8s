...
# 用途: 从 Redis Secret 获取密码并创建 ONLYOFFICE Docs 的 Redis 连接 Secret
# =============================================================================
...
# 计算项目根目录（k8s目录）
# 从 deploy-onlyoffice-redis-secret/ 向上6级到达 k8s/
# deploy-onlyoffice-redis-secret/ -> onlyoffice-redis-secret/ -> secrets/ -> onlyoffice-docs/ -> app-platform/ -> sunmoonai/ -> k8s/
...
main() {
    ...
    if [[ -z "${REDIS_PASSWORD:-}" ]]; then
        ...
        log_error ""
        log_error "示例："
        log_error "  REDIS_PASSWORD=\"your_redis_password\""
        exit 1
    fi
    
    local password="$REDIS_PASSWORD"
    log_success "已从配置文件获取 Redis 密码"
    log_warn "⚠️  请确保此密码与 Redis 部署时使用的密码保持一致！"
    ...
    local secret_yaml="$SECRET_DIR/onlyoffice-redis-secret.yaml"
    ...
    echo ""
    log_success "ONLYOFFICE Redis Secret 部署完成！"
    log_info "部署信息："
