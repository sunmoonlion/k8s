...
# 计算项目根目录（应用根目录）
# 从 deploy-investment-web-backend-secret/ 向上 3 级到达应用根目录
# deploy-investment-web-backend-secret/ -> investment-web-backend-secret/ -> secret/ -> deploy-investment-web-backend/ -> investment-web-backend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/investment-web-backend-secret/generate-investment-web-backend-secret/generate-investment-web-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${INVESTMENT_WEB_BACKEND_SECRET_NAME:-investment-web-backend-secret}"
