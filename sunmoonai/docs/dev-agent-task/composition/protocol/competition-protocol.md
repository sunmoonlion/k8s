...
2026-09-08 R6 实际撞上过这一条（见 tag `dev-plan-final` 中 dev-plan-refact 那次竞争的 findings.md F-21）。
...
原判定脚本 `competition-status.py` 按旧的 `dev-plan/rounds/` 目录判定，2026-09-15 随 dev-plan 删除（原样见 tag `dev-plan-final`）。
按任务目录的结构重写之前：
