...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"investment-admin-backend-pvc-generated.yaml" ]]; then
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${INVESTMENT_ADMIN_BACKEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${INVESTMENT_ADMIN_BACKEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 investment-admin-backend 子组件..."
...
deploy_app() {
    ...
    log_info "镜像: $INVESTMENT_ADMIN_BACKEND_FULL_IMAGE_NAME"
    log_warn "⚠️  请确保该镜像已存在于 Harbor 仓库"

    export NAMESPACE="$NAMESPACE" ENV="$ENV" ENVIRONMENT="$ENVIRONMENT"
    export INVESTMENT_ADMIN_BACKEND_FULL_IMAGE_NAME
    ...
    auto_generate_yaml "$INVESTMENT_ADMIN_BACKEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$INVESTMENT_ADMIN_BACKEND_PVC_YAML" || return 1
