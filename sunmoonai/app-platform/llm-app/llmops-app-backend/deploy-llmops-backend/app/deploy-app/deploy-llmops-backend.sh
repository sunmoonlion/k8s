...
# LLMOps App BFF 部署脚本
# 用法: ./deploy-llmops-backend.sh <action> [project_id] [namespace] [environment] [-c|--cluster CLUSTER]
...
# 项目根目录（deploy-llmops-backend 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-llmops-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（llmops-app-backend 目录）
# 从 deploy-llmops-backend/ 向上 1 级到达 llmops-app-backend/
...
# 加载部署配置文件
LLMOPS_BFF_CONFIG_FILE="$SCRIPT_DIR/deploy-llmops-backend.conf"
...
# 默认配置从配置文件读取（deploy-llmops-backend.conf）
# 如果配置文件未设置，则使用空值（由函数参数默认值处理）
DEFAULT_PROJECT_ID="${LLMOPS_BFF_PROJECT_ID:-}"
DEFAULT_NAMESPACE="${LLMOPS_BFF_NAMESPACE:-}"
...
LLMOPS_BFF_IMAGE="${LLMOPS_BFF_IMAGE:-llmops-app-backend}"
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（llmops-app-backend/）
# app/deploy-app/ -> app/ -> deploy-llmops-backend/ -> llmops-app-backend/
RESOURCES_DIR="$APP_ROOT/resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
LLMOPS_BFF_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/llmops-app-backend-generated.yaml"
LLMOPS_BFF_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-backend-pvc/generate-llmops-backend-pvc/llmops-app-backend-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"llmops-app-backend-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"llmops-app-backend-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-backend-pvc/generate-llmops-backend-pvc/generate-llmops-backend-pvc.sh"
    else
    ...
    if [ -f "$generate_script" ]; then
        ...
        export NAMESPACE="${LLMOPS_BFF_NAMESPACE:-app-platform-dev}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${LLMOPS_BFF_PROJECT_ID:-sunmoonai}"
...
check_env_config() {
    # 自动生成 YAML 文件（如果不存在）
    if ! auto_generate_yaml "$LLMOPS_BFF_YAML" "$K8S_RESOURCE_DIR"; then
        exit 1
    fi
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/llmops-service-config/deploy-llmops-service-config/deploy-llmops-service-config.sh"
        local secret_script="$secrets_dir/llmops-service-secret/deploy-llmops-service-secret/deploy-llmops-service-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_web_api() {
    ...
    export LLMOPS_BFF_IMAGE_REGISTRY="${LLMOPS_BFF_IMAGE_REGISTRY:-$(get_cluster_harbor_registry)}"
    export LLMOPS_BFF_IMAGE_PROJECT="${LLMOPS_BFF_IMAGE_PROJECT:-k8s-images}"
    export LLMOPS_BFF_IMAGE="${LLMOPS_BFF_IMAGE:-llmops-app-backend}"
    ...
    export LLMOPS_BFF_FULL_IMAGE_NAME="$LLMOPS_BFF_FULL_IMAGE_NAME"
    ...
    if ! check_harbor_image_exists "$LLMOPS_BFF_FULL_IMAGE_NAME" "$NAMESPACE" "${LLMOPS_BFF_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"; then
        return 1
    fi
    ...
    # 部署 PVC（如果存在）
    if [ -f "$LLMOPS_BFF_PVC_YAML" ]; then
        log_info "部署 PVC..."
        kubectl apply -f "$LLMOPS_BFF_PVC_YAML" -n "$NAMESPACE"
        if [ $? -eq 0 ]; then
    ...
    echo ""
    log_info "检查部署状态:"
    echo "  kubectl get pods -n $NAMESPACE -l app=llmops-app-backend"
    echo "  kubectl get svc -n $NAMESPACE -l app=llmops-app-backend"
    echo "  kubectl get ingressroute -n $NAMESPACE"
    echo ""
    log_info "查看 Pod 日志:"
    echo "  kubectl logs -n $NAMESPACE -l app=llmops-app-backend -f"
}
...
uninstall_web_api() {
    ...
    if [ ! -f "$LLMOPS_BFF_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $LLMOPS_BFF_YAML，尝试自动生成..."
        if ! auto_generate_yaml "$LLMOPS_BFF_YAML" "$K8S_RESOURCE_DIR"; then
            log_warn "YAML 生成失败，尝试直接删除资源"
            kubectl delete deployment llmops-app-backend -n "$NAMESPACE" --ignore-not-found=true
            kubectl delete service llmops-app-backend-service -n "$NAMESPACE" --ignore-not-found=true
        fi
    fi
...
show_status() {
    ...
    echo ""
    echo "📦 Pods:"
    kubectl get pods -n "$NAMESPACE" -l app=llmops-app-backend 2>/dev/null || echo "  无 Pod 运行"
    echo ""
    echo "🌐 Services:"
    kubectl get svc -n "$NAMESPACE" -l app=llmops-app-backend 2>/dev/null || echo "  无 Service"
    echo ""
    echo "📋 ConfigMaps:"
    kubectl get configmap -n "$NAMESPACE" -l app=llmops-app-backend 2>/dev/null || echo "  无 ConfigMap"
    echo ""
    echo "🔐 Secrets:"
    kubectl get secret -n "$NAMESPACE" -l app=llmops-app-backend 2>/dev/null || echo "  无 Secret"
}
...
main() {
    ...
    case "$ACTION" in
        ...
        *)
            ...
            echo ""
            echo "  # 2. 部署服务"
            echo "  cd ../../k8s/sunmoonai/app-platform/business-apps/llmops-app/llmops-app-backend/deploy-llmops-backend"
            echo "  ./deploy-llmops-backend.sh deploy sunmoonai app-platform-dev development"
            exit 1
            ;;
    esac
}
