...
CONFIG_FILE="$SCRIPT_DIR/deploy-llmops-app-ssr-config.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llmops-app-ssr-config/ 向上 3 级到达应用根目录
# deploy-llmops-app-ssr-config/ -> llmops-app-ssr-config/ -> secrets/ -> deploy-llmops-ssr/ -> llmops-app-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
CONFIGMAP_YAML="$K8S_RESOURCE_DIR/custom-values/configMap/llmops-app-ssr-config/generate-llmops-app-ssr-config/llmops-app-ssr-config-generated.yaml"
...
main() {
  ...
  case "$action" in
    ...
    status)
      ...
      local configmap_name="${LLMOPS_SSR_CONFIGMAP_NAME:-llmops-app-ssr-config}"
