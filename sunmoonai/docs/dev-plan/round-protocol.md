...
> （失败原因见 §11）、2026-09-04 的 refact 轮（异议轮缺失被发现，见 §11）。
...
| 3 | 现在是第几环节 | 跑 §4 的判定命令，**从产物反推，不看任何人的声明** |
...
声明会和事实漂移。判据一律是 §4 的命令，对照 git 提交，不看工作区、不看谁说了什么。
...
2026-09-03 那次失败整合正是这么丢掉了基座候选独有的锚点（§11）。
...
`裁定` 取 `接受` / `部分接受` / `拒绝` 三值之一。§8 的验收方由这张表算出。
...
**验收方由 §7 的处置表算出，不由裁决方凭印象指定**，规则依次适用：
