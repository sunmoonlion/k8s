...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/llmops-bff-ingress/generate-ingress/llmops-app-bff-ingress-generated.yaml"
...
load_config() {
    ...
    # 主配置文件路径：从 deploy-ingress/ -> ingress/ -> deploy-llmops-bff/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-llmops-bff.conf"
...
deploy_http_route() {
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/llmops-bff-ingress/generate-ingress/generate-ingress.sh"
