...
// 获取所有 stage2 文章
const { data: allArticles } = await useAsyncData('all-stage2-articles', () =>
  queryContent('/stage2').find()
)
...
const { data: article } = await useAsyncData(`stage2-${urlSlug}`, () => {
  ...
  for (const item of allArticles.value) {
    ...
    const fileName = itemPath.replace(/^\/stage2\//, '').replace(/\.md$/, '')
...
.stage2-article :deep(h1) {
  font-size: 2rem;
  margin-bottom: 3rem !important; /* 标题和正文之间两个空行的间距 */
}

/* 针对包含标题的 div 也设置间距 */
.stage2-article :deep(div[style*="text-align: center"]) {
  margin-bottom: 3rem !important;
}

/* 段落首行缩进 2 个字符 - 符合中文排版惯例 */
.stage2-article :deep(p) {
  margin-bottom: 1rem;
  line-height: 1.6;
...
.stage2-article :deep(article > p:first-of-type),
.stage2-article :deep(article > div + p) {
  margin-top: 0;
}

/* 排除居中的标题段落不缩进 */
.stage2-article :deep(p[style*="text-align: center"]) {
  text-indent: 0;
}

/* 排除称呼类短句（包含"工会："等称呼的段落）不缩进 */
.stage2-article :deep(p:has(> span.no-indent)),
.stage2-article :deep(p.no-indent) {
  text-indent: 0;
}

/* 列表项也保持首行缩进 */
.stage2-article :deep(li) {
  text-indent: 2em;
}
</style>
