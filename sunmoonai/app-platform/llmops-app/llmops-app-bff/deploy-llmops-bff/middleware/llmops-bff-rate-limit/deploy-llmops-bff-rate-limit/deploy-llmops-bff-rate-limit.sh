...
CONFIG_FILE="$SCRIPT_DIR/deploy-llmops-bff-rate-limit.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-llmops-bff-rate-limit/ -> llmops-bff-rate-limit/ -> middleware/ -> deploy-llmops-bff/ -> llmops-app-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
MIDDLEWARE_YAML="$K8S_RESOURCE_DIR/custom-values/middleware/llmops-bff-rate-limit/generate-llmops-bff-rate-limit/llmops-app-bff-rate-limit-generated.yaml"
...
# 默认配置从配置文件读取（deploy-llmops-bff-rate-limit.conf）
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/middleware/llmops-bff-rate-limit/generate-llmops-bff-rate-limit/generate-llmops-bff-rate-limit.sh"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/middleware/llmops-bff-rate-limit/generate-llmops-bff-rate-limit/generate-llmops-bff-rate-limit.sh"
  if [ -f "$generate_script" ]; then
    if bash "$generate_script"; then
  ...
  case "$action" in
    ...
    status)
      ...
      if kubectl get middleware llmops-app-bff-rate-limit -n "$namespace" 2>/dev/null; then
