...
# =============================================================================
# ONLYOFFICE Redis Secret 部署脚本
# 文件名: deploy-onlyoffice-redis-secret.sh
# 用途: 部署 ONLYOFFICE Docs 连接 Redis 的认证 Secret
...
# 计算项目根目录（应用根目录）
# 从 deploy-onlyoffice-redis-secret/ 向上 3 级到达应用根目录
# deploy-onlyoffice-redis-secret/ -> onlyoffice-redis-secret/ -> secrets/ -> onlyoffice-docs-bff/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由 resources/custom-values/generate.sh 生成）
CUSTOM_VALUES_DIR="$PROJECT_ROOT/resources/custom-values"
SECRET_YAML="$CUSTOM_VALUES_DIR/onlyoffice-redis-secret-generated.yaml"
...
# 加载配置文件
CONFIG_FILE="$SCRIPT_DIR/deploy-onlyoffice-redis-secret.conf"
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        if [ -f "$custom_values_dir/generate.sh" ]; then
            ...
            if [[ -z "${REDIS_PASSWORD:-}" ]]; then
                log_error "Redis 密码未配置！"
                log_error "请在配置文件中设置 REDIS_PASSWORD"
...
main() {
    ...
    log_info "部署 ONLYOFFICE Redis Secret..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    case "$action" in
        deploy)
            ...
            if kubectl apply -f "$SECRET_YAML" -n "$namespace"; then
                log_success "Secret 已部署: ${SECRET_NAME:-onlyoffice-redis-secret} (命名空间: $namespace)"
            else
        ...
        status)
            ...
            local secret_name="${SECRET_NAME:-onlyoffice-redis-secret}"
    ...
    echo ""
    log_success "ONLYOFFICE Redis Secret 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
    log_info "  - Secret 名称: ${SECRET_NAME:-onlyoffice-redis-secret}"
    log_info "  - 命名空间: $namespace"
