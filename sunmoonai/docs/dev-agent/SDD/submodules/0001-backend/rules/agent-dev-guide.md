...
——最后四项是 [§3.6](../SDD/submodules/0002-agent-execution/rules/agent-dev-guide.md) 命名空间纪律在工单上的落点。
...
**这三个字段不是描述性说明，是 [§3.10](../SDD/submodules/0002-agent-execution/rules/agent-dev-guide.md) 写入前门禁的判定输入——派工时缺任一字段，
执行者不得开始写。**
...
产物命名、候选冻结、处置表和验收方算法见 [§3.19](../../../../../dev-human/protocol/competition-operations.md)–[§3.22](../SDD/submodules/0002-agent-execution/rules/agent-dev-guide.md)，来源为现行 competition-protocol。状态脚本从 commit 反推，工作区
...
但**必须可数**，因为成本上界（[§6.2](../SDD/submodules/0001-intake/rules/agent-dev-guide.md)）和绕过口径（[§6.3](../SDD/submodules/0001-intake/rules/agent-dev-guide.md)）都要数它。
...
这是 [§12](../SDD/submodules/0004-acceptance-commit/rules/agent-dev-guide.md)「拿休眠代码当能力证据」那条失败的词汇层防线；配套的可运行验证动作是
**回跑 dormant 测试**并同时确认「锚点仍在」与「能力仍未接线」两个方向（[§2.6](../SDD/submodules/0002-agent-execution/rules/agent-dev-guide.md)）。
...
| # | 独立观察 | 结论与落点 |
| --- | --- | --- |
| K1 | `rg 'interrupt\(|Command\(resume=' investment-backend/app/app`；实际锚见 [§4.3](../SDD/submodules/0003-interrupt-resume/rules/agent-dev-guide.md) | 出向和入向原语都接受业务值；删去平台自造的固定修订 schema，直接绑定产品 Interaction |
...
| 人自己写下一版就必须换 Attempt | **不采用这个触发条件**。人的内容归属保留；执行是否延续按 [§4.3](../SDD/submodules/0003-interrupt-resume/rules/agent-dev-guide.md) checkpoint 和终态判，不按作者类型判 |
