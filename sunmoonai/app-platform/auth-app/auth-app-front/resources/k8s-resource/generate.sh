...
SECRET_CONFIG="../../deploy-auth-app-front/secrets/auth-app-front-secret/deploy-auth-app-front-secret/deploy-auth-app-front-secret.conf"
...
export AUTH_APP_SSR_IMAGE_PULL_SECRET_NAME="${AUTH_APP_SSR_IMAGE_PULL_SECRET_NAME:-harbor-registry-secret}"
export AUTH_APP_SSR_UNIFIED_HOST="${AUTH_APP_SSR_UNIFIED_HOST:-auth-app-front.sunmoonai.com}"
...
HARBOR_DOCKER_SERVER="${HARBOR_DOCKER_SERVER:-${AUTH_APP_SSR_IMAGE_REGISTRY:-harbor.sunmoonai.com:30443}}"
...
export SERVICE_NAME="${SERVICE_NAME:-auth-app-front}"
export SERVICE_PORT="${SERVICE_PORT:-3000}"
export UNIFIED_HOST="${UNIFIED_HOST:-${AUTH_APP_SSR_UNIFIED_HOST}}"
export NODE_IP="${NODE_IP:-${AUTH_APP_SSR_NODE_IP}}"

# PVC 相关变量（用于模板替换，从部署配置文件中读取）
# 注意：这些变量的默认值在 deploy-auth-app-front.conf 中配置
export PVC_NAME="${PVC_NAME:-auth-app-front-pvc}"
