...
# 项目根目录（deploy-celeryworker-incubator 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-celeryworker-incubator/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（celeryworker-incubator 目录）
# 从 deploy-celeryworker-incubator/ 向上 1 级到达 celeryworker-incubator/
...
# 默认配置从配置文件读取（deploy-celeryworker-incubator.conf）
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（celeryworker-incubator/）
# app/deploy-app/ -> app/ -> deploy-celeryworker-incubator/ -> celeryworker-incubator/
RESOURCES_DIR="$APP_ROOT/resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
CELERYWORKER_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/celeryworker-incubator-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"celeryworker-incubator-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
...
uninstall_celeryworker() {
    ...
    if [ ! -f "$CELERYWORKER_YAML" ]; then
        log_warn "生成的 YAML 文件不存在: $CELERYWORKER_YAML，尝试自动生成..."
        if ! auto_generate_yaml "$CELERYWORKER_YAML" "$K8S_RESOURCE_DIR"; then
