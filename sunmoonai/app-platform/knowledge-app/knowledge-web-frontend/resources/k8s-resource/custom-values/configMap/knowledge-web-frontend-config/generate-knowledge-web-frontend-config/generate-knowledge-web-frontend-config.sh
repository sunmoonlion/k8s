#!/bin/bash
# knowledge-web-frontend ConfigMap YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-knowledge-web-frontend-config.conf"
# 从 generate-knowledge-web-frontend-config/ -> knowledge-web-frontend-config/ -> configMap/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-knowledge-web-frontend-config/ -> ... -> k8s-resource/ -> resources/ -> knowledge-web-frontend/
...
main() {
    log_info "开始生成 knowledge-web-frontend ConfigMap YAML 文件..."
