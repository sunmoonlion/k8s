...
# 显示帮助信息
show_help() {
    echo "Celery Worker Incubator Secrets All 部署脚本"
    echo ""
    echo "用法:"
...
main() {
    ...
    log_info "🚀 开始部署 Celery Worker Incubator Secrets..."
    log_info "📋 部署参数："
    log_info "  - 项目ID: $project_id"
    log_info "  - 命名空间: $namespace"
    log_info "  - 环境: $environment"
    ...
    if deploy_secrets "$action" "$project_id" "$namespace" "$environment"; then
        log_success "✅ Celery Worker Incubator Secrets 部署完成！"
        echo ""
        log_info "📋 部署信息："
        log_info "  - 项目ID: $project_id"
        log_info "  - 命名空间: $namespace"
        ...
        log_error "❌ Celery Worker Incubator Secrets 部署失败"
        return 1
    fi
}
