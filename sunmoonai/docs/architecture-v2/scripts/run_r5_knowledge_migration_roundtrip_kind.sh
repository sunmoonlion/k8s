...
RESTORE_DATABASE="${RESTORE_DATABASE:-knowledge_r5_restore_roundtrip_20260811}"
SOURCE_MIGRATION_SECRET="${SOURCE_MIGRATION_SECRET:-knowledge-admin-backend-migration-postgresql-conn}"
ROUNDTRIP_SECRET="r5-knowledge-migration-roundtrip-db"
IMAGE_PULL_SECRET="${IMAGE_PULL_SECRET:-harbor-registry-secret}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORK_DIR="$(mktemp -d /tmp/r5-knowledge-roundtrip.XXXXXX)"
...
cleanup() {
  ...
  kubectl_cmd delete job -n "$APP_NAMESPACE" \
    -l architecture.sunmoonai.com/r5-knowledge-roundtrip=true \
...
run_migration_job() {
  local suffix="$1"
  shift
  local job_name="r5-knowledge-migration-$suffix"
  local command_json
  ...
  jq -n \
    ...
    '{
      ...
      metadata: {
        namespace: $namespace,
        name: $name,
        labels: {"architecture.sunmoonai.com/r5-knowledge-roundtrip": "true"}
      },
      spec: {
        backoffLimit: 0,
        template: {
...
KEEP_RESTORE_DATABASE=true \
  ...
  "$SCRIPT_DIR/restore_r5_knowledge_database_kind.sh" \
...
"$SCRIPT_DIR/audit_r5_knowledge_database_kind.sh" "$RESTORE_DATABASE" \
  >"$WORK_DIR/after-upgrade.txt"

run_migration_job downgrade \
  /app/.venv/bin/alembic -c /app/alembic.ini downgrade 20260715_0003 \
...
test "$(extract_section_json "$WORK_DIR/after-upgrade.txt" alembic_heads)" = '["20260808_0004"]'
test "$(extract_section_json "$WORK_DIR/after-downgrade.txt" alembic_heads)" = '["20260715_0003"]'
test "$(extract_section_json "$WORK_DIR/after-reupgrade.txt" alembic_heads)" = '["20260808_0004"]'

for phase in after-upgrade after-downgrade after-reupgrade; do
  invariants="$(extract_section_json "$WORK_DIR/$phase.txt" knowledge_invariants)"
  test "$(printf '%s' "$invariants" | jq '[.[]] | add')" = "0"
done

echo "section=result"
jq -n \
  --arg task "R5-KNOWLEDGE-MIGRATION-ROUNDTRIP" \
  --arg result "passed" \
  --arg image "$IMAGE" \
  ...
  '{
    ...
    heads: {
      baseline: "20260715_0003",
      upgraded: "20260808_0004",
      downgraded: "20260715_0003",
      reupgraded: "20260808_0004"
    },
    counts: {
...
jq -n \
  ...
  --arg jobs "$(kubectl_cmd get job -n "$APP_NAMESPACE" -l architecture.sunmoonai.com/r5-knowledge-roundtrip=true -o name)" \
  '{
