...
> 依据的通用规范：[多方竞争协议](../competition.md)

> **本文定义一次多方竞争「多家并行出稿 → 互评 → 裁决 → 异议 → 验收 → 确认 → 清理」怎么走。**
> **它是本项目多助手协作的唯一一套流程。**协作的三种形态是同一套流程的三组参数取值，见「一套流程，靠参数覆盖三种协作形态」。
>
...
**那句话的逐字真源只有一处**——[`GO.md`](GO.md) 开头那一句；本节不复制第二份，
因为两份固定指令就是两个入口，谁也保证不了它们一致。

收到它的第一个动作**不是干活，是定位**。按顺序回答四个问题，答不出就停下问人：

| # | 问题 | 怎么答 |
| --- | --- | --- |
| 1 | 我是谁 | 工作目录是 `W`，路径里的 `<X>` 就是你的名字。**若在 `M`，立即停下问人** |
| 2 | 这件事走哪一档 | 见「流程档位」；T0/T1 不走七环节，读到这里就够了 |
| 3 | 现在是哪一次竞争 | 所属任务目录里 `status: ACTIVE` 的那一次（见「本次定义」）。查到零个或多个都**停下问人**，不要自己挑 |
| 4 | 现在是第几环节 | 按「环节判定」那节的判据，**从产物反推，不看任何人的声明** |
| 5 | 我这一环节做什么 | 读本次竞争目录里的 `call-<当前环节>.md` 里**属于你的那一节**；它自足，不需要额外指令 |

⚠ **第 3 问靠声明、第 4 问靠反推，这不是自相矛盾——两问要的东西不是一类。**
第 3 问定的是**范围**：哪一场竞争，由人写定并冻结，本来就是题目的一部分。
第 4 问定的是**状态**：走到第几环节，它会被人误报，所以只能从产物推。
范围靠声明、状态靠反推，各就各位；
真出现 `status` 与产物对不上，按「停止、超时与回退」那节办——**显式写出，不许靠改 `status` 让它消失**。
...
⚠ **参数变了，不能省的三条不变**（「任何档位都不能省的三条」）。跳过环节要在 `round.md` 明写 `skip_stages`，
**留空字段不算数**（「环节判定：命令即判据」）。
...
| 字段 | 内容 |
| --- | --- |
| `status` | `ACTIVE` / `DONE` / `ABORTED`。**一个任务目录下同时只允许一个 `ACTIVE`**；不相关的任务可以并行竞争。⚠ 另一条硬约束：**同一参与方同时只能在一场竞争里**——否则它答不出「现在是哪一次竞争」 |
...
| 形式判据 | 逐条编号，随工单发。**冻结后不得为了让产出通过而修改** |
| 内容判据 | 逐条编号，**与工单同时冻结并记 SHA-256，密封到 ⑤ 才公开**；PRD/SDD 两段密封，IMP/UAT 两段随工单发（见「工单发什么、不发什么」）。同样不得为了让产出通过而修改 |
| 冻结区 | 不许改写的小节清单（如有） |
| 背景与题目细则 | 可另拆 `task.md` 同目录放，`round.md` 只留指针 |

**不变的流程规则不写进 `round.md`**——开工须知、隔离纪律、枚举形状、产物命名、
角色规则、冻结方式全在本文。用户消息重复它们只会产生第二份会漂移的真源。

## 6. 产物、路径与命名

下表的 `R` = 本次竞争的目录。本协议另用三个名字指目录，**换机器只改「本机取值」那一栏**：

| 名字 | 是什么 | 谁能写 | 本机取值 |
| --- | --- | --- | --- |
| `M` | 主线检出：`master` 所在的那个目录 | 竞争期间谁都不写 | `~/master/k8s` |
| `W` | 你的工作目录 | 只有你 | `~/worktrees/<你的名>/k8s` |
| `V` | 临时检视面的落脚处（见「检视面」） | 人 | `~/review/<分支名>` |

⚠ **「本机取值」是这台机器的布局，不是协议的一部分。**换机器时向所有者要这三个值，
其余各节一律按名字读。
...
否则「④ 异议：对整合权的唯一制衡」就是空的——被制衡方在写制衡规则。
实跑中撞上过这一条。

⚠ **「组织者」不是一个可以填的字段，是一个推导值。**原先有脚本按本节规则推导并打印这一行，
脚本已删除，**现在本节就是唯一的判据**：组织者自己按上面两条推导，把结论写进通知首行，
写明依据的是哪一条：

```text
本环节组织者：owner   （依「环节通知：组织者的产物，不是发起人的话术」——遇裁决方即参赛方，回落发起人）
```

**这一行缺失或与本节不符的通知，按未发出处理。**

通知必须自足，含四样：

| 必含 | 说明 |
| --- | --- |
| 取件命令 | 本环节要读的对象怎么取。**必须给完整 40 位 commit OID**；分支只用于发现，不作为冻结对象——`git rev-parse <分支>` 与通知里的 OID 不一致时按通知的 OID 取，并报给组织者 |
...
```bash
git worktree add $V <分支名>    # 开（$V 见「产物、路径与命名」，本机是 ~/review/<分支名>）
git worktree remove $V           # 用完删；分支与提交都还在
```
...
「判据自身的质量」管的是机器判据的覆盖范围；这一节管的是**人写验收判据**时怎么不作弊。
...
4. 判据**不得超出用户消息已向参与方声明的范围**。
   **凡判据有、用户消息没有的要求，一律不得用于扣分，只可用于加分。**

#### 难点清单：记下来是为了检验发起方

事先记下这道题难在哪，**不给解法**。它的用途不是评分，是反查：

> 参与方**都没意识到**某个难点 → 责任在用户消息交代不清，**不扣候选的分**，
> 记为发起方的改进项。
...
事先没写的要求不得用来惩罚。这与「方向不对称」（「方向不对称：这是本协议的统一原则」）是同一条原则的两处应用。
...
#### 各家应当用不同的模型或不同的 provider

**同一个模型跑 N 遍,共同盲区完全一致,N 份候选高度相关**——钱花了 N 份,信息量接近一份。
只能用同一模型时,**必须如实声明「独立性降级」**,并在评优时按降级对待。

⚠ **隔离强度与独立性是竞争特有的两项采信维度。**
[UAT 怎么验](../uat/verify-rules.md)「证据与采信」说采信取各项中**最低**的一项;
在竞争里要把这两项一并计入——**隔离没保证、或各家不独立,整份证据就按最低那一项算**。

隔离若只靠纪律、没有机制保证,**必须如实声明为已知缺口**(本协议当前就是这样,见 9.2 末尾的 ⚠)。
覆盖不全的隔离机制比没有更危险:它会让人以为这次竞争是隔离的。
...
| --- | --- |
| 原始请求 | 所有者原话，一字不改 |
| **形式判据** | 交什么、要回答哪几个问题、交付物的形状与格式。**它不说什么答案算对** |
| 只读输入 | 指向入口文档，让它自己按需深入；**列全**——漏列的材料不得事后用来扣分（「立判据的人怎么约束自己」） |
| 边界 | 含什么 / 不含什么 / 不含的归谁 |

**验收标准分两类，发不发不一样。**两类都**先于产出冻结**（「任何档位都不能省的三条」），
区别只在什么时候发出去：

| 类 | 管什么 | 什么时候发 | 能拿来干什么 |
| --- | --- | --- | --- |
| **形式判据** | 交什么、要回答哪几个问题、边界、格式 | **随工单发** | 判通过与不通过 |
| **内容判据** | **什么答案算对** | **密封到 ⑤ 验收才公开** | 只能用于**排序**；⚠ **不得用来判不通过** |

| 段 | 形式判据 | 内容判据 |
| --- | --- | --- |
| **PRD** / **SDD** | 随工单发 | **密封**——写成条目就把答案空间提前封死，「其实不需要这个机制」这类答案再也交不上来 |
| **IMP** / **UAT** | 随工单发 | **随工单发**：IMP 必须写完成条件，执行者不知道做到哪算完就只能猜；UAT 照抄被验 turn 已冻结的完成条件，不另立、不改 |

⚠ **判不通过只能依据已发出的形式判据。**密封的内容判据在 ⑤ 公开后用于各家之间排序，
不用于否决——这与「没发出去的判据只能用于加分，不得用于扣分」是同一条
（见 [turn 的投影](../turn-project.md)「用户消息写多严」）。
密封判据必须与工单同时冻结并记下 SHA-256，⑤ 公开时可验它没被改过。

| **不给** | 为什么 |
| --- | --- |
| 其他家的候选 | 破坏隔离（「隔离为什么是硬要求」） |
...
⚠ **本机的任何判据都证明不了「这是人做的」。**`author` 可随意填（见「代提交必须用 `--author`」
那节的 ⚠）；签名私钥、`ssh-agent`、git 二进制、检查脚本本身，**对有免密 sudo 或 docker 的 agent 全都可改写**
（[`check-no-owner-creds.sh`](../../tools/check-no-owner-creds.sh) 文件头已把这件事写明）。
**唯一 agent 够不着的位置是托管方的凭据作用域与分支保护。**

| 项 | 要求 |
| --- | --- |
| **判据** | ⑥ 确认 = **主线上出现由所有者在托管方界面完成的 merge 或 tag**。`git log master` 能看到它，但它的可信来源是托管方的操作记录，不是提交里的字段 |
| **写主线的权限** | master 设分支保护：agent 的凭据**只能推自己的分支**，推不了 master。参与方交卷仍按「取件：一律按 commit」，不受影响 |
| **所有者的凭据** | **不能是本机上无口令的私钥**——那等于把写主线的权限交给本机任何进程。加口令、换硬件 key，或只在所有者自己另一台设备上持有 |
| **agent 的动作** | 推自己的分支、开 PR、把 PR 链接写进环节通知。**不合并、不打 tag、不推主线** |

⚠ **这三条不齐，⑥ 就只是纪律。**本仓当前实测（2026-09-22）：
`~/.ssh/id_rsa` **无口令**、免密 sudo 可用、docker 可用——
也就是说**任何在这台机器上跑的 agent 现在都能用所有者的身份推主线**。
`check-no-owner-creds.sh` 的判据 ⓪（「remote 应当拒绝本机身份的 push」）当前**不通过**。
**这是一笔欠账，在分支保护与凭据两件事做完之前，⑥ 的不可伪造性不成立，不得宣称它已落成。**

#### 已记录的例外：产品开发期间，全部 agent 直接推送；合并主线由所有者手动指示

| 项 | 内容 |
| --- | --- |
| **裁定** | 所有者授权（2026-08 口头，2026-09-23 改定）：**整个产品开发期间**，托管方的分支保护与凭据分离暂不执行。**适用于全部 agent，不只某一个**：各 agent 用本机现有身份直接推自己的分支，无须逐次请示；**合并进 `master` 仍由所有者逐次手动指示**，agent 收到指示后代做。产品开发本身照规范走，本条只放宽推送这一个动作 |
| **方向** | **朝省事**——因此按「方向不对称」必须有人确认并记录，两样都有 |
| **理由** | 上面三条前置在托管方，本机做不完；开发期间只有所有者一个人在合，逐条请示推送的成本大于风险；合并这一步保留人的指示，就把「决定它可以上主线」留在了人手里 |
| **边界** | 只限本项目的产品开发期间，不得延用到有外部参与方的竞争，也不得据此推断其他不可逆动作（发布、删远端资产、打 tag）可代做。**没有所有者的指示不合并、不推 `master`**；不改写别人的分支；推送前照旧过门禁 |
| **不变的部分** | 定稿、批准、验收仍是所有者的动作：这条例外只覆盖「把已定稿的东西搬上主线」，不覆盖「决定它可以上主线」。⑥ 的不可伪造性在此期间**不成立**，文档不得宣称已落成 |
| **失效条件** | 分支保护与凭据两件事做完之日；届时删除本条，回到上表的规则。产品上线前必须做完（三条前置本来就是控制面的要求） |

⚠ **这条例外本身就是上面那笔欠账的利息**：正因为强制点还没设，才需要靠一条口头授权顶着。
记在这里是为了**它不会悄悄变成惯例**——下一次竞争开始时，如果这一行还在而前置仍没做完，
那说明欠账在滚，不是在还。
...
因此凡由产出方向审核方交材料的场合（「一套流程，靠参数覆盖三种协作形态」表里第二行的定向审核、以及任何一次 ⑤ 验收），
拆成两阶段：
...
把「经过检视」写成「已验证」，就是「判据自身的质量」那张表的下一行。
...
⚠ **起步阶段不能用 `W`，那里有死锁。**`W` 要等半数交付才算得出，
而「未启动」判据又要用 `W`——若半数参与方都没起来，就永远拿不到 `W`，
也就永远判不了「它们没起来」。**`W` 只用于已有首批样本之后的动态延期。**
首批样本之前用**独立的起步上界 `W0`**，它不是时钟数，判据同样可观测：

| 阶段 | 用什么 | 判据 |
| --- | --- | --- |
| **起步**（`W` 还算不出来） | `W0` | 通知发出后，该家**进程存在、CPU 时间取两次差有增长、输出字节数有增长**，三条至少一条成立即「已起步」。三条全部为零且持续到**第一家交付**为止 → 判「未启动」，不必等 `W` |
| **起步后**（已有 ≥1 家交付） | `W` | 按上表三条判据 |

⚠ **`W0` 要观测的是哪个进程，必须在通知里点名。**GUI agent 的窗口进程、
渲染进程与真正跑推理的子进程不是一个；写「看 CPU」而不写看谁的 CPU，判据落不了地。
观测不到进程身份时判 `undecidable` 交人，不判「未启动」——
那是把「没查到」写成「没有」（本节下面第三条 ⚠ 已就同一件事点名过）。
...
单个执行者的纪律见 [IMP 规则](../imp/message-rules.md);下面这些**只在多家并行时成立**。

**三个路径不是一个**:

| 路径 | 在哪 | 谁写 |
| --- | --- | --- |
| **候选路径** | 各自 worktree 内的共享最终路径同名文件 | 候选作者,只提交自己那份 |
| **发布路径** | 整合者 worktree 内 | **唯一的整合者** |
| **发布引用** | 最终提交或发布版本 | —— |

⚠ **人指定的最终文件名是发布目标,不是所有候选都可以同时写的路径。**

顺序:**私有草稿 → 候选 → 冻结提交 → 选定 → 整合者 worktree → 最终提交 → 发布面**。
N 名写者建 N 个 worktree,另给整合者一个;只读者只取件,不建 worktree。

**整合前显式核对**:候选状态仍有效、提交仍可达、提出方未撤回、基线未失效。
**核对对象是提交与候选状态,不是分支**——分支是会被快进、重置、强推的指针。

**发布必须带预期版本**(比较并交换):目标已变化则发布失败,新建一次整合尝试重来。
**不得以强推、无条件覆盖或先删后写绕过竞争**;发布重试用稳定的幂等键。
人明确只要多个可比较草案时,发布目标是**候选集合及其清单**,不是单一文件。

**用足以产生独立信号的最少候选**:顶层验收满足后,其余在途候选停止,或降为无副作用的只读参考。
⚠ **完整交叉阅读是二次复杂度**——候选多时平衡分配,但评审覆盖的范围必须相同;
无法运行命令的评审必须标「未经事实验证」。

**冲突怎么办**:迟到候选只读保留;两个单元改同一文件交整合者;
合并冲突由整合者解析并记依据,**不按时间自动取新**;删除与修改的冲突交整合者裁决。
整合**从选定的提交取内容**,不拷任何人未提交的文件。
...
| 候选读取其他候选,或偷看发起方倾向 | 独立信号退化成改写 |
| 作者自评自收、候选人互投 | 同一判断链为缺陷背书,独立性是假的 |
| 多数票覆盖失败的测试,或全体一致的共同盲区 | 偏好压过事实;**一致不等于正确** |
...
⚠ **重置之前必须先给每个要保留的提交挂上持久 ref，并推到远端。**上表写「git 历史可取」，
但分支一旦重置，没有 tag、没有归档 ref、不在主线可达范围内的提交就**不再被任何持久 ref 引用**——
reflog 与悬空对象不是归档，它们只在这一台机器上、且会被 gc 回收。
实跑中已经踩到：`refact` 那次的②③④⑤产物与 `attempt1` 的两份候选，
共 7 个标签**只存在于本机磁盘**，两个远端都没有，从 `origin/master` 也走不到。

清理前逐条过这一遍，**任一条不通过就不许重置**：

```bash
# 1 每个要留的提交都有 tag
git tag <竞争名>/<环节>-<名> <完整OID>
# 2 tag 已推到全部远端（本仓 origin 配了双 push URL）
git push origin --tags
# 3 反查：本地有而远端没有的 tag 必须为空
comm -23 <(git tag -l | sort) \
        <(git ls-remote --tags origin | awk '{print $2}' | sed 's#refs/tags/##; s#\^{}##' | sort -u)
# 4 从全新 clone 实测能按记录重取
git clone --mirror <远端> /tmp/verify && git -C /tmp/verify cat-file -e <完整OID>
```

第 3 步输出非空、或第 4 步取不到，就是**证据已经失去长期可达性**，按未归档处理。
...
- **用户消息冻结后不得中途修改**。已开工的执行者按旧版做，改了会让已完成的部分对不上；
  发现用户消息有错，记进处置记录，在下一次竞争时修。
- **引用本协议按标题，不按章节号。**章节会移动，标题不会。本仓已有同源约定：
  [词汇表](../glossary.md)「编号的归属」要求「引用时写编号本身即可，不带章节号——
  章节会移动，编号不会」；没有 ID 的段落用「文件 + 小节名」引用。
