...
# 从 deploy-ingress/ -> auth-app-backend-ingress/ -> ingress/ -> deploy-auth-app-backend/ -> auth-app-backend/
APP_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
RESOURCES_DIR="${APP_ROOT}/resources"
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"

# 导入统一部署模板（建立远程 k8s 连接）
# 从应用根目录（auth-app-backend/）到 k8s/ 需要向上 4 级
# auth-app-backend/ -> auth-app/ -> sunmoonai/ -> k8s/
...
INGRESS_FILE="${K8S_RESOURCE_DIR}/custom-values/ingress/auth-app-backend-ingress/generate-ingress/auth-app-backend-ingress-generated.yaml"
...
# 验证 Auth App BFF 服务是否存在（警告模式，不阻止部署）
verify_service() {
    local namespace="$1"
    ...
    if kubectl get service -n "$namespace" "$service_name" >/dev/null 2>&1; then
        log_success "✅ Auth App BFF 服务存在: $service_name"
        return 0
    else
        if [[ "$warn_only" == "true" ]]; then
            log_warning "⚠️ Auth App BFF 服务不存在: $service_name（将在服务部署后自动生效）"
            return 0  # 返回成功，允许继续部署
        else
            log_error "❌ Auth App BFF 服务不存在: $service_name"
            return 1
        fi
    fi
}
...
# 加载配置
load_config() {
    # 尝试加载主配置文件（如果存在），以获取 AUTH_APP_BFF_NAMESPACE、AUTH_APP_BFF_UNIFIED_HOST 等环境变量
    # 主配置文件路径：从 deploy-ingress/ -> auth-app-backend-ingress/ -> ingress/ -> deploy-auth-app-backend/ -> app/deploy-app/
    local main_config_file="$(cd "$SCRIPT_DIR/../../../app/deploy-app" && pwd)/deploy-auth-app-backend.conf"
    ...
    SERVICE_NAME="auth-app-backend"
    NAMESPACE="${NAMESPACE:-${AUTH_APP_BFF_NAMESPACE:-app-platform-dev}}"
    UNIFIED_HOST="${AUTH_APP_BFF_UNIFIED_HOST:-}"
    NODE_IP="${AUTH_APP_BFF_NODE_IP:-}"
    EXTERNAL_PORT="${AUTH_APP_BFF_EXTERNAL_PORT:-}"
...
deploy_http_route() {
    ...
    log_info "部署 Auth App BFF HTTP 路由..."
    ...
    # 从主配置文件构建配置（所有默认值应在配置文件中定义）
    SERVICE_NAME="auth-app-backend"
    NAMESPACE="${AUTH_APP_BFF_NAMESPACE:-}"
    UNIFIED_HOST="${AUTH_APP_BFF_UNIFIED_HOST:-}"
    NODE_IP="${AUTH_APP_BFF_NODE_IP:-}"
    ...
    if [[ ! -f "$INGRESS_FILE" ]]; then
        ...
        # 注意：这些变量应该从部署脚本的参数或配置中获取，不设置硬编码默认值
        export NAMESPACE="${NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-}"
        export ENV="${ENV:-}"
        
        local generate_script="$K8S_RESOURCE_DIR/custom-values/ingress/auth-app-backend-ingress/generate-ingress/generate-ingress.sh"
    ...
    verify_service "$namespace" "$SERVICE_NAME" "true" || {
        log_warning "⚠️ Auth App BFF 服务尚未创建，Ingress 将在服务部署后自动生效"
    }
    ...
    log_info "应用 Auth App BFF HTTP 路由配置..."
    if kubectl apply -f "$INGRESS_FILE" -n "$namespace"; then
        log_success "✅ Auth App BFF HTTP 路由配置应用成功"
    else
        log_error "❌ Auth App BFF HTTP 路由配置应用失败"
        return 1
    fi
    
    # 检查路由状态
    log_info "检查 Auth App BFF HTTP 路由状态..."
    kubectl get ingressroute -n "$namespace" auth-app-backend-ingress 2>/dev/null || log_warn "IngressRoute 尚未创建"
    kubectl get middleware -n "$namespace" auth-app-backend-stripprefix 2>/dev/null || log_warn "Middleware 尚未创建"
    
    log_success "✅ Auth App BFF HTTP 路由部署完成！"
    return 0
}

# 检查部署状态
check_deployment_status() {
    log_info "检查 Auth App BFF HTTP 路由部署状态..."
    
    echo ""
    echo "=== Auth App BFF HTTP 路由状态 ==="
    kubectl get ingressroute -n "$NAMESPACE" -l component=auth-app-backend 2>/dev/null || echo "IngressRoute 不存在"
    
    echo ""
    echo "=== Middleware 状态 ==="
    kubectl get middleware -n "$NAMESPACE" -l component=auth-app-backend 2>/dev/null || echo "Middleware 不存在"
    
    echo ""
    echo "=== Auth App BFF 服务状态 ==="
    kubectl get svc "$SERVICE_NAME" -n "$NAMESPACE" 2>/dev/null || echo "Auth App BFF 服务不存在"
...
# 删除 HTTP 路由
delete_http_route() {
    # 命名空间从配置文件或参数获取，不设置硬编码默认值
    local namespace="${1:-}"
    
    log_info "删除 Auth App BFF HTTP 路由..."
    log_info "命名空间: $namespace"
    ...
    if kubectl delete -f "$INGRESS_FILE" -n "$namespace" 2>/dev/null; then
        log_success "✅ Auth App BFF HTTP 路由配置删除成功"
    else
        log_warn "⚠️ Auth App BFF HTTP 路由配置不存在或删除失败"
    fi
    log_success "✅ Auth App BFF HTTP 路由删除完成！"
    return 0
}

# 显示帮助信息
show_help() {
    echo "Auth App BFF HTTP 路由部署脚本"
    echo ""
    echo "用法: $0 [命令] [项目ID] [命名空间] [环境]"
    echo ""
    echo "命令:"
    echo "  deploy <project_id> [namespace] [environment]    部署 Auth App BFF HTTP 路由"
    echo "  uninstall <project_id> [namespace] [environment] 卸载 Auth App BFF HTTP 路由"
    echo "  status    检查部署状态"
    echo "  help      显示帮助信息"
    echo ""
    echo "参数:"
    echo "  project_id   项目标识符 (必需，从配置文件读取)"
    echo "  namespace    命名空间 (从配置文件读取)"
    echo "  environment  环境 (从配置文件读取)"
    echo ""
    echo "示例:"
    ...
    case "$action" in
        deploy)
            if [[ -z "$project_id" ]]; then
                ...
                echo "注意：参数可从配置文件读取，详见配置文件说明"
                exit 1
            fi
            
            log_info "开始部署 Auth App BFF HTTP 路由..."
        ...
        uninstall)
            ...
            log_info "开始删除 Auth App BFF HTTP 路由..."
            log_info "项目: $project_id"
            log_info "命名空间: $namespace"
            log_info "环境: $environment"
