...
# 计算项目根目录（应用根目录）
# 从 deploy-tools-admin-backend-secret/ 向上 3 级到达应用根目录
# deploy-tools-admin-backend-secret/ -> tools-admin-backend-secret/ -> secret/ -> deploy-tools-admin-backend/ -> tools-admin-backend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/tools-admin-backend-secret/generate-tools-admin-backend-secret/generate-tools-admin-backend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${TOOLS_ADMIN_BACKEND_SECRET_NAME:-tools-admin-backend-secret}"
