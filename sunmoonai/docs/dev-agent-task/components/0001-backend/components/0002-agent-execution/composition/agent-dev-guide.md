...
⚠ **注意这条与 [§4.3](../../0003-interrupt-resume/composition/agent-dev-guide.md) 的关系**：`dev.change` 走的是 LangGraph checkpointer，**原地 resume 可用**
（[§4.3](../../0003-interrupt-resume/composition/agent-dev-guide.md) 已实测）。本节说的是 Harness 腿——**两条腿的恢复语义不同，不可互相外推**。
...
⚠ **「已派工」「全部返回」「多数一致」都不等于完成**（[§3.12](../../0004-acceptance-commit/composition/agent-dev-guide.md)、§11）。
...
- **「不能倒退的输入」必须写下来**——已经定过的事下次不重新讨论，写进定稿（[`composition/`](../../../../../composition/README.md)），派工时写进 turn 的 `user-message.md`；
