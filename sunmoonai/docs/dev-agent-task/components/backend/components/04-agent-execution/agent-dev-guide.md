...
- 当前推进哪件事、卡在哪、下一动作，由 [`log.md`](../../../../log.md) 推出，不落在脑子里；
- **「不能倒退的输入」必须写下来**——已经定过的事下次不重新讨论，写进 [`task.md`](../../../../task.md) 的「约束」；
- 停下来之前，先让 `log.md` 与 `task.md` 能回答：「接手的人一分钟内要知道什么？」

⚠ **交接投影不是第二真源**，不得在 `log.md` 里复制整张覆盖矩阵。
checkpoint 是恢复输入，不是第二份代码真源，**也不保存凭据**。

⚠ **`log.md` 是单写者面。**多名执行者并行时只有派工的一方（协调者）追加，各执行者的进度写在
**自己的 worktree** 里，由协调者记上去。判据不是「内容重不重要」，而是
「同一事实只能有一个权威写入者」（`I13`，[§1.2](../../composition/agent-dev-guide.md)）——**多人各自往同一份 `log.md` 追加，
就是在共享路径上并发写**。
...
| 每个执行者都写共享 `log.md` | 单写者面被互相覆盖，派工记录失真 |
