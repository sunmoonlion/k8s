...
>
> 规则来自实跑：生命周期文档重写（完整流程）、一次失败整合
> （失败原因见「角色不分会怎样」）、refact 竞争（异议环节缺失被发现，同节）。
...
实测：已发布的 `runtime` 竞争被自己的判据报成「当前环节 ①，缺五家」。
...
| `protocol/competition-status.py --verify` 首版 | 锚点正则只认 `~/repo/` 全路径，短路径形式的一大类**一处未验**，却报「17 处全部可达」 |
| refact 竞争的清理扫描 | 正则漏了一类文件名，扫出「0 份残留」，实际还剩一份 |
...
| `runtime-refact` ② 通知的哈希复核 | 复核命令写成 `git show runtime-refact/$w:路径`——**zsh 把 `$w:s…` 当成参数修饰符**，路径被吃掉，`rev-parse` 解析成 commit 而非 blob。四份哈希全部报「不一致」，而表其实是对的 |
...
它由 refact 竞争裁决方的密封判据升级而来——那份文件按其自身规定
应在环节 ③ 与裁决书一并公开，实际拖到 才拆封，随后并入本节并删除
...
⚠ `runtime` 竞争违反过这一条：工单 §3.2 把发起方的结论
...
由机制保证而非自律。删除，理由三条，都不是「它写坏了」：

- **三次竞争一次没用过**——三次的分发全部是所有者手工投喂；
...
**隔离目前靠纪律，不靠机制**，且无法事后证明某一次竞争真的独立。
重建时不要照抄那份删掉的实现——它按已废除的「模式 A」组织，
且覆盖不全的机制比没有机制更危险（§8.1）：它会让人以为这次竞争是隔离的。
...
**这份协议自己也漏过一次**：异议环节在 那次竞争实际走过，却没被写进环节表，
以致 09-04 的 refact 竞争差点跳过它——**流程里没写下来的环节，等于不存在。**
