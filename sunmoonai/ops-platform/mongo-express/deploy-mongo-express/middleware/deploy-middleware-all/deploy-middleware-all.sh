...
# MongoExpress 中间件总控部署脚本
# 统一部署所有 MongoExpress 中间件组件（policy）
...
MONGO_EXPRESS_MIDDLEWARE_CONFIG_FILE="$SCRIPT_DIR/deploy-middleware-all.conf"

# 加载配置
load_config() {
    if [[ ! -f "$MONGO_EXPRESS_MIDDLEWARE_CONFIG_FILE" ]]; then
        log_error "MongoExpress 中间件总控配置文件不存在: $MONGO_EXPRESS_MIDDLEWARE_CONFIG_FILE"
        exit 1
    fi

    source "$MONGO_EXPRESS_MIDDLEWARE_CONFIG_FILE"
    log_success "✅ MongoExpress 中间件总控配置加载成功"
}
...
deploy_all_middleware() {
    ...
    log_info "开始基于优先级的 MongoExpress 中间件组件部署..."
    log_info "命名空间: $namespace"
    
    local components=(
        "policy:${mongo_express_policy_enabled:-true}:${mongo_express_policy_priority:-100}:MongoExpress Policy:$SCRIPT_DIR/../mongo-express-policy/deploy-mongo-express-policy/deploy-mongo-express-policy.sh"
    )
    ...
    for c in "${enabled[@]}"; do
        ...
        if [[ -f "$path" ]]; then
            # 如果 CLUSTER 环境变量已设置，传递给子脚本
            if [[ -n "${CLUSTER:-}" ]]; then
                if "$path" deploy "$namespace" --cluster "$CLUSTER"; then
    ...
    log_success "✅ MongoExpress 所有中间件组件部署完成！"
    return 0
}
...
delete_all_middleware() {
    ...
    log_info "开始删除 MongoExpress 所有中间件组件（按优先级逆序）..."
    log_info "命名空间: $namespace"
    ...
    for c in "${enabled[@]}"; do
        ...
        if [[ -f "$path" ]]; then
            # 如果 CLUSTER 环境变量已设置，传递给子脚本
            if [[ -n "${CLUSTER:-}" ]]; then
                "$path" uninstall "$namespace" --cluster "$CLUSTER" || log_warn "⚠️ $desc 卸载失败或不存在"
            else
    ...
    log_success "✅ MongoExpress 所有中间件组件删除完成！"
    return 0
}
...
check_all_middleware_status() {
    ...
    log_info "检查 MongoExpress 所有中间件组件状态..."
    log_info "命名空间: $namespace"
    ...
    log_success "✅ MongoExpress 所有中间件组件状态检查完成！"
    return 0
}
...
main() {
    ...
    case "$action" in
        ...
        "help"|"-h"|"--help")
            ...
            echo ""
            echo "操作:"
            echo "  deploy           部署 MongoExpress 所有中间件组件（默认）"
            echo "  uninstall        删除 MongoExpress 所有中间件组件"
