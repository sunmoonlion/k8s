...
# 计算项目根目录（应用根目录）
# 从 deploy-tools-web-frontend-secret/ 向上 3 级到达应用根目录
# deploy-tools-web-frontend-secret/ -> tools-web-frontend-secret/ -> secret/ -> deploy-tools-web-frontend/ -> tools-web-frontend/
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/secret/tools-web-frontend-secret/generate-tools-web-frontend-secret/generate-tools-web-frontend-secret.sh"
  ...
  case "$action" in
    ...
    status)
      ...
      local secret_name="${TOOLS_WEB_FRONTEND_SECRET_NAME:-tools-web-frontend-secret}"
