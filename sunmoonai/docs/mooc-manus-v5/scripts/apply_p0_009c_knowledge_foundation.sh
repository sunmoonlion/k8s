#!/usr/bin/env bash
# Historical P0-009C overlay implementation. It is intentionally retired:
...
archive_dir() {
  local module_root="$1"
  local dest="$module_root/docs/p0-009c-domain-keep"
  mkdir -p "$dest"
  printf '%s\n' "$dest"
}

printf 'P0-009C_STAGE=web-frontend-overlay\n'
KEEP_WEB_FE="$(archive_dir "$KN_WEB_FE")"
...
rsync -a --delete \
  ...
  "$KN_WEB_FE/app/" "$KEEP_WEB_FE/pre-overlay-app/" || true
rsync -a --delete \
  --exclude node_modules --exclude .next --exclude dist --exclude test-results \
  --exclude .env.local --exclude .env \
  "$TPL_WEB_FE/app/" "$KN_WEB_FE/app/"
python3 - <<'PY'
import json
    ...
    if isinstance(msg[section], dict):
      ...
      if 'title' in msg[section] and section in ('Index','Home','Metadata'):
        if 'tpl' in str(msg[section]['title']).lower() or 'template' in str(msg[section]['title']).lower():
          msg[section]['title']='Knowledge'
  path.write_text(json.dumps(msg, indent=2, ensure_ascii=False)+'\n')
PY
rsync -a "$TPL_WEB_FE/mybuild/" "$KN_WEB_FE/mybuild/" \
  --exclude build.conf --exclude '*.log'
cat > "$KN_WEB_FE/mybuild/build.conf" <<EOF
# Knowledge Web Frontend image build config (P0-009C)
WEB_FRONTEND_IMAGE="knowledge-web-frontend"
...
KNOWLEDGE_SSR_IMAGE="knowledge-web-frontend"
KNOWLEDGE_SSR_TAG="${CANDIDATE_TAG}"
TPL_SSR_IMAGE="knowledge-web-frontend"
...
printf 'P0-009C_STAGE=web-backend-overlay\n'
KEEP_WEB_BE="$(archive_dir "$KN_WEB_BE")"
rm -rf "$KEEP_WEB_BE/pre-overlay-nest"
mkdir -p "$KEEP_WEB_BE/pre-overlay-nest"
rsync -a --delete \
  --exclude node_modules --exclude dist --exclude .git \
  "$KN_WEB_BE/app/" "$KEEP_WEB_BE/pre-overlay-nest/app/" || true
[[ -d "$KN_WEB_BE/mybuild" ]] && rsync -a "$KN_WEB_BE/mybuild/" "$KEEP_WEB_BE/pre-overlay-nest/mybuild/" || true
rm -rf "$KN_WEB_BE/app"
mkdir -p "$KN_WEB_BE/app"
rsync -a --delete \
  --exclude .venv --exclude __pycache__ --exclude .env --exclude .env.* \
  --exclude .pytest_cache --exclude htmlcov \
  "$TPL_WEB_BE/app/" "$KN_WEB_BE/app/"
rsync -a "$TPL_WEB_BE/mybuild/" "$KN_WEB_BE/mybuild/"
for f in .dockerignore .gitignore README.md; do
  [[ -f "$TPL_WEB_BE/$f" ]] && cp "$TPL_WEB_BE/$f" "$KN_WEB_BE/$f"
done
python3 - <<'PY'
from pathlib import Path
...
repls={
  ...
  'app_slug: str = "tpl"': 'app_slug: str = "knowledge"',
  'auth_policy_version: str = "tpl-web-v1"': 'auth_policy_version: str = "knowledge-web-v1"',
  'casdoor_application: str = "sunmoonai-tpl-web"': 'casdoor_application: str = "sunmoonai-knowledge-web"',
}
...
PY
cat > "$KN_WEB_BE/mybuild/build.conf" <<EOF
# Knowledge Web Backend (FastAPI) P0-009C
WEB_BACKEND_IMAGE="knowledge-web-backend"
...
printf 'P0-009C_STAGE=admin-frontend-overlay\n'
KEEP_ADMIN_FE="$(archive_dir "$KN_ADMIN_FE")"
...
rsync -a --delete \
  ...
  "$KN_ADMIN_FE/" "$KEEP_ADMIN_FE/pre-overlay-vue/" \
  --exclude 'docs' || true
rm -rf "$KN_ADMIN_FE/app"
mkdir -p "$KN_ADMIN_FE/app"
rsync -a --delete \
  --exclude node_modules --exclude .next --exclude dist --exclude test-results \
  --exclude .env.local --exclude .env \
  "$TPL_ADMIN_FE/app/" "$KN_ADMIN_FE/app/"
rsync -a "$TPL_ADMIN_FE/mybuild/" "$KN_ADMIN_FE/mybuild/"
...
for child in list(root.iterdir()):
  if child.name in keep or child.name.startswith('.'):
    continue
  # leave archived Vue sources only under docs/p0-009c-domain-keep
  if child.is_dir():
    import shutil
...
for locale in ['zh-CN.json','en.json']:
  ...
  for section, key, value in [
    ('Navigation','knowledgeIngestions','知识入库' if zh else 'Knowledge ingestions'),
    ('KnowledgeIngestions','title','知识入库' if zh else 'Knowledge ingestions'),
    ('KnowledgeIngestions','description',
     'Knowledge Admin Dataset/Ingestion 控制台（最小域壳）' if zh
     else 'Knowledge Admin Dataset/Ingestion control surface (minimal shell)'),
  ]:
...
PY
cat > "$KN_ADMIN_FE/mybuild/build.conf" <<EOF
# Knowledge Admin Frontend (Next) P0-009C
ADMIN_FRONTEND_IMAGE="knowledge-admin-frontend"
...
CONTAINER_RUNTIME="docker"
NERDCTL_NAMESPACE="k8s.io"
TPL_SSR_IMAGE="knowledge-admin-frontend"
TPL_SSR_TAG="${CANDIDATE_TAG}"
REGISTRY="\${REGISTRY:-harbor.sunmoonai.com:30443/k8s-images}"
PUSH_IMAGES_AFTER_BUILD="\${PUSH_IMAGES_AFTER_BUILD:-false}"
EOF

printf 'P0-009C_STAGE=admin-backend-kernel-sync\n'
KEEP_ADMIN_BE="$(archive_dir "$KN_ADMIN_BE")"
...
cp "$KN_ADMIN_BE/app/core/config.py" "$KEEP_ADMIN_BE/pre-kernel/core-config.py"
...
repls={
  ...
  'app_slug: str = "tpl"': 'app_slug: str = "knowledge"',
  'auth_policy_version: str = "tpl-admin-v1"': 'auth_policy_version: str = "knowledge-admin-v1"',
  'casdoor_application: str = "sunmoonai-tpl-admin"': 'casdoor_application: str = "sunmoonai-knowledge-admin"',
  'postgresql+asyncpg://tpl:tpl@localhost:5432/tpl': 'postgresql+asyncpg://knowledge:knowledge@localhost:5432/knowledge',
  'default="tpl.admin.default"': 'default="knowledge.admin.default"',
}
for a,b in repls.items():
  if a in text:
    text=text.replace(a,b,1)
...
printf 'P0-009C_STAGE=overlay-complete template_release=%s candidate=%s\n' \
  "$TAG_TEMPLATE_RELEASE" "$CANDIDATE_TAG"
printf 'NEXT: run stitch_p0_009c_knowledge_foundation.py\n'
