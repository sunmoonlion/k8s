...
口令存储（2026-09-26 修）：初始化脚本以前把业务组织的口令类型设成 `plain`，Casdoor 会把口令**明文**入库，自助注册的用户也一样。现在组织用 `bcrypt`（Casdoor 默认）；脚本写入的组织管理员标 `plain`，Casdoor 在其首次登录成功时按组织类型重新哈希（3.42 源码 `CheckPassword`），旧用户同理，不会被锁在外面。同时补了组织的 `account_items`（后台「用户」编辑表单按它生成，为空时「添加」按钮没反应），抄 Casdoor 自带组织的。在 Casdoor 3.42 上从旧状态实测：管理员与旧明文用户都能登录、登录后变成 bcrypt，新注册用户直接是 bcrypt。已在跑的环境里，没登录过的旧用户口令在下次登录前仍是明文。

注册页与登录页的样子（2026-09-26）：用户只看到这两页，碰不到组织、群组、应用这些概念。页面由 `signup-setup.sh` 与 Casdoor 的 Helm 配置决定：中文（组织语言只列中文；`app.conf` 设 `defaultLanguage`、`forceLanguage` 为 `zh`，因为镜像自带配置强制英文、会压过默认语言）；顶部是文字标题「SunMoon AI 投研」（没有图标时藏掉 Casdoor 的标志图位，Casdoor 的 logo 列只收 200 字符的地址）；页脚换成「© SunMoon AI」；注册项顺序为邮箱与验证码 → 用户名 → 密码 → 邀请码，各带中文提示（邀请码旁写"没有邀请码？到首页申请"）；去掉"显示名"一项（新用户显示名为空，我们的后端取不到时用用户名）；藏掉只剩一种语言时多余的语言按钮。用本机 Casdoor 3.42 + 无头浏览器在中文与英文浏览器下截图验证。改 `app.conf` 要重新部署 Casdoor（Helm）才生效。

后台编辑页空白（2026-09-26 修）：脚本建的应用 `providers` 为 NULL，Casdoor 后台打开属于该应用的任何用户（包括 Casdoor 自己注册出来的）都整页空白（`null.filter`）。现在 `create_app` 新建时给 `providers`/`scopes`/`tags`/`token_fields` 空列表，已有的只补 NULL。在本机 Casdoor 3.42 + 无头浏览器复现并验证：用户、应用、组织编辑页都能打开，组织下的「用户 → 添加」直接新建到 sunmoonai 并打开编辑表单（全局「用户」页的「添加」在全新 Casdoor 上也没反应，要从组织进）。
