...
| B1 处置清单 | 候选已验证 | 原任务有去向；旧做法/已有覆盖/独立修复/后续统一开发分开；正式索引文档门禁 2 份通过 |
| B2 抓取防护 | 实施中 | B2.1 网络入口防护先独立验证/集成；B2.2 再实现跨 Worker 来源并发与竞争重排。整项 M1-102 在两者通过前不销账 |
...
| B9 集成与同步 | 待实施 | 每个独立包候选验证后执行，不等全部；子仓先推、父仓 gitlink 后推；master 预期 HEAD 核对、快进/明确合并；再同步两端工作区；不覆写预置草案 |
...
- 当前游标：B1/B2.1 父仓候选提交与集成；B2.2 来源并发尚未开始；未部署。
