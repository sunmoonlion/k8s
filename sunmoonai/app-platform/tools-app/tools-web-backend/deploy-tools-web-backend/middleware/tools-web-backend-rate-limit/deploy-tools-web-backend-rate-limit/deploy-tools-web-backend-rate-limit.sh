#!/bin/bash
# Tools Web Backend Rate Limit Middleware 部署脚本
# 根据配置部署 Rate Limit Middleware 到 Kubernetes 集群

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/deploy-tools-web-backend-rate-limit.conf"
# 计算项目根目录（应用根目录）
# 从 deploy-tools-web-backend-rate-limit/ -> tools-web-backend-rate-limit/ -> middleware/ -> deploy-tools-web-backend/ -> tools-web-backend/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"

# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
K8S_RESOURCE_DIR="$PROJECT_ROOT/resources/k8s-resource"
MIDDLEWARE_YAML="$K8S_RESOURCE_DIR/custom-values/middleware/tools-web-backend-rate-limit/generate-tools-web-backend-rate-limit/tools-web-backend-rate-limit-generated.yaml"
...
main() {
  ...
  local generate_script="$PROJECT_ROOT/resources/k8s-resource/custom-values/middleware/tools-web-backend-rate-limit/generate-tools-web-backend-rate-limit/generate-tools-web-backend-rate-limit.sh"
  ...
  case "$action" in
    deploy)
      log_info "部署 Tools Web Backend Rate Limit Middleware..."
    ...
    uninstall)
      log_info "卸载 Tools Web Backend Rate Limit Middleware..."
      log_info "命名空间: $namespace"
    ...
    status)
      log_info "检查 Tools Web Backend Rate Limit Middleware 状态..."
      log_info "命名空间: $namespace"

      if kubectl get middleware tools-web-backend-rate-limit -n "$namespace" 2>/dev/null; then
