...
定位、领域所有权与旧 Research 处理见 `摘要.md`；本文只写机制。
命名护栏：合法领域概念可保留 `research` 命名（如 `research_web_pilot` graph），但
应用/基础设施身份必须逐项为 `investment-*`；未来 research-app 边界见
`app-platform-architecture/架构.md` §3。
