#!/bin/bash
# info-admin-frontend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-info-admin-frontend-secret.conf"
# 从 generate-info-admin-frontend-secret/ -> info-admin-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-info-admin-frontend-secret/ -> ... -> k8s-resource/ -> resources/ -> info-admin-frontend/
...
main() {
    log_info "开始生成 info-admin-frontend Secret YAML 文件..."
