...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"tools-web-frontend-pvc-generated.yaml" ]]; then
    ...
    if [ -f "$generate_script" ]; then
        export NAMESPACE="${TOOLS_WEB_FRONTEND_NAMESPACE:-}"
        export ENVIRONMENT="${ENVIRONMENT:-development}"
        export ENV="${ENV:-dev}"
        export PROJECT_ID="${TOOLS_WEB_FRONTEND_PROJECT_ID:-}"
...
deploy_sub_components() {
    ...
    log_info "部署 tools-web-frontend 子组件..."
...
deploy_app() {
    ...
    export TOOLS_WEB_FRONTEND_FULL_IMAGE_NAME="${TOOLS_WEB_FRONTEND_IMAGE_REGISTRY}/${TOOLS_WEB_FRONTEND_IMAGE_PROJECT}/${TOOLS_WEB_FRONTEND_IMAGE}:${TOOLS_WEB_FRONTEND_TAG}"

    log_info "镜像: $TOOLS_WEB_FRONTEND_FULL_IMAGE_NAME"
    log_warn "⚠️  请确保该镜像已存在于 Harbor 仓库"

    export NAMESPACE="$NAMESPACE" ENV="$ENV" ENVIRONMENT="$ENVIRONMENT"
    export TOOLS_WEB_FRONTEND_FULL_IMAGE_NAME
    ...
    auto_generate_yaml "$TOOLS_WEB_FRONTEND_YAML" "$K8S_RESOURCE_DIR" || return 1

    log_info "生成并部署 PVC..."
    auto_generate_yaml "$TOOLS_WEB_FRONTEND_PVC_YAML" || return 1
