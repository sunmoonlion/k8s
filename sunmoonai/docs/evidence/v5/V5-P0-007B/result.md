...
- 2026-07-14 本地补齐所有 Info 写操作的统一审计确认：URL crawl、source、collector、discover、upload 和 distribution-create 均先经过 `AuditedActionModal`，再由 `useCrudMutation` 生成并传递 correlation/operation/reason headers；上传从“选中文件即写入”改为“选中文件暂存、确认原因后才上传”。并新增 deferred-upload 回归测试；该改动尚未构建新候选镜像。
