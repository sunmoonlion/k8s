...
SECRET_YAML="$K8S_RESOURCE_DIR/custom-values/secret/pg-inc-db/generate-pg-inc-db/postgresql-incubator-db-secret-generated.yaml"
...
CONFIG_FILE="$SCRIPT_DIR/deploy-pg-inc-db.conf"
...
auto_generate_yaml() {
    ...
    if [ ! -f "$yaml_file" ]; then
        ...
        local generate_script="$k8s_resource_dir/custom-values/secret/pg-inc-db/generate-pg-inc-db/generate-pg-inc-db.sh"
