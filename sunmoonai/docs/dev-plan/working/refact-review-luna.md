...
这是包含我自己候选的完整排序，不是只排另外三家：

| 排名 | 候选 | 排序依据 | 选用前必须修正 |
| ---: | --- | --- | --- |
| 1 | **luna** | 11 项均通过；双腿映射、OpenClaw 两层路由和 crash-loop 熔断的事实口径最准确 | 吸收 Fake/契约测试、dormant capability 强证据；降低分散章节带来的整合成本 |
| 2 | **qwen** | 冻结纪律完整，结构紧凑，生产未接线证据和工具不可见说明最强 | 补 OpenClaw 第二层 runtime route；把 Codex `F-EXEC-03/05` 降为需补法；把版本漂移检查补成同版崩溃熔断 |
| 3 | **kimi** | 七块完整、Gate 0 可证伪条件和契约测试思路突出 | 修正 dsh `F-EXEC-02` 自相矛盾；删除自研 LangGraph 退路；收紧分类器通用兜底和粗粒度锚点 |
| 4 | **cursor** | Port/DTO、上游 G1–G7、进程纪律和盲区清单最具体 | 修正 OpenClaw 第二层路由、审批超时越界造态、Codex `F-EXEC-03/05` 过报及高风险 `llm-review` 边界 |

排序先看硬验收，再看事实错误是否会导致错误放行，最后才看结构与表达。qwen 虽然 OpenClaw
少一层、Codex 能力有过报，但没有 kimi 的 A4 退路冲突和映射表内部矛盾，故列第二；kimi 的
问题仍可局部修补，故高于同时出现产品状态越界定义和 OpenClaw 事实错误的 cursor。

再次声明：排名第 1 的 `luna` 是我自己的候选，存在直接利益冲突。其明确缺陷已经写在 C.2：
新增内容分散、改名触达面较大、Port 尚无 Fake/契约测试、部署/代理/token 撤销链均未运行验证。
裁决方不应把这个排序当作者票，应按 C.1 的固定提交与事实锚点复核。
