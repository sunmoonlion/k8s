...
check_jenkins_status() {
    ...
    if [[ -z "$phases" ]]; then
        ...
        # 统计 Running 个数，并确保为数值
        local pods_ready_str
        pods_ready_str=$(echo "$phases" | tr ' ' '\n' | grep -c '^Running$' 2>/dev/null || true)
        if [[ "$pods_ready_str" =~ ^[0-9]+$ ]]; then
            pods_ready="$pods_ready_str"
        else
            pods_ready=0
        fi
    fi
    if [ "${pods_ready}" -gt 0 ]; then
        log_success "✅ Jenkins Pod 运行正常 (${pods_ready} 个)"
    else
...
show_jenkins_connection_info() {
    ...
    echo "=== Jenkins 连接信息 ==="
    echo "命名空间: $namespace"
    echo "外部访问: https://${JENKINS_UNIFIED_HOST:-llmops.sunmoonai.com}/jenkins"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件（如果需要）:"
    echo "   echo '101.126.151.0 ${JENKINS_UNIFIED_HOST:-llmops.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 访问 Jenkins:"
