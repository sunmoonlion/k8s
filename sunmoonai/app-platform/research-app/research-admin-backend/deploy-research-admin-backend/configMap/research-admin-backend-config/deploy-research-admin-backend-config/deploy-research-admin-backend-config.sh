...
CONFIG_FILE="$SCRIPT_DIR/deploy-research-admin-backend-config.conf"
# 从 deploy-research-admin-backend-config/ -> research-admin-backend-config/ -> configMap/ -> deploy-research-admin-backend/ -> research-admin-backend/
...
main() {
    ...
    local generate_script="$K8S_RESOURCE_DIR/custom-values/configMap/research-admin-backend-config/generate-research-admin-backend-config/generate-research-admin-backend-config.sh"
    ...
    case "$action" in
        ...
        uninstall)
            ...
            log_success "research-admin-backend ConfigMap 卸载完成"
            ;;
        status)
            kubectl get configmap "research-admin-backend-config" -n "$NAMESPACE" 2>/dev/null \
                || log_warn "ConfigMap 不存在: research-admin-backend-config"
            ;;
        generate)
