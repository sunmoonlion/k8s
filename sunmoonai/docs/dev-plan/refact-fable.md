...
> ---
>
> ## ⚠ 本文的架构结论已被推翻；但部分内容仍然有效且被依赖，**在迁出之前不得删除**
>
> **被推翻的**：标题里的「多个 Profile」与 §3.1「一套状态机，两个 Profile」——那对部署形态是**假分叉**，
> 五格只差取值不差结构；且它在内核已有的 Task Profile / Agent Profile 之外造了第三个「Profile」含义，
> 正是本文 §1.2 自己诊断的同名物病。
> **架构结论一律以 `runtime-architecture.md` 为准**（`runtime` 轮 ③ 裁决稿，⑤ 验收有条件通过、条件已补足）。
> 推翻的经过与逐条处置见 `rounds/runtime/runtime-disposition.md`；解冻裁定见
> `rounds/refact-fable/rulings.md` 的 `R9`。
>
> **仍然有效、且当前没有第二个落点的部分**（读这些请直接读本文，不要去读 `runtime-architecture.md`——它没有）：
>
> | 节 | 内容 | 谁在依赖 |
> | --- | --- | --- |
> | §3.13.2 | 三道边界（回执仓 / 主仓写权限移出 VM / 签名不代做） | S1；`runtime-architecture.md` §4.4 只有原则，无实现 |
> | §3.13.3 | 回执仓的对象模型（回执 = 仓内签名 commit，不是跨仓 tag） | **S1 ①③ 直接依赖** |
> | §3.13.4 | 回执 schema | **S1 ③ 直接依赖** |
> | §3.13.5 | 两台机器的身份与凭据分布 | S1 ② |
> | §3.4 | 收件箱字段分级（无默认的字段不得预填） | 现行实践 |
> | §3.6 | 工单与 T0 任务类包 | R1 |
> | §3.7 | 工作区供给 provision 函数 | R2 |
> | §6 | 实施路线 R0 / R0′ / S1 / R1–R5 与各自的机械验收判据 | 整个后续计划 |
>
> **删除条件**：上表每一节都迁出到独立文档、且引用它的地方改指新落点之后，本文才可删。
> 这正是本文 §5.3「删除条件」对两份 lifecycle 文档立的规矩——2026-09-05 曾对本文用了更低的标准
> （只验「锚点可从 git 历史复跑」就删），**那是错的**：`git show <commit>:<路径>` 是可复核性，不是可读性，
> 读的人不会去翻历史。已恢复，登记在 `rounds/runtime/runtime-disposition.md`。
>
> **行号锚说明**：引用本文的活跃文档一律写 `refact-fable.md @ 7e8464c2:<行>`。
> 本抬头使当前文件行号相对 `7e8464c2` **整体下移**，钉 commit 的锚不受影响——这正是钉 commit 的意义。
>
> ---
...
>
> 本文是对 [`working/development-lifecycle-agent.md`](working/development-lifecycle-agent.md)（1603 行）、
> `working/development-lifecycle-human.md`（823 行）与 [`round-protocol.md`](round-protocol.md)
