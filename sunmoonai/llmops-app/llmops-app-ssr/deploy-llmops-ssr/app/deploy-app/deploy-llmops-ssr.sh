...
# 项目根目录（deploy-llmops-ssr 目录）
# 从 app/deploy-app/ 向上 2 级到达 deploy-llmops-ssr/
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
# 应用根目录（llmops-app-ssr 目录）
# 从 deploy-llmops-ssr/ 向上 1 级到达 llmops-app-ssr/
...
# 导入统一部署模板
# 从应用根目录（llmops-app-ssr/）到 k8s/ 需要向上 4 级
# llmops-app-ssr/ -> llmops-app/ -> sunmoonai/ -> k8s/
...
# 默认配置从配置文件读取（deploy-llmops-ssr.conf）
...
# 资源文件路径（对齐项目结构）
# 从 app/deploy-app/ 向上 3 级到达应用根目录（llmops-app-ssr/）
# app/deploy-app/ -> app/ -> deploy-llmops-ssr/ -> llmops-app-ssr/
RESOURCES_DIR="../../../resources"
# 使用生成的 YAML 文件（由各组件自己的 generate-*.sh 生成）
# YAML 文件现在分散在各组件的 generate-* 目录下
K8S_RESOURCE_DIR="${RESOURCES_DIR}/k8s-resource"
LLMOPS_SSR_YAML="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/llmops-app-ssr-generated.yaml"
LLMOPS_SSR_PVC_YAML="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-ssr-pvc/generate-llmops-ssr-pvc/llmops-app-ssr-pvc-generated.yaml"
...
auto_generate_yaml() {
    ...
    if [[ "$yaml_file" == *"llmops-app-ssr-generated.yaml" ]] && [[ "$yaml_file" != *"config"* ]] && [[ "$yaml_file" != *"secret"* ]] && [[ "$yaml_file" != *"pvc"* ]] && [[ "$yaml_file" != *"ingress"* ]]; then
        # 主应用 YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/app/generate-app/generate-app.sh"
    elif [[ "$yaml_file" == *"llmops-app-ssr-pvc-generated.yaml" ]]; then
        # PVC YAML
        generate_script="${K8S_RESOURCE_DIR}/custom-values/pvc/llmops-ssr-pvc/generate-llmops-ssr-pvc/generate-llmops-ssr-pvc.sh"
    else
...
check_env_config() {
    ...
    if [[ "${secrets_enabled:-true}" == "true" ]] || [[ "${configmap_enabled:-true}" == "true" ]]; then
        ...
        local config_script="$configmap_dir/llmops-app-ssr-config/deploy-llmops-app-ssr-config/deploy-llmops-app-ssr-config.sh"
        local secret_script="$secrets_dir/llmops-app-ssr-secret/deploy-llmops-app-ssr-secret/deploy-llmops-app-ssr-secret.sh"
        if [[ "${configmap_enabled:-true}" == "true" ]]; then
...
deploy_app() {
    ...
    export LLMOPS_SSR_IMAGE="${LLMOPS_SSR_IMAGE:-llmops-app-ssr}"
