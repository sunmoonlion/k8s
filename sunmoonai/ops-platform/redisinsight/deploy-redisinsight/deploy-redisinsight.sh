...
# 定义 RedisInsight 所需镜像（保留给镜像检查设计使用）
...
# 使用统一模板的通用按需推送 helper，将 RedisInsight 组件镜像推送到 Harbor
push_redisinsight_images_to_harbor() {
    push_component_images_to_harbor "redisinsight"
}
...
process_redisinsight_values() {
    ...
    if [[ -f "$env_values_file" ]]; then
        ...
        sed -i "s/{{REDISINSIGHT_UNIFIED_HOST}}/${REDISINSIGHT_UNIFIED_HOST:-www.sunmoonai.com}/g" "$redisinsight_values_file"
...
check_redisinsight_status() {
    ...
    local phases
    phases=$(kubectl get pods -n "$namespace" -l "$label_selector" -o jsonpath='{range .items[*]}{.status.phase}{"\n"}{end}' 2>/dev/null || echo "")
    local pods_running pods_pending
    pods_running=$(echo "$phases" | grep -c "^Running$" 2>/dev/null || echo "0")
    pods_running=$(echo "$pods_running" | tr -d '[:space:]')
    pods_running=${pods_running:-0}
    pods_pending=$(echo "$phases" | grep -cE "^(Pending|ContainerCreating)$" 2>/dev/null || echo "0")
    pods_pending=$(echo "$pods_pending" | tr -d '[:space:]')
    pods_pending=${pods_pending:-0}
    
    if [[ "$pods_running" =~ ^[0-9]+$ ]] && [[ "$pods_running" -gt 0 ]]; then
        log_success "✅ RedisInsight Pod 运行正常 ($pods_running 个)"
    elif [[ "$pods_pending" =~ ^[0-9]+$ ]] && [[ "$pods_pending" -gt 0 ]]; then
        log_warn "⏳ RedisInsight Pod 正在启动中（$pods_pending 个 Pending/ContainerCreating，0 个 Running）"
...
show_redisinsight_connection_info() {
    ...
    echo "命名空间: $namespace"
    echo "外部访问: https://${REDISINSIGHT_UNIFIED_HOST:-www.sunmoonai.com}/redisinsight"
    echo ""
    echo "=== 使用说明 ==="
    echo "1. 配置 hosts 文件（如果需要）:"
    echo "   echo '${REDISINSIGHT_NODE_IP:-115.190.153.150} ${REDISINSIGHT_UNIFIED_HOST:-www.sunmoonai.com}' | sudo tee -a /etc/hosts"
    echo ""
    echo "2. 访问 RedisInsight:"
    echo "   https://${REDISINSIGHT_UNIFIED_HOST:-www.sunmoonai.com}/redisinsight"
    echo ""
    echo "3. 查看服务状态:"
