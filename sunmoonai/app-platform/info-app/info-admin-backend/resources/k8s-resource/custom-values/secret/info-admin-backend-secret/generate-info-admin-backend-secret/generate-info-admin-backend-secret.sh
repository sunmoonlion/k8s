#!/bin/bash
# info-admin-backend Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-info-admin-backend-secret.conf"
# 从 generate-info-admin-backend-secret/ -> info-admin-backend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 从 generate-info-admin-backend-secret/ -> ... -> k8s-resource/ -> resources/ -> info-admin-backend/
...
export KNOWLEDGE_APP_SERVICE_CLIENT_ID="${KNOWLEDGE_APP_SERVICE_CLIENT_ID:-}"
export KNOWLEDGE_APP_SERVICE_CLIENT_SECRET="${KNOWLEDGE_APP_SERVICE_CLIENT_SECRET:-}"
...
main() {
    log_info "开始生成 info-admin-backend Secret YAML 文件..."
