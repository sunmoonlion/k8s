...
- KIND 故障验证器：`sunmoonai/docs/mooc-manus-v5/scripts/verify_p0_006_kind.py`。它会临时阻断 API 的 broker wake-up、验证两个 scanner 的单次 lease 竞争、注入“Broker accept 后/published 写库前”与“Provider effect 后/outbox ack 前”中断，并检查 CronJob 恢复后回到 `suspend: true`。临时 API/Knowledge/CronJob/测试队列状态必须在 `finally` 清理；broker 覆盖 helper 自身还含 rollout 失败的内联回滚与 SIGINT/SIGTERM cleanup，不能依赖调用方在函数返回后才登记 cleanup。
