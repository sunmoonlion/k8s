...
| 11 | 定稿 `dev-plan-architecture.md` 内置的 `verify` 要求工作区源文件与冻结输入相同，发布进主线后在仓根跑必失败；改工具（按冻结提交读源文件）还是改文中说明 | `findings.md` F-25 第 3 条 | 已做：改工具，只按冻结提交读源（2026-09-14，所有者让 opus 单独做切换，不另走一轮） |
