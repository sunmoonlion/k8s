...
# 从 generate-app/ -> app/ -> custom-values/ -> k8s-resource/ -> resources/ -> celeryworker-incubator/
...
# 镜像配置
export CELERY_WORKER_IMAGE_REGISTRY="${CELERY_WORKER_IMAGE_REGISTRY:-}"
export CELERY_WORKER_IMAGE_PROJECT="${CELERY_WORKER_IMAGE_PROJECT:-}"
...
export CELERY_WORKER_IMAGE_PULL_SECRET_NAME="${CELERY_WORKER_IMAGE_PULL_SECRET_NAME:-}"

# 后端镜像配置
export BACKEND_IMAGE="${BACKEND_IMAGE:-}"
export BACKEND_TAG="${BACKEND_TAG:-}"
export BACKEND_IMAGE_REGISTRY="${BACKEND_IMAGE_REGISTRY:-}"
export BACKEND_IMAGE_PROJECT="${BACKEND_IMAGE_PROJECT:-}"

# 代码提取配置
export CODE_EXTRACT_SOURCE_DIR="${CODE_EXTRACT_SOURCE_DIR:-}"
export CODE_EXTRACT_DIRS="${CODE_EXTRACT_DIRS:-}"
export CODE_EXTRACT_FILES="${CODE_EXTRACT_FILES:-}"
...
# Secret / ConfigMap 名称
export CELERY_WORKER_SECRET_NAME="${CELERY_WORKER_SECRET_NAME:-}"
export CELERY_WORKER_CONFIGMAP_NAME="${CELERY_WORKER_CONFIGMAP_NAME:-}"
...
# 处理模板文件（替换变量）
# 注意：envsubst 只处理 ${VAR} 格式，不处理 ${VAR:-default} 格式
# 所以我们需要先处理 ${VAR:-default} 格式，将其转换为 ${VAR}
local full_output_path="$OUTPUT_DIR/$OUTPUT_FILE"

# 使用 sed 处理 ${VAR:-default} 格式，然后使用 envsubst 替换变量
# 注意：这里假设模板中已经使用了正确的变量格式
