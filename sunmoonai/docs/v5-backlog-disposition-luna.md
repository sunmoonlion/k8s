...
B7 业务切换/保留策略、B9 最终集成仍未销账。
同日按用户新决定，B8 改为将旧任务集中到 [独立待接收清单](legacy-backlog/README.md)，
等新架构讨论后再正式接收；本轮独立整理已完成，不再用架构未决项阻塞旧账整理。
用户确认三部分职责划分并保留 B-S，不据此取消浏览器入口；本次不改正式模块树。
...
执行范围：现有五仓 Luna 工作区中的清单、代码、测试和必要文档；逐包验证与本地提交。
2026-09-13 所有者更新执行顺序：先彻查历史失败，再完成本轮剩余处置，最后一次性
集成、推送与同步；不再逐包更新 master、推仓库或拉取云端工作区。既有同步历史保留。
“完成处置”包括可独立补齐的旧账通过验收、冲突项明确放弃、需统一开发项被新计划正式
接收；不等于提前做完 N1～N6 全产品，也不豁免实际部署/数据操作的授权和验收门禁。
不自动切生产流量、删除业务数据/历史证据、轮换凭据或强制覆盖分支。
部署、数据迁移、外部状态变更需列出精确对象与回滚条件，按授权和平台批准机制实施。
此前要求保留两份未跟踪的 general/pro 协议草案、不顺带提交；下述暂停指令已更新提交授权，
历史记录中的“未提交/未推送”均指当时状态，不覆盖新的暂停同步要求。

### 2026-09-13 暂停与续作入口

所有者决定今天暂停，要求删除旧便签、更新项目指南并同步现有成果；随后明确两份
general/pro 协议草案一起提交同步，**不改内容**。本次先保存已验证候选，覆盖此前
“全部处置完成后才同步”的执行顺序；不是宣告所有旧账完成。

- 本包同步范围：四 Backend 现有提交先推送既有仓库 master，再集成五父仓；
  本地与云端 master/Luna 普通快进同步，不重置其它工位、不新建分支、不启动本地 Opus。
  本节是提交前的收尾计划，实际完成以两端与 GitHub/Gitee 引用核验回执为准。
- B7u 联合验证已完成，固定后端 tpl `44e74fe`、Info `5fb909b`、Knowledge `e79a712`、
  Investment `68f776e`；四完整后端套件共 **1581 passed / 0 skipped**，四组外层各 36 项
  （含各自完整套件包装），Ruff/Pyright 通过。详见
  [联合运行证据](v5-backlog-joint-runtime-identity-luna.md)，不重复计算包装测试为新增用例。
- 下次先同步，再从 **B7 运行身份供给/重启一致性、旧连接排空与撤权、备份恢复和受控切换前置**
  接续；候选 ACL、预声明拓扑与隔离进程证明不能代替业务切换验收。
- B7 整体仍未完成；B8/N1～N6 的新 dev-plan 正式接收仍待完善，B9 不能因本次
  阶段同步而整体销账。实际 Prometheus/Alertmanager 采集与告警到达验收继续留在
  [未来 N4-OPS-01](tasks/N4-OPS-01/thread/0001-imp/0001/user-message.md)，本次不安装、不发送外部通知。
- 本次不改业务数据/Secret、不构建或推送 Harbor 镜像、不部署。旧 `接着做.txt` 已删除，
  可从 Git 历史恢复；后续只用本清单与对应证据接续，不再维护过时仓根便签。

暂停同步回执：四后端已先普通推送到 GitHub/Gitee 的既有 master；五父仓候选
tpl `33f5b636`、Info `08c1505b`、Knowledge `92e310ad`、Investment `9728cf1c`、
k8s 内容 `0f18e1c4` 已完成两处 master/Luna 推送、本地 master 快进和最终拉取、
云端 master/Luna 拉取。SSH 逐一核对两工作区五父仓完整 SHA、全部实际子模块和
干净状态通过；两处仓库实时引用也逐一与预期一致。本段为该同步的后续回填。
两份草案在提交前、已提交对象及云端两工作区的 SHA-256 一致（general `427da4ac…`，
pro `60c9b00f…`），未修改内容；14 份文档门禁及四后端 44 项休眠能力核验通过。
无强推/reset/realign、新分支或其它工位重置，暂停并不销掉上述未完成项。

同日追加：按所有者新请求完成 [Knowledge Provider 内部解耦](knowledge-provider-decoupling-luna.md)，
Backend `26aa071`、父仓 `8f87061`、k8s 内容 `c299c6b5` 已同步到 GitHub/Gitee 和
本地/云端 master、Luna，五父仓及实际子模块核验一致、干净；本段为后续回填。
默认仍为 RAGFlow，未接入 WeKnora、未部署；再次暂停，B7 及上述续作入口不变。
...
| B8 后续移交 | 按 2026-09-19 新决定，独立整理完成；正式接收延期 | [独立待接收清单](legacy-backlog/README.md) 保存 N1～N6 原编号、目标、已有基础与剩余项；等架构讨论后再接收，不声明产品功能完成或正式迁入 |
...
## 6. 新 dev-plan 接收包（历史分类，仅监控子项已迁入）

2026-09-19 更新：下表保留为原分类快照；后续统一查看
[独立待接收清单](legacy-backlog/README.md)，不以表内旧架构表述作为新的实施指令。
按用户新决定，正式接收待新架构讨论，不是本轮独立整理的阻塞条件。

N4 的监控部署/采集/告警送达子项于 2026-09-13 按所有者要求迁入
[实施计划 N4-OPS-01](tasks/N4-OPS-01/thread/0001-imp/0001/user-message.md)。任务已定义，不等于实施完成；
其他 N1～N6 子项仍待 B8 正式接收，不以局部接收关闭整包。
...
- B7s：模板 Backend `44e74fe` 提供显式预建拓扑及确认/不可路由返回；模板父仓
  ...
  父仓普通 16 项通过，均零跳过。初始化管理员回环限制、两阶段启动竞争及测试接口误用
  均已查清修复，未放宽业务权限。临时 RabbitMQ/PG/三卷清理；三领域源码、部署、
  master/gitlink/远端未改。下一游标 Info→Knowledge→Investment 对齐、联合身份及供给切换。
- B7r：模板 `9abc39d` 新增 Investment 测试确认域；k8s `15588f3a` 固定 18 表权限
  策略、两类测试与说明。[证据](v5-backlog-investment-database-policy-luna.md)。模板→Info→
  Knowledge→Investment 共 94 PG、模板普通 12、平台串行 42 项通过；不使用内存 Saver
  冒充断点验证，真实 factory 认证为 Worker。API owner/token/取消与 Worker lease/回执
  原用例可用，但不外推未来统一产品审批已完成。一次性 PG/三卷清理，业务部署/身份/
  master/gitlink/远端不变；下一游标 broker 精确权限与真实启动/拒绝、受控供给和切换。
- B7q：模板 `2faa367` 仅新增 Knowledge 测试确认域与说明；k8s `81c990e0` 固定
  Knowledge 领域策略与原用例/拒绝验证。[完整证据](v5-backlog-knowledge-database-policy-luna.md)。
  固定模板 18 PG+12 单元→Info 25 PG→Knowledge 27 PG 严格串行通过，平台 28+7+3
  共 38 项及 Ruff/diff 通过；本包无真实 PG 失败，静态格式问题已修复。
  测试库/账号清零后，一次性 PG 及三卷经批准删除；四后端/父仓预置增量/草案均保留，
  不更改 master/gitlink，不推送。下一权限游标 Investment，其后 broker/实际供给与切换。
- B7p：模板 `96d4017` 提取无 App 导入的公共隔离夹具，固定复验 12 单元及 18 PG
  通过后，Info 部署策略/测试固定 k8s `58980ce6`，25 PG（19.49 秒）、平台串行
  28+3+3=34 项通过，Ruff/diff 检查通过。[范围与证据](v5-backlog-info-database-policy-luna.md)。
  四后端、三个领域父仓与预置增量均未改；不更新 gitlink/master，不中间同步。
  仅新测试角色前置下的加法 GRANT 候选，不将其宣称为现有业务权限收敛；下一领域
  权限游标 Knowledge→Investment，其后 broker/真实供给与受控切换继续留 B7。
...
- B7n：新增单一平台只读 PostgreSQL 目录工具及 7 项安全测试，实查九角色有效
  ...
  缺口、下一精确供给/拒绝矩阵；内部 char→bytes 序列化失败已定位并改为 SQL 转文本，
  九角色重新采集成功。模板 8、平台串行共 30 项通过，非真实拒绝或切换验收。
  不改四后端/父仓 gitlink/master/业务凭据，不作中间同步；B7 整包仍未完成。
- B7m 固定复验：tpl-app 10acfdc 的三个脚手架文件本地提交，backend gitlink 不变；
  k8s 内容 0bd49272，Ruff 通过；模板 8 项，再串行 Info 17 / Knowledge 20 /
  Investment 23 项通过、零跳过。三份文档门禁通过，详细时间见本包报告。
  没有修改或生成到旧发布目录、没有真实凭据/集群变更，不作中间同步。
- B7m：模板补 Worker release-id 后 8 项通过；Info → Knowledge → Investment
  串行保留模板 DB/broker 角色键、修外部 Secret 依赖；共享 runtime_role_env 对
  丢失/重复/错角色/内联/可选化等失败关闭。实际完整渲染及开发 finalizer 验证领域
  env/envFrom/挂载、迁移凭据、探针和发布标识；最终脚本 23 项通过，无跳过，七个
  Python 文件 Ruff 通过。[本包报告](v5-backlog-runtime-rendering-luna.md)区分引用
  契约和真实凭据/权限，也记录 import 排序与工具 cwd 错误的修正。
  旧 bundle、四后端、gitlink/master、Secret/数据库/集群未改；不构建/推送/同步。
  下一项准备精确角色权限/供给与拒绝矩阵，实际外部操作须先冻结对象及回滚；
  B7/B8/B9 整体不销账，两份预置协议草案保持原样。
...
- B7k：固定四后端 a91eb3e/7dafb34/5df4f17/3d9531d 的源码引用审计完成，
  ...
  CASCADE 保护。冷副本不能替代在线去重与恢复；只读引用快照也不能关闭并发窗口。
  本包只改三份 k8s 文档，不改生产代码/迁移/触发器，不规定保留天数；无业务扫描、
  备份/清理/部署，没有新跑后端测试或冒认历史测试数。归档实施及 M1-105 仍未收口，
  产品全引用生命周期仍待 B8 正式接收，不擅自把 B7 整体转入未来计划。
  三份文档门禁与 diff 检查通过，四后端 HEAD/干净状态及父仓原 gitlink 复核不变。
  下一游标为 B7 当前角色/身份/发布与数据切换只读预检；实际外部操作另列精确对象、
  授权与回滚。父仓 gitlink/master/远端和两份预置草案不动，最终一次性集成同步不变。
...
- B7i 最终源码结果：模板→Info→Knowledge→Investment 固定本地提交
  ...
  [本包证据](v5-backlog-scheduler-activity-luna.md)保留上一轮 AMQP ack 测试接口误用
  的根因与修正，原调度和实际 broker 消息断言不放宽。本轮固定复验无新失败。
  四后端已本地提交且干净，三个对齐报告仅工作区增量；父仓 gitlink、五 master 未动，
  未 push/云端同步/镜像/部署，两个预置协议草案不修改或暂存。
  四个本轮临时测试容器及四个匿名卷已核归属精确清理；KIND/业务资源/镜像未动。
  下一游标是 B7 Worker 消费进展、保留归档和其余运行门禁，再 B8；仅监控接线按
  所有者明确决定迁入未来 N4-OPS-01，不将 B7/B8/B9 整体标完成。
- 2026-09-13 所有者将 Prometheus/Alertmanager 监控部署与告警送达交未来 dev-plan。
  已在实施计划登记 N4-OPS-01，含环境/资源/身份/接收渠道前置、L1/L2/L5/L7、
  真实送达与恢复判据及回滚。只迁移这部分运行接线，不扩大为整个 B7 延后；无安装、
  通知或业务部署。继续 B7i，模板上一轮 243 项通过，待固定提交及串行三实例复验。
- B7i 开工：后端起点 cd89d1f/a9f0d7b/ffc88f9/681256f，k8s 726cc4a3；
  冻结公共 Scheduler 活动适配器、只读本机 CLI、scheduler bootstrap 及对应测试/说明。
  复用 Celery PersistentScheduler，不替换调度算法/任务清单/重试；仅调度循环返回后
  原子写本进程临时快照，分列发送调用返回/异常，不冒充 broker 确认或 Inbox 完成。
  Linux 容器适配用 boot ID、PID 启动标识和 BOOTTIME 防旧文件/PID 复用/墙钟回拨；
  CLI 要求显式 schedule 路径及最大年龄，限制输入大小、拒绝符号链接/非普通文件。
  D1/D2 快照非持久业务账，无数据库迁移或跨库；D8 不升级数据库；T3 保留四角色；
  R6 模板完整门禁后串行 Info→Knowledge→Investment；T4/R1 不更新 gitlink 或发布。
  验收含真实独立 Beat/RabbitMQ 的发布、暂停/恢复/退出/重启，单测失败/缺失/旧快照/
  原子写失败和调度语义保持，以及四仓完整门禁。不接自动重启、KIND 探针或告警策略；
  Worker 真实消费进展仍另验，不能以本机 Scheduler 活动关闭 B7。
  外部测试仅本机回环随机测试队列和一次性容器，临时文件用专属目录，结束精确清理。
  回滚为 bootstrap 退回 Celery 默认类并撤销新增适配器/CLI；业务状态和 schedule 文件
  格式不变。非 Linux 的该探针失败关闭，不改变未来本机 Agent 核心的跨平台边界。
- B7h 最终结果：模板→Info→Knowledge→Investment 固定提交
  cd89d1f/a9f0d7b/ffc88f9/681256f，200/445/360/343 passed，合计 1,348、零跳过；
  Ruff/Pyright 全通过，公共五文件 SHA-256 一致，模板部署脚手架另 8 项通过。
  复验中发现的 50 ms 故障预算泄漏已确定性复现并修正；没有改变正式 2 秒预算。
  四后端工作区干净，父仓 gitlink 留原值，对齐报告不暂存；五个本地 master 未动。
  四个一次性测试容器及四个自建匿名卷已按归属清理，KIND/旧容器/原镜像未动。
  只读核对 KIND 未见 Prometheus/Alertmanager，已询问告警渠道，未擅自接外部通知。
  本包关闭受保护 HTTP 采集的源码缺口；下一游标仍是 B7 消费进展/Scheduler、实际
  采集/告警、保留归档及环境门禁，再 B8 正式接收；不把 B7/B8/B9 整体打勾。
- B7h 固定提交复验暂停：模板 199 passed，Info 在 389 passed 后旧 schema readiness
  恢复用例返回 503，日志 TimeoutError。代码确认 50 ms 故障注入覆盖了锁释放后的
  恢复断言，而正式预算为 2 秒；不据此猜测当次延迟的操作系统/连接池唯一来源。
  冻结追加公共 `test_schema_readiness.py`：模板先在恢复期注入 100 ms 正常慢响应，
  复现旧 503，再将短超时仅限持锁阶段；该延迟用于制造红例，不是等待重试至绿。
  模板完整通过后才按 Info→Knowledge→Investment 修正同一测试和重跑全量。
  同类复查另见 `test_delivery_observation.py` 的 SQL statement-timeout 用例：5 秒
  放宽预算本为隔离服务端超时，但也泄漏到恢复检查；同时收窄到故障上下文，恢复用
  原 2 秒。30 ms Redis 负例没有恢复阶段，不属于此缺陷，不顺带修改。
- B7h 门禁反馈：专项 24 项先通过；模板全量在 99 passed 后由旧“Internal 无 router”
  休眠断言拦截。实际入口已激活，按该门禁规定将旧休眠条目替换为路由已挂载的正向
  检查，并更新项目指南；不跳过门禁或放松签名/授权测试。冻结范围相应增加模板/Info
  `test_dormant_capabilities.py` 和指南当前能力投影；Info 在模板全量通过后再串行修改。
- B7h 开工：冻结四后端 B7f 提交 1c5173b/fbb48ef/94512d5/24a7512、k8s
  4761b4f；只补受保护的投递指标 HTTP 入口及测试、说明。路径为公共
  `interfaces/http/internal/delivery_metrics.py`、HTTP 路由注册、API no-store
  响应头、对应测试和对齐报告。复用实际 delivery observer 和 API 已有数据库池；
  不新增指标主档、数据库迁移、默认身份授权或监控平台。专用服务 scope
  `delivery:observe`，精确 subject/audience/签名校验；浏览器 Cookie 不可替代。
  每进程最多一个在途采集，忙/数据库异常返回失败，不缓存旧快照或输出半份零指标。
  D1/D2/I1 共享既有只读观测；I2/I3/I7 分离服务身份、按能力命名；D8 无迁移；
  R6 模板完整门禁后按 Info→Knowledge→Investment 串行；T4/R1 仍无中间发布。
  验收：真实签名正反例、未授权零数据库访问、真实 PG 只读与锁超时恢复、取消/并发
  释放、四仓完整静态/测试门禁。回滚仅精确撤销本包新增入口与注册，不影响 CLI 或投递。
  实际采集器身份配置、监控部署/告警、Worker 消费进展和 Scheduler 活性不冒充完成。
- B7g 结果：原首放行探针在新集群再次失败，捕获同秒 exit 1 的 `wget: bad address`；
  后续未改配置就解析正确并 HTTP 200，明确是 DNS 阶段失败而非后端网络拒绝。
  缺失服务发现启动门禁已补，且 DNS/OOM/启动错误不再当拒绝成功；更早失败细分 DNS
  RCODE 未捕获，不编造具体内部唯一触发点。完整新集群六条放行/拒绝全部通过，
  8 个门禁单测通过，证据见本包文档与 JSON。下一游标 B7 消费进展/Scheduler/
  scrape/告警、保留归档与其余环境门禁，随后 B8 正式接收；仍无中间同步。
- B7g 开工：独立复核 Info 2026-09-11 Calico 放行失败。冻结 k8s 的现有
  `architecture-v2/scripts/verify_r3_network_policy_calico.sh`、对应门禁测试与证据；
  先修“只能清理本次创建资源”和失败日志/Pod 现场保留，再验证原探针，不先加重试掩盖。
  网络复验只允许另建指定名称的临时 Calico KIND，使用当前 bundle 的 NetworkPolicy
  和合成 HTTP 服务；不向常驻 kind 应用配置，不构建/推送业务镜像，不动业务数据/Secret。
  旧首次失败现场不存在时不编造根因；新增失败必须保留具体 DNS/连接/退出状态并查清。
  当前单机可用内存约 75 GiB，已有 KIND/Calico/HTTP 探针镜像缓存；启动需平台授权。
  R1/R6 分开源码与实际包级门禁，I3 无业务凭据，D1/D8 不涉及业务主档或迁移。
  仍仅本地工作，不合并 master、推送或同步。
- B7f 固定提交复验：严格串行 176/421/335/318 passed，分别 28.98/56.85/47.14/51.68 秒，
  全部零跳过；源文件与提交一致，四后端工作区干净。三个父仓增量对齐报告保存在
  工作区（尚未 git add），父仓 gitlink 留待最终一起更新；五个本地 master 仍为 B7e
  起点且干净。本包未调用任何 push/云端同步，没有新分支；预置两个协议草案继续保留。
  下一游标先处置独立 Calico 失败与门禁证据/清理安全，再继续 B7 运行观测及 B8 接收。
- B7f 首轮结果：模板→Info→Knowledge→Investment 完整 176/421/335/318 passed，
  全部零跳过，Ruff/Pyright 通过。本地后端提交 1c5173b/fbb48ef/94512d5/24a7512，
  公共六文件 SHA-256 一致；未更新父仓 gitlink、master、任何远端引用或云端工作区。
  原场景直接复用且原断言未改；具体红→绿证据在本包文档。另纳入更早 Calico 允许
  探针失败复核，不能当投递时钟缺陷一并销账；先补诊断保留和只清理自建资源的边界。
- B7f 冻结：起点为 B7e 四后端 5369862/7755da2/7b11608/d231266，父仓
  ee0978a/016517b/8150c52/fc4503f、k8s 4833a2d；只在既有 Luna 开发。
  范围为公共租约显式释放、无预约/重放/对账的立即可投递语义、观测与运行语义对齐，
  对应真实 PG 时钟故障回归及 Info/Knowledge 原失败场景、Investment 租约 overlay。
  D1/D2 不新增主档或跨域读取，C2 保留 Inbox/死信/epoch 和真正 not-before/退避，
  D6/D8 无新迁移、不动业务库，R6 模板门禁后串行实例，T4/R1 最终一起发布源码。
  只给测试引擎的显式 SQL 时钟读数注入偏移，不修改系统时间/数据库全局函数。
  模板旧生产代码上的第一轮确定性回归为 6 failed / 3 passed：释放后再 claim 被拒、
  旧租约 renew/guard 被接受、无预约消息被误判未来任务、重放及对账不能立即再投递。
  修复拟用 PostgreSQL 原生负无穷表示“已释放/无等待门槛”，保留 epoch 和所有权谓词；
  时间戳审计字段继续真实时间，不声称墙钟 TTL/deadline 全部不受回拨影响。
  验收为原失败场景、故障注入正反例、四仓全量/契约/静态门禁；无中间同步。
  回滚为精确反向修改；负无穷能被旧 SQL 时间比较读取，不需要降级迁移。
- 2026-09-13 最新执行顺序：历史失败根因复核优先于新增能力。首项为 B7c Info
  `test_inflight_worker_blocks_same_host_not_other_host_and_cancel_releases` 的重试
  `consume=False`，并复核 B7d/B7e Knowledge 重放观测、响应丢失恢复与轮询时限失败。
  捕获过真实 DB 时钟回退不等于每次历史失败都有因果证据；须区分已证实机制、确定性
  回归与无法恢复的历史现场。禁止用加 sleep、放宽断言或跳过用例销账。
  本轮后续不作中间合并/推送/云端同步，保留本地验证和提交检查点，最终一起同步。
...
- B7e（2026-09-13，所有者“继续”）：[冻结范围、失败与断点](v5-backlog-worker-readiness-luna.md)。
  新增本节点消费队列/交换机/路由/持久性/任务注册检查，子进程 6 秒预算；模板未来 bundle
  接线，旧实例 release/bundle 不动。模板固定 5369862 全量 167 项、Info 7755da2 全量
  411 项均零跳过；两子仓已先推既有远端 master/Luna ref，父仓 master 尚未整合。
  Knowledge 7b11608 常规全量先后在旧重放观测、上传响应丢失恢复 consume=False 失败；
  诊断完整 322 项通过不抵消常规失败，根因仍未确定，停止推进 Investment。
  仅修正公共旧锁超时测试的 50ms 注入作用域，未改生产超时/原重放断言；其它时序缺陷
  不标已修复。当前优先查清投递时序并重过模板→Info→Knowledge 门禁，不继续叠新能力。
  Luna 候选本地保留；五父仓本地 master/云端工作区未由本包更新，未运行 remote-pull。
- B7e 续查：所有者要求“出现问题总要解决”，持续诊断捕获 DB 时钟倒退 1.875799 秒，
  系统日志亦记录跳变。WSL Hyper-V 隐式校时与 timesyncd 同开，经授权停用并禁用后，
  180 秒 31,940 次读取无倒退/大跳变。另修 Windows 原时间源不可达，获准改到已验证
  可达的 Ubuntu NTP，读回宿主已同步。未改投递生产代码或旧重放/恢复断言。
  原始完整门禁模板 5369862：167、Info 7755da2：411、Knowledge 7b11608：322
  （连续两轮）、Investment d231266：305，均零跳过；不是以带诊断复跑抵消失败。
  四后端对象已推，父仓候选 tpl ee0978a、Info 016517b、Knowledge 8150c52、
  Investment fc4503f；当前进入父仓集成与最终同步，不将本机系统配置误称应用部署。
- B7e 收口：宿主同步后 Knowledge 同提交再过 322 项（44.81 秒），最终另一次 180 秒/
  31,858 次 DB 时钟审计无倒退或超过 50ms 跳变。四后端先推、五父仓预期 HEAD 快进，
  GitHub/Gitee master/Luna 均推送成功；remote-pull master/luna 成功，SSH 复核两工作区
  五父仓及实际后端 SHA/干净状态正确。初次文档 k8s@82108cb2，本条为后续回填。
  两批一次性测试资源及三份临时诊断草件已清理；两协议草稿未改，业务数据/Secret/
  应用镜像/部署未变。下一游标 B7 消费进展、Scheduler、采集/告警与归档/实际环境门禁，
  不再以旧的“未查清时序”停止该包；B7 整体与 B8/N1～N6 正式接收仍未完成。
