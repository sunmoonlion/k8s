#!/bin/bash
# Tools Admin Frontend Secret YAML 生成脚本
# 根据配置生成 Secret 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-tools-admin-frontend-secret.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-tools-admin-frontend-secret/ -> tools-admin-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-tools-admin-frontend-secret/ -> tools-admin-frontend-secret/ -> secret/ -> custom-values/ -> k8s-resource/ -> resources/ -> tools-admin-frontend/
...
CONFIGMAP_CONFIG_FILE="$K8S_RESOURCE_DIR/custom-values/configMap/tools-admin-frontend-config/generate-tools-admin-frontend-config/generate-tools-admin-frontend-config.conf"
...
# 生成 YAML
main() {
    log_info "开始生成 Tools Admin Frontend Secret YAML 文件..."
