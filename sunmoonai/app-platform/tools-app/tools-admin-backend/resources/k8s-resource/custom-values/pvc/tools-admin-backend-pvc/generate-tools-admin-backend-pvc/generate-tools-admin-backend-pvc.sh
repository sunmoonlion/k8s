#!/bin/bash
# Tools Admin Backend PVC YAML 生成脚本
# 根据配置生成 PVC 的 YAML 文件

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-tools-admin-backend-pvc.conf"
# 计算 resources/k8s-resource 目录（模板文件所在位置）
# 从 generate-tools-admin-backend-pvc/ -> tools-admin-backend-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/
K8S_RESOURCE_DIR="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# 计算应用根目录（用于查找主应用的 deploy-*.conf）
# 从 generate-tools-admin-backend-pvc/ -> tools-admin-backend-pvc/ -> pvc/ -> custom-values/ -> k8s-resource/ -> resources/ -> tools-admin-backend/
...
# 生成 YAML
main() {
    log_info "开始生成 Tools Admin Backend PVC YAML 文件..."
