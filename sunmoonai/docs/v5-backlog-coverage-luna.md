...
| 服务身份 / M1-002 | `infrastructure/security/service_identity.py`、`core/config.py` 的 subject/scope 精确绑定；[B7l](v5-backlog-runtime-preflight-luna.md) 确认身份共用；[B7m](v5-backlog-runtime-rendering-luna.md) 修角色键；[B7n](v5-backlog-runtime-permissions-luna.md) 实查 ACL/broker；[B7o](v5-backlog-template-database-policy-luna.md) 模板 18 PG；[B7p](v5-backlog-info-database-policy-luna.md) Info 25 PG；[B7q](v5-backlog-knowledge-database-policy-luna.md) Knowledge 摄入/回执/轮询及拒绝 27 PG 通过 | 真实集群仍旧身份、版本表 CRUD 与 broker 共用仍未切换；模板/Info/Knowledge 加法候选不收敛旧账号，Investment overlay、broker、历史权限/供给/撤销/轮换继续受控验收，不同键名不等于不同凭据 |
