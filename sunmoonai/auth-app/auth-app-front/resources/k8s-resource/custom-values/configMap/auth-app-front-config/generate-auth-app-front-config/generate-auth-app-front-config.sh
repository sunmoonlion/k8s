#!/bin/bash
# Auth App Front ConfigMap YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-auth-app-front-config.conf"
# 从 generate-auth-app-front-config/ -> auth-app-front-config/ -> configMap/ -> custom-values/ -> k8s-resource/
...
export NITRO_HOST="${NITRO_HOST:-}"
export NITRO_PORT="${NITRO_PORT:-}"
export AUTH_BACKEND_URL="${AUTH_BACKEND_URL:-}"
...
main() {
    log_info "开始生成 Auth App Front ConfigMap YAML..."
