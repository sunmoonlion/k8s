...
# =============================================================================
# Traefik nerdctl 客户端部署插件
# 文件名: traefik-N-deploy-client.sh
# 用途: Traefik的nerdctl客户端CA证书分发
# =============================================================================
...
deploy_traefik_nerdctl() {
    ...
    if ! distribute_ca_certificate_to_client "$combo"; then
        log_error "Traefik nerdctl 客户端CA证书分发失败"
        return 1
    fi
    ...
    log_success "Traefik nerdctl 客户端部署完成"
    return 0
}
