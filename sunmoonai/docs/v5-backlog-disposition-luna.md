...
| M1-203 | B6a/B6b 提供持久单次轮询源码；部署及共享 SQL/HTTP 日志降噪尚未验收 | B6 源码；B7 运行态/日志 |
...
| B6 解析调度 | B6a 已集成；B6b 源码已验证待集成 | [公共同步证据](v5-backlog-scheduling-luna.md)；[Knowledge 单次轮询证据](v5-backlog-parse-polling-luna.md)。deadline/backoff、回执原子接续及重试代次故障回归通过；运行态及日志子项交 B7，不把 M1-203 整项销账 |
| B7 已有覆盖收口 | 待核验 | 各包新增现状纳入；逐验收子项核投递、身份/配置、运维指标、归档、健康与恢复；接 B4 对账周期/权限/保留策略、B5 真实映射/旧任务/一致配置切换、B6 旧协议调查/排空/回滚及 SQL/HTTP 日志降噪；自动修复回收与全引用门禁交 N4；未验证项不销账 |
