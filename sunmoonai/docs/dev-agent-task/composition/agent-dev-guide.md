...
| turn 的本地编号（`01`、`02`、`03a`） | 无对应：Codex 的 thread id 与 turn id 都是 UUID（turn id 为 UUIDv7），thread id 还编进会话记录文件名 `rollout-<时间>-<uuid>.jsonl` | `protocol/src/thread_id.rs:16`、`rollout/src/list.rs:427`、`app-server-protocol/src/protocol/v2/thread_data.rs:355` | 本地号只排先后，它的 id 原样接在目录名后面（`01-<turn id>`）；会话 id 记在 `thread` 字段，会话记录在哪记 `provider_record` |
| 并行尝试（`03a`、`03b`…） | 云端 `TurnAttempt.attempt_placement`：同一 Task 下的第几个尝试 | `cloud-tasks-client/src/api.rs:67` | 共用一份任务书；各家一个尝试子目录 `a-<turn id>`，各写各的回执 |
