...
- 当时恢复顺序已执行到 P0-008B/B5；本节的版本矩阵/生成清单清理、
  `P0-007A2/A2.2~A2.5`、`P0-007B/C`、`P0-008A` 和 B1~B5 均不得重放。ADR-018
  接受后，当前游标改为 §14 的 `P0-007D -> P0-007E -> B6 -> P0-009A~E`：先冻结并
  改名 React Router legacy，再建立 canonical Next Admin，随后完成 Web B6；两条 Next
  模板资格门接受后，立即把共同底座按 Info → Knowledge → Research 串行同步。P0-009
  接受后才执行 P0-008C；M1-411/413 只补完整业务等价、切流和 legacy 退出，不再承担
  首次基础同步。
...
Spike，重复投递/外部副作用恢复并入可靠交付 Spike；P0-007A~D 固定 React Router
Admin 的历史能力和 legacy 输入，P0-007E 建立默认 Next Admin；P0-008 再基线 Next
Web/Web Backend，P0-009 在任何新增业务开发前传播统一 Next/FastAPI 模板 release。
...
- 实施：对未触发硬淘汰规则的候选运行相同 interrupt/tool graph；测创建、stream、SSE 断线对账、cancel、worker kill、resume、同 Thread 并发；以最小浏览器 harness 验证 Runtime stream 可被产品客户端恢复，不能只用命令行读取流。许可、合规或未经批准 egress 先触发淘汰时，不为形式比较部署不可进入生产的候选。
- 测试：L2/L5/L7。
- 验收：评分矩阵、许可结论、运行证据、退出成本和 ADR-001 获批。
- 回滚：Spike 不接生产数据；删除部署即可。
- 状态：ACCEPTED / CANDIDATE_A_SELECTED（2026-07-16；自建 Runtime 的 PostgreSQL checkpoint、提交前/后真实 SIGKILL、running cancel、双 worker、PostgreSQL fail-closed/recovery、同 Thread reject、durable dispatch intent、服务端 cursor 与 Chromium 断线对账均通过；API/worker 双副本验证后恢复为 1。Agent Server/Hybrid 因未批准的生产许可、采购和 beacon egress/usage reporting 触发预设硬淘汰规则。ADR：`sunmoonai/docs/mooc-manus-v5/adr/ADR-001-runtime-selection.md`；证据：`sunmoonai/docs/evidence/v5/V5-P0-001/result.md`）
...
- 状态：ACCEPTED（2026-07-16；Retrieval v1 契约、稳定领域身份/migration、Knowledge RAGFlow adapter、Research KnowledgePort、独立 retrieval 服务身份和 Citation projection 已以固定 r2 digest 通过真实 KIND 全矩阵。KIND 使用默认关闭、显式传入的 RAGFlow-only egress proxy；Pod 自动代理连续探测 `10/10`，真实 retrieval 返回一条可回溯 Evidence。未知 dataset `403`、空结果、不可映射 chunk、Provider `504`、token budget、JWT/credential 隔离负向矩阵和 citation -> Knowledge version -> Info source version 回溯全部通过；临时 override/provider double 已清理，正式 `15/20s` timeout 未放宽。ADR-004 已接受；证据：`sunmoonai/docs/evidence/v5/V5-P0-004/result.md`。开发桌面代理不构成生产 egress 方案，生产仍须受治理 NAT/egress gateway/proxy。）
...
### 前端/配对后端物理仓库与差异基线（初始 2026-07-13，ADR-018 修订 2026-07-26）
...
### V5-P0-007 Admin Frontend Template Rollup

- 类型/优先级：ARCH/FRONTEND/P0
- ADR：`sunmoonai/docs/mooc-manus-v5/adr/ADR-013-frontend-technology-rendering.md`
  （历史能力输入）与 `ADR-018-unified-next-frontend-surfaces.md`（目标态）。
- 目标：历史链
  `P0-007A -> P0-007A2 -> P0-005 -> P0-007B -> P0-007C` 已冻结 React Router
  Admin v1；ADR-018 增补 `P0-007D -> P0-007E`，先完成 legacy 最终配对/改名，再建立
  canonical Next Admin 并逐项消费完整通用能力矩阵。父任务不直接写代码。
...
- 状态：ACCEPTED（P0-007A/A2/B/C/D/E 已全部接受。历史 React Router v1 已固定并
  改名为 `tpl-admin-frontend-react`，只作 legacy/reference；canonical Next Admin 已由
  P0-007E 接受并成为默认 Admin 模板。）
...
- 类型/优先级：ARCH/FRONTEND/RELEASE/P0；ADR-018 接受后的历史已完成任务。
- 仓库：`tpl-app/tpl-admin-frontend`、`tpl-app/tpl-admin-backend`、`tpl-app`、`k8s`。
- 前置：P0-007C、P0-008B/B5 ACCEPTED；Admin Backend 必须使用 B5 固定 commit/tree，
  不得用业务实例后端或 mock 代替。
- 实施：
  1. 固定 React Router Admin 与 B5 Admin Backend 的 source commit、tag、依赖锁和
     immutable image digest；
  2. 以独立 Casdoor Admin client/audience/cookie/namespace 完成真实严格 TLS 配对，
     覆盖登录/回调、session/CSRF、受保护读取、受审计 mutation、403/过期 session、
     多副本、滚动和双向回滚；
  3. 确认能力矩阵、Vue/React 映射、迁移指南和 clean-room 仍能从固定提交重放；
  4. 配对接受后，将远端 `tpl-admin-frontend`、本地目录、父仓 submodule path/url/gitlink
     和相关 tag/文档原子改名为 `tpl-admin-frontend-react`，验证旧名不存在、新名可
     clean clone，且父仓可递归初始化；
  5. 记录独立恢复 runbook；不得把该 legacy profile 加入任何业务 App 发布矩阵。
- 不做：不创建 Next Admin、不修改三个 App、不把 React Router/Nginx 产物提升为新的默认
...
- 类型/优先级：ARCH/FRONTEND/SECURITY/P0。
- ADR：`sunmoonai/docs/mooc-manus-v5/adr/ADR-018-unified-next-frontend-surfaces.md`。
- 仓库：新 canonical `tpl-app/tpl-admin-frontend`、`tpl-app/tpl-admin-backend`、
  `tpl-app/tpl-web-frontend`（只读工程种子）、`tpl-app`、`k8s`。
- 前置：P0-007D ACCEPTED；远端新的空 `tpl-admin-frontend` 已由项目负责人创建或明确
  授权创建；必须从 P0-007D 固定 legacy 和 B5 Admin Backend tuple 工作。
- 初始化纪律：从固定 `tpl-web-frontend` 工程树复制到空目录并创建**全新 Git 历史**，
  不保留 Web 仓祖先或业务语义；随后改为独立 Admin 应用名、Casdoor client/audience、
  cookie、session namespace、route/layout、环境变量、镜像、K8s 和发布单元。Web 只提供
  Next 工程基线，Admin 功能真相来自 P0-007D 固定的完整能力矩阵。
- 实施：采用 React 19 + Next 16 App Router + Node standalone；public/authenticated
  rendering/cache matrix、server-only DAL、浏览器 typed client、TanStack Query、
  i18n、主题/响应式、权限导航、CRUD/Query/审计/上传、富工具边界、错误/a11y/可观测
  接入点全部逐项标记 `adopted/adapted/deferred/rejected`。浏览器与 Next DAL 均只访问
  Admin FastAPI BFF 的同源 `/api/*`；Next 不承载 OIDC token exchange、session 真相、
  CSRF 真相或第三套 BFF。
- 配对：以 B5 `tpl-admin-backend` 完成真实 Casdoor、严格 TLS、session/CSRF/audience、
  读写/错误、缓存不泄漏、两副本、滚动/version-skew、回滚及 immutable release tuple。
- 测试：Node/pnpm clean-room、typecheck/lint/unit/component、Playwright/a11y、SSR/
  hydration、public/static 与 authenticated/dynamic、base path、Docker standalone、
  non-root/read-only/probe、KIND 严格 TLS 和失败注入。
...
- 状态：ACCEPTED（2026-07-28；实现 `tpl-admin-frontend@6d3b28f`，验收矩阵 release
  `tpl-admin-frontend@fb69795` / tag `p0-007e-next-admin-20260728`，配对 FastAPI
  Admin `69e634b` / tag `p0-007e-admin-backend-20260728`，父仓 `7089e19` / tag
  `p0-007e-next-admin-parent-20260728`。42 个 unit/component、10 个 Playwright、
  clean-room、Docker standalone、真实 Casdoor、严格 TLS、2+2 Pod、缓存/角色隔离、
  41/44 次回滚/前滚连续探测和双向 18 个静态资源门禁通过；三个 App 未修改。证据：
...
- 顺序纪律：P0-008B/B5 已完成；ADR-018 插入 P0-007D/E，故 B6 只有在 P0-007E
...
- 状态：IN_PROGRESS / B5_ACCEPTED / B6_CURRENT_UNIQUE_TASK（P0-008A 与 ADR-014 已于 2026-07-16
  ...
  配对、双向回滚和原子改名；同日 P0-007E 完成 canonical Next Admin、FastAPI Admin
  真实配对、clean-room 与双向回滚并接受。当前唯一任务已恢复为 B6；B6 之后唯一下一
  任务是 P0-009，不得跳到 008C。）
...
- 验收：ADR-014/016 Accepted；一张当前/目标拓扑、route rendering matrix、cache owner
  matrix、BFF allowlist、backend profile/仓库迁移、FastAPI 母版来源、环境变量和部署兼容
  矩阵获批；所有未决项都有 owner/阻断任务，不以“模板以后处理”放行。
- 状态：ACCEPTED / REVISION_2（2026-07-18；原 2026-07-16 结果及固定 ixartz SHA
  `9926cc1f8664f67eca63065bf1c31bc4f60b09c2` 继续有效；ADR-016 将原 Nest 默认改为
  FastAPI 默认、Nest 受维护可选 profile，并冻结 B2~B6、仓库改名、新 FastAPI 仓建立、
  母版来源和双实现共享契约纪律。原证据：`sunmoonai/docs/evidence/v5/V5-P0-008A/result.md`；
  修订附录：`sunmoonai/docs/evidence/v5/V5-P0-008A/revision-20260718.md`。原运行证据保持
  不变；本附录只修订后续决策，不冒充 B2~B6 运行证据。）

### V5-P0-008B tpl-app Next Web v2 + 双 Web BFF 模板（FastAPI 默认）

- 类型/优先级：ARCH/FRONTEND/P0
- 仓库：`tpl-app`
- 前置：P0-008A ACCEPTED。
- 实施：保留 React 19、Next 16 App Router、next-intl、Tailwind/shadcn/Base UI 候选和
  `standalone` 自托管。先在现有 `tpl-web-backend` 完成 Nest 生产 BFF 并与 Next 验收，
  固化后改名为 `tpl-web-backend-nest`；再补齐 canonical `tpl-admin-backend` 的 FastAPI
  通用安全母版，并创建新的默认 `tpl-web-backend` FastAPI Web surface。Next 实现统一的
  Server/Client、server-only DAL/DTO、typed client、render/cache、stream UI adapter、
  错误/correlation、安全、测试和 Docker/KIND 骨架。只允许 ADR-016 批准的一个模板
  backend profile 仓，不创建第二套 Next 前端或任何平行业务仓库。
...
- 串行施工包：
  1. `B1 Repo/Env/Rendering/Test Baseline`：Node 24/Next/Nest/clean-room（已接受）。
  2. `B2 Nest BFF Identity + Next DAL/DTO`：按 ADR-005 重写 Nest 的 PKCE/state/nonce/
     JWKS、最小 Redis session、Principal、CSRF/Origin、POST logout、Web audience；Next
     只经统一 server-only DAL/DTO 使用它。
  3. `B3 UI/Query/Stream/Citation + Nest Pair`：完成中性 UI 与共享 Auth/Error/SSE/
     Citation contract，不把 fixture 当真实业务。
  4. `B4 Nest Security/Paired Test/Deploy/Freeze`：Next+Nest 双 Pod、浏览器、滚动/回滚和
     安全负向矩阵通过后，固定 tag/digest/contract，原子改名为
     `tpl-web-backend-nest` 并更新父仓 gitlink/远端；未通过不得改名冒充完成模板。
  5. `B5 FastAPI Canonical Kernel + Default Web BFF`：先从三套已通过 P0-005 的业务后端
...
- 不做：不做无 tag/digest 的不可回滚覆盖、不改三个 App 流量、不把 Admin/Web 合成一个
...
- 状态：IN_PROGRESS / B5_ACCEPTED / B6.1_ACCEPTED / B6.2_ACCEPTED /
  ...
  原 Node 20 B1 为 `SUPERSEDED / NO_NEW_RELEASE`。2026-07-22：B2 固定
  `tpl-web-frontend@b1730a6`、`tpl-web-backend@839ea09`、`tpl-app@4cae61d`；PKCE/state/
  nonce/JWKS/audience、原子 Redis transaction、最小 session、CSRF/Origin、POST logout、
  server-only DAL、严格浏览器 DTO 和受控 Next+Nest 配对门禁均通过。证据：
  `sunmoonai/docs/evidence/v5/V5-P0-008B/B2/result.md`。2026-07-22：B3 固定
  ...
  `tpl-web-backend-nest`。2026-07-26：B5 固定 canonical Admin
  `456bd65` / tag `p0-008b-b5-admin-kernel-20260726`、默认 FastAPI Web
  `6b6c71e` / tag `p0-008b-b5-fastapi-web-20260726`、父仓 `bd19ee2`；新 Web 仓的
  初始化 tree 与固定 Admin tree 完全一致，随后只实施 Web surface/audience/cookie/
...
- 前置：P0-007E、P0-008B/B6 ACCEPTED；四个默认组件及可选 Nest/legacy React profile 均有固定
...
- 目标：在继续 P0-008C 或任何新增业务功能前，把 `tpl-admin-frontend`、
  ...
  `template_release_id` 串行应用到 Info、Knowledge、Research。Nest 和 React Router
  legacy profile 只留在模板门，不进入业务实例。
- 串行施工包：
...
- ADR-001~006、ADR-013/014/016/017/018 Accepted。
...
- P0-007A/A2/B/C/D/E 全部 ACCEPTED；React Router legacy 的最终配对/原子改名、
  Next Admin 默认模板、完整能力矩阵、真实 FastAPI Admin 配对、固定 commit 干净重建、
  Docker/KIND/回滚证据通过；Vue 与 React Router 回滚由固定 tag/镜像提供。
...
P0-007A ────────────────────────────────────────> P0-007C
P0-008A/B1~B5 + P0-007C ──> P0-007D Legacy closure/rename
                                      v
                              P0-007E Next Admin
                                      v
                              P0-008B/B6 Template release
                                      v
...
- R6 Frontend/Experience：历史链严格按 P0-007A -> A2.1 -> P0-005 -> A2.2 -> A2.3 ->
  A2.4 -> A2.5 -> P0-007B -> P0-007C，Web 链已完成 P0-008A -> B1~B5。ADR-018 后当前
  顺序固定为 `P0-007D -> P0-007E -> P0-008B/B6`：D 冻结并改名 React Router legacy，
  E 建立 Next Admin 并与 FastAPI Admin 配对，B6 再冻结统一默认 release 与 Nest 可选
  profile。随后不得插入其他开发，必须执行 P0-009A -> Info -> Knowledge -> Research ->
  Convergence Gate。三个实例共同底座全部对齐后才执行 P0-008C，并只用 FastAPI 默认
  profile。M1-411B/C/D 和 M1-413 只做完整业务等价、切流和 legacy 退出，不再次替换共同
  底座。

粗粒度工作量用于容量规划，不是承诺日期：ADR-016 将 P0-008 从单一 Nest 配对扩展为
Nest 可选模板、FastAPI 通用母版/默认 Web BFF 和双实现共享门禁，P0-008A/B/C 重新估算为
约 11~19 人周（A 1~2、B1~B6 8~13、C 2~4）；ADR-018 新增 P0-007D/E 约 5~9 人周；
P0-009 四组件三实例收敛粗估约 6~12 人周，完整 Phase 0 重估为约 36~62 人周。
P0-007A/A2/B/C 是历史投入，D/E 单独计量；411B/C/D
...
1~2 个自然周。每次 Gate 后重估，禁止把区间当固定 deadline。
...
ADR-001 已选择 Custom Runtime。任务适用性如下：

```text
Runtime Common：M1-301、303、305~309、312（实现形态按选型适配）
Custom only：M1-302、310、311
Agent Server only：M1-313（NOT_APPLICABLE）
Hybrid only：M1-314（NOT_APPLICABLE）
```

未选分支已按 ADR-001 标记为 `NOT_APPLICABLE`。Common 任务和 Custom 任务必须实现 ADR-001 冻结的产品领域、ACL、durability 与 recovery 边界，不得把 P0 Spike 直接升级为生产 Runner。
...
- 状态：NOT_APPLICABLE（ADR-001 于 2026-07-16 选择 Custom Runtime；只有新 ADR 解除 Agent Server 许可/egress 硬门并重开选型后才能激活。）
...
- 状态：NOT_APPLICABLE（ADR-001 于 2026-07-16 选择 Custom Runtime；不得平行建设第二套执行面。）
...
### V5-M1-411 三个 Next Admin 业务等价迁移 Rollup
...
### V5-M1-411B Info Admin Next 等价迁移

- 类型/优先级：FRONTEND/FUNC/P0
- 仓库：`info-app`、`k8s`
- 前置：P0-009B、Gate P0；复用 P0-007B 试点，不重新实现同一页面；必须消费
  P0-007A2 的通用能力映射。
- 实施：在 P0-009 已同步的固定 Next Admin 上迁移现有真实 Info 页面以及 M1-404
  Artifact/Delivery 治理范围；建立 Vue/React Router route -> Next route ->
  API/contract -> E2E 的逐项映射。P0-007B 只作为业务与回归输入，不直接提升 legacy
  React Router 实现。
- 验收：功能、安全、可访问性、性能、浏览器 E2E 和部署等价；隔离流量切换与回滚演练通过。M1a 可只接受 Artifact/Delivery 最小面，M1b 前完成双方约定的全部保留页面。
- 状态：NOT_STARTED

### V5-M1-411C Knowledge Admin Next 等价迁移
...
### V5-M1-411D Research Admin Next 等价迁移
...
| 前端产品闭环 | P0-001/004/005/007A/A2/B/C/D/E/008/009, ADR-016/017/018, M1-400~413, M1-600/601 | React Router legacy 完整能力矩阵与最终配对、Next Admin/Web 默认模板、FastAPI Admin/Web 默认 BFF 与 Nest 可选 profile 共享契约、统一模板 release、六实例共同底座立即串行收敛、浏览器真实 E2E、HITL、刷新/断网恢复、跨 Admin lineage、legacy 退出 |
...
17. Web Architecture Freeze：V5-P0-008A（`ACCEPTED / REVISION_2`，2026-07-18）；
...
19. Template-first Adoption：ADR-017（`ACCEPTED`，2026-07-22）；四默认模板组件形成统一
    ...
    业务开发。
20. Web Re-baseline B2 Nest BFF Identity + Next DAL/DTO（`ACCEPTED`，2026-07-22）：
    固定 `tpl-web-frontend@b1730a6`、`tpl-web-backend@839ea09`、`tpl-app@4cae61d`；身份
    内核、严格 DTO、SSR DAL 与受控 Next+Nest 配对测试通过，未提前改名仓库或修改业务实例。
21. Web Re-baseline B3 UI/Query/Stream/Citation + Nest Pair（`ACCEPTED`，2026-07-22）：
...
22. Web Re-baseline B4 Nest Security/Paired Deploy/Freeze（`ACCEPTED`，2026-07-26）：
    ...
    已原子改名为 `tpl-web-backend-nest`，三个业务实例未修改。当时的下一任务 B5 已于
    同日接受；后续严格执行 `B5（FastAPI Admin 母版/默认
...
23. Web Re-baseline B5 FastAPI Canonical Kernel + Default Web BFF（`ACCEPTED`，
    2026-07-26）：固定 Admin `456bd65`、FastAPI Web `6b6c71e`、父仓 `bd19ee2` 及两份
    Harbor immutable digest；Web 初始化 commit 与 Admin 固定 tree 完全一致，随后显式
    替换 Web surface/audience/cookie/namespace/interaction/downstream。独立源码、迁移
    roundtrip、生产配置负向、Docker 和 KIND 两副本门禁通过，三个业务实例未修改。
...
26. Next Admin Default：V5-P0-007E（`ACCEPTED`，2026-07-28）；固定 Next Admin
    `fb69795`、FastAPI Admin `69e634b`、父仓 `7089e19`、三份 Git tag 及前后端
...
退出。完成全部 Phase 0 后更新 v5，再进入 M1a；禁止绕过模板与实例收敛门直接扩建
Walking Skeleton、Memory 或 Subagent。
