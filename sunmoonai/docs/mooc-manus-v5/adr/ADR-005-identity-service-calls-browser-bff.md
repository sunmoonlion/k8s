...
2. Knowledge internal resource server 使用 application-specific discovery/JWKS 验证签名、issuer、精确 target audience、`exp`/`iat`，再以本地 binding 把 service subject 映射为 scopes。未知 subject 或 binding 默认拒绝；Provider 返回的 `scope`/`scp` 只作为格式化输入，不能替代本地关系授权（Casdoor client-credentials 令牌可能只返回 provider 级 `openid` scope）。
