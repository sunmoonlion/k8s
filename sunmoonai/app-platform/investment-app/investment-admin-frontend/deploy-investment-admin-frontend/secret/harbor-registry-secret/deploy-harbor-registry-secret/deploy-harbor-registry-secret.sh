...
# =============================================================================
# Harbor Registry Secret 部署脚本（Investment Admin Frontend）
...
# deploy-harbor-registry-secret/ -> harbor-registry-secret/ -> secret/ -> deploy-investment-admin-frontend/ -> investment-admin-frontend/
...
# 尝试加载主配置文件
MAIN_CONFIG_FILE="$(cd "$SCRIPT_DIR/../../.." && pwd)/deploy-investment-admin-frontend.conf"
...
main() {
    ...
    log_info "部署 Harbor Registry Secret (Investment Admin Frontend)..."
    log_info "部署参数："
    log_info "  - 操作: $action"
    ...
    echo ""
    log_success "Harbor Registry Secret (Investment Admin Frontend) 操作完成！"
    log_info "操作信息："
    log_info "  - 操作: $action"
...
# 显示帮助信息
show_help() {
    echo "Harbor Registry Secret 部署脚本 (Investment Admin Frontend)"
    echo ""
    echo "用法:"
