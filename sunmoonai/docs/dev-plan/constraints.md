...
| 分支与主线的关系 | git 的行为 | 谁挡住 |
| --- | --- | --- |
| 主线在分叉后**没改过**该文件（本轮的 opus） | 干净删除，自动合并成功 | `pre-merge-commit` 拦下合并提交 |
| 主线在分叉后**改过**该文件（cursor/kimi/luna/qwen3.8——主线把 human 那份从 169 行重写到 821 行） | `modify/delete` 冲突，合并当场停下，主线版本留在工作树 | git 自己；若人在冲突里选择接受删除再提交，`pre-commit` 再拦一道 |

第二种情况的第一道保护取决于「主线恰好改过这份文件」，不能指望它长期成立；能长期
成立的是门禁那道。另：合并助手分支到主线还会在 `codex-reference/README.md` 上产生
`add/add` 冲突——各分支只放自己写的那份，本来就会撞，按各自保留处理即可。
