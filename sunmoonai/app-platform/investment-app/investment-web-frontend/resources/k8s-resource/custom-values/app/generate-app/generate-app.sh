#!/bin/bash
# investment-web-frontend Deployment YAML 生成脚本
...
export INVESTMENT_WEB_FRONTEND_POSTGRESQL_SECRET_NAME="${INVESTMENT_WEB_FRONTEND_POSTGRESQL_SECRET_NAME:-}"
export INVESTMENT_WEB_FRONTEND_REDIS_SECRET_NAME="${INVESTMENT_WEB_FRONTEND_REDIS_SECRET_NAME:-}"
export INVESTMENT_WEB_FRONTEND_MONGODB_SECRET_NAME="${INVESTMENT_WEB_FRONTEND_MONGODB_SECRET_NAME:-}"
export INVESTMENT_WEB_FRONTEND_DATABASE_ENV_FROM=""
if [[ -n "$INVESTMENT_WEB_FRONTEND_POSTGRESQL_SECRET_NAME" ||
      -n "$INVESTMENT_WEB_FRONTEND_REDIS_SECRET_NAME" ||
      -n "$INVESTMENT_WEB_FRONTEND_MONGODB_SECRET_NAME" ]]; then
    if [[ -z "$INVESTMENT_WEB_FRONTEND_POSTGRESQL_SECRET_NAME" ||
          -z "$INVESTMENT_WEB_FRONTEND_REDIS_SECRET_NAME" ||
          -z "$INVESTMENT_WEB_FRONTEND_MONGODB_SECRET_NAME" ]]; then
        log_error "PostgreSQL、Redis 和 MongoDB Secret 名称必须同时设置"
        exit 1
    fi
    printf -v INVESTMENT_WEB_FRONTEND_DATABASE_ENV_FROM \
      '        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$INVESTMENT_WEB_FRONTEND_POSTGRESQL_SECRET_NAME" \
      "$INVESTMENT_WEB_FRONTEND_REDIS_SECRET_NAME" \
      "$INVESTMENT_WEB_FRONTEND_MONGODB_SECRET_NAME"
    export INVESTMENT_WEB_FRONTEND_DATABASE_ENV_FROM
fi
export INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME="${INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME:-}"
export INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME="${INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME:-}"
export INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM=""
if [[ -n "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" ||
      -n "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME" ]]; then
    if [[ -z "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" ||
          -z "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME" ]]; then
        log_error "对象存储 ConfigMap 和 Secret 名称必须同时设置"
        exit 1
    fi
    printf -v INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM \
      '        - configMapRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_CONFIGMAP_NAME" \
      "$INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_SECRET_NAME"
    export INVESTMENT_WEB_FRONTEND_OBJECT_STORAGE_ENV_FROM
fi
export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME="${INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME:-}"
export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME="${INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME:-}"
export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM=""
export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT=""
export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_VOLUME=""
if [[ -n "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" ||
      -n "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME" ]]; then
    if [[ -z "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" ||
          -z "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME" ]]; then
        log_error "Elasticsearch ConfigMap 和 Secret 名称必须同时设置"
        exit 1
    fi
    printf -v INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM \
      '        - configMapRef:\n            name: %s\n        - secretRef:\n            name: %s' \
      "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_CONFIGMAP_NAME" \
      "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME"
    printf -v INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT \
      '        - name: elasticsearch-ca\n          mountPath: /var/run/secrets/sunmoonai/elasticsearch\n          readOnly: true'
    printf -v INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_VOLUME \
      '      - name: elasticsearch-ca\n        secret:\n          secretName: %s\n          items:\n          - key: ca.crt\n            path: ca.crt' \
      "$INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_SECRET_NAME"
    export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_ENV_FROM
    export INVESTMENT_WEB_FRONTEND_ELASTICSEARCH_VOLUME_MOUNT
