...
**今后每一个 agent 都按这里的规范开发。**它与任务的生命周期正交——任何开发任务（如 [`../dev-agent-task/`](../dev-agent-task/)）
...
这里放两类东西：与具体项目无关的原则和交付规范；以及本平台上所有 agent 开发任务都必须遵守的代码规则
（[`constraints.md`](constraints.md)，动代码前必读）。具体项目怎样落实，写在各任务自己的目录里
